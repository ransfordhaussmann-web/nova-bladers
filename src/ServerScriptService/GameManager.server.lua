...
local function tryStartMatch(playerList, modeId)
...
MatchmakingManager.onMatchReady(function(playerList, modeId)
	local readyPlayers = {}
	for _, player in playerList do
		if player.Parent then
	...
	tryStartMatch(readyPlayers, modeId)
end)

MatchmakingManager.setCanStartMatch(function()
