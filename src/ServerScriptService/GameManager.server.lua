...
local function getModeFromCount(count, forcedMode)
	if forcedMode then
		return forcedMode
	end
	if count >= 3 then
	...
	for _, player in state.players do
		if player.Parent then
			Remotes.MatchState:FireClient(player, {
				phase = phase,
				mode = getModeFromCount(#state.players, state.mode),
...
local function startFighting()
	...
	local mode = getModeFromCount(#state.players, state.mode)
...
Bindables.MatchReady.Event:Connect(function(payload)
	...
	local playerList = payload.players
