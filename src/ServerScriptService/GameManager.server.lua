...
local function beginMatch(playerList)
	...
	if Bindables.MatchStarted then
		Bindables.MatchStarted:Fire({
			mode = getModeFromCount(#playerList),
			players = playerList,
		})
	end
...
Bindables.MatchReady.Event:Connect(function(matchData)
	if state.phase ~= MatchPhase.Idle then
		return
	end
	if typeof(matchData) ~= "table" or typeof(matchData.players) ~= "table" then
		return
	end
	if #matchData.players == 0 then
		return
	end

	beginMatch(matchData.players)
end)
