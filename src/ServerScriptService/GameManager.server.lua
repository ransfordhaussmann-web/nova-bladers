...
MatchmakingQueue.registerHandlers({
	isBusy = function(player)
		return isPlayerInMatch(player) or state.phase ~= MatchPhase.Idle
	end,
	start = beginMatch,
})
...
Bindables.EnterArena.Event:Connect(function(player)
	if MatchmakingQueue.join(player) then
		-- Queue joined; match starts via MatchmakingQueue when ready.
	end
end)
