	...
	for _, player in state.players do
		if player.Parent then
			Remotes.MatchState:FireClient(player, {
				phase = phase,
				mode = state.matchModeId or getModeFromCount(#state.players),
...
local function startFighting()
	...
	local mode = state.matchModeId or getModeFromCount(#state.players)
...
local function beginMatch(playerList, modeId)
	...
	state.matchModeId = modeId or getModeFromCount(#playerList)
	state.phase = MatchPhase.Selecting
...
Bindables.MatchReady.Event:Connect(function(players, modeId)
