...
Bindables.MatchReady.Event:Connect(function(match)
	if state.phase ~= MatchPhase.Idle then
		MatchmakingService.cancelStart(match)
		return
	end
	if typeof(match) ~= "table" or typeof(match.players) ~= "table" then
		return
	end
