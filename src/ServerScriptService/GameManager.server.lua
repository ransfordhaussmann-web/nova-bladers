...
local function beginMatch(playerList, modeId)
	...
	matchModeId = modeId or getModeFromCount(#playerList)
	state.players = playerList
	state.phase = MatchPhase.Selecting
	GameMatchState.setArenaBusy(true, matchModeId)
...
Bindables.MatchReady.Event:Connect(function(modeId, playerList)
	if typeof(modeId) ~= "string" or typeof(playerList) ~= "table" or #playerList == 0 then
		return
	end
