...
Bindables.MatchReady.Event:Connect(function(playerList, modeId)
	...
	local resolvedMode = modeId or getModeFromCount(#playerList)
	local started = beginMatch(playerList, resolvedMode)
	if not started then
		MatchmakingService.requeuePending(playerList, resolvedMode)
	end
end)
