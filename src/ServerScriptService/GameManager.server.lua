...
local function beginMatch(matchPayload)
	if state.phase ~= MatchPhase.Idle then
		return
	end

	state.players = matchPayload.players
	state.mode = matchPayload.modeId or "training"
	state.phase = MatchPhase.Selecting
...
Bindables.MatchReady.Event:Connect(function(matchPayload)
	if typeof(matchPayload) ~= "table" or typeof(matchPayload.players) ~= "table" then
		return
	end
	beginMatch(matchPayload)
end)

print("[GameManager] Match system ready (queue-driven)")
