...
local function getModeFromCount(count, modeId)
	if modeId then
		return modeId
	end
	if count >= 3 then
	...
	for _, player in state.players do
		if player.Parent then
			Remotes.MatchState:FireClient(player, {
				phase = phase,
				mode = getModeFromCount(#state.players, state.matchModeId),
...
local function startFighting()
	...
	local mode = getModeFromCount(#state.players, state.matchModeId)
...
MatchReady.Event:Connect(function(payload)
	...
	beginMatch(activePlayers, payload.modeId)
end)
