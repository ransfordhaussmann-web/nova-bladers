...
local function startFighting()
	...
	if getActiveModeId() == "training" then
...
local function beginMatch(matchData)
	if state.phase ~= MatchPhase.Idle then
		return
	end

	local playerList = {}
	for _, player in matchData.players do
		if player.Parent then
	...
	state.modeId = matchData.modeId or "training"
	state.phase = MatchPhase.Selecting
