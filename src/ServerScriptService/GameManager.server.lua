	...
	for _, player in state.players do
		if player.Parent then
			Remotes.MatchState:FireClient(player, {
				phase = phase,
				mode = state.mode or getModeFromCount(#state.players),
...
local function startFighting()
	...
	local mode = state.mode or getModeFromCount(#state.players)
...
local function beginMatch(playerList, mode)
	state.players = playerList
	state.mode = mode or getModeFromCount(#playerList)
	state.phase = MatchPhase.Selecting
...
Bindables.MatchReady.Event:Connect(function(payload)
	...
	if state.phase ~= MatchPhase.Idle then
		for _, player in payload.players or {} do
			HubService.returnPlayerToHub(player)
		end
		MatchStateService.setBusy(false)
		return
	end
	if typeof(payload.players) ~= "table" or #payload.players == 0 then
		return
	end
	beginMatch(payload.players, payload.mode)
end)
