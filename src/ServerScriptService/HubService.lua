...
function HubService.setQueued(player, modeId)
	if handlers.setQueued then
		handlers.setQueued(player, modeId)
	end
end

function HubService.clearQueued(player)
	if handlers.clearQueued then
		handlers.clearQueued(player)
	end
end
