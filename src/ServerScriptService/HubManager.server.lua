...
local function enterQueuePhase(player)
...
local function setPhase(player, phase)
	...
	elseif phase == "queue" then
		enterQueuePhase(player)
	elseif phase == "arena" then
...
hub.portalPrompt.Triggered:Connect(function(player)
	local modeId = MatchmakingService.resolveModeForServer()
...
MatchmakingService.init(HubService)
...
print("[HubManager] 3D Hub ready — walk to mode pads or Arena Portal to queue")
