...
local function onJoinQueue(player, modeId)
	...
	MatchmakingBridge.joinQueue(player, resolvedMode)
end
...
MatchReady.Event:Connect(function(players)
	for _, matchPlayer in players do
		if matchPlayer.Parent then
			leaveHubForArena(matchPlayer)
		end
	end
end)
...
print("[HubManager] 3D Hub ready — Mode-Pads oder Portal für Warteschlange")
