...
local function updateModePads()
	local recommendedId = getRecommendedModeId()
	for _, pad in hub.modePads do
		pad.setActive(pad.config.id == recommendedId)
	end
end
...
local function buildLobbyPayload(player)
	...
	local queuedModeId = MatchmakingService.getPlayerQueue(player)
	...
	return {
		...
		losses = data.Losses,
		rank = rank,
		modeLabel = getModeLabel(activeModeId),
...
local function enterHub(player)
	...
	HubState:FireClient(player, {
		phase = "hub",
		modeLabel = getModeLabel(getRecommendedModeId()),
	})
	ReturnToHub:FireClient(player)
end

local function leaveHubForArena(player, modeId)
	if playerPhase[player] == "arena" then
		return
	end
	playerPhase[player] = "arena"
	local labelMode = modeId or getRecommendedModeId()
	HubState:FireClient(player, { phase = "arena", modeLabel = getModeLabel(labelMode) })
end
