...
local function onEnterArena(player)
	joinModeQueue(player, getActiveModeId())
end
...
for _, pad in hub.modePads do
	...
	pad.part.Touched:Connect(function(hit)
		...
		local now = os.clock()
		local last = padDebounce[touchPlayer]
		if last and now - last < MatchmakingConfig.PAD_TOUCH_DEBOUNCE then
			return
		end
		padDebounce[touchPlayer] = now

		joinModeQueue(touchPlayer, modeId)
	end)
end
