...
MatchmakingService.start(getActiveModeId)
...
local function joinQuickMatch(player)
	...
	MatchmakingService.joinQueue(player, "quick", getActiveModeId)
end

hub.portalPrompt.Triggered:Connect(joinQuickMatch)
...
for _, pad in hub.modePads do
	pad.queuePrompt.Triggered:Connect(function(player)
		...
		MatchmakingService.joinQueue(player, pad.config.id, getActiveModeId)
	end)
end

Remotes.EnterArena.OnServerEvent:Connect(joinQuickMatch)
