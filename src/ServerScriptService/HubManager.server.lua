...
local function getModeLabel()
	local mode = MatchModes.resolveFromPlayerCount(#Players:GetPlayers())
...
local function onQueueJoin(player, modeId)
	...
	if modeId == "auto" or modeId == nil then
		joined = MatchmakingService.joinAuto(player)
	else
		joined = MatchmakingService.join(player, modeId)
	end
...
for _, pad in hub.modePads do
	pad.prompt.Triggered:Connect(function(player)
		onQueueJoin(player, pad.config.id)
	end)
end
...
print("[HubManager] 3D Hub + Matchmaking ready")
