...
local function requestQueueJoin(player, modeId)
	if playerPhase[player] == "arena" then
		return
	end
...
hub.portalPrompt.Triggered:Connect(function(player)
	requestQueueJoin(player, nil)
end)

for _, pad in hub.modePads do
	pad.joinPrompt.Triggered:Connect(function(player)
		requestQueueJoin(player, pad.config.id)
	end)
end
...
MatchmakingService.init({
	remotes = Remotes,
	onMatchStart = function(playerList)
...
print("[HubManager] 3D Hub ready — Portal / Mode-Pads / Schnell-Match für Warteschlange")
