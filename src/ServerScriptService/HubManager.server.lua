...
local function leaveHubForArena(player, modeLabel)
	...
	HubState:FireClient(player, {
		phase = "arena",
		modeLabel = modeLabel or getModeLabel(),
	})
end
...
local function onJoinQueue(player, modeId)
	...
	MatchmakingService.joinQueue(player, modeId or MatchmakingService.getRecommendedMode())
end

hub.portalPrompt.Triggered:Connect(function(player)
	onJoinQueue(player, MatchmakingService.getRecommendedMode())
end)
...
print("[HubManager] 3D Hub ready — use Mode-Pads or Portal to join queue")
