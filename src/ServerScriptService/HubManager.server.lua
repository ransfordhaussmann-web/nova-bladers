...
local function enterQueuePhase(player, modeId)
	playerPhase[player] = "queue"
	local mode = HubConfig.MODE_PADS
	local label = getModeLabel()
	for _, padConfig in mode do
		if padConfig.id == modeId then
			label = "Warteschlange: " .. padConfig.label
			break
		end
	end
	HubState:FireClient(player, { phase = "queue", modeLabel = label, queueModeId = modeId })
end

local function leaveQueuePhase(player)
...
local function onEnterArena(player)
	onJoinQueue(player, MatchmakingService.getRecommendedModeId())
end
...
HubService.register({
	...
	getPhase = getPhase,
	enterQueue = enterQueuePhase,
	leaveQueue = leaveQueuePhase,
