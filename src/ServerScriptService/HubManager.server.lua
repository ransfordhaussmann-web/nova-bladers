...
local function joinActiveQueue(player)
	MatchmakingService.joinQueue(player, getActiveModeId(), leaveHubForArena)
end
...
for _, pad in hub.modePads do
	pad.prompt.Triggered:Connect(function(player)
		MatchmakingService.joinQueue(player, pad.config.id, leaveHubForArena)
	end)
end
