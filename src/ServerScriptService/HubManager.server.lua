...
local function getActiveModeId()
	return MatchCoordinator.resolveAutoMode(#Players:GetPlayers())
end

local function getModeLabel(modeId)
	local mode = MatchmakingConfig.getMode(modeId or getActiveModeId())
	if mode then
...
local function enterQueue(player, modeId)
	...
	if not MatchmakingService.joinQueue(player, modeId) then
		return
	end
...
local function leaveHubForMatch(player, modeId)
