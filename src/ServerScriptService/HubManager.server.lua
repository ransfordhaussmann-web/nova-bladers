...
local function getModeLabel()
	local count = #Players:GetPlayers()
	if count >= 3 then
		return "Modus: FFA (Portal)"
	elseif count == 2 then
		return "Modus: 1v1 PvP (Portal)"
	end
	return "Modus: Training (Portal)"
end
...
local function markQueued(player, modeId)
	...
	HubState:FireClient(player, { phase = "queued", modeId = modeId, modeLabel = getModeLabel() })
end
...
hub.portalPrompt.Triggered:Connect(function(player)
	onJoinQueue(player, MatchmakingManager.getDefaultModeId())
end)
...
print("[HubManager] 3D Hub ready — Mode-Pads oder Arena-Portal für Queue")
