...
local function getModeLabel(modeId)
	local mode = MatchmakingConfig.MODES[modeId or getActiveModeId()]
	if mode then
...
local function enterQueue(player, modeId)
	...
	HubState:FireClient(player, { phase = "queue", modeLabel = getModeLabel(modeId) })
	MatchmakingService.joinQueue(player, modeId)
end

local function enterArena(player, modeId)
	playerPhase[player] = "arena"
	HubState:FireClient(player, { phase = "arena", modeLabel = getModeLabel(modeId) })
end
