...
local function getActiveModeId()
	...
	return MatchModes.resolveQuickMatch(count)
end
...
local function buildLobbyPayload(player)
	...
	return {
		...
		queueModeId = MatchmakingService.getQueueMode(player),
	}
end
...
local function setPlayerQueued(player, modeId)
...
local function joinQueue(player, modeId)
	...
	local resolvedMode = modeId or MatchModes.resolveQuickMatch(#Players:GetPlayers())
...
MatchmakingService.init({
	...
	onMatchReady = function(modeId, matchedPlayers)
		...
		MatchReady:Fire(modeId, matchedPlayers)
	end,
})

hub.portalPrompt.Triggered:Connect(function(player)
	joinQueue(player, MatchModes.resolveQuickMatch(#Players:GetPlayers()))
end)
