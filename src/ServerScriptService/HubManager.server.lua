...
local function getActiveModeId()
	...
	return MatchModes.resolveQuickMode(count)
end

local function getModeLabel()
	local activeId = getActiveModeId()
	local mode = MatchModes.get(activeId)
	return "Empfohlen: " .. (mode and mode.label or "Training")
end
...
local function joinQuickMatch(player)
	MatchmakingService.joinQueue(player, "quick")
end
...
EnterArena.OnServerEvent:Connect(joinQuickMatch)
...
HubService.register({
	...
	getPhase = getPhase,
	onMatchStarting = leaveHubForArena,
})
