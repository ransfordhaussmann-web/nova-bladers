...
local function onJoinAutoQueue(player)
	if playerPhase[player] == "arena" then
		return
	end
...
HubService.register({
	...
	getPhase = getPhase,
	prepareForMatchmaking = leaveHubForArena,
})
...
print("[HubManager] 3D Hub ready — Portal / Mode-Pads starten die Matchmaking-Queue")
