...
local function enterArena(player)
	local state = playerState[player]
	if not state or not state.inHub then
		return
	end
...
local function handleInteract(player)
	local state = playerState[player]
	if not state or not state.inHub or not state.activeZone then
		return
	end
...
local function setupPlayer(player)
	...
	if player.Character and playerState[player].inHub then
		teleportCharacter(player, HubConfig.SPAWN)
	end
