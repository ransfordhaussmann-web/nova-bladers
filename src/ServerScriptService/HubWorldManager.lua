...
local playerInArena: { [Player]: boolean } = {}

local function getRemotes()
	return ReplicatedStorage:WaitForChild("NovaBladers"):WaitForChild("Remotes")
end

local function createSign(parent: BasePart, text: string, color: Color3)
...
local function createZone(zoneConfig, origin: Vector3, onTriggered)
...
function HubWorldManager.isInArena(player: Player)
	return playerInArena[player] == true
end

function HubWorldManager.teleportToHub(player: Player)
...
function HubWorldManager.init(onZoneAction)
	...
	spawnPart = hubFolder:FindFirstChild("HubSpawn") :: BasePart
	...
	if arena and arena:IsA("Model") then
		for _, descendant in arena:GetDescendants() do
			if descendant:IsA("BasePart") then
				descendant:SetAttribute("HubHidden", descendant.Transparency)
			end
		end
	end
...
function HubWorldManager.onPlayerAdded(player: Player, onReady)
	playerInArena[player] = false
