...
local function resolveArenaSpawn()
	...
	if arena then
		local spawn = arena:FindFirstChild("ArenaSpawn", true)
			or arena:FindFirstChild("Spawn", true)
	...
	local rootSpawn = workspace:FindFirstChild("ArenaSpawn")
	if rootSpawn and rootSpawn:IsA("BasePart") then
		return rootSpawn.CFrame + Vector3.new(0, 3, 0)
	end
