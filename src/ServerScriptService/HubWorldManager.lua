...
local function findArenaSpawn()
	...
	if bowl then
		local bowlSpawn = bowl:FindFirstChild("Spawn")
		if bowlSpawn and bowlSpawn:IsA("BasePart") then
			return bowlSpawn.CFrame + Vector3.new(0, 3, 0)
		end
		if bowl:IsA("BasePart") then
			return bowl.CFrame + Vector3.new(0, 5, 0)
		end
	end
	local fallback = arena:FindFirstChildWhichIsA("BasePart")
	if fallback then
		return fallback.CFrame + Vector3.new(0, 5, 0)
	end
	return nil
end
