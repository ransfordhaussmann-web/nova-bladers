...
local function buildPathways(parent)
	...
	for _, def in pairs(ZONE_DEFS) do
...
local function resolveArenaSpawnCFrame()
	local arena = Workspace:FindFirstChild("Arena")
	if arena then
		local spawn = arena:FindFirstChild(ARENA_SPAWN_NAME)
		if spawn and spawn:IsA("BasePart") then
			return spawn.CFrame + Vector3.new(0, 3, 0)
		end
	end
	local spawn = Workspace:FindFirstChild(ARENA_SPAWN_NAME)
	if spawn and spawn:IsA("BasePart") then
		return spawn.CFrame + Vector3.new(0, 3, 0)
	end
	return CFrame.new(0, 6, 80)
end
...
function HubWorldManager.build()
	...
	arenaSpawnCFrame = resolveArenaSpawnCFrame()
