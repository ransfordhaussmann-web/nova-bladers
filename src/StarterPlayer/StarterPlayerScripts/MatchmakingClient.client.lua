...
local function showQueue(payload)
	...
	local lines = { payload.message or "In Warteschlange…" }
	if payload.queueSize and payload.minPlayers then
		table.insert(lines, string.format("Spieler: %d / %d", payload.queueSize, payload.maxPlayers or payload.minPlayers))
	end
...
Remotes.HubState.OnClientEvent:Connect(function(state)
	if state.phase == "arena" and not gui.Enabled then
		return
	end
