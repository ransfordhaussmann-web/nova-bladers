...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	local lines = { payload.message or "Suche Match..." }
	if payload.queueSize and payload.minPlayers then
		table.insert(lines, string.format("Spieler: %d / %d", payload.queueSize, payload.minPlayers))
	end
	if payload.status == "pending" then
		table.insert(lines, "⏳ Arena noch belegt")
	end
