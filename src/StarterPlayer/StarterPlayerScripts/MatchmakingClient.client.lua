...
local function applyQueueUpdate(payload)
	if payload.status == "left" or payload.status == "starting" or not payload.inQueue then
		gui.Enabled = false
		return
	end
	...
	local playerText = formatPlayerList(payload.players or {})
	local statusText = countText .. "\n" .. playerText
