...
local function statusText(payload)
	...
	elseif payload.status == "filling" and payload.fillTimeLeft then
		return string.format("Lobby füllt sich… (%ds)", payload.fillTimeLeft)
	end
...
local function applyUpdate(payload)
	...
	statusLabel.Text = string.format(
		"%s\n%d / %d Spieler\n%s",
		statusText(payload),
		payload.currentPlayers or 0,
		payload.maxPlayers or 0,
		payload.queuedNames and #payload.queuedNames > 0
			and table.concat(payload.queuedNames, ", ")
			or ""
	)
end
