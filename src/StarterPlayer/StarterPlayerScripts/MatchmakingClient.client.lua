...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d / %d Spieler", payload.modeLabel, payload.waiting, payload.maxPlayers),
	}
	if payload.status == "pending" then
		lines[#lines + 1] = "Arena belegt — warte auf freien Slot"
	elseif payload.waiting < payload.minPlayers then
		lines[#lines + 1] = string.format("Warte auf %d Spieler...", payload.minPlayers - payload.waiting)
	elseif payload.fillTimeout then
		lines[#lines + 1] = string.format("Start in max. %ds", payload.fillTimeout)
	else
		lines[#lines + 1] = "Match startet gleich..."
	end
