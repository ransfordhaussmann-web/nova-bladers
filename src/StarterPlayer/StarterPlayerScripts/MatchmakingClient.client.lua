...
local function statusText(payload)
	local lines = {
		string.format("%s — %d / %d Spieler", payload.modeLabel, payload.players, payload.minPlayers),
	}

	if payload.status == MatchState.QueueStatus.Pending then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.status == MatchState.QueueStatus.Filling and payload.fillRemaining then
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	if not payload or payload.players == 0 then
		gui.Enabled = false
		return
	end
