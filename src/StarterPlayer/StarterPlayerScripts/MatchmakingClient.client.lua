...
local function formatStatus(payload)
	...
	if payload.status == "pending" or payload.arenaBusy then
		return string.format("%s\nArena belegt — warte auf freien Slot…", countText)
	end
	if payload.status == "filling" then
		return string.format("%s\nMatch startet bald…", countText)
	end
	if payload.status == "ready" or payload.status == "full" then
		return string.format("%s\nMatch wird vorbereitet…", countText)
	end
