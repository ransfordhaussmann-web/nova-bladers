...
local function showQueue(payload)
	...
	local lines = {
		string.format("%d / %d Spieler", payload.players or 0, payload.required or 1),
	}
	...
	if payload.pending then
		table.insert(lines, "Arena belegt — du bist dran, sobald das Match endet")
	elseif payload.status == "pending" then
		table.insert(lines, "Match bereit — warte auf freie Arena")
	end
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	if payload.status == "waiting" or payload.status == "pending" or payload.inQueue then
		showQueue(payload)
	else
		hideQueue()
	end
