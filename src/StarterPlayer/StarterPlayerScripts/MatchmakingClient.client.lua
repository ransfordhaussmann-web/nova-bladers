...
local function formatStatus(payload)
	...
	local countText = string.format("%d / %d Spieler", payload.players, payload.maxPlayers)
	local lines = {
		payload.modeLabel or "Queue",
		countText,
		payload.statusLabel or "",
	}

	if payload.status == "pending" then
		lines[3] = "Arena belegt — du bist als Nächstes dran"
	elseif payload.status == "filling" then
		lines[3] = "Warte auf weitere Spieler…"
	elseif payload.status == "starting" then
		lines[3] = "Match startet!"
	end
