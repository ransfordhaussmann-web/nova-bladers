...
local function showQueue(payload)
	...
	if payload.status == "pending" then
		status.Text = payload.message or "Arena belegt — du startest als Nächstes"
	elseif payload.status == "queued" then
		local line = payload.message or "Suche Mitspieler..."
		if payload.total and payload.needed then
			line = line .. string.format("\n(%d/%d)", payload.total, payload.maxPlayers or payload.needed)
		end
		status.Text = line
	else
