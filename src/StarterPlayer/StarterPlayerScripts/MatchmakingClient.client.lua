...
local function showQueue(payload)
	...
	local lines = { payload.message or "" }
	if payload.status == "pending" then
		table.insert(lines, "Match läuft noch...")
	elseif payload.status == "starting" then
		table.insert(lines, "Bereit zum Kampf!")
	else
		table.insert(lines, string.format("Max. %d Spieler", payload.maxPlayers or 0))
	end
