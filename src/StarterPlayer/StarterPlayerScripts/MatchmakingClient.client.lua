...
local function showQueue(payload)
	...
	local lines = {
		string.format("%d / %d Spieler", payload.players or 0, payload.maxPlayers or 1),
	}
	if payload.pending then
		table.insert(lines, payload.status or "Warte auf freie Arena…")
	else
		table.insert(lines, payload.status or "Suche Mitspieler…")
	end
