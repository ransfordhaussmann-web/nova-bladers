...
local function statusText(payload)
	local lines = {
		string.format("Modus: %s", payload.label),
		string.format("Spieler: %d / %d", payload.total, payload.maxPlayers),
	}

	if payload.status == "pending" then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.status == "ready" and payload.fillSecondsLeft then
		table.insert(lines, string.format("Start in %ds", payload.fillSecondsLeft))
	elseif payload.total < payload.minPlayers then
