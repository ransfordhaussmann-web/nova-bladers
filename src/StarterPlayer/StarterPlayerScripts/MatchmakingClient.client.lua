...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d/%d Spieler", payload.modeLabel or "Queue", payload.playersInQueue or 0, payload.maxPlayers or 1),
	}

	if payload.status == "pending" then
		table.insert(lines, "Arena belegt — warte auf freien Slot...")
	elseif payload.status == "ready" and payload.secondsLeft then
		table.insert(lines, string.format("Start in %ds (oder bei %d Spielern)", payload.secondsLeft, payload.maxPlayers))
	elseif payload.playersInQueue < payload.minPlayers then
		table.insert(lines, string.format("Warte auf %d Spieler...", payload.minPlayers - payload.playersInQueue))
	end
