...
local function showQueue(payload)
	...
	local lines = {
		string.format("Spieler: %d / %d", payload.queueSize or 1, payload.maxPlayers or 1),
	}

	if payload.pending then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.modeId == "ffa" and payload.queueSize and payload.minPlayers and payload.queueSize >= payload.minPlayers then
		table.insert(lines, "Start in Kürze...")
	else
