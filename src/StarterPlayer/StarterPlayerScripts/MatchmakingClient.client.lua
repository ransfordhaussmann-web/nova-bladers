...
local function showQueue(snapshot)
	if not snapshot.inQueue then
		hideQueue()
		return
	end

	gui.Enabled = true
	title.Text = string.format("Queue: %s", snapshot.modeLabel or snapshot.modeId or "?")

	local lines = {
		string.format("Spieler in Queue: %d / %d", snapshot.queueSize or 0, snapshot.maxPlayers or 0),
	}

	if snapshot.pending then
		table.insert(lines, "Arena belegt — warte auf freien Slot...")
	elseif snapshot.queueSize and snapshot.needed and snapshot.queueSize < snapshot.needed then
		table.insert(lines, string.format("Noch %d Spieler bis Start", snapshot.needed - snapshot.queueSize))
	else
