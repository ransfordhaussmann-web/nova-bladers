...
panel.Size = UDim2.fromOffset(320, 160)
...
playersLabel.Size = UDim2.new(1, -16, 0, 48)
...
local function formatStatus(payload)
	...
	if payload.fillTimer and payload.fillTimer > 0 then
...
local function formatPlayers(payload)
	local lines = {}
	for i, entry in payload.players do
		local prefix = if entry.userId == player.UserId then "• Du" else string.format("• %s", entry.name)
