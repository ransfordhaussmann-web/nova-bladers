...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d / %d Spieler", payload.modeLabel or "Queue", payload.count or 0, payload.maxPlayers or 0),
	}
	...
	elseif payload.fillRemaining and payload.fillRemaining > 0 then
		table.insert(lines, string.format("Start in ca. %ds", payload.fillRemaining))
	else
