...
local function showQueue(payload)
	...
	title.Text = string.format("Queue: %s", payload.modeLabel or payload.mode or "?")
	local lines = { payload.message or "Warte auf Match..." }
	if payload.queueSize and payload.required then
		table.insert(lines, string.format("Spieler: %d / %d", payload.queueSize, payload.required))
	end
	if payload.status == "pending" then
		table.insert(lines, "Status: Arena belegt")
	end
