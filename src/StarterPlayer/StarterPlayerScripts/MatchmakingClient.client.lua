...
local function showQueue(payload)
	...
	local countText = string.format("%d / %d Spieler", payload.players or 0, payload.needed or 1)
	if payload.maxPlayers and payload.maxPlayers > payload.needed then
		countText = string.format("%d / %d–%d Spieler", payload.players or 0, payload.needed, payload.maxPlayers)
	end
	...
	elseif payload.fillTimeout then
		status.Text = countText .. string.format("\nStart in %ds (oder bei %d Spielern)", payload.fillTimeout, payload.maxPlayers or payload.needed)
	else
