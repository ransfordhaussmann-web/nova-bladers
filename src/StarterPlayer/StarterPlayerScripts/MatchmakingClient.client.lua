...
panel.Size = UDim2.fromOffset(300, 88)
...
local function showQueue(payload)
	...
	if payload.pending then
		title.Text = "Warteschlange — Arena belegt"
	else
		title.Text = string.format("Warteschlange — %s", mode.label)
	end

	local lines = {
		string.format("%d / %d Spieler", mode.count, mode.maxPlayers),
	}
	if mode.fillTimeout and mode.count >= mode.minPlayers and not payload.pending then
		table.insert(lines, string.format("Start in max. %ds", mode.fillTimeout))
	end
	if payload.pending then
