...
panel.Position = UDim2.new(0.5, -120, 0, 16)
...
local function formatStatus(payload)
	if payload.status == "idle" or not payload.status then
		return nil
	end

	if payload.status == "starting" then
		return string.format("Starte %s …", payload.modeLabel or "Match")
	end

	if payload.status == "pending" then
		return string.format(
			"%s — Warteschlange\nArena belegt · %d/%d Spieler",
