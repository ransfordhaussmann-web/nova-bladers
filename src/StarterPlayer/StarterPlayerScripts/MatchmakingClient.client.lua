...
local function statusText(payload)
	...
	local lines = {
		string.format("%s — %d / %d Spieler", payload.label, payload.count, payload.max),
	}
	if payload.timeRemaining and payload.timeRemaining > 0 then
		table.insert(lines, string.format("Start in %ds", payload.timeRemaining))
	else
