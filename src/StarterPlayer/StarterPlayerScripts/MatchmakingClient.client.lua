...
local statusLabel = panel.Status
...
local function showQueue(payload)
	gui.Enabled = true
	titleLabel.Text = string.format("Warteschlange — %s", payload.modeLabel or "Arena")

	local statusText
	if payload.status == MatchmakingConfig.QUEUE_STATUS.Pending or payload.arenaBusy then
		statusText = string.format(
			"Arena belegt — %d/%d Spieler warten",
			payload.queued or 0,
			payload.required or 1
		)
	elseif payload.status == MatchmakingConfig.QUEUE_STATUS.Starting then
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	if payload.error == "queue_full" then
		statusLabel.Text = "Queue ist voll — versuche es gleich erneut"
		gui.Enabled = true
