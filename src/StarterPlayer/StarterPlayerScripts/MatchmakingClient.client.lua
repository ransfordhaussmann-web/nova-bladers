...
local function formatStatus(payload)
	if payload.pending or payload.arenaBusy then
		return "Arena belegt — dein Match startet gleich..."
	end

	local lines = {
		string.format("%s (%d/%d)", payload.modeLabel or "Queue", payload.queueSize or 0, payload.maxPlayers or 0),
	}
	...
	elseif payload.queueSize and payload.minPlayers and payload.queueSize < payload.minPlayers then
		table.insert(lines, string.format("Warte auf %d Spieler...", payload.minPlayers - payload.queueSize))
	else
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	gui.Enabled = true
	title.Text = payload.pending and "Match bereit" or "In Queue"
