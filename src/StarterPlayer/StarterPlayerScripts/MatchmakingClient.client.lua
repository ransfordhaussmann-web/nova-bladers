...
local function updateQueue(payload)
	...
	gui.Enabled = true
	title.Text = string.format("Queue: %s", payload.modeLabel or payload.modeId)

	local lines = {
		string.format("Spieler: %d / %d", payload.playersWaiting or 0, payload.maxPlayers or 1),
	}
	if payload.pending then
		table.insert(lines, "Arena belegt — warte...")
	elseif (payload.playersWaiting or 0) < (payload.minPlayers or 1) then
		table.insert(lines, string.format("Noch %d Spieler nötig", (payload.minPlayers or 1) - (payload.playersWaiting or 0)))
	else
