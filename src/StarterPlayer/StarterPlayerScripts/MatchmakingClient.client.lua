...
local function formatStatus(payload)
	...
	elseif payload.status == "full" then
		table.insert(lines, "Queue voll — Start wird vorbereitet...")
	elseif payload.count < payload.minPlayers then
		table.insert(lines, string.format("Noch %d Spieler benötigt", payload.minPlayers - payload.count))
	else
		table.insert(lines, "Startbereit — warte auf weitere Spieler oder Timeout")
	end
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	gui.Enabled = true
	title.Text = "In Queue: " .. payload.modeLabel
...
Remotes.HubState.OnClientEvent:Connect(function(state)
	if state.phase == "queue" then
		gui.Enabled = true
		if state.queueModeLabel then
			title.Text = "In Queue: " .. state.queueModeLabel
		end
