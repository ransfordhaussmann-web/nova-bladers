...
playersLabel.Position = UDim2.fromOffset(8, 76)
...
local function showQueue(payload)
	...
	if payload.status == "pending" then
		statusLabel.Text = "Arena belegt — du bist als Nächster dran.\n" .. countText
	elseif (payload.count or 0) < (payload.minPlayers or 1) then
		statusLabel.Text = "Warte auf Mitspieler...\n" .. countText
	else
		local timeout = payload.fillTimeout
		if timeout then
			statusLabel.Text = string.format("Start in max. %ds...\n%s", timeout, countText)
		else
			statusLabel.Text = "Match startet gleich...\n" .. countText
		end
	end
