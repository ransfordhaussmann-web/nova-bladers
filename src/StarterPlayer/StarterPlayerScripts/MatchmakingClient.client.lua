...
local function showQueue(payload)
	...
	if payload.status == "pending" then
		status.Text = string.format(
			"Arena belegt — %d Spieler warten (%s)",
	...
	elseif payload.status == "waiting" then
		...
		if (payload.playersNeeded or 0) > 0 then
			table.insert(lines, string.format("Noch %d benötigt", payload.playersNeeded))
		end
		if payload.fillTimeout then
			table.insert(lines, string.format("Start in %ds", payload.fillTimeout))
		end
