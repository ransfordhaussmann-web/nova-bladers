...
local function statusText(payload)
	...
	elseif payload.status == "filling" and payload.fillRemaining then
		table.insert(lines, string.format("Start in %ds (Plätze werden gefüllt)", payload.fillRemaining))
	elseif payload.status == "waiting" then
		table.insert(lines, string.format("Warte auf %d Spieler", payload.minPlayers - payload.queued))
	else
