...
local function formatStatus(payload)
	...
	if payload.status == "waiting" then
		...
		if payload.fillSecondsLeft and payload.fillSecondsLeft > 0 then
			table.insert(lines, string.format("Start in %ds (max. %d)", payload.fillSecondsLeft, payload.maxPlayers or 6))
		end
		return table.concat(lines, "\n")
	end
	return "Queue verlassen"
end
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	if payload.status == "starting" or payload.inQueue then
		title.Text = "Matchmaking — " .. (payload.modeLabel or "")
