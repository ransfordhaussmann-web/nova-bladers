...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d/%d Spieler", payload.modeLabel, payload.count, payload.minPlayers),
	}

	if payload.status == "pending" then
		table.insert(lines, "Arena belegt — warte auf freien Slot...")
	elseif payload.status == "filling" and payload.secondsLeft then
		table.insert(lines, string.format("Start in %ds (min. %d)", payload.secondsLeft, payload.minPlayers))
	else
		table.insert(lines, string.format("Warte auf %d Spieler...", math.max(0, payload.minPlayers - payload.count)))
	end
