...
local function formatStatus(payload)
	local lines = {
		...
		string.format("Spieler: %d / %d", payload.total or 0, payload.needed or 0),
	}

	if payload.status == "pending" or payload.arenaBusy then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.total and payload.needed and payload.total >= payload.needed then
		table.insert(lines, "Match startet gleich...")
	else
		local missing = math.max(0, (payload.needed or 0) - (payload.total or 0))
		table.insert(lines, string.format("Noch %d Spieler nötig", missing))
	end
