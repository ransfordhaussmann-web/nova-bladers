...
local function statusText(payload)
	...
	if payload.status == "filling" and payload.fillEndsAt then
		local remaining = math.max(0, math.ceil(payload.fillEndsAt - Workspace:GetServerTimeNow()))
		return string.format(
			"%s\nMatch startet in %ds…\nSpieler: %d / %d",
			payload.modeLabel,
			remaining,
...
task.spawn(function()
	while true do
		task.wait(0.5)
		if gui.Enabled and lastPayload and lastPayload.status == "filling" then
			statusLabel.Text = statusText(lastPayload)
		end
	end
end)
