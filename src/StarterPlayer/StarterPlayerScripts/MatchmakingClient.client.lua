...
local function showQueue(payload)
	gui.Enabled = true
	title.Text = string.format("Queue: %s", payload.modeLabel or "Match")

	local lines = {
		string.format("Spieler: %d / %d", payload.players or 1, payload.maxPlayers or 1),
	}

	if payload.pending then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.fillSeconds then
