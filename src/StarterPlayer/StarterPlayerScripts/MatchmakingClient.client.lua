...
playersLabel.Size = UDim2.new(1, -120, 0, 56)
...
local function applyQueueUpdate(payload)
	...
	local names = payload.players or {}
	local playerLines = {}
	for i, name in names do
		table.insert(playerLines, string.format("%d. %s", i, name))
	end
	if #playerLines > 0 then
		playersLabel.Text = table.concat(playerLines, "\n")
	else
		playersLabel.Text = "Noch keine Spieler"
	end
