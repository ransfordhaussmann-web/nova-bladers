...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d/%d Spieler", payload.modeLabel or "Modus", payload.players or 0, payload.maxPlayers or 1),
	}

	if payload.status == "pending" then
		table.insert(lines, "Arena belegt — warte auf freien Slot…")
	elseif payload.secondsLeft then
		table.insert(lines, string.format("Start in %ds (oder bei %d Spielern)", payload.secondsLeft, payload.maxPlayers))
	else
		table.insert(lines, string.format("Warte auf %d Spieler…", math.max(0, (payload.minPlayers or 1) - (payload.players or 0))))
	end
