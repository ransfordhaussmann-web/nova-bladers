...
local function formatStatus(payload)
	...
	elseif payload.fillEndsAt then
...
RunService.Heartbeat:Connect(function()
	if not currentPayload or not currentPayload.inQueue or not gui.Enabled then
		return
	end
	if currentPayload.fillEndsAt or currentPayload.pendingArena then
		status.Text = formatStatus(currentPayload)
	end
end)
