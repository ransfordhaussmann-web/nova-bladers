...
local function formatStatus(payload)
	...
	elseif payload.playersNeeded and (payload.playersInQueue or 0) < payload.playersNeeded then
		table.insert(lines, string.format("Noch %d Spieler nötig", payload.playersNeeded - (payload.playersInQueue or 0)))
	else
		table.insert(lines, "Bereit — warte auf Matchstart")
	end
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	if not payload or payload.inQueue ~= true then
		gui.Enabled = false
		return
	end
