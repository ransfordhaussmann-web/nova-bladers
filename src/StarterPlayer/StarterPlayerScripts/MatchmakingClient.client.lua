...
local function formatQueueNames(names)
...
local function render(payload)
	...
	gui.Enabled = true
	title.Text = string.format("Warteschlange — %s", payload.modeLabel or "?")

	local lines = {
		string.format("%d / %d Spieler", payload.queue and payload.queue.count or 0, payload.queue and payload.queue.maxPlayers or 0),
		string.format("Platz: %d", payload.position or 1),
		"Spieler: " .. formatQueueNames(payload.queue and payload.queue.playerNames),
	}
...
Remotes.QueueUpdate.OnClientEvent:Connect(render)
