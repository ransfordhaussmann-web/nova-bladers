...
local function showQueue(payload)
	...
	if payload.fillRemaining then
		lines[#lines + 1] = string.format("Start in ~%ds", payload.fillRemaining)
	end

	if payload.pending then
		lines[#lines + 1] = "⏳ Arena belegt — warte..."
	end

	if payload.playerNames and #payload.playerNames > 0 then
		lines[#lines + 1] = table.concat(payload.playerNames, ", ")
	end
