...
local function showQueue(payload)
	gui.Enabled = true
	title.Text = "Queue: " .. (payload.modeLabel or payload.mode or "?")
	...
	local countText = string.format(
		...
		payload.playersNeeded or payload.minPlayers or 1
	)

	if payload.fillSecondsRemaining and payload.fillSecondsRemaining > 0 then
		countText ..= string.format(" · Start in %ds", payload.fillSecondsRemaining)
	end
