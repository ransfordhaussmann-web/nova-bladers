...
local function formatStatus(payload)
	...
	if players then
		lines[#lines + 1] = string.format(
			"Spieler: %d / %d (min. %d)",
			players.count or 0,
			players.max or players.required or 1,
			players.required or 1
		)
	end

	if payload.status == "pending" then
		lines[#lines + 1] = "Arena belegt — warte auf freien Slot"
	elseif payload.fillCountdown then
		lines[#lines + 1] = string.format("Start in %ds …", payload.fillCountdown)
	else
		lines[#lines + 1] = "Warte auf Mitspieler …"
	end
