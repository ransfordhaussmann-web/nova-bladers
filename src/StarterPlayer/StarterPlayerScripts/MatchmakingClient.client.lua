...
local function applyPayload(payload)
	...
	local playerText = string.format("%d / %d Spieler", payload.players, payload.maxPlayers)
	local namesText = formatNames(payload.names or {})
	local statusText = playerText
	if namesText ~= "" then
		statusText = statusText .. "\n" .. namesText
	end

	if payload.status == "pending" then
		title.Text = "Arena belegt — warte..."
		statusText = "Dein Match startet, sobald die Arena frei ist.\n" .. statusText
	else
...
Remotes.QueueJoin.OnClientEvent:Connect(applyPayload)
