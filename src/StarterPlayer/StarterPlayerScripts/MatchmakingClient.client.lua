...
local function applyPayload(payload)
	...
	local countText = string.format("%d / %d Spieler", payload.waiting or 0, payload.maxPlayers or 0)
	if payload.status == "pending" then
		status.Text = (payload.message or "Arena belegt") .. "\n" .. countText
	elseif payload.status == "filling" then
		status.Text = (payload.message or "Start bald") .. "\n" .. countText
	else
