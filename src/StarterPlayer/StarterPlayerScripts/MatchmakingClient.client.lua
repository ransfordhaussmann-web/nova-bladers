...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d / %d Spieler", payload.label, payload.queued, payload.maxPlayers),
	}

	if payload.pending then
		lines[#lines + 1] = "Arena belegt — Match startet gleich..."
	elseif payload.arenaBusy then
		lines[#lines + 1] = "Arena belegt — warte auf freien Slot"
	end

	if payload.fillSecondsLeft and payload.fillSecondsLeft > 0 then
		lines[#lines + 1] = string.format("Start in %ds (oder bei %d Spielern)", payload.fillSecondsLeft, payload.maxPlayers)
	end
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	hideLobby()
	gui.Enabled = true
	title.Text = "Warteschlange — " .. payload.label
