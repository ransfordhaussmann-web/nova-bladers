...
local function formatStatus(payload)
	local lines = {
		...
		string.format("Spieler in Queue: %d / %d", payload.playersInQueue or 0, payload.maxPlayers or 1),
	}

	if payload.waitingForArena then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.fillDeadline and payload.modeId == "ffa" then
		table.insert(lines, "Startet bald mit aktueller Gruppe")
	else
		table.insert(lines, string.format("Benötigt min. %d Spieler", payload.minPlayers or 1))
	end
