...
local function showQueue(payload)
	...
	if payload.status == "pending" then
		status.Text = payload.reason or "Arena belegt — Warte auf freie Arena"
	elseif payload.status == "waiting" then
		local countText = string.format("%d / %d Spieler", payload.count or 0, payload.max or 0)
		if payload.fillSecondsLeft and payload.fillSecondsLeft > 0 then
			status.Text = string.format("%s\nStart in %ds", countText, payload.fillSecondsLeft)
		else
