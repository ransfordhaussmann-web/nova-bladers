...
local function formatStatus(payload)
	...
	local lines = {
		...
		string.format("Spieler: %d / %d", payload.players or 0, payload.max or 0),
	}
	...
	elseif payload.secondsLeft and payload.secondsLeft > 0 then
		...
		local current = payload.players or 0
		if current < needed then
			table.insert(lines, string.format("Noch %d Spieler nötig", needed - current))
		else
...
Remotes.HubState.OnClientEvent:Connect(function(state)
	if state.phase == "hub" and not status.Text:find("Match startet") then
		gui.Enabled = false
	end
end)
