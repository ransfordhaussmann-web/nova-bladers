...
local function statusText(payload)
	...
	if payload.queueStatus == "pending" or payload.arenaBusy then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.queueStatus == "filling" and payload.fillSecondsLeft then
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(playerPayload)
	if playerPayload.inQueue then
		showQueue(playerPayload)
	else
		hideQueue()
	end
end)
