...
local function statusText(payload)
	...
	local lines = {
		string.format("Spieler in Queue: %d / %d", payload.playersInQueue, payload.maxPlayers),
	}
	if payload.playersInQueue < payload.playersNeeded then
		table.insert(lines, string.format("Noch %d Spieler benötigt", payload.playersNeeded - payload.playersInQueue))
	end
...
local function applyQueueState(payload)
	...
	gui.Enabled = true
	title.Text = "Queue: " .. (payload.modeLabel or "?")
