...
local function formatStatus(payload)
	...
	local lines = {
		string.format("%s — %d/%d Spieler", payload.modeLabel or payload.modeId, payload.queued or 0, payload.maxPlayers or 1),
	}

	if payload.status == "pending" or payload.arenaBusy then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.status == "ready" then
		table.insert(lines, "Bereit — Match startet bald")
	else
