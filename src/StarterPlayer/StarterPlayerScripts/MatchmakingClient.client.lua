...
local function formatStatus(payload)
	...
	elseif payload.status == "filling" and payload.fillSecondsLeft then
		table.insert(lines, string.format("Start in %ds (oder bei %d Spielern)", math.ceil(payload.fillSecondsLeft), payload.maxPlayers or 6))
...
Remotes.HubState.OnClientEvent:Connect(function(state)
	...
	elseif state.phase == "hub" then
		-- queue UI re-shown via QueueUpdate if still queued
	end
end)
