...
local function formatStatus(payload)
	...
	elseif payload.fillSecondsLeft then
		table.insert(lines, string.format("Start in %ds (weitere Spieler können joinen)", payload.fillSecondsLeft))
	else
		table.insert(lines, string.format("Warte auf %d Spieler…", math.max(0, payload.minPlayers - payload.playersInQueue)))
	end
...
-- Expose join helper for other client scripts
local joinEvent = Instance.new("BindableEvent")
joinEvent.Name = "JoinRecommendedQueue"
joinEvent.Parent = gui

joinEvent.Event:Connect(function(modeId)
	QueueJoin:FireServer(modeId)
end)
