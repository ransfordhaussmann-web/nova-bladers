...
local function statusText(payload)
	...
	if payload.secondsLeft then
		return string.format("Warte auf Spieler (%ds)...", payload.secondsLeft)
	end
	return "Warte auf Spieler..."
end

local function updatePanel(payload)
	if not payload.inQueue or payload.status == "left" then
		gui.Enabled = false
		return
	end

	gui.Enabled = true
	title.Text = string.format("%s — %d/%d", payload.modeLabel or "Queue", payload.count or 0, payload.maxPlayers or 1)

	local lines = { statusText(payload) }
