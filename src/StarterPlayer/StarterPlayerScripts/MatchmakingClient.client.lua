...
local function formatStatus(payload)
	local lines = {
		string.format("%s — %d / %d Spieler", payload.modeLabel or "Match", payload.players or 0, payload.needed or 0),
	}

	if payload.status == MatchState.QueueStatus.Pending then
		table.insert(lines, "Arena belegt — warte...")
	elseif payload.maxPlayers and payload.players and payload.players < payload.maxPlayers then
