...
local function formatStatus(payload)
	local lines = {
		...
		string.format("Spieler in Queue: %d / %d", payload.playersInQueue or 0, payload.maxPlayers or 0),
	}

	if payload.status == "pending" then
		table.insert(lines, payload.pendingReason or "Arena belegt")
	else
		table.insert(lines, string.format("Benötigt: %d Spieler", payload.playersNeeded or 0))
	end
...
Remotes.QueueUpdate.OnClientEvent:Connect(function(payload)
	...
	title.Text = if payload.status == "pending" then "Warteschlange (Pending)" else "Warteschlange"
