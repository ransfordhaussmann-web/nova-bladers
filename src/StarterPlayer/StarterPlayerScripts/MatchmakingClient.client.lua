...
local function formatStatus(payload)
	local progress = string.format("%d / %d Spieler", payload.queueSize, payload.minPlayers)
	if payload.status == "pending" or payload.arenaBusy then
		return string.format("Arena belegt — Ausstehend\n%s", progress)
	end
	if payload.queueSize >= payload.minPlayers then
		return string.format("Match startet gleich…\n%s", progress)
	end
	return string.format("Suche Gegner…\n%s", progress)
end
