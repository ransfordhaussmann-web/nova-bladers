...
local function formatStatus(payload)
	...
	local lines = {
		string.format("%s: %d / %d Spieler", payload.modeLabel or "Modus", payload.playersWaiting or 0, payload.maxPlayers or 1),
	}
	if payload.secondsLeft then
		table.insert(lines, string.format("Start in %ds (oder bei %d Spielern)", payload.secondsLeft, payload.maxPlayers or 6))
	elseif payload.minPlayers and (payload.playersWaiting or 0) < payload.minPlayers then
		table.insert(lines, string.format("Noch %d Spieler nötig", payload.minPlayers - (payload.playersWaiting or 0)))
	end
