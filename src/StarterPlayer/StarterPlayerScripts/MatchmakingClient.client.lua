...
local function showQueue(payload)
	...
	if payload.players and #payload.players > 0 then
		table.insert(lines, string.format("Spieler (%d/%d): %s", payload.count or #payload.players, payload.maxPlayers or "?", table.concat(payload.players, ", ")))
	end
	if payload.arenaBusy and payload.status == "pending" then
		table.insert(lines, "Arena belegt — du startest sobald sie frei ist.")
	end
