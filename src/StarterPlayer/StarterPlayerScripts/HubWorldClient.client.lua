...
local function fireInteract()
	if not inHub or not canInteract then
		return
	end
...
UserInputService.InputBegan:Connect(function(input, processed)
	if processed or not inHub or not canInteract then
		return
	end
	if input.KeyCode == HubConfig.INTERACT_KEY or input.KeyCode == Enum.KeyCode.ButtonX then
		fireInteract()
	end
end)
