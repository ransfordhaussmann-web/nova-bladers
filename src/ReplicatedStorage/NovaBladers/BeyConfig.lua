local BeyConfig = {
	...
	SPECIAL_MOVES = {
		...
		--[[ Starfall Edge — comet rush / rapid meteor hits ]]
		StarfallRush = {
			id = "StarfallRush",
			name = "Starfall Rush",
			mode = "meteor",
			...
			phases = {
				...
				{ id = "shower", duration = 0.78, hitInterval = 0.14, hitRadius = 5, damage = 12, hits = 5 },
			},
		},
		--[[ Stone Halo — earth wall / quake pulses ]]
		ShellGuard = {
			id = "ShellGuard",
			name = "Shell Guard",
			mode = "fortress",
			...
			spinLoss = 6,
			damageReduction = 0.6,
			color = Color3.fromRGB(170, 155, 120),
			phases = {
		...
		--[[ Thunder Coil — lightning spirals / tight orbit ]]
		ThunderLoop = {
			id = "ThunderLoop",
			name = "Thunder Loop",
			mode = "sonic",
		...
		--[[ Night Maw — shadow dive / venom burst ]]
		NightFang = {
			id = "NightFang",
			name = "Night Fang",
			mode = "eclipse",
			...
			damage = 44,
			spinLoss = 20,
			color = Color3.fromRGB(90, 50, 150),
			phases = {
