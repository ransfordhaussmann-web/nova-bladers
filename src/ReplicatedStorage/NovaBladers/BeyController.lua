...
function BeyController:applySlow(mult, duration)
	...
	self.slowUntil = math.max(self.slowUntil, os.clock() + (duration or 0.5))
end

function BeyController:applyFreeze(duration)
	self.frozenUntil = math.max(self.frozenUntil, os.clock() + (duration or 0.5))
	self.velocity = Vector3.zero
end

function BeyController:getMoveSpeedMult()
	local now = os.clock()
...
function BeyController:update(dt, allControllers)
	...
	local statusMult = self:getMoveSpeedMult()
