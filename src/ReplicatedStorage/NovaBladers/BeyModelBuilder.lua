...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Jagged attack blades (spin fast)
	for i = 0, 3 do
		local angle = i * 90 + 15
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 1.0, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.4, 0, 0))
		table.insert(spinVisuals, seg)
	end

	-- Frost crystal spikes (static)
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.3, 1.5)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
	...
	outerShell:SetAttribute("SpinMult", -0.25)
	table.insert(spinVisuals, outerShell)
