...
local function buildAzureHalo(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i, radius in ipairs({ 1.05, 1.45, 1.85 }) do
		local halo = part({
			name = "HaloRing_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1 + i * 0.04, radius * 2, radius * 2),
		...
		halo:SetAttribute("SpinMult", 1.0 - i * 0.08)
