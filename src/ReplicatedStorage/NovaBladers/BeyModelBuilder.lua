...
local function buildFrostAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.5),
		})
		spike:SetAttribute("SpinMult", 0.65)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.5))
