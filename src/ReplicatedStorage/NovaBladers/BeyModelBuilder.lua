...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Saw-tooth fang blades (spin fast)
	for i = 0, 3 do
		local angle = i * 90 + 15
...
local function buildNeonPulse(parent, color, accent, baseCFrame)
	...
	-- Concentric neon rings (counter-rotating)
	for i, mult in ipairs({ 1.0, -0.7, 0.5 }) do
		local ring = part({
			name = "PulseRing_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.12, 2.8 + i * 0.5, 2.8 + i * 0.5),
		...
		ring:SetAttribute("SpinMult", mult)
	...
	for i = 0, 2 do
		...
		node:SetAttribute("SpinMult", 0.9)
