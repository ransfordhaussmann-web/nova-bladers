...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	bloodRing:SetAttribute("SpinMult", 1.5)
...
local function buildFrostRing(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local crystal = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.4, 1.1),
		})
		crystal:SetAttribute("SpinMult", -0.4)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.4, 1.1))
