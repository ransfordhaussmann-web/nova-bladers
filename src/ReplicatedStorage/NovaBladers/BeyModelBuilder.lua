...
local function buildCrimsonComet(parent, color, accent, baseCFrame)
	...
	tail:SetAttribute("SpinOffset", CFrame.new(0, 0, -1.5))
	table.insert(spinVisuals, tail)

	return visuals, spinVisuals, spinRing
end

local function buildFrostRegent(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.1)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
