...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.5)
		local anchor = part({
			name = "AnchorFluke_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.4, 1.0),
			color = color,
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		anchor:SetAttribute("SpinMult", 0.75)
		anchor:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, anchor)
	end

	local waterRing = part({
		name = "WaterRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	waterRing:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, waterRing)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i, angle in ipairs({ -40, 40 }) do
		...
		local claw = part({
			name = "Claw_" .. i,
			parent = parent,
			size = Vector3.new(0.5, 0.45, 1.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		claw:SetAttribute("SpinMult", 1)
		claw:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, claw)
	end

	local bloodAura = part({
		name = "BloodAura",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.22, 3.4, 3.4),
		color = color,
		material = Enum.Material.Neon,
		transparency = 0.3,
		canCollide = false,
	...
	bloodAura:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, bloodAura)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.1)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.15, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 100, 120),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	phaseRing:SetAttribute("SpinMult", 1.1)
