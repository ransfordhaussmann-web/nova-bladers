...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- 4 ripper blades (aggressive attack profile)
	for i = 0, 3 do
		local angle = i * 90 + 20
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Crest petals (slow counter-spin)
	for i = 0, 5 do
		local angle = i * 60
	...
	shieldDome:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, shieldDome)
	...
	outerHalo:SetAttribute("SpinMult", 0.55)
