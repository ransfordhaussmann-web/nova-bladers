...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local block = ringSegment(parent, 1.0, 1.6, 1.0, color, Enum.Material.Concrete, i * 45, "StoneBlock_" .. i)
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.3, 0, 0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(140, 145, 170),
		material = Enum.Material.Glass,
		transparency = 0.35,
		canCollide = false,
		cframe = baseCFrame,
	...
	ghost:SetAttribute("SpinMult", -0.6)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		spike:SetAttribute("SpinMult", 1.05)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.2))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local flame = part({
			name = "CrownFlame_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.9, 1.4),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.15),
		})
		flame:SetAttribute("SpinMult", 1.25)
		flame:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.15))
