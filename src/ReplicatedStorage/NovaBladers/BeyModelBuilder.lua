...
local function buildCrimsonApex(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.35, 1.25)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
