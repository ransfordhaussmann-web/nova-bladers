...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0.1, 1.15)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
		...
		feather:SetAttribute("SpinMult", 1.05)
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(30, 70, 110),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(32)) * CFrame.new(0, 0, 1.3)
		local fang = part({
			name = "Fang_" .. i,
			parent = parent,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local block = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.5, 0, 0),
		})
		block:SetAttribute("SpinMult", 0.4)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.5, 0, 0))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.15, 1.2),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.6),
		})
		ray:SetAttribute("SpinMult", 1.05)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.6))
