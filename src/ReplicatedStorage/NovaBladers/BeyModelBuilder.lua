...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 90, 120),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	local anchor = part({
		...
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.1) * CFrame.Angles(math.rad(90), 0, 0),
	})
	anchor:SetAttribute("SpinMult", 0.5)
	anchor:SetAttribute("SpinOffset", CFrame.new(0, 0, 1.1) * CFrame.Angles(math.rad(90), 0, 0))
	table.insert(spinVisuals, anchor)

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.8, 0.85, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i, angle in ipairs({ -40, 40 }) do
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.35)
		local fang = part({
			name = "Fang_" .. i,
			parent = parent,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.25, 2.7, 2.7),
		color = Color3.fromRGB(70, 65, 58),
	...
	for i = 0, 5 do
		local block = ringSegment(parent, 1.0, 1.7, 1.0, color, Enum.Material.Concrete, i * 60, "StoneBlock_" .. i)
		block:SetAttribute("SpinMult", 0.7)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.35, 0, 0))
	...
	rampart:SetAttribute("SpinMult", 0.55)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.15)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.22, 0.35, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		ray:SetAttribute("SpinMult", 1.0)
		ray:SetAttribute("SpinOffset", offset)
	...
	corona:SetAttribute("SpinMult", 0.9)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.2)
		local blade = part({
			name = "GhostBlade_" .. i,
			parent = parent,
	...
	ghostAura:SetAttribute("SpinMult", 0.85)
