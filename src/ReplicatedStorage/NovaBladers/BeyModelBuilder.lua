...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Neon,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, -0.35, 1.1) * CFrame.Angles(math.rad(55), 0, 0),
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.35)
		local fang = part({
