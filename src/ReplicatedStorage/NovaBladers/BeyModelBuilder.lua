...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset * CFrame.new(0, 0, 1.3),
		})
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	-- Heavy anchor flukes (spin slowly)
	for i = 0, 2 do
		local angle = i * 120
	...
	waveRing:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, waveRing)

	-- Chain links around rim
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.3, 1.85, 0.5, Color3.fromRGB(60, 100, 130), Enum.Material.Metal, i * 60, "ChainSeg_" .. i)
		seg:SetAttribute("SpinMult", -0.4)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.575, 0, 0))
		table.insert(spinVisuals, seg)
	end
