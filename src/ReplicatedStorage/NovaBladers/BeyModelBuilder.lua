...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.3, 1.35),
		})
		spike:SetAttribute("SpinMult", 1.2)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.3, 1.35))
