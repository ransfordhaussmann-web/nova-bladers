...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local flame = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = tooth.CFrame * CFrame.new(0, 0, 1.05),
		})
		flame:SetAttribute("SpinMult", 1.15)
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.05))
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	veilRing:SetAttribute("SpinMult", 0.65)
