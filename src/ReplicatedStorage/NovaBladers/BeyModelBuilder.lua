...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.75, color, Enum.Material.Concrete, i * 45, "RockSeg_" .. i)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(math.rad(35), math.rad(angle), 0) * CFrame.new(0, 0, 1.1)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
