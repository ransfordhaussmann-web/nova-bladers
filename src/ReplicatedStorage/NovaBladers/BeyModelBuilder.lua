...
local function buildCrimsonEdge(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	emberGlow:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, emberGlow)
...
local function buildFrostHalo(parent, color, accent, baseCFrame)
	...
	haloGlow:SetAttribute("SpinMult", -0.3)
	table.insert(spinVisuals, haloGlow)

	return visuals, spinVisuals, spinRing
end

local function buildNebulaShard(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	nebula:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, nebula)

	for i = 0, 2 do
		local angle = i * 120 + 15
		local offset = CFrame.Angles(math.rad(30), math.rad(angle), 0) * CFrame.new(0, 0.2, 1.15)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
...
local BUILDERS = {
	...
	CrimsonEdge = buildCrimsonEdge,
	FrostHalo = buildFrostHalo,
	NebulaShard = buildNebulaShard,
}
