...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.42, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	crown:SetAttribute("SpinMult", 0.35)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.35, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.1),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.1))
	...
	corona:SetAttribute("SpinMult", 1.3)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local blade = part({
			...
			transparency = 0.25,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.25),
		})
		blade:SetAttribute("SpinMult", 1.25)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, blade)
	end
	...
	halo:SetAttribute("SpinMult", -0.6)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0, 1.5),
		})
		spike:SetAttribute("SpinMult", 0.75)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0, 1.5))
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 25, 45),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	driftRing:SetAttribute("SpinMult", 1.5)
	...
	for i = 0, 1 do
		local wing = part({
			name = "Wing_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.2, 2.5),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 180 + 90), math.rad(5)) * CFrame.new(0, 0, 1.2),
		})
		wing:SetAttribute("SpinMult", 1.4)
		wing:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 180 + 90), math.rad(5)) * CFrame.new(0, 0, 1.2))
	...
	exhaust:SetAttribute("SpinMult", 0.7)
	table.insert(spinVisuals, exhaust)
