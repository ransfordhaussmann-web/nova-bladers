...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(100, 20, 30),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local block = ringSegment(parent, 1.2, 1.8, 1.0, color, Enum.Material.Concrete, i * 45, "Block_" .. i)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostLayer:SetAttribute("SpinMult", 1.1)
