...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, accent, Enum.Material.Glass, i * 60, "CrownSeg_" .. i)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.4, 1.05)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
