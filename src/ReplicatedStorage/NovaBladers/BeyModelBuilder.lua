...
local function buildBlazeHorn(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		...
		horn:SetAttribute("SpinMult", 1.05)
		horn:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, horn)
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = horn.CFrame * CFrame.new(0, 0, 1.35),
		})
...
local function buildCrystalWard(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(1.5, 0.2, 0)
		local prism = part({
			name = "Prism_" .. i,
			parent = parent,
		...
		prism:SetAttribute("SpinMult", 0.75)
	...
	outerBumper:SetAttribute("SpinMult", -0.3)
	table.insert(spinVisuals, outerBumper)
...
local BUILDERS = {
	...
	BlazeHorn = buildBlazeHorn,
	CrystalWard = buildCrystalWard,
}
