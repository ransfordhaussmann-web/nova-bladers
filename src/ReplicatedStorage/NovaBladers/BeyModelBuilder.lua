...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0.2, 1.15)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.2)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.9, 2.0, 2.0),
		color = Color3.fromRGB(180, 50, 20),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
