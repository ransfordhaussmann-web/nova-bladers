...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	vortexRing:SetAttribute("SpinMult", 1.3)
...
local function buildGlacierCrown(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		...
		local spike = part({
			...
			cframe = baseCFrame
				...
				* CFrame.new(1.85, 0.35, 0)
				* CFrame.Angles(math.rad(-20), 0, 0),
		})
		spike:SetAttribute("SpinMult", 0.75)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.85, 0.35, 0) * CFrame.Angles(math.rad(-20), 0, 0))
...
local BUILDERS = {
	...
	GlacierCrown = buildGlacierCrown,
}
