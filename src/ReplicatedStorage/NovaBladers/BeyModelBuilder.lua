...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.7, 1.0, color, Enum.Material.Concrete, i * 45, "FortSeg_" .. i)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	veil:SetAttribute("SpinMult", 0.85)
	...
	for i = 0, 2 do
		...
		local edge = part({
			...
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.3),
		})
		edge:SetAttribute("SpinMult", 0.85)
		edge:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.3))
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(200, 240, 250),
		material = Enum.Material.Glass,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.3, 1.1),
		})
		spike:SetAttribute("SpinMult", 1)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.3, 1.1))
