...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, 0.55, 0.55),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(35), math.rad(angle), 0) * CFrame.new(1.15, 0, 0),
		})
		coil:SetAttribute("SpinMult", 1.3)
		coil:SetAttribute("SpinOffset", CFrame.Angles(math.rad(35), math.rad(angle), 0) * CFrame.new(1.15, 0, 0))
