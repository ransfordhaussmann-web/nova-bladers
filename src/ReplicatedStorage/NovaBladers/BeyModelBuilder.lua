...
local function tryExternalMesh(beyData, parent, baseCFrame)
	...
	local creatorStore = beyData.creatorStore
	local meshId = (assets and assets.meshId) or (creatorStore and creatorStore.meshId)
	if not meshId then
		return nil
	end
	...
	local mp = meshPart({
		...
		parent = parent,
		meshId = meshId,
		textureId = (assets and assets.textureId) or (creatorStore and creatorStore.textureId),
		size = (assets and assets.size) or (creatorStore and creatorStore.size) or Vector3.new(3.6, 1.2, 3.6),
		color = beyData.color,
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	-- Crystal veil panels
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.Glass, i * 60, "VeilSeg_" .. i)
	...
	frostAura:SetAttribute("SpinMult", 0.6)
	...
	table.insert(visuals, crystalTip)
