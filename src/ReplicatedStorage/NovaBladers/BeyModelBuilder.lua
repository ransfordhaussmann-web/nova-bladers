...
local function buildCrimsonForge(parent, color, accent, baseCFrame)
	...
	forgePlate:SetAttribute("SpinMult", 0.95)
	...
	table.insert(visuals, hammerHead)
...
local function buildGlacierCrest(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.25)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.65, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
...
local BUILDERS = {
	...
	GlacierCrest = buildGlacierCrest,
}
