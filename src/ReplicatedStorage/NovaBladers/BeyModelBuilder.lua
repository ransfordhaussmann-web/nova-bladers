...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.85, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.5, 1.1),
		})
		spike:SetAttribute("SpinMult", 0.8)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.5, 1.1))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for layer = 0, 2 do
		local radius = 1.1 + layer * 0.35
		local coil = part({
			name = "CoilLayer_" .. layer,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, radius * 2, radius * 2),
			color = layer == 1 and accent or color,
			material = Enum.Material.Neon,
			transparency = 0.15 + layer * 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, layer * 0.12, 0),
		})
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.2, 1.4)
		local flare = part({
			name = "SolarFlare_" .. i,
			parent = parent,
