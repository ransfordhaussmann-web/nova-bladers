...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Layered aurora rings (slow counter-rotation)
	for i, mult in ipairs({ 1.0, -0.7, 0.5 }) do
		local ring = part({
			name = "AuroraRing_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.2 + i * 0.08, 2.8 + i * 0.5, 2.8 + i * 0.5),
	...
	local crest = part({
		name = "CrystalCrest",
		parent = parent,
		size = Vector3.new(0.5, 1.2, 0.8),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.15,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0.6, 0) * CFrame.Angles(math.rad(15), 0, 0),
	})
