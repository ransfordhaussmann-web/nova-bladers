...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Spiral vortex blades (counter-rotating layers)
	for layer, spinMult in ipairs({ 1.2, -0.85 }) do
		for i = 0, 3 do
			local angle = i * 90 + layer * 22
			local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0, 1.25 + layer * 0.2)
			local blade = part({
				name = ("VortexBlade_%d_%d"):format(layer, i),
				parent = parent,
				size = Vector3.new(0.4, 0.5, 2.0),
				color = layer == 1 and color or accent,
				material = Enum.Material.Neon,
				transparency = layer == 2 and 0.15 or 0,
				canCollide = false,
				cframe = baseCFrame * offset,
			})
			blade:SetAttribute("SpinMult", spinMult)
	...
	local tip = part({
		...
		color = Color3.fromRGB(255, 180, 160),
		material = Enum.Material.Glass,
		canCollide = false,
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	mantle:SetAttribute("SpinMult", 0.6)
	...
	frostRing:SetAttribute("SpinMult", -0.4)
