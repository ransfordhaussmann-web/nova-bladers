...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.1, 1.25)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.85, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.6, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.35)
		local blade = part({
			name = "PhantomBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.55, 2.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
