...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local spike = part({
			name = "Spike_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.55, 1.0),
			color = accent,
			material = Enum.Material.Rock,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.6),
		})
		spike:SetAttribute("SpinMult", 0.4)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.6))
