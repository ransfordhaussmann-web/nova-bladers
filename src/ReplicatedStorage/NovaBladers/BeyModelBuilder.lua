--[[
	...
	Use modelAssets.creatorStoreSearch as Toolbox search hint in Studio.
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local petal = part({
			name = "SolarPetal_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.25, 1.4),
			color = color,
			material = Enum.Material.SmoothPlastic,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	halo:SetAttribute("SpinMult", 1.05)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	wisp:SetAttribute("SpinMult", 0.85)
