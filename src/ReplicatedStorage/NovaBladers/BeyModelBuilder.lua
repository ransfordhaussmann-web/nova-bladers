...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.1) * CFrame.Angles(0, 0, math.rad(30)),
	})
	anchor:SetAttribute("SpinMult", 0.8)
	anchor:SetAttribute("SpinOffset", CFrame.new(0, 0, 1.1) * CFrame.Angles(0, 0, math.rad(30)))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.3)
		local blade = part({
			name = "Blade_" .. i,
			parent = parent,
