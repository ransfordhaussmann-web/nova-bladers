...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.1, 1.1)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0.3, 0.6),
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 20, 35),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.85, 2.0, 2.0),
		color = Color3.fromRGB(100, 95, 130),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 180 + 45), math.rad(15)) * CFrame.new(0, 0, 1.15)
		local blade = part({
			name = "PhaseBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.3, 1.7),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
