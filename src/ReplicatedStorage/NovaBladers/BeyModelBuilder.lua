...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.1, 1.7, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.6)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.4, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0, 1.5),
		})
		ray:SetAttribute("SpinMult", 1.3)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0, 1.5))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(140, 140, 170),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.2)
		local edge = part({
			name = "GhostEdge_" .. i,
			parent = parent,
