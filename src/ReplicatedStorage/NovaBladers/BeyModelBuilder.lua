...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Glass, i * 60, "CrestSeg_" .. i)
	...
	for i, hue in ipairs({ 0.55, 0.75, 0.9 }) do
		local band = part({
			name = "AuroraBand_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.08, 3.6 + i * 0.15, 3.6 + i * 0.15),
			color = Color3.fromHSV(hue, 0.6, 1),
			material = Enum.Material.Neon,
			transparency = 0.45,
			canCollide = false,
			cframe = baseCFrame,
		})
		band:SetAttribute("SpinMult", 0.5 + i * 0.15)
		table.insert(spinVisuals, band)
	end
