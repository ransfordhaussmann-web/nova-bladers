...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.7, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	bloodRing:SetAttribute("SpinMult", 0.95)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local block = part({
			name = "StoneBlock_" .. i,
			parent = parent,
			size = Vector3.new(0.7, 0.8, 0.9),
			color = color,
			material = Enum.Material.Concrete,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.55, 0, 0),
		})
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.55, 0, 0))
