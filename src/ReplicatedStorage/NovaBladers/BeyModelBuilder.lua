...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 0.9, 1.55, 0.55, color, Enum.Material.Neon, i * 90, "CrestSeg_" .. i)
		seg.Transparency = 0.2
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 90), 0) * CFrame.new(1.22, 0, 0))
		table.insert(spinVisuals, seg)
	end
