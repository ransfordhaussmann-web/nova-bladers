...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local seg = part({
			name = "RockPlate_" .. i,
			parent = parent,
			size = Vector3.new(0.7, 0.55, 1.6),
			color = color,
			material = Enum.Material.Concrete,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 72), math.rad(12)) * CFrame.new(0, 0, 1.5),
		})
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 72), math.rad(12)) * CFrame.new(0, 0, 1.5))
		table.insert(spinVisuals, seg)
	end
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local blade = part({
			...
			transparency = 0.25,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.3),
		})
		blade:SetAttribute("SpinMult", 1.1)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.3))
		table.insert(spinVisuals, blade)
	end
