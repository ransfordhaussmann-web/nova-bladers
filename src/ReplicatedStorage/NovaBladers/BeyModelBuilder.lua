...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.SmoothPlastic, i * 60, "TideSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end

	local anchorBlade = part({
		name = "AnchorBlade",
		parent = parent,
		size = Vector3.new(0.55, 0.7, 2.2),
		color = Color3.fromRGB(70, 110, 140),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.Angles(0, 0, math.rad(15)) * CFrame.new(0, 0, 1.1),
	})
	anchorBlade:SetAttribute("SpinMult", 0.5)
	table.insert(visuals, anchorBlade)
	...
	ripple:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, ripple)
