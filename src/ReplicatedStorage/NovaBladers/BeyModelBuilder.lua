...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.2),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.2))
		table.insert(spinVisuals, fang)
		table.insert(spinVisuals, tip)
	end
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.9, 1.0, color, Enum.Material.Concrete, i * 45, "FortSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.55, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	for i = 0, 3 do
		...
		local pillar = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.6, 0.2, 0),
		})
		pillar:SetAttribute("SpinMult", 0.4)
		pillar:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.6, 0.2, 0))
		table.insert(spinVisuals, pillar)
	end
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 90, 110),
		material = Enum.Material.Glass,
		transparency = 0.15,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.25)
		local blade = part({
			name = "EdgeBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.6, 2.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
	...
	ghostRing:SetAttribute("SpinMult", -0.7)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.7, 0.9),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.35),
		})
		spike:SetAttribute("SpinMult", 0.8)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.35))
	...
	crown:SetAttribute("SpinMult", 0.75)
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	driftRing:SetAttribute("SpinMult", 1.25)
	table.insert(spinVisuals, driftRing)
	...
	for i = 0, 2 do
		...
		local wing = part({
			name = "DriftWing_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.2, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.5),
		})
		wing:SetAttribute("SpinMult", 1.3)
		wing:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.5))
	...
	trailGlow:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, trailGlow)
...
local BUILDERS = {
	...
	CrimsonDrift = buildCrimsonDrift,
}
