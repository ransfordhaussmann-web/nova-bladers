...
local function tryExternalMesh(beyData, parent, baseCFrame)
	...
	if (not assets or not assets.meshId) and beyData.creatorStore then
		assets = beyData.creatorStore
	end
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.75, color, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 0.95)
		local shard = part({
			name = "IceShard_" .. i,
			parent = parent,
