...
local function tryCloneStudioModel(beyData, model, visualFolder, baseCFrame, hull)
	...
	local modelName = (beyData.modelRef and beyData.modelRef.studioModelName)
		or (beyData.modelAssets and beyData.modelAssets.studioModelName)
		or beyData.id
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.4))
		table.insert(spinVisuals, blade)
	end
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.8, 0.35),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(20), math.rad(angle), 0) * CFrame.new(0, 0, 1.25),
		})
		shard:SetAttribute("SpinMult", 1.05)
		shard:SetAttribute("SpinOffset", CFrame.Angles(math.rad(20), math.rad(angle), 0) * CFrame.new(0, 0, 1.25))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(-15), math.rad(angle), 0) * CFrame.new(0, 0, 1.35),
		})
		spike:SetAttribute("SpinMult", 1)
		spike:SetAttribute("SpinOffset", CFrame.Angles(math.rad(-15), math.rad(angle), 0) * CFrame.new(0, 0, 1.35))
	...
	flameRing:SetAttribute("SpinMult", 1.25)
	table.insert(spinVisuals, flameRing)

	local spinRing = flameRing
	return visuals, spinVisuals, spinRing
end
