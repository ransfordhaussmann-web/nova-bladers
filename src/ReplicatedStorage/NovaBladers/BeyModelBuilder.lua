...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		size = Vector3.new(0.85, 2.0, 2.0), color = Color3.fromRGB(200, 80, 40),
	...
	local spinRing = part({
		...
		size = Vector3.new(0.14, 3.6, 3.6), color = accent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.2, 2.8, 2.8), color = Color3.fromRGB(50, 90, 140),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	...
	local anchor = part({
		...
		material = Enum.Material.Neon, canCollide = false,
		cframe = baseCFrame * CFrame.new(0, -0.4, 0),
	})
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.3, 2.8, 2.8), color = Color3.fromRGB(90, 85, 75),
	...
	spinRing:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, spinRing)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.7, 1.7, 1.7), color = Color3.fromRGB(240, 200, 60),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	...
	flatRing:SetAttribute("SpinMult", 1.3)
	table.insert(spinVisuals, flatRing)
	...
	for i = 0, 5 do
		...
		local ray = part({
			...
			size = Vector3.new(0.15, 0.6, 1.0), color = accent,
			material = Enum.Material.Neon, canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.1),
		})
		ray:SetAttribute("SpinMult", 1.3)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.1))
		table.insert(spinVisuals, ray)
	end

	local spinRing = flatRing
	return visuals, spinVisuals, spinRing
end
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.8, 2.0, 2.0), color = Color3.fromRGB(60, 100, 120),
	...
	local aura = part({
		...
		size = Vector3.new(0.25, 3.2, 3.2), color = color,
		material = Enum.Material.ForceField, transparency = 0.5,
		canCollide = false, cframe = baseCFrame,
	})
	aura:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, aura)

	local spinRing = part({
		name = "SpinRing", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.12, 3.6, 3.6), color = accent,
