...
local function buildEmberClaw(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(24)) * CFrame.new(0, 0, 1.3)
		local claw = part({
			name = "FlameClaw_" .. i,
			parent = parent,
...
local function buildGlacierShield(parent, color, accent, baseCFrame)
	...
	frostWall:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, frostWall)
