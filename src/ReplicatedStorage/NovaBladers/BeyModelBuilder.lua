...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.5)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local radius = 1.0 + i * 0.45
		local height = 0.22 - i * 0.04
		local coil = part({
			name = "HelixCoil_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.2)
		local flare = part({
			name = "SolarFlare_" .. i,
			parent = parent,
