...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Glass, i * 60, "CrestSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.4, 0.9)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
	...
	frostHalo:SetAttribute("SpinMult", -0.4)
