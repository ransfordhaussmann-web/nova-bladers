...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.18, 0.35, 1.4),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local edge = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.15),
		})
		edge:SetAttribute("SpinMult", 0.95)
		edge:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.15))
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			...
			parent = parent,
			size = Vector3.new(0.3, 0.55, 1.5),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(32)) * CFrame.new(0, 0, 1.2),
		})
		shard:SetAttribute("SpinMult", 1.08)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(32)) * CFrame.new(0, 0, 1.2))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 50, 20),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 4 do
		...
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.6, 1.3),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.1),
		})
		spike:SetAttribute("SpinMult", 1)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.1))
	...
	flameRing:SetAttribute("SpinMult", 1.1)
