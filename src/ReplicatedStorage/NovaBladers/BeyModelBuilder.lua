...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	-- Crown spikes (spin slowly)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.35)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	-- Heavy frost shell segments
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.85, color, Enum.Material.Ice, i * 90, "FrostSeg_" .. i)
