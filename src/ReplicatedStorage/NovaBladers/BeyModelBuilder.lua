...
local function buildGlacierShield(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(1.5, 0, 0)
		local crystal = part({
			name = "IceCrystal_" .. i,
			parent = parent,
	...
	frostShell:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, frostShell)
	...
	local outerRim = part({
		...
		color = Color3.fromRGB(100, 150, 190),
		material = Enum.Material.DiamondPlate,
		transparency = 0.1,
		canCollide = false,
		cframe = baseCFrame,
