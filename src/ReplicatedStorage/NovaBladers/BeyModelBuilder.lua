...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	vortexRing:SetAttribute("SpinMult", 1.6)
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local crystal = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.35, 1.05),
		})
		crystal:SetAttribute("SpinMult", 0.9)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.35, 1.05))
