...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		spoke:SetAttribute("SpinMult", 1.3)
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	-- Frost veil layers (counter-rotate)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Glass, i * 60, "VeilSeg_" .. i)
	...
	shield:SetAttribute("SpinMult", 0.6)
	table.insert(spinVisuals, shield)
