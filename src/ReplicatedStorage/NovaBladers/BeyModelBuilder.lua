...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local spinRing = part({
		...
		color = Color3.fromRGB(255, 120, 130),
		material = Enum.Material.Neon,
		transparency = 0.35,
		canCollide = false,
		cframe = baseCFrame,
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.SmoothPlastic, i * 60, "CrestSeg_" .. i)
	...
	crestPlate:SetAttribute("SpinMult", 0.55)
