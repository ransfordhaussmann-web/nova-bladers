...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Neon,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, -0.4, 1.0),
	})
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0, 2.0),
		})
		ray:SetAttribute("SpinMult", 1.15)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0, 2.0))
