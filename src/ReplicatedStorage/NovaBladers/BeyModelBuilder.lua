...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	outerHeat:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, outerHeat)
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	iceLayer:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, iceLayer)

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.65, 0.75, accent, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
