...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.15),
		})
		ray:SetAttribute("SpinMult", 0.95)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.15))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i, angle in ipairs({ -50, 50 }) do
