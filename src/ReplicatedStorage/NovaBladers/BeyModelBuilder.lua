...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- 4 sharp fang blades (aggressive attack profile)
	for i = 0, 3 do
		local angle = i * 90 + 20
...
local function buildGlacierShield(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.3, 1.1)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
