...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local ember = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = spoke.CFrame * CFrame.new(0, 0, 1.35),
		})
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.05, 1.7, 0.75, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
		seg.Transparency = 0.15
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.375, 0, 0))
		table.insert(spinVisuals, seg)
	end

	for i, angle in ipairs({ 0, 120, 240 }) do
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0.2, 1.05)
		local shard = part({
			name = "IceShard_" .. i,
			parent = parent,
			size = Vector3.new(0.28, 0.85, 0.55),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.1,
			canCollide = false,
