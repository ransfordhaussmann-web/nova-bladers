...
local function buildBlazeSpike(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = spike.CFrame * CFrame.new(0, 0, 1.1),
		})
...
local function buildFrostHalo(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 220, 250),
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.2, 1.0)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
