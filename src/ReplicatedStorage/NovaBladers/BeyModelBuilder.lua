...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local blade = part({
			...
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset * CFrame.new(0.3, 0, 0.8),
		})
		blade:SetAttribute("SpinMult", 1.15)
		blade:SetAttribute("SpinOffset", offset * CFrame.new(0.3, 0, 0.8))
		table.insert(spinVisuals, blade)
	end
...
local function buildGlacierCore(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.2, 0.9)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
