...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.65, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	corona:SetAttribute("SpinMult", 0.75)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		edge:SetAttribute("SpinMult", 1.3)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.35, 1.0)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local angle = i * 180
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.4)
		local wing = part({
			name = "DriftWing_" .. i,
			parent = parent,
		...
		wing:SetAttribute("SpinMult", 1.25)
