...
local function buildEmberCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local flame = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = coil.CFrame * CFrame.new(0, 0, 0.95),
		})
		flame:SetAttribute("SpinMult", 1.15)
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 0.95))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60 + 15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.2, 1.35)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
