...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local edge = part({
			name = "RipperEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.25, 0.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0, 0, 1.05),
		})
		edge:SetAttribute("SpinMult", 1.15)
		edge:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.05))
		table.insert(spinVisuals, blade)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.65, 1.0, color, Enum.Material.Concrete, i * 45, "FortSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local ring = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1, 3.5 + i * 0.8, 3.5 + i * 0.8),
			color = i == 0 and color or accent,
			material = Enum.Material.Neon,
			transparency = 0.25 + i * 0.15,
			canCollide = false,
			cframe = baseCFrame,
		})
		ring:SetAttribute("SpinMult", 0.9 - i * 0.2)
	...
	for i = 0, 5 do
		...
		flare:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.15))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(35, 90, 100),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 3 do
		...
		local edge = part({
			name = "PhaseEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.35, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
