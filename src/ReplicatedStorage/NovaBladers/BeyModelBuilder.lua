...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.9, 1.0, color, Enum.Material.Concrete, i * 60, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.55, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.15)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 90, 110),
		material = Enum.Material.Glass,
		transparency = 0.3,
		canCollide = false,
		cframe = baseCFrame,
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-30)) * CFrame.new(0, 0.35, 1.1)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(100, 15, 35),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.5)
		local fin = part({
			name = "DriftFin_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.35, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		fin:SetAttribute("SpinMult", 1.25)
