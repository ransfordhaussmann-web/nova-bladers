...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.9, 1.0, color, Enum.Material.Slate, i * 45, "StoneSeg_" .. i)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.2)
		local spike = part({
			name = "FlameSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.8, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
