...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.3, 1.0)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.9, 1.2),
			color = accent,
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Ice, i * 90, "IceSeg_" .. i)
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, 3.2 + i * 0.6, 3.2 + i * 0.6),
			color = color,
			material = Enum.Material.Neon,
	...
	for i = 0, 5 do
		...
		local flare = part({
			name = "SolarFlare_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.5, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.4),
		})
		flare:SetAttribute("SpinMult", 1.3)
		flare:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.4))
