...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "BlockSeg_" .. i)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local crown = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.45, 0.9, 0.45),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		crown:SetAttribute("SpinMult", 1.08)
		crown:SetAttribute("SpinOffset", offset)
