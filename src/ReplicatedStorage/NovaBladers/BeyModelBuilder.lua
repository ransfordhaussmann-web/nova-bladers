...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.2, 1.3)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.15, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	flameRing:SetAttribute("SpinMult", 1.2)
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 100, 120),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	local anchor = part({
		...
		material = Enum.Material.DiamondPlate,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 0.5),
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.85, 2.1, 2.1),
		color = Color3.fromRGB(120, 20, 30),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	local bloodRing = part({
		name = "BloodRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.18, 3.5, 3.5),
		color = color,
		material = Enum.Material.Neon,
		transparency = 0.25,
		canCollide = false,
	...
	bloodRing:SetAttribute("SpinMult", 1.1)
	table.insert(spinVisuals, bloodRing)

	local spinRing = part({
		name = "SpinRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.12, 3.9, 3.9),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.35,
		canCollide = false,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.3, 2.9, 2.9),
		color = Color3.fromRGB(70, 72, 65),
	...
	for i = 0, 7 do
		local block = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "StoneBlock_" .. i)
	...
	rampart:SetAttribute("SpinMult", 0.35)
	table.insert(spinVisuals, rampart)

	local spinRing = part({
		name = "SpinRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.28, 4.6, 4.6),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.5,
		canCollide = false,
	...
	spinRing:SetAttribute("SpinMult", 0.3)
	table.insert(spinVisuals, spinRing)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisc:SetAttribute("SpinMult", 0.9)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.8, 2.0, 2.0),
		color = Color3.fromRGB(80, 75, 110),
		material = Enum.Material.Glass,
		transparency = 0.3,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 1 do
		local angle = i * 180 + 40
		local blade = part({
			name = "GhostBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.3, 2.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.25),
		})
		blade:SetAttribute("SpinMult", 1.05)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, blade)
	end

	local ghostAura = part({
		name = "GhostAura",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.15, 3.4, 3.4),
		color = color,
		material = Enum.Material.Neon,
		transparency = 0.5,
		canCollide = false,
	...
	ghostAura:SetAttribute("SpinMult", 0.8)
	table.insert(spinVisuals, ghostAura)

	local spinRing = part({
		name = "SpinRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.1, 3.8, 3.8),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.45,
		canCollide = false,
