...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for layer = 1, 2 do
		local ring = part({
			name = "CoronaRing_" .. layer,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1, 3.2 + layer * 0.6, 3.2 + layer * 0.6),
			color = layer == 1 and color or accent,
			material = Enum.Material.Neon,
			transparency = 0.25 + layer * 0.1,
			canCollide = false,
			cframe = baseCFrame,
		})
		ring:SetAttribute("SpinMult", 1.0 + layer * 0.15)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 210, 225),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
