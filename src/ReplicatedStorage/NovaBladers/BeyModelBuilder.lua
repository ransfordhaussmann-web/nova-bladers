...
local function buildBlazeLance(parent, color, accent, baseCFrame)
	...
	-- Flame lance blades (3 sharp attack fins)
	for i = 0, 2 do
		local angle = i * 120
		...
		local flame = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = lance.CFrame * CFrame.new(0, 0, 1.55),
		})
...
local function buildCoralTide(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 120, 110),
	...
	-- Coral reef segments around the rim
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Cobblestone, i * 60, "CoralSeg_" .. i)
	...
	tideGlow:SetAttribute("SpinMult", -0.4)
