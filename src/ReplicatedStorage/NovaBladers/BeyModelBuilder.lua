...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Spiral attack fins (3 curved blades)
	for i = 0, 2 do
		...
		local ember = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fin.CFrame * CFrame.new(0, 0, 1.35),
		})
...
local function buildGlacierPeak(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 1.0, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.4, 1.1)
		local spike = part({
			name = "FrostSpike_" .. i,
			parent = parent,
