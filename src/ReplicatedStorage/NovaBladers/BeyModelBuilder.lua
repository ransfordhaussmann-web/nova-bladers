...
local function buildEmberVortex(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildGlacierCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 0)
		local crystal = part({
			name = "CrownCrystal_" .. i,
			parent = parent,
