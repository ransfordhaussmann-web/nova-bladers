...
local function buildBlazeOrbit(parent, color, accent, baseCFrame)
	...
	cometRing:SetAttribute("SpinMult", 1.3)
	table.insert(spinVisuals, cometRing)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	-- Ice crown spikes (slow spin)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.25, 1.3)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
