...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	-- Flame spikes on outer wheel
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0, 1.45)
		local spike = part({
			name = "FlameSpike_" .. i,
			parent = parent,
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	-- Crystal shield segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, accent, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
	...
	veilRing:SetAttribute("SpinMult", -0.3)
