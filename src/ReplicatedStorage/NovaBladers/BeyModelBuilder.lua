...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Spiral attack fins (counter-rotating layers)
	for layer, spinMult in ipairs({ 1.15, -0.75 }) do
		for i = 0, 2 do
			local angle = i * 120 + layer * 25
			local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0, 1.25)
			local fin = part({
				name = ("SpiralFin_%d_%d"):format(layer, i),
				parent = parent,
				size = Vector3.new(0.4, 0.5, 2.0),
				color = layer == 1 and color or accent,
				material = Enum.Material.Neon,
				transparency = layer == 2 and 0.1 or 0,
				canCollide = false,
				cframe = baseCFrame * offset,
			})
			fin:SetAttribute("SpinMult", spinMult)
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	local outerRim = part({
		...
		color = Color3.fromRGB(100, 160, 200),
		material = Enum.Material.Metal,
		transparency = 0.1,
		canCollide = false,
		cframe = baseCFrame,
