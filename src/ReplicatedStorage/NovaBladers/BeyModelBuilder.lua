...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Blood-red energy ring
	local energyRing = part({
		name = "EnergyRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	energyRing:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, energyRing)
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Crest shield segments (6 panels)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.SmoothPlastic, i * 60, "CrestSeg_" .. i)
	...
	glowRing:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, glowRing)

	-- Outer barrier ring (counter-rotates)
	local barrierRing = part({
		name = "BarrierRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	barrierRing:SetAttribute("SpinMult", -0.4)
	table.insert(spinVisuals, barrierRing)
