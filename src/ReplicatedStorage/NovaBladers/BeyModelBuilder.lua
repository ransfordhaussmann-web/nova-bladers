...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0.1, 1.25)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.55, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	ember:SetAttribute("SpinMult", 1.2)
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, accent, Enum.Material.Glass, i * 90, "WaveSeg_" .. i)
		seg.Transparency = 0.35
		seg:SetAttribute("SpinMult", 0.9)
	...
	spinRing:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, spinRing)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.4)
		local fang = part({
			name = "Fang_" .. i,
			parent = parent,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		block:SetAttribute("SpinMult", 0.55)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.18, 0.35, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.5),
		})
		ray:SetAttribute("SpinMult", 0.7)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.5))
	...
	corona:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, corona)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local blade = part({
			name = "GhostBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.5, 2.2),
			color = accent,
			material = Enum.Material.ForceField,
			transparency = 0.4,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.3),
		})
		blade:SetAttribute("SpinMult", 1.25)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.3))
		table.insert(spinVisuals, blade)
	end
	...
	mist:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, mist)

	local spinRing = part({
		name = "SpinRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.1, 3.9, 3.9),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.5,
		canCollide = false,
