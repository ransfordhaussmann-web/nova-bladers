...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.3),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.3))
		table.insert(spinVisuals, tip)
	end
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.9, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.45, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostAura:SetAttribute("SpinMult", -0.6)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(math.rad(20), math.rad(angle), 0) * CFrame.new(0, 0, 1.3)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
		...
		shard:SetAttribute("SpinMult", 1.25)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.3, 1.2)
		local flame = part({
			name = "CrownFlame_" .. i,
			parent = parent,
	...
	crown:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, crown)
