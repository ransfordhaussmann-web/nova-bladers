...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 100, 130),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 4 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.55, color, Enum.Material.Glass, i * 72, "CrestSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 72), 0) * CFrame.new(1.28, 0.2, 0))
		table.insert(spinVisuals, seg)
	end
