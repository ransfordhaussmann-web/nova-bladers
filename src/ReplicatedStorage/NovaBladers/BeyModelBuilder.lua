...
local function buildFrostPrism(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			...
			parent = parent,
			size = Vector3.new(0.35, 0.85, 1.4),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
	...
	icicleRing:SetAttribute("SpinMult", -0.4)
	table.insert(spinVisuals, icicleRing)
...
local function buildBlazeForge(parent, color, accent, baseCFrame)
	...
	forgeGlow:SetAttribute("SpinMult", 1.15)
	table.insert(spinVisuals, forgeGlow)
