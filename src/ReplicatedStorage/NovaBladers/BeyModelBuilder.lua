...
local function buildCrimsonLance(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 50, 35),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 2 do
		...
		flameTip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.55))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.35, 1.35)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Ice, i * 90, "FrostSeg_" .. i)
...
local BUILDERS = {
	...
	CrimsonLance = buildCrimsonLance,
