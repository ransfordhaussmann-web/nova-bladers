...
local function buildStarfallEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(200, 140, 90),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	comet:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, comet)
...
local function buildStoneHalo(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.9, 0.75, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
	...
	halo:SetAttribute("SpinMult", 0.4)
...
local function buildThunderCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local coil = part({
			name = "CoilArc_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.6, 1.3),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.1),
		})
		coil:SetAttribute("SpinMult", 1.25)
		coil:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.1))
	...
	coilRing:SetAttribute("SpinMult", 1.5)
	table.insert(spinVisuals, coilRing)
...
local function buildNightMaw(parent, color, accent, baseCFrame)
	...
	mawAura:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, mawAura)
