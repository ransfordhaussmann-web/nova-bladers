...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "Plate_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 2.0)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostAura:SetAttribute("SpinMult", 1.05)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(math.rad(20), math.rad(angle), 0) * CFrame.new(0, 0, 1.4)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.7, 1.5),
			color = color,
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(math.rad(-35), math.rad(angle), 0) * CFrame.new(0, 0.5, 1.1)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
