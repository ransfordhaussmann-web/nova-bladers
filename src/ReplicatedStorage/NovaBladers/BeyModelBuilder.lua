...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, 2.8 + i * 0.3, 2.8 + i * 0.3),
			color = (i % 2 == 0) and color or accent,
			material = Enum.Material.Neon,
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)),
		})
		coil:SetAttribute("SpinMult", 1.0 + i * 0.15)
		coil:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)))
	...
	flare:SetAttribute("SpinMult", 0.7)
