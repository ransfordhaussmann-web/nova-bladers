...
local function tryCloneStudioModel(beyData, model, visualFolder, baseCFrame, hull)
	...
	local modelName = (beyData.modelRef and beyData.modelRef.studioModelName)
		or (beyData.creatorStore and beyData.creatorStore.studioModelName)
		or beyData.id
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "FortSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.22, 0.35, 1.4),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.75),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.75))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostRing:SetAttribute("SpinMult", 1.05)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.55, 1.2),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.05,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.15),
		})
		shard:SetAttribute("SpinMult", 1.0)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.15))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.85, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.35, 1.05),
		})
		spike:SetAttribute("SpinMult", 0.95)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.35, 1.05))
	...
	flameRing:SetAttribute("SpinMult", 1.0)
