...
local function buildBlazeComet(parent, color, accent, baseCFrame)
	...
	tail:SetAttribute("SpinMult", -0.5)
	table.insert(spinVisuals, tail)

	return visuals, spinVisuals, spinRing
end

local function buildCrystalGuard(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.05)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
...
local BUILDERS = {
	...
	CrystalGuard = buildCrystalGuard,
}
