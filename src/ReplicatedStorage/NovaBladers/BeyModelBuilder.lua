...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Glass, i * 90 + 45, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 90 + 45), 0) * CFrame.new(1.3, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	local anchor = part({
		name = "AnchorSpike",
		parent = parent,
		size = Vector3.new(0.5, 0.9, 1.4),
		color = accent,
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 0.9) * CFrame.Angles(math.rad(90), 0, 0),
	})
