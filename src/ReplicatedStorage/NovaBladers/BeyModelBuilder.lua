...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		ripper:SetAttribute("SpinMult", 1.15)
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = ripper.CFrame * CFrame.new(0, 0, 1.05),
		})
	...
	chainRing:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, chainRing)
...
local function buildCrystalTide(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			...
			parent = parent,
			size = Vector3.new(0.35, 0.85, 1.1),
			color = accent,
	...
	outerMist:SetAttribute("SpinMult", 0.5)
	table.insert(spinVisuals, outerMist)
