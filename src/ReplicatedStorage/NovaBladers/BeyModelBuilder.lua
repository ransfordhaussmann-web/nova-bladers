...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	-- Heavy stone blocks around perimeter
	for i = 0, 7 do
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		petal:SetAttribute("SpinMult", 0.9)
	...
	outerHalo:SetAttribute("SpinMult", 0.5)
	table.insert(spinVisuals, outerHalo)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	-- Ghost edge blades (counter-rotating)
	for i = 0, 1 do
	...
	phaseRing:SetAttribute("SpinMult", 1.0)
