...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
	...
	local anchor = part({
		...
		material = Enum.Material.DiamondPlate,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.5) * CFrame.Angles(0, 0, math.rad(30)),
	})
	anchor:SetAttribute("SpinMult", 0.4)
	anchor:SetAttribute("SpinOffset", CFrame.new(0, 0, 1.5) * CFrame.Angles(0, 0, math.rad(30)))
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		seg:SetAttribute("SpinMult", 0.35)
	...
	rampart:SetAttribute("SpinMult", 0.25)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.8),
		})
		ray:SetAttribute("SpinMult", 1.2)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.8))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.35)
		local blade = part({
			name = "PhantomBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.9, 2.2),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.2,
			canCollide = false,
