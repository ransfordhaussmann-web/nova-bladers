...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.55)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.2, 1.15)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
		...
		spike:SetAttribute("SpinMult", 0.85)
