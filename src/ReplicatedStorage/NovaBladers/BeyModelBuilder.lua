...
local function buildEmberFang(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(22)) * CFrame.new(0, 0, 1.4)
		local claw = part({
			name = "FlameClaw_" .. i,
			parent = parent,
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = claw.CFrame * CFrame.new(0, 0, 1.35),
		})
	...
	heatRing:SetAttribute("SpinMult", 0.7)
