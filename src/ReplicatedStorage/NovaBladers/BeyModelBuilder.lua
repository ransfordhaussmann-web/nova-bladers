...
local function buildCrimsonOrbit(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildFrostAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.85, accent, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
	...
	local anchorSpike = part({
		name = "AnchorSpike",
		parent = parent,
		size = Vector3.new(0.55, 0.55, 1.4),
		color = color,
		material = Enum.Material.Metal,
		canCollide = false,
...
local BUILDERS = {
	...
	CrimsonOrbit = buildCrimsonOrbit,
