...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(35, 80, 120),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 0.9, 1.5, 0.75, color, Enum.Material.Glass, i * 90, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 90), 0) * CFrame.new(1.2, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	local anchor = part({
		name = "AnchorSpike",
		parent = parent,
		size = Vector3.new(0.6, 1.1, 0.6),
		color = accent,
		material = Enum.Material.Neon,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, -0.65, 0),
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 25, 35),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(32)) * CFrame.new(0, 0, 1.4)
		local fang = part({
			name = "CrimsonFang_" .. i,
			parent = parent,
		...
		fang:SetAttribute("SpinMult", 1.25)
		fang:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, fang)
	end
