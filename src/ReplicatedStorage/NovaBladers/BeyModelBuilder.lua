...
local function buildCrystalVortex(parent, color, accent, baseCFrame)
	...
	vortex:SetAttribute("SpinMult", 1.2)
...
local function buildEmberRing(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.1, 1.15)
		local ember = part({
			name = "Ember_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.35, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	outerGlow:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, outerGlow)
