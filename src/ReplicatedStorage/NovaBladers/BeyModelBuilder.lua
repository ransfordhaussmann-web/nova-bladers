...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	auroraRing:SetAttribute("SpinMult", -0.4)
	...
	for i = 0, 2 do
		...
		local crystal = part({
			...
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.05),
		})
		crystal:SetAttribute("SpinMult", 0.6)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.05))
