...
local function buildCinderVortex(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	emberRing:SetAttribute("SpinMult", 1)
...
local function buildArcticCrown(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	-- Crown spikes (slow counter-spin)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.85, color, Enum.Material.Ice, i * 60, "CrownSpike_" .. i)
...
local BUILDERS = {
	...
	CinderVortex = buildCinderVortex,
	ArcticCrown = buildArcticCrown,
}
