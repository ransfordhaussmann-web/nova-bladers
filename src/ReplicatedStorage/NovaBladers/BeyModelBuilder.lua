...
local function buildBlazeFang(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 3 do
		...
		local tip = part({
			name = "FangTip_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.35, 0.65),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Ice, i * 60, "IceShard_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.28, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	veil:SetAttribute("SpinMult", 0.55)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.0),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.0))
		table.insert(spinVisuals, tip)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisk:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, sunDisk)

	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 2.05)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local BUILDERS = {
	...
	BlazeFang = buildBlazeFang,
	FrostVeil = buildFrostVeil,
