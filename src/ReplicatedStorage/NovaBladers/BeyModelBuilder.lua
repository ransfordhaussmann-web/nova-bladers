...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	bastion:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, bastion)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local edge = part({
			name = "PhantomEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.25, 2.6),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
	...
	ghostRing:SetAttribute("SpinMult", 0.85)
