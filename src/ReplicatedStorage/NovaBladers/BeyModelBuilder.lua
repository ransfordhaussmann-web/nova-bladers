...
local function buildGlacierShield(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.85, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Ice,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.65),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.65))
