...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.6, 1.1, color, Enum.Material.Concrete, i * 45, "FortSeg_" .. i)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.4, 1.55)
		local pillar = part({
			name = "Pillar_" .. i,
			parent = parent,
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.45)
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
