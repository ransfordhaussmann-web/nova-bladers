...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	heatHaze:SetAttribute("SpinMult", 0.7)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local spikeOffset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.35, 1.5)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.9, 1.1),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.15,
			canCollide = false,
	...
	frostAura:SetAttribute("SpinMult", -0.3)
