...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.5) * CFrame.Angles(0, 0, math.rad(45)),
	})
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	shield:SetAttribute("SpinMult", 0.55)
	table.insert(spinVisuals, shield)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(10)) * CFrame.new(0, 0, 1.3)
		local blade = part({
