...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local blade = part({
			...
			parent = parent,
			size = Vector3.new(0.35, 0.3, 2.0),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 180 + 45), math.rad(15)) * CFrame.new(0, 0, 1.25),
		})
		blade:SetAttribute("SpinMult", 1.05)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 180 + 45), math.rad(15)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, blade)
	end
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.6, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(40)) * CFrame.new(0, 0, 1.2),
		})
		shard:SetAttribute("SpinMult", 1.1)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(40)) * CFrame.new(0, 0, 1.2))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.7, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 72), math.rad(30)) * CFrame.new(0, 0.2, 1.15),
		})
		spike:SetAttribute("SpinMult", 1.0)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 72), math.rad(30)) * CFrame.new(0, 0.2, 1.15))
