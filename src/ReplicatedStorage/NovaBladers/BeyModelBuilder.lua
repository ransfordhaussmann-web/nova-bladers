...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Aggressive ripper blades (4 blades)
	for i = 0, 3 do
		local angle = i * 90 + 15
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Ice, i * 60, "CrestSeg_" .. i)
	...
	outerCrest:SetAttribute("SpinMult", -0.3)
	table.insert(spinVisuals, outerCrest)
