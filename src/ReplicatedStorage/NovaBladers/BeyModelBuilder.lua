...
local function buildCrimsonFlare(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 40, 25),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildAquaSpiral(parent, color, accent, baseCFrame)
	...
	for layer = 0, 1 do
		for i = 0, 2 do
			...
			local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, layer * 0.12, 1.1)
			local fin = part({
			...
			fin:SetAttribute("SpinMult", layer == 0 and 0.9 or -0.7)
	...
	waveRing:SetAttribute("SpinMult", 0.85)
