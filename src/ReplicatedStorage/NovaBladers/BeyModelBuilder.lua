...
local function buildStoneHalo(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Concrete, i * 45, "HaloSeg_" .. i)
	...
	haloRing:SetAttribute("SpinMult", -0.4)
	table.insert(spinVisuals, haloRing)
...
local function buildThunderCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.15, 3.2 + i * 0.3, 3.2 + i * 0.3),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25 + i * 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0),
		})
	...
	for i = 0, 3 do
		...
		local bolt = part({
			name = "Arc_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.6, 0.9),
			color = Color3.fromRGB(220, 250, 255),
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.0),
		})
		bolt:SetAttribute("SpinMult", 1.3)
		bolt:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.0))
		table.insert(spinVisuals, bolt)
	end
...
local function buildNightMaw(parent, color, accent, baseCFrame)
	...
	for i, angle in ipairs({ -50, 0, 50 }) do
		...
		fang:SetAttribute("SpinMult", 0.85)
		fang:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, fang)
	end
