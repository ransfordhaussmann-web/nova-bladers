...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	bloodGlow:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, bloodGlow)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.1, 1.15)
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(80, 110, 130),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
