...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, -0.3, 1.1)
		local anchor = part({
			name = "AnchorArm_" .. i,
			parent = parent,
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 1.55),
		})
		ray:SetAttribute("SpinMult", 1.0)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 1.55))
