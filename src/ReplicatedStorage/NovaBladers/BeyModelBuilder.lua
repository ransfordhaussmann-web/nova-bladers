...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.55, 1.3),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(32)) * CFrame.new(0, 0, 1.15),
		})
		shard:SetAttribute("SpinMult", 1.08)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(32)) * CFrame.new(0, 0, 1.15))
	...
	spinRing:SetAttribute("SpinMult", 1.12)
	table.insert(spinVisuals, spinRing)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.2, 1.2),
		})
		spike:SetAttribute("SpinMult", 1)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.2, 1.2))
