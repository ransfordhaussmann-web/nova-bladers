...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Crest shield plates (slow spin)
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Glass, i * 90, "CrestPlate_" .. i)
	...
	auroraRing:SetAttribute("SpinMult", 0.8)
	...
	local spinRing = part({
		...
		color = Color3.fromRGB(100, 255, 220),
		material = Enum.Material.Neon,
		transparency = 0.4,
		canCollide = false,
		cframe = baseCFrame,
