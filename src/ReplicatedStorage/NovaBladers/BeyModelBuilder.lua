...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.1, 1.3)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.5, accent, Enum.Material.Glass, i * 90, "WaveSeg_" .. i)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.8, 1.0, color, Enum.Material.Concrete, i * 60, "RockSeg_" .. i)
