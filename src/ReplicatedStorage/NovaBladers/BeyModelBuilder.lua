...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			name = "FangTip_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.35, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		edge:SetAttribute("SpinMult", 1.0)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60 + 10
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.1)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.7, 1.2),
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.5, 0.9)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.9, 0.4),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
