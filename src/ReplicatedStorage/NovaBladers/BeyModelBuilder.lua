...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.4, 1.0)
		local spike = part({
			name = "StoneSpike_" .. i,
			parent = parent,
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.1, 1.5)
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	bladeRing:SetAttribute("SpinMult", 1.1)
	table.insert(spinVisuals, bladeRing)
