...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	crown:SetAttribute("SpinMult", 0.4)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.1, 1.4)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.3)
		local blade = part({
			name = "PhantomBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.55, 2.0),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
	...
	ghostRing:SetAttribute("SpinMult", 0.75)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 240, 250),
		material = Enum.Material.Glass,
		transparency = 0.1,
		canCollide = false,
		cframe = baseCFrame,
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.25, 1.15)
		local flame = part({
			name = "FlameSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.8, 0.5),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	crown:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, crown)
