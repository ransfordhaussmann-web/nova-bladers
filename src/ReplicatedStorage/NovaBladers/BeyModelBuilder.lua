...
local function buildBlazeRing(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildCrystalCrown(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildNeonDrift(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 180), 0) * CFrame.new(0, 0.2, 1.1)
		local wing = part({
			name = "NeonWing_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.15, 2.2),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local BUILDERS = {
	...
	BlazeRing = buildBlazeRing,
	CrystalCrown = buildCrystalCrown,
	NeonDrift = buildNeonDrift,
}
