...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Aurora glow layers
	for i, mult in ipairs({ 0.85, 1.0, 1.15 }) do
		local glow = part({
			name = "AuroraGlow_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.12, 3.2 * mult, 3.2 * mult),
			color = i == 2 and accent or color,
			material = Enum.Material.Neon,
			transparency = 0.35 + i * 0.1,
			canCollide = false,
			cframe = baseCFrame,
		})
		glow:SetAttribute("SpinMult", 0.7 - i * 0.15)
