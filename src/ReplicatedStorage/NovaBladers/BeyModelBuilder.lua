...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	crownRing:SetAttribute("SpinMult", 0.8)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.35)
		local flare = part({
			name = "SolarFlare_" .. i,
			parent = parent,
		...
		flare:SetAttribute("SpinMult", 1.05)
