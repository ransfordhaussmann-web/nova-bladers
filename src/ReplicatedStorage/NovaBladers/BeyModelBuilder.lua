...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local ringSize = 3.2 + i * 0.35
		local ring = part({
			name = "AuroraRing_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.14, ringSize, ringSize),
			color = Color3.fromRGB(100 + i * 30, 200, 230),
			material = Enum.Material.Neon,
			transparency = 0.25 + i * 0.08,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, 0.05 * i, 0),
		})
		ring:SetAttribute("SpinMult", 0.5 - i * 0.1)
