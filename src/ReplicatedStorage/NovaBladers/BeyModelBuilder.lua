...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Aggressive ripper blades (4-way)
	for i = 0, 3 do
		local angle = i * 90 + 15
		...
		local edge = part({
			name = "BladeEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.25, 0.25, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 230, 240),
	...
	-- Crystalline crest segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Glass, i * 60, "CrestSeg_" .. i)
	...
	shimmer:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, shimmer)

	local frostDome = part({
		name = "FrostDome",
		parent = parent,
		shape = Enum.PartType.Ball,
	...
	table.insert(visuals, frostDome)
