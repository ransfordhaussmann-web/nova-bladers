...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		ember:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.1))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.75, color, Enum.Material.Glass, i * 60, "CrownSeg_" .. i)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.5),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.5))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for layer = 1, 3 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1 + layer * 0.04, radius * 2, radius * 2),
	...
	for i = 0, 2 do
		...
		local flare = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.1, 1.2),
		})
		flare:SetAttribute("SpinMult", 1.3)
		flare:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.1, 1.2))
