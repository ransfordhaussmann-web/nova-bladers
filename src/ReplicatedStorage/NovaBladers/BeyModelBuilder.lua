...
local function buildCrimsonSpike(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildFrostCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.55, color, Enum.Material.Glass, i * 90 + 15, "CoilSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 90 + 15), 0) * CFrame.new(1.28, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local BUILDERS = {
	...
	CrimsonSpike = buildCrimsonSpike,
