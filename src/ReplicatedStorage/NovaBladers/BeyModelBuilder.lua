...
local function buildFrostPrism(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(math.rad(25), math.rad(angle), 0) * CFrame.new(0, 0, 1.1)
		local facet = part({
...
local function buildBlazeRipper(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		tip:SetAttribute("SpinOffset", blade:GetAttribute("SpinOffset") * CFrame.new(0, 0, 1.15))
		table.insert(spinVisuals, tip)
	end
