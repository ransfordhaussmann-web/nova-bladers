...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	-- Crystal facets (slow counter-spin)
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.4)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
		...
		crystal:SetAttribute("SpinMult", -0.5)
	...
	veilRing:SetAttribute("SpinMult", 0.75)
