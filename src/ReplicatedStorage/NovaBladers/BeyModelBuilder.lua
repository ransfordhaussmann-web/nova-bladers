...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	bloodRing:SetAttribute("SpinMult", 1.15)
...
local function buildFrostRing(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 0.75, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
	...
	frostAura:SetAttribute("SpinMult", 0.9)
	...
	for i = 0, 2 do
		...
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.55, 0.9),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.15,
			canCollide = false,
		...
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.15))
