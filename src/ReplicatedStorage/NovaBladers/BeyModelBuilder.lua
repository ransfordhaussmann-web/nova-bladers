...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	frostAura:SetAttribute("SpinMult", 0.55)
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 0.9)
		local coil = part({
			name = "HelixCoil_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
	...
	solarRing:SetAttribute("SpinMult", 1.25)
	table.insert(spinVisuals, solarRing)
