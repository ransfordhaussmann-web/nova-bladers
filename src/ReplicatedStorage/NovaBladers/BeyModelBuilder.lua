...
local function buildCrimsonRipper(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local blade = ringSegment(parent, 0.85, 1.4, 0.55, color, Enum.Material.Neon, i * 120, "Blade_" .. i)
		blade:SetAttribute("SpinMult", 1.15)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 120), 0) * CFrame.new(1.3, 0, 0))
		table.insert(spinVisuals, blade)
	end
...
local function buildFrostHalo(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local crystal = part({
			...
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(25)) * CFrame.new(0, 0, 1.35),
		})
		crystal:SetAttribute("SpinMult", 0.75)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(25)) * CFrame.new(0, 0, 1.35))
	...
	haloRing:SetAttribute("SpinMult", 0.65)
