	...
	for i = 0, 5 do
		...
		local spike = part({
			...
			cframe = baseCFrame
				...
				* CFrame.new(0, 0.55, 1.05),
		})
		table.insert(visuals, spike)
	end

	-- Frost ring segments (slow spin)
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.7, color, Enum.Material.Ice, i * 90, "FrostSeg_" .. i)
