...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	veil:SetAttribute("SpinMult", 1.15)
	table.insert(spinVisuals, veil)

	for i = 0, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 180 + 40), math.rad(15)) * CFrame.new(0, 0, 1.15)
		local blade = part({
			name = "GhostBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.08, 1.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			...
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.3),
		})
		shard:SetAttribute("SpinMult", 1)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.3))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.55, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
	...
	flameRing:SetAttribute("SpinMult", 1.35)
