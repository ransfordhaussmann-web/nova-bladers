...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.05),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.05))
		table.insert(spinVisuals, tip)
	end
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.7, 0.3, 0),
		})
		spike:SetAttribute("SpinMult", 0.4)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.7, 0.3, 0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	halo:SetAttribute("SpinMult", -0.7)
	table.insert(spinVisuals, halo)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.4, 1.35),
		})
		spike:SetAttribute("SpinMult", 0.75)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.4, 1.35))
		table.insert(spinVisuals, spike)
	end
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	streak:SetAttribute("SpinMult", 1.5)
	table.insert(spinVisuals, streak)

	for i = 0, 1 do
		local wing = part({
			name = "Wing_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.15, 1.8),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 180 + 90), math.rad(8)) * CFrame.new(0, 0, 1.1),
		})
		wing:SetAttribute("SpinMult", 1.5)
		wing:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 180 + 90), math.rad(8)) * CFrame.new(0, 0, 1.1))
