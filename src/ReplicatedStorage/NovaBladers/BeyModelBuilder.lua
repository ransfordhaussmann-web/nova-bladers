...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.05),
		})
		spike:SetAttribute("SpinMult", 0.5)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.05))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.22, 3.6 + i * 0.15, 3.6 + i * 0.15),
			color = (i % 2 == 0) and color or accent,
			material = Enum.Material.Neon,
			transparency = 0.2 + i * 0.05,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(i * 8)),
		})
		coil:SetAttribute("SpinMult", 1.0 + i * 0.08)
		coil:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(i * 8)))
