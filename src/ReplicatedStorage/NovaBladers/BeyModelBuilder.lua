...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Cross blades (X pattern, spin fast)
	for i, angle in ipairs({ 0, 90 }) do
		...
		local edge = part({
			name = "BladeEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.35, 2.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0, 0.15, 0),
		})
		edge:SetAttribute("SpinMult", 1.15)
		edge:SetAttribute("SpinOffset", offset * CFrame.new(0, 0.15, 0))
