...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.85)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	veil:SetAttribute("SpinMult", 1.05)
	table.insert(spinVisuals, veil)

	for i, angle in ipairs({ -50, 0, 55 }) do
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.25)
		local edge = part({
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 40, 10),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-35)) * CFrame.new(0, 0.45, 1.1)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.9, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		spike:SetAttribute("SpinMult", 1.2)
