...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Neon, angle, "CrestSeg_" .. i)
		seg.Transparency = 0.15
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end
