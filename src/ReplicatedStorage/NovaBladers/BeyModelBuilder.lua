...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "RockSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local ring = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1, 3.5 + i * 0.6, 3.5 + i * 0.6),
		...
		ring:SetAttribute("SpinMult", 1.0 - i * 0.2)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	shield:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, shield)
