...
local function tryExternalMesh(beyData, parent, baseCFrame)
	...
	local mp = meshPart({
		...
		meshId = meshId,
		textureId = textureId,
		size = (assets and assets.size) or (store and store.size) or Vector3.new(3.6, 1.2, 3.6),
		color = beyData.color,
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, accent, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
