...
local function buildGlacierCrown(parent, color, accent, baseCFrame)
	...
	frostCrown:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, frostCrown)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(25), math.rad(angle), 0) * CFrame.new(0, 0.55, 1.1),
		})
		spike:SetAttribute("SpinMult", 0.8)
		spike:SetAttribute("SpinOffset", CFrame.Angles(math.rad(25), math.rad(angle), 0) * CFrame.new(0, 0.55, 1.1))
