...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.0)
		local pillar = part({
			name = "Pillar_" .. i,
			parent = parent,
		...
		pillar:SetAttribute("SpinOffset", offset)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.55)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
