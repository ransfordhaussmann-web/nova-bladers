...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.85, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
	...
	frostAura:SetAttribute("SpinMult", 0.65)
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, 0.9, 0.9),
			color = color,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.0 + i * 0.12),
		})
		coil:SetAttribute("SpinMult", 1.0 + i * 0.08)
		coil:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.0 + i * 0.12))
	...
	helixRing:SetAttribute("SpinMult", 1.3)
