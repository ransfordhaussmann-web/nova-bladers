...
local function buildCrystalFang(parent, color, accent, baseCFrame)
	...
	prism:SetAttribute("SpinMult", 1.2)
...
local function buildGraniteCrush(parent, color, accent, baseCFrame)
	...
	crushPlate:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, crushPlate)
...
local function buildEmberOrbit(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.1)
		local flame = part({
			name = "Flame_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.6, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	outerFlare:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, outerFlare)
