...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	outerFlare:SetAttribute("SpinMult", 0.75)
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60 + 15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.3)
		local shard = part({
			name = "IceShard_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.65, 1.1),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.15,
			canCollide = false,
		...
		shard:SetAttribute("SpinMult", 0.8)
