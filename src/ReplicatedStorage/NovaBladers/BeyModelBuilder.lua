...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.05)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Glass, i * 90, "IceSeg_" .. i)
