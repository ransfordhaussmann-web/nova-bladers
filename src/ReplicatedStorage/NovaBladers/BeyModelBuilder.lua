...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		size = Vector3.new(0.9, 2.0, 2.0), color = Color3.fromRGB(240, 180, 80),
	...
	for i = 0, 3 do
		...
		feather:SetAttribute("SpinMult", 1.15)
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.2, 2.8, 2.8), color = Color3.fromRGB(50, 90, 120),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Glass, i * 90, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 90), 0) * CFrame.new(1.3, 0, 0))
		table.insert(spinVisuals, seg)
	end
	local anchor = part({
		name = "Anchor", parent = parent, size = Vector3.new(0.8, 0.6, 1.2),
		color = accent, material = Enum.Material.Metal, canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.1),
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.85, 2.1, 2.1), color = Color3.fromRGB(80, 20, 25),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 2 do
		...
		local fang = part({
			...
			size = Vector3.new(0.55, 0.4, 2.0), color = accent,
			material = Enum.Material.Neon, canCollide = false, cframe = baseCFrame * offset,
		})
		fang:SetAttribute("SpinMult", 1.05)
		fang:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, fang)
	end
	local spinRing = part({
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.25, 2.7, 2.7), color = Color3.fromRGB(90, 85, 80),
		material = Enum.Material.Slate, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 60, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.52, 0, 0))
		table.insert(spinVisuals, seg)
	end
	local spinRing = part({
		name = "SpinRing", parent = parent, shape = Enum.PartType.Cylinder,
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.7, 1.7, 1.7), color = Color3.fromRGB(255, 230, 120),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	local sunDisc = part({
		name = "SunDisc", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.2, 4.0, 4.0), color = color,
		material = Enum.Material.Neon, transparency = 0.2, canCollide = false, cframe = baseCFrame,
	})
	sunDisc:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, sunDisc)
	for i = 0, 7 do
		local angle = i * 45
		local ray = part({
			name = "Ray_" .. i, parent = parent,
			size = Vector3.new(0.15, 0.15, 1.4), color = accent,
			material = Enum.Material.Neon, canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 2.0),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 2.0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.8, 2.0, 2.0), color = Color3.fromRGB(40, 30, 70),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	local ghostRing = part({
		name = "GhostRing", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.25, 3.3, 3.3), color = color,
		material = Enum.Material.ForceField, transparency = 0.4, canCollide = false, cframe = baseCFrame,
	})
	ghostRing:SetAttribute("SpinMult", 1.1)
	table.insert(spinVisuals, ghostRing)
	for i = 0, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 180 + 45), math.rad(15)) * CFrame.new(0, 0, 1.25)
		local blade = part({
			name = "PhaseBlade_" .. i, parent = parent,
			size = Vector3.new(0.35, 0.3, 1.9), color = accent,
			material = Enum.Material.Neon, transparency = 0.15, canCollide = false, cframe = baseCFrame * offset,
		})
	...
	local spinRing = part({
		name = "SpinRing", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.12, 3.8, 3.8), color = accent,
