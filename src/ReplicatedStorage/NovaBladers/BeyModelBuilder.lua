...
local function buildBlazeDrift(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 3 do
		local angle = i * 90 + 15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.25)
		local spike = part({
			name = "FlameSpike_" .. i,
			parent = parent,
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(55)) * CFrame.new(0, 0.35, 1.1)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
