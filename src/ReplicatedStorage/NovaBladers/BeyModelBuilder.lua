...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		petal:SetAttribute("SpinMult", 1.25)
	...
	fireRing:SetAttribute("SpinMult", 1.1)
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	-- Ice crystal segments (slow counter-spin)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, accent, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
