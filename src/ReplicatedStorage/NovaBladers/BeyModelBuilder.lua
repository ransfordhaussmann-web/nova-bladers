...
local function buildCrimsonEdge(parent, color, accent, baseCFrame)
	...
	-- 4 sharp crimson blades
	for i = 0, 3 do
		local angle = i * 90 + 15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(22)) * CFrame.new(0, 0, 1.3)
		local blade = part({
			name = "FlameBlade_" .. i,
			parent = parent,
...
local function buildFrostHalo(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.85, color, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
