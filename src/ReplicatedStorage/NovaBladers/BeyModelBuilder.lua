...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	-- Frost crest plates (6 segments)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Ice, i * 60, "CrestSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end

	local auroraGlow = part({
		name = "AuroraGlow",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	auroraGlow:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, auroraGlow)
	...
	local frostShield = part({
		...
		color = Color3.fromRGB(200, 255, 250),
		material = Enum.Material.Glass,
		transparency = 0.55,
		canCollide = false,
		cframe = baseCFrame,
	})
	frostShield:SetAttribute("SpinMult", -0.4)
