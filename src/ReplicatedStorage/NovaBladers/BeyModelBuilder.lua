...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 35, 30),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildCrystalHalo(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.55, accent, Enum.Material.Neon, i * 60, "PrismSeg_" .. i)
	...
	halo:SetAttribute("SpinMult", 0.75)
