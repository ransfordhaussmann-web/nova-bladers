...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisc:SetAttribute("SpinMult", 1.15)
	table.insert(spinVisuals, sunDisc)

	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.15)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 90, 100),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.1, 1.2)
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.55, 1.4),
			color = color,
		...
		shard:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, shard)
	end

	local crystalRing = part({
		name = "CrystalRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	crystalRing:SetAttribute("SpinMult", 1.1)
	table.insert(spinVisuals, crystalRing)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.9, 2.0, 2.0),
		color = Color3.fromRGB(120, 50, 15),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.25, 1.1)
		local flame = part({
			name = "CrownFlame_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.6, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
		...
		flame:SetAttribute("SpinMult", 1.0)
		flame:SetAttribute("SpinOffset", offset)
	...
	fireAura:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, fireAura)
