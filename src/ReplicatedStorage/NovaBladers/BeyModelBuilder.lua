...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Spiral vortex blades (4 curved segments)
	for i = 0, 3 do
		local angle = i * 90 + 20
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	-- Crown spikes (static, don't spin)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.6, 1.1)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	iceRing:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, iceRing)

	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.5, accent, Enum.Material.Ice, i * 90, "FrostSeg_" .. i)
