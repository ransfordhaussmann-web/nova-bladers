...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Three curved attack fins (spin fast)
	for i = 0, 2 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fin.CFrame * CFrame.new(0, 0, 1.15),
		})
		tip:SetAttribute("SpinMult", 1.25)
	...
	vortexRing:SetAttribute("SpinMult", -1.1)
	...
	local tip = part({
		...
		color = Color3.fromRGB(255, 200, 190),
		material = Enum.Material.Glass,
		canCollide = false,
