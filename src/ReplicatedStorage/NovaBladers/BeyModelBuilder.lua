...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.15)
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.0)
		local ember = part({
			name = "Ember_" .. i,
			parent = parent,
			shape = Enum.PartType.Ball,
		...
		ember:SetAttribute("SpinOffset", offset)
