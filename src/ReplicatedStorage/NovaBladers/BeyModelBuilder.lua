...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		anchor:SetAttribute("SpinMult", 0.7)
	...
	waterRing:SetAttribute("SpinMult", 0.85)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.9)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
