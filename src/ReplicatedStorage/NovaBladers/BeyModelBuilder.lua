...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.85),
		})
		ray:SetAttribute("SpinMult", 1.0)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.85))
	...
	local flare = Instance.new("Fire")
	flare.Size = 2
	flare.Heat = 4
	flare.Color = color
	flare.SecondaryColor = accent
	flare.Parent = core
