...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	-- Feather blades (4 angled quills)
	for i = 0, 3 do
		local angle = i * 90 + 15
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	-- Anchor hooks (2 opposing)
	for i, angle in ipairs({ 0, 180 }) do
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-40)) * CFrame.new(0, 0, 1.4)
		local hook = part({
			name = "AnchorHook_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.45, 1.6),
			color = Color3.fromRGB(70, 100, 130),
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		hook:SetAttribute("SpinMult", 0.75)
		hook:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, hook)
	end

	-- Wave ring segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.35, accent, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
	...
	tideRing:SetAttribute("SpinMult", 0.55)
