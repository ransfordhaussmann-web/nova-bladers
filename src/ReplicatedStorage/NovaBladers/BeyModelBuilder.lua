...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		edge:SetAttribute("SpinMult", 1.2)
		edge:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.35))
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local pillar = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.65),
		})
		pillar:SetAttribute("SpinMult", 0.4)
		pillar:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.65))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local edge = part({
			name = "PhantomEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.55, 1.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
