...
local function buildBlazeOrbit(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		comet:SetAttribute("SpinMult", 1.3)
		comet:SetAttribute("SpinOffset", offset)
	...
	emberGlow:SetAttribute("SpinMult", 0.75)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.55, 0.35)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	frostAura:SetAttribute("SpinMult", 0.8)
