...
local function buildCrystalPrism(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	prismRing:SetAttribute("SpinMult", 0.75)
...
local function buildFlameCrest(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		...
		crest:SetAttribute("SpinMult", 1.15)
		crest:SetAttribute("SpinOffset", offset)
		...
		local flame = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = crest.CFrame * CFrame.new(0, 0, 1.1),
		})
...
local BUILDERS = {
	...
	CrystalPrism = buildCrystalPrism,
	FlameCrest = buildFlameCrest,
}
