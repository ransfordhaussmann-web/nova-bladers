...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0, 1.7),
		})
		spike:SetAttribute("SpinMult", 0.4)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0, 1.7))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local ring = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1, 3.2 + i * 0.5, 3.2 + i * 0.5),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25 + i * 0.1,
			canCollide = false,
			cframe = baseCFrame,
		})
		ring:SetAttribute("SpinMult", 0.9 - i * 0.15)
