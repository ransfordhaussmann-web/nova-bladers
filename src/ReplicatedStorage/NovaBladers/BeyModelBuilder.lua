...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		fang:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, fang)
	end
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.85, 1.0, color, Enum.Material.Concrete, i * 60, "RockSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0.1, 1.1)
		local flare = part({
			name = "Flare_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.6, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 0.9)
		local ghost = part({
			name = "GhostShard_" .. i,
			parent = parent,
			shape = Enum.PartType.Ball,
		...
		ghost:SetAttribute("SpinOffset", offset)
