...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Glass,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.5, 1.55),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.5, 1.55))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		coil:SetAttribute("SpinMult", 1.15)
	...
	helix:SetAttribute("SpinMult", 1.3)
