...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostRing:SetAttribute("SpinMult", -0.6)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			name = "Shard_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.55, 0.9),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.15),
		})
		shard:SetAttribute("SpinMult", 1.15)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.15))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.9, 2.0, 2.0),
		color = Color3.fromRGB(200, 80, 20),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 4 do
		...
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.7, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.35, 1.1),
		})
		spike:SetAttribute("SpinMult", 0.8)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.35, 1.1))
	...
	flameRing:SetAttribute("SpinMult", 1)
