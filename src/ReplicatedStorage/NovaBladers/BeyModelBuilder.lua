...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Swirling vortex blades (4 curved segments)
	for i = 0, 3 do
		local angle = i * 90 + 20
		...
		flare:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.3))
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	-- Ice crystal segments (6 hexagonal shell pieces)
	for i = 0, 5 do
	...
	-- Crystal spikes on outer rim
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0, 1.85)
		local spike = part({
