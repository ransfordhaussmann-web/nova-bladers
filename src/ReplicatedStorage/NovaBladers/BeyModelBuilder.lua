...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.2, 1.3)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, -0.3, 1.0) * CFrame.Angles(math.rad(45), 0, 0),
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0, 1.4)
		local fang = part({
			name = "Fang_" .. i,
			parent = parent,
	...
	bloodRing:SetAttribute("SpinMult", 1)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.3, 3.0, 3.0),
		color = Color3.fromRGB(70, 68, 65),
	...
	for i = 0, 5 do
		local block = part({
			name = "StoneBlock_" .. i,
			parent = parent,
			size = Vector3.new(0.7, 0.9, 1.0),
			color = color,
			material = Enum.Material.Concrete,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.55, 0, 0),
		})
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.55, 0, 0))
	...
	rampart:SetAttribute("SpinMult", 0.45)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.8),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.8))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.7, 1.9, 1.9),
		color = Color3.fromRGB(100, 130, 150),
		material = Enum.Material.Glass,
		transparency = 0.3,
		canCollide = false,
		cframe = baseCFrame,
	...
	ghostAura:SetAttribute("SpinMult", 1.1)
	...
	spinRing:SetAttribute("SpinMult", 1.25)
	table.insert(spinVisuals, spinRing)
