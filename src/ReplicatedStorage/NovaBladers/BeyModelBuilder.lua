...
local function buildEmberCoil(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.1)
		local coil = part({
			name = "FireCoil_" .. i,
			parent = parent,
	...
	innerFlame:SetAttribute("SpinMult", -0.9)
	...
	for i = 0, 5 do
		local angle = i * 60 + 15
		local emberOffset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.55)
		local ember = part({
			name = "Ember_" .. i,
			parent = parent,
			shape = Enum.PartType.Ball,
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * emberOffset,
		})
		ember:SetAttribute("SpinMult", 1.3)
		ember:SetAttribute("SpinOffset", emberOffset)
...
local function buildFrostHalo(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.85, color, Enum.Material.Glass, i * 60, "FrostSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local shardOffset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.25)
		local shard = part({
			name = "IceShard_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.8, 1.2),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * shardOffset,
		})
		shard:SetAttribute("SpinMult", 0.85)
		shard:SetAttribute("SpinOffset", shardOffset)
...
local function buildGraniteCrown(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.75, 0.95, color, Enum.Material.Concrete, i * 60, "StoneSeg_" .. i)
	...
	for i = 0, 3 do
		local angle = i * 90 + 10
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.25, 1.0)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.45, 0.55, 1.4),
			color = accent,
	...
	bumper:SetAttribute("SpinMult", -0.3)
...
local BUILDERS = {
	...
	EmberCoil = buildEmberCoil,
	FrostHalo = buildFrostHalo,
	GraniteCrown = buildGraniteCrown,
}
