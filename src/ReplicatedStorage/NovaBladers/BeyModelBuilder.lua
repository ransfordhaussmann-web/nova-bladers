...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local fluke = part({
			name = "AnchorFluke_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.35, 1.4),
			color = color,
			material = Enum.Material.DiamondPlate,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	-- Heavy outer anchor ring (counter-rotates)
	local anchorRing = part({
		name = "AnchorRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	anchorRing:SetAttribute("SpinMult", -0.3)
	table.insert(spinVisuals, anchorRing)
