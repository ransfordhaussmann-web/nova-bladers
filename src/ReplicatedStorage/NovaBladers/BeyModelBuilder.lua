...
local function buildCrimsonForge(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		hammer:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.25))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.65, 0.75, color, Enum.Material.Glass, i * 60, "CrownSeg_" .. i)
	...
	for i = 0, 5 do
		...
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.85))
