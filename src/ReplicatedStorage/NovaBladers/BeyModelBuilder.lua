...
local function buildBlazeReactor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local flame = part({
			name = "Flame_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.5, 0.6),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * offset * CFrame.new(0, 0, 0.55),
		})
		flame:SetAttribute("SpinMult", 1)
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 0.55))
