...
local function buildStarfallEdge(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 3 do
		...
		spike:SetAttribute("SpinMult", 1.15)
...
local function buildStoneHalo(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.28, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	halo:SetAttribute("SpinMult", 0.55)
...
local function buildThunderCoil(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for layer = 1, 2 do
		local coil = part({
			name = "CoilRing_" .. layer,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.15 + layer * 0.05, 3.2 + layer * 0.6, 3.2 + layer * 0.6),
			color = layer == 1 and color or accent,
			material = Enum.Material.Neon,
			transparency = 0.2 + layer * 0.1,
			canCollide = false,
			cframe = baseCFrame,
		})
		coil:SetAttribute("SpinMult", layer == 1 and 1.2 or -0.9)
	...
	for i = 0, 3 do
		...
		local bolt = part({
			name = "CoilBolt_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.55, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.15),
		})
		bolt:SetAttribute("SpinMult", 1.3)
		bolt:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.15))
		table.insert(spinVisuals, bolt)
	end
...
local function buildNightMaw(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	maw:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, maw)
	...
	for i = 0, 2 do
		...
		local claw = part({
			name = "NightClaw_" .. i,
			parent = parent,
			size = Vector3.new(0.45, 0.35, 1.5),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		claw:SetAttribute("SpinMult", 0.85)
...
local BUILDERS = {
	...
	StarfallEdge = buildStarfallEdge,
	StoneHalo = buildStoneHalo,
	ThunderCoil = buildThunderCoil,
	NightMaw = buildNightMaw,
}
