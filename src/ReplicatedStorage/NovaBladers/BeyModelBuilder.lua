...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Glass, i * 60, "CrownSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 0.85)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	-- Helix coil segments (counter-rotating)
	for i = 0, 5 do
		local angle = i * 60
		local tilt = math.rad(30 + i * 8)
		local offset = CFrame.Angles(0, math.rad(angle), tilt) * CFrame.new(0, 0.1 * (i % 2 == 0 and 1 or -1), 1.0)
		local coil = part({
			name = "CoilSeg_" .. i,
			parent = parent,
		...
		coil:SetAttribute("SpinMult", i % 2 == 0 and 1.3 or -0.9)
	...
	sunRing:SetAttribute("SpinMult", 1.1)
