...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.35),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.35))
		table.insert(spinVisuals, tip)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.25)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.55, 1.4),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		ray:SetAttribute("SpinMult", 0.95)
	...
	halo:SetAttribute("SpinMult", 1.3)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(math.rad(20), math.rad(angle), 0) * CFrame.new(0, 0, 1.5)
		local prism = part({
			name = "Prism_" .. i,
			parent = parent,
		...
		prism:SetAttribute("SpinMult", 1)
		prism:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, prism)
	end
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.3, 1.2)
		local flame = part({
			name = "CrownFlame_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.9, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		flame:SetAttribute("SpinMult", 1.3)
	...
	fireRing:SetAttribute("SpinMult", 1.5)
	table.insert(spinVisuals, fireRing)

	local spinRing = part({
		name = "SpinRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.16, 4.2, 4.2),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.35,
		canCollide = false,
