...
Each catalog entry may include modelAssets.creatorStoreSearch as a Toolbox hint.

Search Roblox Studio Toolbox → Creator Store → "spinning top" / bey name
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.55, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.45)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
