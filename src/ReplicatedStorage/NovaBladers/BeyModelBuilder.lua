...
local CreatorStoreMeshes = require(script.Parent.CreatorStoreMeshes)

local function mergeCreatorStoreAssets(beyData)
	local store = CreatorStoreMeshes[beyData.id]
	if not store then
		return beyData.modelAssets
	end
	if beyData.modelAssets then
		return beyData.modelAssets
	end
	if store.meshId then
		return {
			meshId = store.meshId,
			textureId = store.textureId,
			size = store.size,
		}
	end
	return nil
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 50, 45),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Ice, i * 60, "CrownSeg_" .. i)
...
function BeyModelBuilder.build(beyData, spawnCFrame)
	...
	if imported then
		...
		local modelAssets = mergeCreatorStoreAssets(beyData)
		local meshBeyData = beyData
		if modelAssets then
			meshBeyData = table.clone(beyData)
			meshBeyData.modelAssets = modelAssets
		end
		local external = tryExternalMesh(meshBeyData, visualFolder, baseCFrame)
