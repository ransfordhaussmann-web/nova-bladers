...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = claw.CFrame * CFrame.new(0, 0, 1.05),
		})
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Ice, i * 60, "FrostSeg_" .. i)
	...
	crest:SetAttribute("SpinMult", 0.5)
	...
	local auroraGlow = part({
		...
		color = Color3.fromRGB(160, 255, 220),
		material = Enum.Material.Neon,
		transparency = 0.5,
		canCollide = false,
		cframe = baseCFrame,
