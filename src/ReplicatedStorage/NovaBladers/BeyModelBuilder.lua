...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local radius = 1.0 + i * 0.35
		local coilColor = i == 0 and color or accent
		local coil = part({
			name = "CoilRing_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18 - i * 0.04, radius * 2 + 0.6, radius * 2 + 0.6),
			color = coilColor,
			material = Enum.Material.Neon,
			transparency = 0.1 + i * 0.15,
			canCollide = false,
			cframe = baseCFrame,
		})
		coil:SetAttribute("SpinMult", 1.0 + i * 0.25)
		table.insert(spinVisuals, coil)
	end

	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.6, 0.1, 0)
		local flare = part({
			name = "SolarFlare_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.15, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
