...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.85, color, Enum.Material.Glass, i * 60, "CrownSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Glass,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.5),
		})
		spike:SetAttribute("SpinMult", 0.8)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.5))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for layer = 1, 3 do
		...
		local coil = part({
			...
			material = layer == 2 and Enum.Material.Neon or Enum.Material.SmoothPlastic,
			transparency = layer == 3 and 0.3 or 0,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, (layer - 2) * 0.08, 0),
		})
