...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 0.95, 1.55, 0.75, color, Enum.Material.Glass, i * 60, "CrownSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.25, 0, 0))
		table.insert(spinVisuals, seg)
	end

	for i = 0, 2 do
		local angle = i * 120 + 15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-35)) * CFrame.new(0, 0.55, 0.95)
		local spike = part({
