...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		tooth:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.35))
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.SmoothPlastic, i * 60, "CrestPetal_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end

	local auroraInner = part({
		name = "AuroraInner",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	auroraInner:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, auroraInner)

	local auroraOuter = part({
		name = "AuroraOuter",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	auroraOuter:SetAttribute("SpinMult", -0.5)
	table.insert(spinVisuals, auroraOuter)
