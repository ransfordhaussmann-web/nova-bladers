...
local function buildAquaSpiral(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		fin:SetAttribute("SpinMult", 1.05)
...
local function buildCrimsonBlaze(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local flame = part({
			name = "FlameTip_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.6, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0, 0.1, 1.2),
		})
		flame:SetAttribute("SpinMult", 1.15)
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0.1, 1.2))
