...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Spiral vortex blades (fast spin)
	for i = 0, 3 do
		local angle = i * 90 + 22
		...
		blade:SetAttribute("SpinMult", 1.35)
	...
	innerVortex:SetAttribute("SpinMult", 1.6)
	table.insert(spinVisuals, innerVortex)
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	iceRing:SetAttribute("SpinMult", -0.3)
