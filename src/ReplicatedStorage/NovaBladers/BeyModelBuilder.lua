...
local function buildCrystalVortex(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(math.rad(35), math.rad(angle), 0) * CFrame.new(0, 0, 1.15)
		local shard = part({
			name = "CrystalShard_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.85, 0.35),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
	...
	prismRing:SetAttribute("SpinMult", 0.95)
