--[[
	...
	or BeyCatalog.creatorStore.meshId (search terms in creatorStore.searchTerms).
]]
...
local function tryExternalMesh(beyData, parent, baseCFrame)
	...
	local creatorMesh = beyData.creatorStore and beyData.creatorStore.meshId
	local meshId = (assets and assets.meshId) or creatorMesh
	if not meshId then
		return nil
	end
	...
	local mp = meshPart({
		...
		parent = parent,
		meshId = meshId,
		textureId = (assets and assets.textureId) or (beyData.creatorStore and beyData.creatorStore.textureId),
		size = (assets and assets.size) or (beyData.creatorStore and beyData.creatorStore.size) or Vector3.new(3.6, 1.2, 3.6),
		color = beyData.color,
...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		spoke:SetAttribute("SpinMult", 1.2)
	...
	outer:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, outer)
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
