...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Aggressive ripper fangs (asymmetric, spin fast)
	for i, angle in ipairs({ 0, 110, 235 }) do
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Layered aurora crest rings (slow counter-rotation)
	for i = 0, 2 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.35, color, Enum.Material.Neon, i * 120, "CrestSeg_" .. i)
		seg.Transparency = 0.2
		seg:SetAttribute("SpinMult", 0.75 - i * 0.1)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 120), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	prism:SetAttribute("SpinMult", 0.6)
	...
	outerHalo:SetAttribute("SpinMult", -0.4)
