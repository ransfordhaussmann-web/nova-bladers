...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	slashRing:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, slashRing)
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Crest shield petals (slow spin)
	for i = 0, 5 do
		local seg = ringSegment(parent, 0.9, 1.55, 0.75, color, Enum.Material.SmoothPlastic, i * 60, "CrestPetal_" .. i)
	...
	shimmer:SetAttribute("SpinMult", -0.5)
