...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.5)
		local flare = part({
			name = "SolarFlare_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.15, 1.0),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		flare:SetAttribute("SpinMult", 1.0)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	crown:SetAttribute("SpinMult", 0.7)
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	driftRing:SetAttribute("SpinMult", 1.05)
	...
	for i = 0, 2 do
		...
		wing:SetAttribute("SpinMult", 1.05)
