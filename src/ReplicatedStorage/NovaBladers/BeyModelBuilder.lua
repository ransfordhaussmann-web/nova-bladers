...
local function buildCrimsonRipper(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	vortexRing:SetAttribute("SpinMult", -0.9)
...
local function buildFrostMonarch(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.35, 1.05),
		})
		spike:SetAttribute("SpinMult", 0.6)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.35, 1.05))
	...
	frostAura:SetAttribute("SpinMult", 0.5)
...
local BUILDERS = {
	...
	CrimsonRipper = buildCrimsonRipper,
	FrostMonarch = buildFrostMonarch,
}
