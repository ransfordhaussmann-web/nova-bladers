...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.6, 1.0, color, Enum.Material.Concrete, i * 45, "Rampart_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.3, 0, 0))
		table.insert(spinVisuals, seg)
	end

	local battlements = part({
		name = "Battlements",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	battlements:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, battlements)
