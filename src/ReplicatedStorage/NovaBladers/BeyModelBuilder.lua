...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		size = Vector3.new(0.9, 2.0, 2.0), color = Color3.fromRGB(80, 40, 20),
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		name = "Anchor", parent = parent, shape = Enum.PartType.Ball,
		size = Vector3.new(0.9, 0.9, 0.9), color = accent,
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.85, 2.1, 2.1), color = Color3.fromRGB(60, 20, 25),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 1 do
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.3, 3.0, 3.0), color = Color3.fromRGB(70, 65, 60),
		material = Enum.Material.Slate, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 60, "FortSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	flatRing:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, flatRing)
	for i = 0, 5 do
		local angle = i * 60
		local ray = part({
			name = "Ray_" .. i, parent = parent,
			size = Vector3.new(0.15, 0.6, 0.9), color = accent,
			material = Enum.Material.Neon, canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.1),
		})
		ray:SetAttribute("SpinMult", 1.2)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.1))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.8, 2.0, 2.0), color = Color3.fromRGB(100, 110, 130),
	...
	for i = 0, 2 do
		...
		local blade = part({
			...
			size = Vector3.new(0.2, 0.5, 1.7), color = accent,
	...
	local spinRing = part({
		...
		material = Enum.Material.Neon, transparency = 0.55, canCollide = false, cframe = baseCFrame,
	})
