...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.85, 0.2, 0),
		})
		spike:SetAttribute("SpinMult", 0.4)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.85, 0.2, 0))
	...
	shield:SetAttribute("SpinMult", 0.35)
	table.insert(spinVisuals, shield)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostLayer:SetAttribute("SpinMult", 1.05)
	...
	mirageRing:SetAttribute("SpinMult", -0.6)
	table.insert(spinVisuals, mirageRing)
