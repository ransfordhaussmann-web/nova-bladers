...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.15),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.15))
		table.insert(spinVisuals, tip)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.85)
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.3)
		local blade = part({
			name = "PhantomBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.7, 2.2),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.25,
			canCollide = false,
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	ring:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, ring)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.2, 1.1)
		local flame = part({
			name = "FlameCrown_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.55, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
		...
		flame:SetAttribute("SpinMult", 1.05)
	...
	crown:SetAttribute("SpinMult", 0.9)
	...
	ember:SetAttribute("SpinMult", 0.75)
