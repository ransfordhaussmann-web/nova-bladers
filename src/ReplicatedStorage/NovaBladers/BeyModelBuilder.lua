...
local function buildCrystalVortex(parent, color, accent, baseCFrame)
	...
	frost:SetAttribute("SpinMult", 1.05)
...
local function buildEmberRing(parent, color, accent, baseCFrame)
	...
	heat:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, heat)

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.55, color, Enum.Material.Neon, i * 60, "EmberSeg_" .. i)
	...
	for i = 0, 1 do
		...
		wing:SetAttribute("SpinMult", 0.8)
