...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0, 0, 1.15),
		})
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 2.0)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	veil:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, veil)

	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.25)
		local edge = part({
			name = "GhostEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.6, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
		...
		edge:SetAttribute("SpinMult", 0.75)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.1)
		local shard = part({
	...
	frostRing:SetAttribute("SpinMult", 1)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.3, 1.15)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
