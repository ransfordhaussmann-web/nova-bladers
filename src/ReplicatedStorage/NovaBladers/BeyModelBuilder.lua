...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Aggressive saw-tooth fangs (4 blades)
	for i = 0, 3 do
		local angle = i * 90 + 15
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 140, 150),
		material = Enum.Material.Glass,
		transparency = 0.15,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.SmoothPlastic, i * 60, "CrestSeg_" .. i)
	...
	glow:SetAttribute("SpinMult", 0.6)
	...
	barrier:SetAttribute("SpinMult", -0.4)
	table.insert(spinVisuals, barrier)
