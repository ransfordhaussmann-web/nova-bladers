...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.1, 1.25)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.2, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		name = "Anchor",
		parent = parent,
		size = Vector3.new(0.6, 0.8, 1.4),
		color = accent,
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.1) * CFrame.Angles(math.rad(25), 0, 0),
	})
	anchor:SetAttribute("SpinMult", 0.5)
	table.insert(spinVisuals, anchor)

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.75, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.3)
		local fang = part({
			name = "Fang_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.4, 1.9),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		local block = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.5, 0, 0),
		})
		block:SetAttribute("SpinMult", 0.4)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.5, 0, 0))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.5),
		})
		ray:SetAttribute("SpinMult", 1.05)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.5))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local blade = part({
			name = "PhantomBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.5, 2.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.2),
		})
		blade:SetAttribute("SpinMult", 1.2)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.2))
		table.insert(spinVisuals, blade)
	end
	...
	ghostAura:SetAttribute("SpinMult", 0.9)
