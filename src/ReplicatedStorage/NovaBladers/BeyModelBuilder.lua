...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local edge = part({
			name = "FangEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.25, 0.25, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildFrostRing(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local shard = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(15), math.rad(angle), 0) * CFrame.new(0, 0.4, 1.1),
		})
		shard:SetAttribute("SpinMult", 0.9)
		shard:SetAttribute("SpinOffset", CFrame.Angles(math.rad(15), math.rad(angle), 0) * CFrame.new(0, 0.4, 1.1))
