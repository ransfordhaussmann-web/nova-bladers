...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(140, 30, 40),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	-- Vortex spiral blades (4 curved segments)
	for i = 0, 3 do
		local angle = i * 90 + 22
	...
	innerRing:SetAttribute("SpinMult", -0.8)
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.4)
		local spike = part({
			name = "FrostSpike_" .. i,
			parent = parent,
	...
	frostShield:SetAttribute("SpinMult", 0.45)
