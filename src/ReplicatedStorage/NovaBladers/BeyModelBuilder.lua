...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for layer = 1, 3 do
		...
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.12 + layer * 0.04, radius * 2, radius * 2),
			color = layer % 2 == 0 and color or accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
			cframe = baseCFrame,
		})
		coil:SetAttribute("SpinMult", 1 + layer * 0.15)
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(55)) * CFrame.new(0, -0.1, 1.0)
		local fluke = part({
			name = "AnchorFluke_" .. i,
			parent = parent,
	...
	local shaft = part({
		name = "AnchorShaft",
		parent = parent,
		size = Vector3.new(0.35, 0.35, 1.8),
		color = Color3.fromRGB(70, 70, 80),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, -0.6),
	})
	table.insert(visuals, shaft)
