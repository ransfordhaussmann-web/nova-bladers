...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 90, 130),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local tip = part({
			name = "FangTip_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.3, 0.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset * CFrame.new(0, 0, 1.2),
		})
		tip:SetAttribute("SpinMult", 1.05)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local block = part({
			name = "Bastion_" .. i,
			parent = parent,
			size = Vector3.new(0.8, 0.9, 1.0),
			color = color,
			material = Enum.Material.Concrete,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.55),
		})
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.55))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(240, 180, 50),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	sunDisc:SetAttribute("SpinMult", 1.3)
	...
	for i = 0, 5 do
		local ray = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.1),
		})
		ray:SetAttribute("SpinMult", 1.3)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.1))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(100, 105, 130),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	local blade = part({
		name = "PhantomBlade",
		parent = parent,
		size = Vector3.new(0.25, 0.6, 2.2),
		color = accent,
		material = Enum.Material.Glass,
		transparency = 0.25,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.3),
	})
	...
	ghostRing:SetAttribute("SpinMult", -0.8)
