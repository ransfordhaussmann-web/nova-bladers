...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local edge = part({
			name = "BladeEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.35, 2.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0.15, 0, 0),
		})
		edge:SetAttribute("SpinMult", 1.15)
		edge:SetAttribute("SpinOffset", offset * CFrame.new(0.15, 0, 0))
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 1.0, color, Enum.Material.Concrete, i * 60, "FortSeg_" .. i)
	...
	for i = 0, 3 do
		...
		local pillar = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.55, 0.2, 0),
		})
		pillar:SetAttribute("SpinMult", 0.4)
		pillar:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.55, 0.2, 0))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
			size = Vector3.new(0.18, 0.5, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.65),
		})
		ray:SetAttribute("SpinMult", 1.0)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.65))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local edge = part({
			name = "PhantomEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.4, 1.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.2,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.25),
		})
		edge:SetAttribute("SpinMult", 1.1)
		edge:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, edge)
	end
