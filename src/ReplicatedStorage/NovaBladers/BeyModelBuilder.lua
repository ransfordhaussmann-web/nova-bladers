...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(180, 40, 45),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	-- Spiral attack wings (4 blades, fast spin)
	for i = 0, 3 do
		local angle = i * 90 + 22
...
local function buildGlacierShield(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	-- Heavy ice plate segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 0.85, color, Enum.Material.Ice, i * 60, "IcePlate_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.2, 1.0)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
...
local BUILDERS = {
	...
	GlacierShield = buildGlacierShield,
}
