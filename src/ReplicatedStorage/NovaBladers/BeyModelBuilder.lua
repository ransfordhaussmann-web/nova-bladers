...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.0))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.5, 1.05),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.5, 1.05))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.22, 3.6 + i * 0.4, 3.6 + i * 0.4),
			color = i == 0 and color or accent,
			material = Enum.Material.Neon,
			transparency = i == 0 and 0.15 or 0.35,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, i * 0.08, 0),
		})
		coil:SetAttribute("SpinMult", 1.0 + i * 0.15)
		coil:SetAttribute("SpinOffset", CFrame.new(0, i * 0.08, 0))
