--[[
	...
	Import Creator Store models: set meshId in BeyCatalog.creatorStore or modelAssets.

	Search Roblox Studio Toolbox → Creator Store using creatorStore.searchTerms
...
local function getMeshAssets(beyData)
	...
	local cs = beyData.creatorStore
	if cs and cs.meshId then
		return {
			meshId = cs.meshId,
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, accent, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
