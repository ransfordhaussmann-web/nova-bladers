...
local function buildCrimsonForge(parent, color, accent, baseCFrame)
	...
	forgeGlow:SetAttribute("SpinMult", 0.95)
	...
	for i = 0, 3 do
		...
		ember:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.15))
		table.insert(spinVisuals, ember)
	end

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, Color3.fromRGB(120, 45, 30), Enum.Material.Metal, i * 60, "MoltenSeg_" .. i)
...
local function buildCrystalTide(parent, color, accent, baseCFrame)
	...
	table.insert(visuals, innerGlow)
