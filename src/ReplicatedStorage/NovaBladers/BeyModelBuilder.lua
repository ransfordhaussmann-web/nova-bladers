...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.5)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	iceRing:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, iceRing)
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local helix = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.15, 3.2 + i * 0.4, 3.2 + i * 0.4),
			color = i == 1 and accent or color,
			material = Enum.Material.Neon,
			transparency = 0.25 + i * 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 40), 0),
		})
		helix:SetAttribute("SpinMult", 1.0 + i * 0.15)
...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.1, 1.25)
		local quill = part({
			name = "Quill_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.25, 1.6),
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local fluke = part({
			name = "AnchorFluke_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.35, 1.1),
			color = color,
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset,
