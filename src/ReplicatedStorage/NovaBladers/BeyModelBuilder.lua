...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.2, 1.15)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.12, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	disc:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, disc)
	...
	for i = 0, 7 do
		...
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.6),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.6))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(10)) * CFrame.new(0, 0, 1.25)
		local blade = part({
