...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.5, 1.1)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
...
local function buildNeonSpire(parent, color, accent, baseCFrame)
	...
	for _, radius in ipairs({ 3.2, 3.8 }) do
		local pulseRing = part({
			name = "PulseRing_" .. radius,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.1, radius, radius),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.5,
			canCollide = false,
		...
		pulseRing:SetAttribute("SpinMult", 1.5)
		table.insert(spinVisuals, pulseRing)
	end
