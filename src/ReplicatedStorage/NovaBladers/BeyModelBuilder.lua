...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.2, 1.2)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.7, 1.3),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.1,
			canCollide = false,
