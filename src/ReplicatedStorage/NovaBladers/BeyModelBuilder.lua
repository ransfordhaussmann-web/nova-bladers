...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.35, 1.1)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
...
local function buildNeonSpire(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		node:SetAttribute("SpinMult", 1.3)
		node:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, node)
	end
	...
	outerPulse:SetAttribute("SpinMult", -0.4)
