...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i, spinMult in ipairs({ 0.75, 1.0, -0.5 }) do
		local ring = part({
			name = "AuroraRing_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18 + i * 0.06, 2.8 + i * 0.5, 2.8 + i * 0.5),
		...
		ring:SetAttribute("SpinMult", spinMult)
	...
	for i = 0, 5 do
		...
		crest:SetAttribute("SpinMult", 0.85)
