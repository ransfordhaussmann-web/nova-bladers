...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local panel = part({
			...
			material = Enum.Material.SmoothPlastic,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.5),
		})
		panel:SetAttribute("SpinMult", 1.05)
		panel:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.5))
		table.insert(spinVisuals, panel)
	end
