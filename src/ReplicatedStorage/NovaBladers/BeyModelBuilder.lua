...
local function buildCrimsonComet(parent, color, accent, baseCFrame)
	...
	-- Comet tail blades (asymmetric spin)
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(18)) * CFrame.new(0, 0, 1.15)
		local blade = part({
			name = "CometBlade_" .. i,
			parent = parent,
	...
	tailGlow:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, tailGlow)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.25, 1.0)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
