...
local function buildBlazeRipper(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		...
		ripper:SetAttribute("SpinMult", 1.2)
		ripper:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, ripper)
		...
		local flame = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = ripper.CFrame * CFrame.new(0, 0, 1.35),
		})
