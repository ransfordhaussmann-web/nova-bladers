...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fang.CFrame * CFrame.new(0, 0, 1.1),
		})
		tip:SetAttribute("SpinMult", 1.15)
		tip:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.1))
		table.insert(spinVisuals, tip)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		ray:SetAttribute("SpinMult", 0.85)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.55))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	mirage:SetAttribute("SpinMult", 1.05)
	...
	for i = 0, 1 do
		...
		local blade = part({
			...
			parent = parent,
			size = Vector3.new(0.15, 0.55, 2.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.2,
			canCollide = false,
		...
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, blade)
	end
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	crown:SetAttribute("SpinMult", 0.8)
...
local function buildCrimsonDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.72, 1.75, 1.75),
	...
	for i = 0, 2 do
		...
		local streak = part({
			name = "BloodStreak_" .. i,
			parent = parent,
			size = Vector3.new(0.18, 0.12, 1.3),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.2),
		})
		streak:SetAttribute("SpinMult", 1.3)
		streak:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.2))
