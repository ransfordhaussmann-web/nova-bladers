...
local function buildBlazeWheel(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		wheel:SetAttribute("SpinMult", 1.2)
		wheel:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, wheel)
	end
...
local function buildFrostVeil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, accent, Enum.Material.Glass, i * 60, "VeilSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.2, 1.1)
		local shard = part({
			name = "IceShard_" .. i,
			parent = parent,
