...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.2)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		facet:SetAttribute("SpinMult", 1.0)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-30)) * CFrame.new(0, 0.5, 0.9)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
		...
		spike:SetAttribute("SpinMult", 1.3)
	...
	flameRing:SetAttribute("SpinMult", 1.5)
