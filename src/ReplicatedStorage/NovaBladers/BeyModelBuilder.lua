...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.7, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.35, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunRing:SetAttribute("SpinMult", 1.0)
	table.insert(spinVisuals, sunRing)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 90, 100),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	ghost:SetAttribute("SpinMult", 0.75)
	...
	for i = 0, 1 do
		...
		local blade = part({
			...
			parent = parent,
			size = Vector3.new(0.35, 0.4, 1.9),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.15),
		})
		blade:SetAttribute("SpinMult", 0.85)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.15))
		table.insert(spinVisuals, blade)
	end
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local shard = part({
			...
			parent = parent,
			size = Vector3.new(0.4, 0.55, 1.5),
			color = color,
			material = Enum.Material.Glass,
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.2),
		})
		shard:SetAttribute("SpinMult", 1.15)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.2))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.6, 1.1),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.25),
		})
		spike:SetAttribute("SpinMult", 1.0)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.25))
	...
	flameRing:SetAttribute("SpinMult", 1.05)
