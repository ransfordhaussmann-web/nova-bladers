...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 1.7),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 1.7))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	veil:SetAttribute("SpinMult", 0.8)
	...
	for i = 0, 2 do
		local blade = part({
			...
			transparency = 0.3,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 120), math.rad(15)) * CFrame.new(0, 0, 1.25),
		})
		blade:SetAttribute("SpinMult", 0.95)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 120), math.rad(15)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, blade)
	end
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		crystal:SetAttribute("SpinMult", 1)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.15))
		table.insert(spinVisuals, crystal)
	end
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.9, 2.0, 2.0),
		color = Color3.fromRGB(180, 80, 20),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 4 do
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.6, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 72), math.rad(30)) * CFrame.new(0, 0.3, 1.1),
		})
		spike:SetAttribute("SpinMult", 1)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 72), math.rad(30)) * CFrame.new(0, 0.3, 1.1))
