...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 25, 20),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	-- Ice shell segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.8, 0.85, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.5, 0, 0))
		table.insert(spinVisuals, seg)
	end

	-- Crown spikes (slow spin)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-40)) * CFrame.new(0, 0.6, 1.1)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
...
local function buildNeonSpire(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	-- Tall neon spire fins
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(55)) * CFrame.new(0, 0.3, 0.9)
		local fin = part({
			name = "SpireFin_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 1.4, 0.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		fin:SetAttribute("SpinMult", 1.3)
		fin:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, fin)
	end

	-- Wide stamina base ring
	local baseRing = part({
		name = "StaminaBase",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	baseRing:SetAttribute("SpinMult", 1.0)
	table.insert(spinVisuals, baseRing)
	...
	for i = 0, 5 do
		...
		dot:SetAttribute("SpinMult", 1.5)
		dot:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, dot)
	end
	...
	local outerGlow = part({
		...
		color = Color3.fromRGB(180, 255, 230),
		material = Enum.Material.Neon,
		transparency = 0.55,
		canCollide = false,
		cframe = baseCFrame,
