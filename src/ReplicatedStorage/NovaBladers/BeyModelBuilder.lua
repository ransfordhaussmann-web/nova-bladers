...
local function buildFrostCoil(parent, color, accent, baseCFrame)
	...
	-- Coiled ice segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 0.9, 1.5, 0.7, color, Enum.Material.Ice, i * 60, "CoilSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.2, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	outerCoil:SetAttribute("SpinMult", -0.4)
