...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(24)) * CFrame.new(0, 0, 1.25)
		local fang = part({
			name = "FlameFang_" .. i,
			parent = parent,
...
local function buildAuroraMantle(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 0.75, color, Enum.Material.Ice, i * 60, "CrystalSeg_" .. i)
	...
	mantle:SetAttribute("SpinMult", 0.55)
...
local BUILDERS = {
	...
	AuroraMantle = buildAuroraMantle,
}
