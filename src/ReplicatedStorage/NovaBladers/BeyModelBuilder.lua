...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Jagged ripper blades (3, aggressive angles)
	for i = 0, 2 do
		...
		local edge = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0, 0, 1.55),
		})
		edge:SetAttribute("SpinMult", 1.15)
		edge:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.55))
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	-- Crest petals (6, slow counter-spin)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Glass, i * 60, "CrestPet_" .. i)
	...
	frostShield:SetAttribute("SpinMult", 0.6)
	...
	auroraGlow:SetAttribute("SpinMult", 0.4)
