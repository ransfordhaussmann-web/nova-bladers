...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.4, 1.1)
		local spike = part({
			name = "Crystal_" .. i,
			parent = parent,
...
local function buildSolarBlaze(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
