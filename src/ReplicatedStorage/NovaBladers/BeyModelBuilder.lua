...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	-- Spiral blade arms (3 curved segments)
	for i = 0, 2 do
		local angle = i * 120
	...
	vortex:SetAttribute("SpinMult", -1.5)
	table.insert(spinVisuals, vortex)
	...
	outer:SetAttribute("SpinMult", 0.6)
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	-- Heavy ice shell plates (6 segments)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.Ice, i * 60, "IcePlate_" .. i)
	...
	mantle:SetAttribute("SpinMult", 0.4)
	...
	for i, angle in ipairs({ 0, 90, 180, 270 }) do
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.55)
		local crystal = part({
			name = "IceCrystal_" .. i,
			parent = parent,
		...
		crystal:SetAttribute("SpinMult", 0.4)
