...
local function buildCrimsonReaver(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
...
local function buildFrostWard(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	-- Crystalline shield plates
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.75, color, Enum.Material.Glass, i * 60, "IcePlate_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0, 1.65)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.35, 1.0),
...
local BUILDERS = {
	...
	CrimsonReaver = buildCrimsonReaver,
	FrostWard = buildFrostWard,
}
