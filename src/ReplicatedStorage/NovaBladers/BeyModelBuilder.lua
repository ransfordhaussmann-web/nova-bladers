...
local function buildCrystalVortex(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.55, color, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.2, 0.95)
		local shard = part({
			name = "IceShard_" .. i,
			parent = parent,
	...
	vortexRing:SetAttribute("SpinMult", 0.65)
...
local function buildEmberRing(parent, color, accent, baseCFrame)
	...
	magma:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, magma)
	...
	phoenixGlow:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, phoenixGlow)
