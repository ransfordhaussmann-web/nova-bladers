...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 0.85, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.1),
		})
		spike:SetAttribute("SpinMult", 0.5)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.1))
	...
	frostShield:SetAttribute("SpinMult", 0.65)
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for layer = 1, 3 do
		local radius = 1.0 + layer * 0.35
		local coil = part({
			name = "HelixCoil_" .. layer,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, radius * 2, radius * 2),
			color = layer == 2 and accent or color,
			material = layer == 3 and Enum.Material.Neon or Enum.Material.SmoothPlastic,
			transparency = layer == 3 and 0.2 or 0,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, (layer - 2) * 0.12, 0),
		})
		coil:SetAttribute("SpinMult", 0.9 + layer * 0.15)
