...
local function buildFrostWard(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.65, 0.75, color, Enum.Material.Ice, i * 60, "FrostSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(math.rad(15), math.rad(angle), 0) * CFrame.new(0, 0.6, 0.5)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
