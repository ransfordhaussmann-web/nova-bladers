...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local quill = part({
			name = "Quill_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.5, 2.0),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		quill:SetAttribute("SpinMult", 1.15)
		...
		local tip = part({
			name = "QuillTip_" .. i,
			parent = parent,
			size = Vector3.new(0.28, 0.28, 0.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = quill.CFrame * CFrame.new(0, 0, 1.15),
		})
	...
	emberRing:SetAttribute("SpinMult", 1.3)
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local fluke = part({
			name = "AnchorFluke_" .. i,
			parent = parent,
			size = Vector3.new(0.5, 0.35, 1.6),
			color = Color3.fromRGB(70, 110, 140),
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-30)) * CFrame.new(0, 0, -1.1),
		})
		table.insert(visuals, fluke)
	end

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.2, 1.85, 0.75, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
	...
	tideRing:SetAttribute("SpinMult", 0.65)
	table.insert(spinVisuals, tideRing)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 1.4)
		local petal = part({
			name = "FlarePetal_" .. i,
			parent = parent,
		...
		petal:SetAttribute("SpinMult", 0.95)
		petal:SetAttribute("SpinOffset", offset)
