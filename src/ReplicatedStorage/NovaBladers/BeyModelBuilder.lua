...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.2, 2.8, 2.8), color = Color3.fromRGB(40, 80, 120),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	local anchor = part({
		name = "Anchor", parent = parent,
		size = Vector3.new(0.6, 0.8, 1.4), color = accent,
		material = Enum.Material.Metal, canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.5),
	})
	anchor:SetAttribute("SpinMult", 0.5)
	anchor:SetAttribute("SpinOffset", CFrame.new(0, 0, 1.5))
	table.insert(spinVisuals, anchor)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 0.7, color, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.8)
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.9, 2.1, 2.1), color = Color3.fromRGB(80, 20, 30),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 2 do
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(1.3, 2.9, 2.9), color = Color3.fromRGB(70, 65, 60),
		material = Enum.Material.Slate, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.6)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.52, 0, 0))
		table.insert(spinVisuals, seg)
	end
	local spinRing = part({
		name = "SpinRing", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.25, 4.3, 4.3), color = accent,
		material = Enum.Material.Neon, transparency = 0.5,
		canCollide = false, cframe = baseCFrame,
	})
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.7, 1.7, 1.7), color = Color3.fromRGB(255, 240, 180),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	local sun = part({
		name = "SunCore", parent = parent, shape = Enum.PartType.Ball,
		size = Vector3.new(1.0, 1.0, 1.0), color = accent,
		material = Enum.Material.Neon, transparency = 0.1,
		canCollide = false, cframe = baseCFrame * CFrame.new(0, 0.3, 0),
	})
	table.insert(visuals, sun)
	for i = 0, 5 do
		local angle = i * 60
		local ray = part({
			name = "Ray_" .. i, parent = parent,
			size = Vector3.new(0.2, 0.2, 1.2), color = color,
			material = Enum.Material.Neon, canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.0),
		})
		ray:SetAttribute("SpinMult", 1.3)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		name = "Core", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.8, 2.0, 2.0), color = Color3.fromRGB(30, 60, 70),
		material = Enum.Material.Metal, canCollide = false, cframe = baseCFrame,
	})
	table.insert(visuals, core)
	local blade = part({
		name = "PhantomBlade", parent = parent,
		size = Vector3.new(0.25, 0.5, 2.2), color = accent,
		material = Enum.Material.ForceField, transparency = 0.3,
		canCollide = false, cframe = baseCFrame * CFrame.new(0, 0, 1.3),
	})
	blade:SetAttribute("SpinMult", 1)
	blade:SetAttribute("SpinOffset", CFrame.new(0, 0, 1.3))
	table.insert(spinVisuals, blade)
	local aura = part({
		name = "PhaseAura", parent = parent, shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.2, 3.2, 3.2), color = color,
		material = Enum.Material.ForceField, transparency = 0.5,
		canCollide = false, cframe = baseCFrame,
	})
	aura:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, aura)
