...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.2, 1.15)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	anchor:SetAttribute("SpinMult", 0.8)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisc:SetAttribute("SpinMult", 1.0)
	...
	for i = 0, 7 do
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.15, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(0, 0, 2.0),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(0, 0, 2.0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local ghostBlade = part({
		name = "GhostBlade",
		parent = parent,
		size = Vector3.new(0.25, 0.6, 2.8),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame * CFrame.Angles(0, 0, math.rad(15)) * CFrame.new(0, 0, 1.1),
	})
	ghostBlade:SetAttribute("SpinMult", 1.1)
	ghostBlade:SetAttribute("SpinOffset", CFrame.Angles(0, 0, math.rad(15)) * CFrame.new(0, 0, 1.1))
	table.insert(spinVisuals, ghostBlade)

	local phaseRing = part({
		name = "PhaseRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.14, 3.4, 3.4),
		color = color,
	...
	phaseRing:SetAttribute("SpinMult", -0.6)
	table.insert(spinVisuals, phaseRing)
