...
local function buildCrimsonEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		tip:SetAttribute("SpinOffset", blade:GetAttribute("SpinOffset") * CFrame.new(0, 0, 1.2))
		table.insert(spinVisuals, tip)
	end
	...
	heatHaze:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, heatHaze)
...
local function buildFrostCore(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.35, 0.15, 0)
		local crystal = part({
			name = "IceCrystal_" .. i,
			parent = parent,
	...
	local outerShell = part({
		...
		color = Color3.fromRGB(120, 170, 210),
		material = Enum.Material.DiamondPlate,
		transparency = 0.1,
		canCollide = false,
		cframe = baseCFrame,
