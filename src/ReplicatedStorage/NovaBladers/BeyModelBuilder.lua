...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	counterRing:SetAttribute("SpinMult", -0.7)
	table.insert(spinVisuals, counterRing)
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.6, 0.9)
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 1.0, 0.35),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.15,
			canCollide = false,
