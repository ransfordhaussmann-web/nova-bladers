...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.2, 1.25)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local block = ringSegment(parent, 1.1, 1.7, 1.0, color, Enum.Material.Concrete, i * 60, "StoneBlock_" .. i)
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.4, 0, 0))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.2, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 1.6),
		})
		ray:SetAttribute("SpinMult", 1.05)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 1.6))
