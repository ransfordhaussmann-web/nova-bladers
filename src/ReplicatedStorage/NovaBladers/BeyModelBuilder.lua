...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		name = "Anchor",
		parent = parent,
		size = Vector3.new(0.6, 1.2, 0.8),
		color = accent,
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.1) * CFrame.Angles(math.rad(30), 0, 0),
	})
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	corona:SetAttribute("SpinMult", 1.2)
