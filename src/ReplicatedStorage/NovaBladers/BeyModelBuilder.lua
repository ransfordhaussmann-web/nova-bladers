...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.SmoothPlastic, i * 60, "CrestSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.2, 0.85)
		local spike = part({
			name = "CrestSpike_" .. i,
			parent = parent,
