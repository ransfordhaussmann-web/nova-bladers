...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	waterRing:SetAttribute("SpinMult", 0.8)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	fortRing:SetAttribute("SpinMult", 0.35)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.7)
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.2, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local blade = part({
			name = "GhostBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.08, 1.7),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
