...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 120, 140),
		material = Enum.Material.Glass,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	-- Crystalline crest dome (static)
	local crest = part({
		name = "Crest",
		parent = parent,
		shape = Enum.PartType.Ball,
		size = Vector3.new(1.6, 1.0, 1.6),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.2,
		canCollide = false,
	...
	-- Aurora ring segments (slow spin)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Glass, i * 60, "CrystalSeg_" .. i)
	...
	outerShell:SetAttribute("SpinMult", -0.4)
