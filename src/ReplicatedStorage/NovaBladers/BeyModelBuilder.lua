...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local fang = part({
			name = "FangBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.42, 0.38, 1.9),
			color = color,
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildFrostCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		local coil = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.22 + i * 0.08, 3.2 + i * 0.5, 3.2 + i * 0.5),
			color = color,
			material = Enum.Material.Glass,
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.35, accent, Enum.Material.Neon, i * 60, "FrostSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.85)
