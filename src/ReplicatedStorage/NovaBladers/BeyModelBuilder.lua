...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		shard:SetAttribute("SpinMult", 1.05)
	...
	frostRing:SetAttribute("SpinMult", 0.95)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.7, 0.8),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(-30)) * CFrame.new(0, 0.3, 1.5),
		})
		spike:SetAttribute("SpinMult", 0.8)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(-30)) * CFrame.new(0, 0.3, 1.5))
