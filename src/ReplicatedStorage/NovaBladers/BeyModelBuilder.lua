...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	-- Crown spikes (ice crystals)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-20)) * CFrame.new(0, 0.5, 1.3)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	-- Heavy frost shield ring
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, color, Enum.Material.Ice, i * 90, "FrostSeg_" .. i)
