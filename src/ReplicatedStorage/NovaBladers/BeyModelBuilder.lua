...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			name = "SunRay_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.6, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0.1, 1.4),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0.1, 1.4))
		table.insert(spinVisuals, ray)
	end

	local spinRing = sunRing
	return visuals, spinVisuals, spinRing
end
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(130, 135, 150),
		material = Enum.Material.Metal,
		transparency = 0.15,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i = 0, 2 do
		local blade = part({
			name = "PhaseBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.4, 1.5),
			color = color,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 120), math.rad(15)) * CFrame.new(0, 0, 1.1),
		})
		blade:SetAttribute("SpinMult", 1.1)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 120), math.rad(15)) * CFrame.new(0, 0, 1.1))
		table.insert(spinVisuals, blade)
	end
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local crown = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.2, 1.2),
		})
		crown:SetAttribute("SpinMult", 1.3)
		crown:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.2, 1.2))
