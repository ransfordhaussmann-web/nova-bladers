...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.SmoothPlastic, i * 72, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 72), 0) * CFrame.new(1.3, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.1, 1.8)
		local ray = part({
			name = "Ray_" .. i,
			parent = parent,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	aura:SetAttribute("SpinMult", 1.1)
	table.insert(spinVisuals, aura)
