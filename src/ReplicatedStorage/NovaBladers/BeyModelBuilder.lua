...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = blade.CFrame * CFrame.new(0, 0, 1.2),
		})
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local spike = part({
			name = "RockSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.75, 0.55),
			color = accent,
			material = Enum.Material.Rock,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.55),
		})
		spike:SetAttribute("SpinMult", 0.65)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 1.55))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
			size = Vector3.new(0.18, 0.55, 1.4),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostRing:SetAttribute("SpinMult", 1)
	...
	for i = 0, 2 do
		...
		edge:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.3))
