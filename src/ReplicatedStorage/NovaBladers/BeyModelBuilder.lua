...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0, 1.1) * CFrame.Angles(0, 0, math.rad(15)),
	})
	anchor:SetAttribute("SpinMult", 0.5)
	anchor:SetAttribute("SpinOffset", CFrame.new(0, 0, 1.1) * CFrame.Angles(0, 0, math.rad(15)))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisc:SetAttribute("SpinMult", 1.25)
	...
	for i = 0, 5 do
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.4, 1.2),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.8),
		})
		ray:SetAttribute("SpinMult", 1.25)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.8))
