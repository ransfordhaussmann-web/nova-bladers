...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local block = part({
			name = "FortBlock_" .. i,
			parent = parent,
			size = Vector3.new(0.8, 0.9, 1.2),
			color = color,
			material = Enum.Material.Concrete,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.25),
		})
		block:SetAttribute("SpinMult", 0.4)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 1.25))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		local ray = part({
			name = "Ray_" .. i,
			parent = parent,
			size = Vector3.new(0.15, 0.15, 1.0),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		edge:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.3))
