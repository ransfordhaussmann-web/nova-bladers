...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	bloodRing:SetAttribute("SpinMult", 1.2)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.9, 1.0, color, Enum.Material.Concrete, i * 45, "StoneSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	corona:SetAttribute("SpinMult", 1.0)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local edge = part({
			...
			transparency = 0.25,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.25),
		})
		edge:SetAttribute("SpinMult", 1.05)
		edge:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 1.25))
	...
	spinRing:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, spinRing)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.4),
		})
		shard:SetAttribute("SpinMult", 1.2)
		shard:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.4))
		table.insert(spinVisuals, shard)
	end
	...
	frostRing:SetAttribute("SpinMult", 1.1)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 4 do
		...
		local spike = part({
			name = "FlameSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.9, 1.2),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.2, 1.35),
		})
		spike:SetAttribute("SpinMult", 1.25)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0.2, 1.35))
	...
	inferno:SetAttribute("SpinMult", 1.3)
	table.insert(spinVisuals, inferno)
...
local BUILDERS = {
	...
	CrystalEdge = buildCrystalEdge,
	BlazeCrown = buildBlazeCrown,
}
