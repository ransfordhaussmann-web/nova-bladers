...
local function buildCrimsonVortex(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = fin.CFrame * CFrame.new(0, 0, 1.05),
		})
...
local function buildGlacierMantle(parent, color, accent, baseCFrame)
	...
	mantleRing:SetAttribute("SpinMult", 0.55)
	table.insert(spinVisuals, mantleRing)
