...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	-- Blocky granite plates
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.55, 0, 0)
		local plate = part({
			name = "GranitePlate_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.85, 1.1),
			color = color,
	...
	fortRing:SetAttribute("SpinMult", 0.65)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	halo:SetAttribute("SpinMult", 1.25)
