...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 90, 120),
		material = Enum.Material.Glass,
		transparency = 0.15,
		canCollide = false,
		cframe = baseCFrame,
	...
	phaseAura:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, phaseAura)

	for i, angle in ipairs({ -20, 70, 160 }) do
