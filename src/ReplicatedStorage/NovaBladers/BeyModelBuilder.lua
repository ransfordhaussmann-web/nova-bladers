...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 1.0, color, Enum.Material.Concrete, i * 60, "RockSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisk:SetAttribute("SpinMult", 1.2)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 180), math.rad(15)) * CFrame.new(0, 0, 1.4)
		local blade = part({
			name = "PhaseBlade_" .. i,
			parent = parent,
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.35, 1.0)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
	...
	crownRing:SetAttribute("SpinMult", 1.15)
	table.insert(spinVisuals, crownRing)
