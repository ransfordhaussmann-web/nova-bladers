...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.35, 1.05)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.9, 0.55),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.15,
			canCollide = false,
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Ice, i * 90, "IceRing_" .. i)
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local r = 0.85 + (i % 2) * 0.35
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0.08, r)
		local coil = part({
			name = "CoilSeg_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.22, 1.4, 1.4),
