...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 30, 25),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildFrostRing(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local crystal = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.2, 1.1),
		})
		crystal:SetAttribute("SpinMult", 0.9)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.2, 1.1))
