...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0.2, 1.25)
		local quill = part({
			name = "Quill_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.75, color, Enum.Material.Glass, i * 90, "WaveSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.7)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 90), 0) * CFrame.new(1.32, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local block = ringSegment(parent, 1.1, 1.7, 1.0, color, Enum.Material.Brick, i * 60, "FortBlock_" .. i)
		block:SetAttribute("SpinMult", 0.65)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	flatRing:SetAttribute("SpinMult", 1.25)
	table.insert(spinVisuals, flatRing)

	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0.1, 1.55)
		local flare = part({
			name = "Flare_" .. i,
			parent = parent,
		...
		flare:SetAttribute("SpinMult", 1.25)
		flare:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, flare)
	end
