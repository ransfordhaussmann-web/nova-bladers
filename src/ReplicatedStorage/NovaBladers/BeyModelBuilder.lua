...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.6, 1.0, color, Enum.Material.Concrete, i * 45, "FortWall_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	outerGlow:SetAttribute("SpinMult", 0.7)
	table.insert(spinVisuals, outerGlow)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostAura:SetAttribute("SpinMult", 1.0)
