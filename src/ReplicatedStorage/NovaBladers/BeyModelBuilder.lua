...
local function buildBlazeLance(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local flame = part({
			name = "FlameTip_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.35, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = spike.CFrame * CFrame.new(0, 0, 1.55),
		})
		flame:SetAttribute("SpinMult", 1.15)
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.55))
...
local function buildCoralTide(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		wave:SetAttribute("SpinMult", 0.75)
