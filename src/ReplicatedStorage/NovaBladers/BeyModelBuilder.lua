...
local function buildCrimsonOrbit(parent, color, accent, baseCFrame)
	...
	orbitRing:SetAttribute("SpinMult", 1.3)
...
local function buildFrostAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Glass,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.1),
		})
		spike:SetAttribute("SpinMult", 0.4)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0.2, 1.1))
