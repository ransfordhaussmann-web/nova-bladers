...
local function buildBlazeLance(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		lance:SetAttribute("SpinMult", 1.15)
		lance:SetAttribute("SpinOffset", offset)
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = lance.CFrame * CFrame.new(0, 0, 1.35),
		})
...
local function buildCoralTide(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(30, 120, 140),
		material = Enum.Material.Glass,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 3 do
		...
		coral:SetAttribute("SpinMult", 0.8)
		coral:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, coral)
	end
