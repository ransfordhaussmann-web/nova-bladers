...
local function buildCrimsonForge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.Metal, i * 60, "ForgeSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local rivet = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.1),
		})
		rivet:SetAttribute("SpinMult", 0.9)
		rivet:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.1))
		table.insert(spinVisuals, rivet)
	end
