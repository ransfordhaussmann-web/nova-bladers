...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	ripperGlow:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, ripperGlow)
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.55, color, Enum.Material.Glass, i * 60, "CrestSeg_" .. i)
	...
	local crestPlate = part({
		name = "CrestPlate",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.35, 2.8, 2.8),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.2,
		canCollide = false,
	...
	crestPlate:SetAttribute("SpinMult", 0.5)
	table.insert(spinVisuals, crestPlate)
	...
	local auroraRing = part({
		...
		color = Color3.fromRGB(120, 255, 220),
		material = Enum.Material.Neon,
		transparency = 0.45,
		canCollide = false,
		cframe = baseCFrame,
