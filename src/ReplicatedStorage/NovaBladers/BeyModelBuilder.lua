...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(0, 0, 1.55),
		})
		seg:SetAttribute("SpinMult", 0.5)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(0, 0, 1.55))
		table.insert(spinVisuals, seg)
	end
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(140, 130, 180),
		material = Enum.Material.Glass,
		transparency = 0.3,
		canCollide = false,
		cframe = baseCFrame,
	...
	for i, angle in ipairs({ -50, 20, 140 }) do
