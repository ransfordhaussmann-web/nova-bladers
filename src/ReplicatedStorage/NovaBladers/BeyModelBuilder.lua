...
local function buildBlazeFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local blade = part({
			name = "FlameBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.5, 0.45, 2.2),
			color = color,
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(22)) * CFrame.new(0, 0, 1.25),
		})
		blade:SetAttribute("SpinMult", 1.05)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(22)) * CFrame.new(0, 0, 1.25))
		table.insert(spinVisuals, blade)
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.75, color, Enum.Material.Glass, i * 45, "IceSeg_" .. i)
