...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Jagged ripper blades (4, spin fast)
	for i = 0, 3 do
		local angle = i * 90 + 15
...
local function buildAuroraCrest(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.7, color, Enum.Material.Glass, i * 60, "CrestPetals_" .. i)
	...
	auroraGlow:SetAttribute("SpinMult", 0.6)
	...
	shieldCap:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, shieldCap)
