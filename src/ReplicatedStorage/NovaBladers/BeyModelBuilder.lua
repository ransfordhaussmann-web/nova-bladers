...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local block = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.Concrete, i * 45, "StoneBlock_" .. i)
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 45), 0) * CFrame.new(1.32, 0, 0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	halo:SetAttribute("SpinMult", 0.85)
