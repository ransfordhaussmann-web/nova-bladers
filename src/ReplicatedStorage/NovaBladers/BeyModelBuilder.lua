...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local edge = part({
			name = "BladeEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.25, 0.35, 0.9),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.85, 1.0, color, Enum.Material.Concrete, i * 60, "StoneSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local pillar = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.05),
		})
		pillar:SetAttribute("SpinMult", 0.4)
		pillar:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.05))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	wideRing:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, wideRing)
	...
	for i = 0, 5 do
		...
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.15),
		})
		ray:SetAttribute("SpinMult", 0.95)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.15))
