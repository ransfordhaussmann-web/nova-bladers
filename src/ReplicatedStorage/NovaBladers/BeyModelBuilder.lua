...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0, 1.15)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		...
		block:SetAttribute("SpinMult", 0.35)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(100, 140, 160),
		material = Enum.Material.Glass,
		transparency = 0.3,
		canCollide = false,
		cframe = baseCFrame,
