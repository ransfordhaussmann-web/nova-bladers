...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(240, 100, 40),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.2, 1.1)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local seg = ringSegment(parent, 1.0, 1.6, 1.0, color, Enum.Material.Metal, i * 90, "AnchorSeg_" .. i)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local block = part({
			name = "FortBlock_" .. i,
			parent = parent,
			size = Vector3.new(0.7, 0.9, 1.2),
			color = color,
			material = Enum.Material.Concrete,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.5, 0, 0),
		})
		block:SetAttribute("SpinMult", 0.5)
		block:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.5, 0, 0))
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisk:SetAttribute("SpinMult", 1.15)
	table.insert(spinVisuals, sunDisk)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0, 1.8),
		})
		ray:SetAttribute("SpinMult", 1.15)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0, 1.8))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	ghostLayer:SetAttribute("SpinMult", 0.95)
	table.insert(spinVisuals, ghostLayer)

	for i = 0, 1 do
		local blade = part({
			name = "PhaseBlade_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.4, 2.2),
			color = color,
			material = Enum.Material.Neon,
			transparency = 0.25,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 180), math.rad(20)) * CFrame.new(0, 0, 1.3),
		})
		blade:SetAttribute("SpinMult", 1.05)
		blade:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 180), math.rad(20)) * CFrame.new(0, 0, 1.3))
		table.insert(spinVisuals, blade)
	end
