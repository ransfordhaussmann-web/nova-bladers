...
local function buildFrostSpiral(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 2 do
		local angle = i * 120
		local seg = ringSegment(parent, 0.95, 1.55, 0.35, accent, Enum.Material.Glass, angle, "IceSeg_" .. i)
		seg:SetAttribute("SpinMult", 0.95)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.25, 0, 0))
		table.insert(spinVisuals, seg)
	end
	...
	for i = 0, 5 do
		local crystal = part({
			name = "Crystal_" .. i,
			parent = parent,
			size = Vector3.new(0.3, 0.55, 0.3),
			color = accent,
			material = Enum.Material.Glass,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame
				* CFrame.Angles(0, math.rad(i * 60), math.rad(15))
				* CFrame.new(0, 0.2, 1.45),
		})
		crystal:SetAttribute("SpinMult", 1.05)
		crystal:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(15)) * CFrame.new(0, 0.2, 1.45))
