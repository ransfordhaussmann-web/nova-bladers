...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		...
		local shard = part({
			...
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(25), math.rad(angle), 0) * CFrame.new(0, 0, 1.4),
		})
		shard:SetAttribute("SpinMult", 1)
		shard:SetAttribute("SpinOffset", CFrame.Angles(math.rad(25), math.rad(angle), 0) * CFrame.new(0, 0, 1.4))
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(math.rad(-30), math.rad(angle), 0) * CFrame.new(0, 0.5, 1.3),
		})
		spike:SetAttribute("SpinMult", 0.95)
		spike:SetAttribute("SpinOffset", CFrame.Angles(math.rad(-30), math.rad(angle), 0) * CFrame.new(0, 0.5, 1.3))
