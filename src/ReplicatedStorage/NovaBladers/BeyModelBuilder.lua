...
local function buildCrimsonForge(parent, color, accent, baseCFrame)
	...
	forgeGlow:SetAttribute("SpinMult", 1)
	table.insert(spinVisuals, forgeGlow)

	-- Flame lance blades (3)
	for i = 0, 2 do
		local angle = i * 120
	...
	emberRing:SetAttribute("SpinMult", 0.7)
...
local function buildFrostPrism(parent, color, accent, baseCFrame)
	...
	-- Crystal facets (hexagonal shell)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.Ice, i * 60, "CrystalSeg_" .. i)
	...
	prismRing:SetAttribute("SpinMult", 0.65)
