...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Jagged attack fangs (spin with ring)
	for i = 0, 3 do
		local angle = i * 90 + 20
	...
	chainRing:SetAttribute("SpinMult", 0.5)
...
local function buildCrystalTide(parent, color, accent, baseCFrame)
	...
	shell:SetAttribute("SpinMult", 0.6)
	table.insert(spinVisuals, shell)
	...
	tideGlow:SetAttribute("SpinMult", 0.9)
