...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.DiamondPlate,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0.3, 0.9),
	})
	table.insert(visuals, anchor)

	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 0.75, accent, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local seg = ringSegment(parent, 1.2, 1.85, 1.0, color, Enum.Material.Rock, i * 45, "StoneSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisk:SetAttribute("SpinMult", 1.0)
