...
local function buildCrystalBloom(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(200, 180, 240),
		material = Enum.Material.Glass,
		transparency = 0.15,
		canCollide = false,
		cframe = baseCFrame,
	...
	gemRing:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, gemRing)
	...
	local tip = part({
		...
		color = Color3.fromRGB(240, 220, 255),
		material = Enum.Material.Glass,
		canCollide = false,
...
local function buildEmberForge(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		glow:SetAttribute("SpinMult", 1)
		glow:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 0.85))
