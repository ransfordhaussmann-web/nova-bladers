...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		quill:SetAttribute("SpinMult", 1.1)
		quill:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, quill)
	end
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, accent, Enum.Material.Glass, i * 60, "WaveSeg_" .. i)
		seg.Transparency = 0.3
		seg:SetAttribute("SpinMult", 0.75)
		seg:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(1.3, 0, 0))
		table.insert(spinVisuals, seg)
	end
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	fortRing:SetAttribute("SpinMult", 0.5)
	table.insert(spinVisuals, fortRing)

	local spinRing = part({
		name = "SpinRing",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.22, 4.4, 4.4),
		color = accent,
		material = Enum.Material.Neon,
		transparency = 0.45,
		canCollide = false,
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	sunDisc:SetAttribute("SpinMult", 1.05)
	...
	for i = 0, 5 do
		local ray = part({
			name = "Ray_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.15, 1.0),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 2.0),
		})
		ray:SetAttribute("SpinMult", 1.05)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), 0) * CFrame.new(0, 0, 2.0))
	...
	spinRing:SetAttribute("SpinMult", 1.35)
	table.insert(spinVisuals, spinRing)
	...
	glow:SetAttribute("SpinMult", 0.9)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		edge:SetAttribute("SpinMult", 1.1)
		edge:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, edge)
	end
