...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	-- Sharp fang blades (4-way, spin fast)
	for i = 0, 3 do
		local angle = i * 90 + 45
...
local function buildGlacierSpin(parent, color, accent, baseCFrame)
	...
	-- Frost crystal segments (slow counter-spin)
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.7, accent, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
