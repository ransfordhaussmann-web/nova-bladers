...
local function buildFlameCrown(parent, color, accent, baseCFrame)
	local visuals = {}
	local spinVisuals = {}
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(28)) * CFrame.new(0, 0.2, 1.15)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.55, 1.6),
			color = color,
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset,
		...
		local tip = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = spike.CFrame * CFrame.new(0, 0, 0.95),
		})
...
local function buildCrystalBloom(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.0)
		local facet = part({
			name = "Facet_" .. i,
			parent = parent,
	...
	prismRing:SetAttribute("SpinMult", 1.15)
	...
	outerRing:SetAttribute("SpinMult", 0.75)
