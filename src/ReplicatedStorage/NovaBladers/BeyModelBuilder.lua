...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(50, 110, 125),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	for layer, mult in ipairs({ 0.85, 1.05 }) do
		local ghost = part({
			name = "GhostLayer_" .. layer,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.15, 2.8 * mult, 2.8 * mult),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.45 + layer * 0.1,
			canCollide = false,
			cframe = baseCFrame,
		})
		ghost:SetAttribute("SpinMult", layer == 1 and 1.1 or -0.7)
		table.insert(spinVisuals, ghost)
	end

	for i = 0, 1 do
		local angle = i * 180 + 30
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, 1.15)
		local edge = part({
			name = "PhantomEdge_" .. i,
			parent = parent,
