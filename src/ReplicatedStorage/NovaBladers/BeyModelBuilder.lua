...
local function buildFrostCore(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.55, 0.75, color, Enum.Material.Ice, i * 60, "CrystalSeg_" .. i)
	...
	for i = 0, 2 do
		local angle = i * 120
		local spikeOffset = CFrame.Angles(0, math.rad(angle), math.rad(-15)) * CFrame.new(0, 0.2, 1.1)
		local spike = part({
			name = "IceSpike_" .. i,
			parent = parent,
			...
			transparency = 0.1,
			canCollide = false,
			cframe = baseCFrame * spikeOffset,
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", spikeOffset)
