...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 7 do
		local angle = i * 45
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(12)) * CFrame.new(0, 0, 2.0)
		local flare = part({
			name = "Flare_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.2, 1.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.2,
			canCollide = false,
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local angle = i * 180
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(8)) * CFrame.new(0, 0, 1.45)
		local edge = part({
			name = "PhantomEdge_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.9, 3.2),
			color = accent,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
