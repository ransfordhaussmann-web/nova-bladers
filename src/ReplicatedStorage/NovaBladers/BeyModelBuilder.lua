...
local function buildBlazeLance(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		flare:SetAttribute("SpinMult", 1.15)
		flare:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.35))
...
local function buildCoralTide(parent, color, accent, baseCFrame)
	...
	-- Wave crest segments
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.55, color, Enum.Material.SmoothPlastic, i * 60, "WaveSeg_" .. i)
	...
	tideRing:SetAttribute("SpinMult", 1.05)
	...
	local foamRing = part({
		...
		color = Color3.fromRGB(200, 250, 255),
		material = Enum.Material.Neon,
		transparency = 0.5,
		canCollide = false,
		cframe = baseCFrame,
	})
	foamRing:SetAttribute("SpinMult", 0.75)
	table.insert(spinVisuals, foamRing)
