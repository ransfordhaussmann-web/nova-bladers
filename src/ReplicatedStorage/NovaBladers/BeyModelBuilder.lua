...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local fang = part({
			name = "RipFang_" .. i,
			parent = parent,
			size = Vector3.new(0.42, 0.38, 1.7),
			color = color,
			material = Enum.Material.Metal,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	tornado:SetAttribute("SpinMult", 1.3)
	table.insert(spinVisuals, tornado)
...
local function buildFrostRing(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.05, 1.65, 0.75, color, Enum.Material.Ice, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 2 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.5, 1.1),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.5, 1.1))
