...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 90, 140),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 25, 40),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	bloodRing:SetAttribute("SpinMult", 1.05)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		block:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, block)
	end

	local outerWall = part({
		name = "OuterWall",
		parent = parent,
		shape = Enum.PartType.Cylinder,
		size = Vector3.new(0.45, 4.4, 4.4),
		color = accent,
		material = Enum.Material.Slate,
		transparency = 0.1,
		canCollide = false,
	...
	outerWall:SetAttribute("SpinMult", 0.4)
	table.insert(spinVisuals, outerWall)
