...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.25, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	local anchor = part({
		...
		material = Enum.Material.DiamondPlate,
		canCollide = false,
		cframe = baseCFrame * CFrame.new(0, 0.2, 1.1) * CFrame.Angles(math.rad(25), 0, 0),
	})
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 2.0),
		})
		ray:SetAttribute("SpinMult", 0.9)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 2.0))
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		...
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(15)) * CFrame.new(0, 0, 1.15)
		local blade = part({
			name = "PhaseBlade_" .. i,
			parent = parent,
	...
	ghostRing:SetAttribute("SpinMult", 0.95)
