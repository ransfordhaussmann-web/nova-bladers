...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.2))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.1, 1.7, 0.85, color, Enum.Material.Glass, i * 60, "IceSeg_" .. i)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.55, 0.35, 0),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(1.55, 0.35, 0))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 1 do
		local coilColor = i == 0 and color or accent
		local coil = part({
			name = "HelixCoil_" .. i,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.22, 3.2 + i * 0.6, 3.2 + i * 0.6),
			color = coilColor,
			material = Enum.Material.Neon,
			transparency = 0.15 + i * 0.1,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, i * 0.08, 0),
		})
	...
	for i = 0, 3 do
		...
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.1, 1.0))
