...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	-- Crown ice spikes (spin slowly)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.3, 1.25)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local height = (i % 2 == 0) and 0.15 or -0.15
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, height, 1.1)
		local coil = part({
			name = "CoilSeg_" .. i,
			parent = parent,
	...
	flare:SetAttribute("SpinMult", 1.5)
