...
local function buildStarfallEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(200, 80, 50),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for i = 0, 4 do
		...
		local star = part({
			name = "StarPoint_" .. i,
			parent = parent,
			size = Vector3.new(0.4, 0.35, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		star:SetAttribute("SpinMult", 1.15)
		star:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, star)
	end

	local cometTail = part({
		name = "CometTail",
		parent = parent,
		shape = Enum.PartType.Cylinder,
	...
	cometTail:SetAttribute("SpinMult", 0.9)
	table.insert(spinVisuals, cometTail)
...
local function buildStoneHalo(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.6, 0.85, color, Enum.Material.Concrete, i * 60, "StoneSeg_" .. i)
...
local function buildThunderCoil(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(40, 80, 160),
		material = Enum.Material.Metal,
		canCollide = false,
		cframe = baseCFrame,
	})
	...
	for layer = 1, 3 do
		local r = 0.9 + layer * 0.35
		local coil = part({
			name = "Coil_" .. layer,
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.18, r * 2, r * 2),
			color = layer % 2 == 0 and accent or color,
			material = Enum.Material.Neon,
			transparency = 0.15,
			canCollide = false,
			cframe = baseCFrame * CFrame.new(0, (layer - 2) * 0.12, 0),
		})
	...
	for i = 0, 2 do
		...
		bolt:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0.2, 1.1))
		table.insert(spinVisuals, bolt)
	end
...
local function buildNightMaw(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		fang:SetAttribute("SpinMult", 0.95)
		fang:SetAttribute("SpinOffset", offset)
		table.insert(spinVisuals, fang)
	end
	...
	jawAura:SetAttribute("SpinMult", 0.85)
	table.insert(spinVisuals, jawAura)
