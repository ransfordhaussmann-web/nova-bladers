...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local ray = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.5),
		})
		ray:SetAttribute("SpinMult", 1.15)
		ray:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(i * 60), math.rad(12)) * CFrame.new(0, 0, 1.5))
	...
	disk:SetAttribute("SpinMult", 1.2)
	table.insert(spinVisuals, disk)
...
local function buildPhantomEdge(parent, color, accent, baseCFrame)
	...
	local core = part({
		...
		color = Color3.fromRGB(120, 125, 145),
		material = Enum.Material.Glass,
		transparency = 0.2,
		canCollide = false,
		cframe = baseCFrame,
	...
	aura:SetAttribute("SpinMult", 0.8)
	table.insert(spinVisuals, aura)
...
local function buildCrystalEdge(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, 1.1)
		local shard = part({
			name = "CrystalShard_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.7, 1.4),
			color = color,
			material = Enum.Material.Glass,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	frost:SetAttribute("SpinMult", 1)
	table.insert(spinVisuals, frost)
...
local function buildBlazeCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 4 do
		local angle = i * 72
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(-25)) * CFrame.new(0, 0.4, 1.0)
		local spike = part({
			name = "CrownSpike_" .. i,
			parent = parent,
			size = Vector3.new(0.35, 0.8, 1.2),
			color = color,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
	...
	flame:SetAttribute("SpinMult", 1.1)
