...
local function buildBlazeQuill(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90 + 20
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.1, 1.3)
		local feather = part({
			name = "Feather_" .. i,
			parent = parent,
			size = Vector3.new(0.2, 0.5, 1.6),
			color = accent,
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * offset,
...
local function buildTideAnchor(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, -0.3, 1.1)
		local fluke = part({
			name = "AnchorFluke_" .. i,
			parent = parent,
			size = Vector3.new(0.55, 0.35, 1.0),
			color = color,
			material = Enum.Material.DiamondPlate,
			canCollide = false,
			cframe = baseCFrame * offset,
		})
		fluke:SetAttribute("SpinMult", 0.5)
	...
	waveRing:SetAttribute("SpinMult", 0.7)
	table.insert(spinVisuals, waveRing)
...
local function buildGraniteFort(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.7, 1.0, color, Enum.Material.Concrete, i * 60, "StoneSeg_" .. i)
...
local function buildSolarDrift(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.2, 1.0)
		local ray = part({
			name = "SolarRay_" .. i,
			parent = parent,
