...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			material = Enum.Material.Neon,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.55),
		})
		spike:SetAttribute("SpinMult", 0.5)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.55, 1.55))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for layer = 1, 2 do
		local ring = part({
			...
			parent = parent,
			shape = Enum.PartType.Cylinder,
			size = Vector3.new(0.22, 3.2 + layer * 0.5, 3.2 + layer * 0.5),
