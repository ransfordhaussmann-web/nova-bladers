...
local function buildCrimsonFang(parent, color, accent, baseCFrame)
	...
	for i = 0, 2 do
		...
		flame:SetAttribute("SpinOffset", offset * CFrame.new(0, 0, 1.35))
...
local function buildFrostCrown(parent, color, accent, baseCFrame)
	...
	for i = 0, 5 do
		local seg = ringSegment(parent, 1.0, 1.65, 0.85, color, Enum.Material.Ice, i * 60, "CrownSeg_" .. i)
	...
	for i = 0, 3 do
		...
		local spike = part({
			...
			material = Enum.Material.Glass,
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.7),
		})
		spike:SetAttribute("SpinMult", 0.9)
		spike:SetAttribute("SpinOffset", CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.5, 1.7))
...
local function buildSolarCoil(parent, color, accent, baseCFrame)
	...
	for i = 0, 3 do
		...
		local coil = part({
			...
			canCollide = false,
			cframe = baseCFrame * CFrame.Angles(0, math.rad(angle), math.rad(8)),
		})
