...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "TideAnchorCrash" then
		...
		elseif phase.id == "wave" then
			...
			if controller.pulseTimer >= (phase.interval or 0.35) then
				controller.pulseTimer = 0
				SpecialVFX.waterWave(controller.part.Position, phase.range or 8, move.color, folder)
				controller:areaHit(allControllers, phase.range or 8, phase.damage or 13, true)
			end
		end
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "tremor" then
			...
			if controller.pulseTimer >= (phase.interval or 0.4) then
				controller.pulseTimer = 0
				SpecialVFX.stoneTremor(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "loop" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			controller.spin = math.min(BeyConfig.MAX_SPIN, controller.spin + (move.spinRecovery or 0) * dt * 0.5)
			controller:checkCollisions(allControllers, true)
		end
