...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			controller.velocity = Vector3.zero
			local heal = move.healAmount or 15
			controller.hp = math.min(BeyConfig.MAX_HP, controller.hp + heal)
		elseif phase.id == "burst" then
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.15) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.ripperTrail(controller.ripLastPos, pos, move.color, folder)
