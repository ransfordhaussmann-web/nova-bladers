...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "dash1" or phase.id == "dash2" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.mirageDash(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "SolarFlareOrbit" then
		if phase.id == "orbit" and controller.orbitCenter then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burn" then
			controller.solarBurnTimer = (controller.solarBurnTimer or 0) + dt
			if controller.solarBurnTimer >= (phase.interval or 0.25) then
				controller.solarBurnTimer = 0
				SpecialVFX.solarBurnRing(controller.part.Position, 5, move.color, folder)
