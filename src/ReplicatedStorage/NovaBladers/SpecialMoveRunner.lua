...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" and controller.cycloneCenter then
			...
			local r = phase.orbitRadius or 4
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "seismic" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.seismicCrack(controller.part.Position, phase.range or 9, move.color, folder)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" and controller.orbitCenter then
			controller.orbitAngle = (controller.orbitAngle or 0) + (controller.orbitSpeed or 12) * dt
			...
			if controller.driftTimer >= (phase.interval or 0.25) then
				...
				controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 7, true)
			end
		elseif phase.id == "flare" then
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			SpecialVFX.phantomTrail(controller.part.Position, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "echo" then
			controller.echoTimer = (controller.echoTimer or 0) + dt
			if controller.echoTimer >= (phase.hitInterval or 0.2) and (controller.echoHitsLeft or 0) > 0 then
				controller.echoTimer = 0
				controller.echoHitsLeft -= 1
