...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "comet" then
		if phase.id == "windup" then
			SpecialVFX.cometCharge(controller, color, phase.duration)
		elseif phase.id == "launch" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
	...
	elseif mode == "comet" then
		...
		if phase.id == "flare" then
			...
			if controller.flareTimer >= (phase.hitInterval or 0.2) then
				controller.flareTimer = 0
				local pos = controller.part.Position
				SpecialVFX.cometTrail(controller.flareLastPos, pos, move.color, folder)
				SpecialVFX.flareBurst(pos, move.color, folder)
