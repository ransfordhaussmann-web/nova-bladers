...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			...
			controller.spiralShrink = 0.85
		elseif phase.id == "erupt" then
			SpecialVFX.blazeErupt(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.hitInterval or 0.22) then
				controller.shardTimer = 0
				controller.shardCount = (controller.shardCount or 0) + 1
				local range = (phase.hitRadius or 6) + controller.shardCount * 0.8
	...
	elseif mode == "blaze" then
		if phase.id == "spiral" and controller.orbitCenter then
			...
			local r = (controller.orbitRadius or 7) * (controller.spiralShrink or 1)
			controller.spiralShrink = math.max(0.3, (controller.spiralShrink or 1) - dt * 0.5)
			local center = controller.orbitCenter
			local y = controller.part.Position.Y
			local pos = center + Vector3.new(math.cos(controller.orbitAngle) * r, 0, math.sin(controller.orbitAngle) * r)
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.blazeTrail(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "erupt" then
