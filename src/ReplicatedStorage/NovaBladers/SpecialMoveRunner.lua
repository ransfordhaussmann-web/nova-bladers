...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "burst" then
			controller.burstTriggered = false
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastion" then
		...
		elseif phase.id == "slam" then
			if not controller.slamTriggered then
				...
				SpecialVFX.graniteSlam(controller.part.Position, phase.range or 9, move.color, folder)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burst" then
			if not controller.burstTriggered then
				controller.burstTriggered = true
				SpecialVFX.solarFlare(controller.part.Position, phase.range or 7, move.color, folder)
