...
function SpecialMoveRunner.updateOrbit(controller, dt, allControllers, move, phase)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "sonic" then
		...
		elseif phase.id == "orbit" then
			SpecialMoveRunner.updateOrbit(controller, dt, allControllers, move, phase)
		end
	...
	elseif mode == "rampart" then
		if phase.id == "brace" or phase.id == "rampart" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
