...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRendingArc" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "saw" then
			controller.sawTimer = 0
			controller.sawHitsLeft = phase.hits or 4
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendingArc" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "saw" then
			controller.sawTimer = (controller.sawTimer or 0) + dt
			if controller.sawTimer >= (phase.interval or 0.16) then
				controller.sawTimer = 0
				controller.sawAngle = (controller.sawAngle or 0) + math.rad(90)
				local slashDir = Vector3.new(math.cos(controller.sawAngle), 0, math.sin(controller.sawAngle))
