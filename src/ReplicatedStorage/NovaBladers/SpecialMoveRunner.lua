...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeSpiralDrive" then
		...
		elseif phase.id == "spiral" then
			...
			controller.spiralForward = (controller.spiralForward or 0) + dt * (phase.rushSpeed or move.rushSpeed or 85)
			local perp = Vector3.new(-controller.facing.Z, 0, controller.facing.X)
			local offset = perp * math.sin(controller.spiralAngle) * (controller.spiralRadius or 3.5)
			local forward = controller.facing * controller.spiralForward
			controller.velocity = (forward + offset).Unit * (phase.rushSpeed or move.rushSpeed or 85)
			controller.spiralTimer = (controller.spiralTimer or 0) + dt
			if controller.spiralTimer >= (phase.interval or 0.12) then
