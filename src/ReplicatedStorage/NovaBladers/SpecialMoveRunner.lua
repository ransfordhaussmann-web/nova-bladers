...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "phantom" then
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "rift" then
			SpecialVFX.voidRift(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "maelstrom" then
		...
		elseif phase.id == "vortex" then
			...
			if controller.vortexTimer >= (phase.interval or 0.15) then
				controller.vortexTimer = 0
				controller.vortexCount = (controller.vortexCount or 0) + 1
				local range = 3 + controller.vortexCount * 1.2
				SpecialVFX.sonicRing(controller.part.Position, range, move.color, folder)
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "flare" then
			...
			if controller.solarTimer >= (phase.interval or 0.22) then
				...
				local maxR = phase.maxRadius or 12
				local range = 3 + controller.solarCount * (maxR / 4)
