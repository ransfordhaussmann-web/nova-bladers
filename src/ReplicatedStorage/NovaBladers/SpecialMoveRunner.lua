...
local function onMeteorPhaseStart(controller, move, phase)
...
local function onFortressPhaseStart(controller, move, phase)
	local color = move.color
	local useBurrow = move.underground ~= false

	if phase.id == "burrow" and useBurrow then
		SpecialVFX.setUnderground(controller, true)
		SpecialVFX.burrowCloud(controller, color)
		controller.velocity = Vector3.zero
	elseif phase.id == "windup" and not useBurrow then
...
local function onSonicPhaseStart(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
...
local function onEclipsePhaseStart(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if mode == "meteor" then
		onMeteorPhaseStart(controller, move, phase)
	elseif mode == "fortress" then
		onFortressPhaseStart(controller, move, phase)
	elseif mode == "sonic" then
		onSonicPhaseStart(controller, move, phase)
	elseif mode == "eclipse" then
		onEclipsePhaseStart(controller, move, phase)
	end
end
