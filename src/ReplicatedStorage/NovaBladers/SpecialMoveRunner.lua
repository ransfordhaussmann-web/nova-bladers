...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "frost" then
		...
		elseif phase.id == "citadel" then
			...
			SpecialVFX.citadelRing(controller, color, phase.duration)
	...
	elseif mode == "slash" then
		...
		if phase.id == "barrage" then
			...
			if controller.meteorTimer >= (phase.hitInterval or 0.14) then
				controller.meteorTimer = 0
				local pos = controller.part.Position
				SpecialVFX.slashArc(controller.meteorLastPos, pos, move.color, folder)
				SpecialVFX.slashImpact(pos, move.color, folder)
				controller.meteorLastPos = pos
