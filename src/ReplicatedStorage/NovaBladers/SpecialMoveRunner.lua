...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "prism" then
		...
		elseif phase.id == "shatter" then
			SpecialVFX.prismShatter(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "prism" then
		if phase.id == "prism" or phase.id == "reflect" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
		...
		if phase.id == "reflect" then
			...
			if controller.pulseTimer >= (phase.interval or 0.35) then
				controller.pulseTimer = 0
				SpecialVFX.prismPulse(controller.part.Position, phase.range or 7, move.color, folder)
