...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipper" then
		...
		elseif phase.id == "finisher" then
			SpecialVFX.fangBurst(controller.part.Position, color, folder)
		end
	...
	elseif move.id == "PhantomMirage" then
		if phase.id == "split" then
			SpecialVFX.mirageClone(controller, color, phase.duration)
		elseif phase.id == "blink" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.blinkFlash(controller.part.Position, color, folder)
			local behind = targetPos
			if target and target.part then
				local tFacing = target.facing or Vector3.new(0, 0, -1)
				behind = target.part.Position - tFacing * 4
			end
			controller.part.CFrame = CFrame.new(behind + Vector3.new(0, 0.5, 0))
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipper" then
		...
		elseif phase.id == "lunge" then
			...
			if controller.lungeTimer >= (phase.hitInterval or 0.28) then
				controller.lungeTimer = 0
				controller.lungeHitsLeft = (controller.lungeHitsLeft or 3) - 1
				SpecialVFX.fangStrike(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 12, true)
				if controller.lungeHitsLeft <= 0 and controller.specialPhase then
