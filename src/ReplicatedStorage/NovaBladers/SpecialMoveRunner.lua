...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonCrossSlash" then
		...
		elseif phase.id == "slash1" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.crossSlashDir = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "slash2" then
			local base = controller.crossSlashDir or controller.facing
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonCrossSlash" then
		...
		elseif phase.id == "slash1" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			SpecialVFX.crossSlash(controller.part.Position, controller.facing, move.color, folder, 1)
			controller:checkCollisions(allControllers, true)
			controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 14, true)
		elseif phase.id == "slash2" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			SpecialVFX.crossSlash(controller.part.Position, controller.facing, move.color, folder, 2)
			controller:checkCollisions(allControllers, true)
			controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 14, true)
		elseif phase.id == "impact" then
