...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "drift" then
			...
			SpecialVFX.burnOrbit(controller, color, phase.duration)
		end
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "mirage" then
			controller.mirageDashesLeft = phase.dashes or 3
			controller.mirageTimer = 0
			controller.mirageDashInterval = phase.duration / (phase.dashes or 3)
		elseif phase.id == "blade" then
			SpecialVFX.setPhantom(controller, false)
			SpecialVFX.bladeFinale(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirage" then
			...
			if controller.mirageTimer >= (controller.mirageDashInterval or 0.18) then
				controller.mirageTimer = 0
				...
				dir = Vector3.new(dir.X, 0, dir.Z).Unit
				controller.facing = dir
				controller.velocity = dir * (phase.dashSpeed or move.rushSpeed or 78)
