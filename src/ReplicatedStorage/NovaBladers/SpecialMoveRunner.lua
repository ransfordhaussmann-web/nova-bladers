...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "edge" then
			...
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.edgeSlash(controller.part.Position, controller.facing, color, folder, phase.range or 6)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			local spinOffset = CFrame.Angles(0, controller.cycloneSpin, 0)
			...
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				...
				SpecialVFX.bladeArc(controller.part.Position, move.color, folder, phase.hitRadius or 5)
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "quake" then
			...
			if controller.quakeTimer >= (phase.interval or 0.38) then
				controller.quakeTimer = 0
				SpecialVFX.quakeRipple(controller.part.Position, phase.range or 9, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			...
			if controller.dashTimer >= dashInterval then
				...
				controller.facing = dir
				local dashDist = math.min((targetPos - fromPos).Magnitude, 14)
				local toPos = fromPos + dir * dashDist
				SpecialVFX.phantomDash(fromPos, toPos, move.color, folder)
				controller.part.CFrame = CFrame.new(Vector3.new(toPos.X, controller.part.Position.Y, toPos.Z), toPos + dir)
