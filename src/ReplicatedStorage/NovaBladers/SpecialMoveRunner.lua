...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonInfernoRush" then
		if phase.id == "windup" then
			SpecialVFX.flameCharge(controller, color, phase.duration)
		...
		elseif phase.id == "inferno" then
			controller.infernoHitsLeft = phase.hits or 5
	...
	elseif move.id == "CrimsonInfernoRush" then
		...
		if phase.id == "inferno" then
			...
			if controller.infernoTimer >= (phase.hitInterval or 0.15) then
				controller.infernoTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameTrail(controller.infernoLastPos or pos, pos, move.color, folder)
