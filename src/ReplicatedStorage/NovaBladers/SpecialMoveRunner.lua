...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "lacerate" then
			controller.lacerateTimer = 0
			controller.lacerateHitsLeft = phase.hits or 3
		end
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "burst" then
			controller.solarBurstTimer = 0
			controller.solarBurstCount = 0
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "lacerate" then
			...
			controller.lacerateTimer = (controller.lacerateTimer or 0) + dt
			if controller.lacerateTimer >= (phase.hitInterval or 0.16) then
				controller.lacerateTimer = 0
				SpecialVFX.slashArc(controller.part.Position, controller.facing, move.color, folder)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "brace" or phase.id == "pillars" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "loop" and controller.orbitCenter then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burst" then
			controller.solarBurstTimer = (controller.solarBurstTimer or 0) + dt
			if controller.solarBurstTimer >= (phase.interval or 0.3) then
				controller.solarBurstTimer = 0
				controller.solarBurstCount = (controller.solarBurstCount or 0) + 1
				local range = 3.5 + controller.solarBurstCount * 1.8
				SpecialVFX.sonicRing(controller.part.Position, range, move.color, folder)
