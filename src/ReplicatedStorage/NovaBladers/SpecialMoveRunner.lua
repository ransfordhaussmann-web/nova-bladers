...
local function onPhaseStartSlam(controller, move, phase)
	if phase.id == "leap" then
		SpecialVFX.chargeAura(controller, move.color, phase.duration)
		controller.verticalVelocity = phase.leapForce or 38
		controller.airborne = true
		controller.velocity *= 0.5
	elseif phase.id == "slam" then
		controller.verticalVelocity = -55
	elseif phase.id == "ripple" then
		controller.pulseTimer = 0
	end
end

local function onPhaseStartPhase(controller, move, phase, target, folder)
	if phase.id == "vanish" then
		SpecialVFX.phaseVanish(controller, move.color, phase.duration)
		controller.velocity = Vector3.zero
	elseif phase.id == "blink" then
		...
		SpecialVFX.phaseBlink(controller.part.Position, behind, move.color, folder)
		controller.part.CFrame = CFrame.new(behind, targetPos)
		controller.facing = (targetPos - behind).Unit
	elseif phase.id == "slash" then
		SpecialVFX.phaseSlash(controller, move.color, folder)
	end
end

local PHASE_START = {
	meteor = function(c, m, p, t) onPhaseStartMeteor(c, m, p, t) end,
	fortress = function(c, m, p) onPhaseStartFortress(c, m, p) end,
	sonic = function(c, m, p, t) onPhaseStartSonic(c, m, p, t) end,
	eclipse = function(c, m, p, t, f) onPhaseStartEclipse(c, m, p, t, f) end,
	slam = function(c, m, p) onPhaseStartSlam(c, m, p) end,
	phase = function(c, m, p, t, f) onPhaseStartPhase(c, m, p, t, f) end,
}
...
local function updateEclipse(controller, move, phase, _dt, allControllers, _folder)
...
local function updateSlam(controller, move, phase, dt, allControllers, folder)
	if phase.id == "leap" then
		controller.velocity *= 0.85
	elseif phase.id == "slam" then
		if controller.part.Position.Y <= controller.floorY + 0.5 then
			SpecialVFX.slamImpact(controller.part.Position, phase.range or 9, move.color, folder)
...
local function updatePhase(controller, move, phase, _dt, allControllers, _folder)
