...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			controller.shatterHitsLeft = phase.hits or 4
		end
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			...
			controller.spiralRadius = move.spiralRadius or 8
			controller.spiralSpeed = move.spiralSpeed or 18
			controller.spiralTimer = 0
		elseif phase.id == "finale" then
			SpecialVFX.blazeTrail(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.hitInterval or 0.2) then
				controller.shatterTimer = 0
				local pos = controller.part.Position
				SpecialVFX.shatterBurst(pos, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 6, phase.damage or 13, true)
			end
		end
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			controller.spiralAngle = (controller.spiralAngle or 0) + (controller.spiralSpeed or 18) * dt
			local r = controller.spiralRadius or 8
			local origin = controller.arenaOrigin
			local y = controller.part.Position.Y
			local pos = origin + Vector3.new(math.cos(controller.spiralAngle) * r, 0, math.sin(controller.spiralAngle) * r)
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), origin)
			controller.velocity = Vector3.zero
