...
local function onNovaPhaseStart(controller, move, phase, color, folder)
	if phase.id == "charge" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
	elseif phase.id == "nova" then
		SpecialVFX.novaBurst(controller.part.Position, color, phase.range or 8, folder)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "nova" then
		onNovaPhaseStart(controller, move, phase, color, folder)
	end
end
...
local function updateNova(controller, move, phase, dt, allControllers, folder)
	if phase.id == "charge" then
	...
	elseif phase.id == "flare" then
		...
		if controller.flareTimer >= (phase.interval or 0.22) then
			controller.flareTimer = 0
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "nova" then
		updateNova(controller, move, phase, dt, allControllers, folder)
	end
