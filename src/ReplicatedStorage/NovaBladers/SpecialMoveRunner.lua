...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "fang" then
		if phase.id == "parry" then
			controller.guardReduction = move.damageReduction or 0.7
			SpecialVFX.parryFlash(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "riposte" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.fangRush(controller, color, folder)
	...
	elseif mode == "veil" then
		if phase.id == "veil" then
			SpecialVFX.veilShroud(controller, color, phase.duration)
			controller.velocity *= 0.5
		elseif phase.id == "slash" then
			SpecialVFX.setVeil(controller, false)
			local targetPos = getTargetPos(controller, target)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.phantomSlash(controller, targetPos, color, folder)
		elseif phase.id == "aftershock" then
			SpecialVFX.aftershockBurst(controller.part.Position, color, folder)
		end

	elseif mode == "inferno" then
		if phase.id == "crown" then
			SpecialVFX.crownCharge(controller, color, phase.duration)
			controller.velocity *= 0.85
		elseif phase.id == "inferno" then
			controller.infernoTimer = 0
			controller.infernoCount = 0
		elseif phase.id == "finale" then
			SpecialVFX.crownFinale(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "inferno" then
		if phase.id == "inferno" then
			...
			if controller.infernoTimer >= (phase.interval or 0.28) then
				controller.infernoTimer = 0
				controller.infernoCount = (controller.infernoCount or 0) + 1
				local range = 3 + controller.infernoCount * 1.2
				SpecialVFX.infernoRing(controller.part.Position, range, move.color, folder)
