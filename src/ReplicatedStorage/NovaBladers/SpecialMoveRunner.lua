...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFangRend" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "lunge" or phase.id == "rend" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		end
		if phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.17) then
				controller.rendTimer = 0
				controller.rendAngle = (controller.rendAngle or 0) + math.pi / 2
				local pos = controller.part.Position
				SpecialVFX.slashArc(pos, controller.rendAngle, move.color, folder)
	...
	elseif move.id == "FrostCrownShatter" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.26) then
				controller.shardTimer = 0
				controller.shardCount = (controller.shardCount or 0) + 1
				local range = 3.5 + controller.shardCount * 1.2
