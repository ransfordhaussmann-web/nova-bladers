...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameSpiral" then
		...
		if phase.id == "spiral" then
			...
			local spiralOffset = Vector3.new(math.cos(controller.spiralAngle) * 2.5, 0, math.sin(controller.spiralAngle) * 2.5)
			controller.part.CFrame = CFrame.new(controller.part.Position + spiralOffset * dt * 3, controller.part.Position + controller.facing)
			controller.meteorTimer = (controller.meteorTimer or 0) + dt
	...
	elseif move.id == "FrostBastion" then
		...
		elseif phase.id == "pulse" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.icePulse(controller.part.Position, phase.range or 8.5, move.color, folder)
