...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "rupture" then
			SpecialVFX.auroraRupture(controller.part.Position, color, phase.range or 9, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.2) then
				controller.ripTimer = 0
				controller.ripHitsLeft = (controller.ripHitsLeft or 3) - 1
				local pos = controller.part.Position
				SpecialVFX.crimsonFangArc(pos, controller.facing, move.color, folder)
