...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRendClaw" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
	...
	elseif move.id == "FrostHaloCrown" then
	...
	elseif move.id == "CrimsonRendClaw" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.22) then
				controller.slashTimer = 0
				local side = ((controller.slashHitsLeft or 1) % 2 == 0) and 1 or -1
				local slashDir = (CFrame.new(Vector3.zero, controller.facing) * CFrame.Angles(0, math.rad(55 * side), 0)).LookVector
				controller.facing = slashDir
				controller.velocity = slashDir * (move.rushSpeed or 80)
				SpecialVFX.clawSlash(controller.part.Position, slashDir, move.color, folder)
		...
		elseif phase.id == "rend" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
			if now >= controller.specialPhaseEnd - 0.08 then
				SpecialVFX.clawBurst(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.range or 7, phase.damage or 34, true)
			end
		end

	elseif move.id == "FrostHaloCrown" then
		if phase.id == "crown" or phase.id == "ring" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shards" then
			controller.shardTimer = (controller.shardTimer or 0) + dt
			if controller.shardTimer >= (phase.interval or 0.3) then
				controller.shardTimer = 0
				SpecialVFX.frostShards(controller.part.Position, phase.range or 7.5, move.color, folder)
