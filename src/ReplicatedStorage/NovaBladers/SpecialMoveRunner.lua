...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeInfernoLoop" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			controller.blazeHitTimer = (controller.blazeHitTimer or 0) + dt
			if controller.blazeHitTimer >= (phase.hitInterval or 0.2) then
				controller.blazeHitTimer = 0
				SpecialVFX.fireOrbitSpark(controller, move.color, folder)
