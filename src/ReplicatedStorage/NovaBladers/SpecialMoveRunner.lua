...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "ripper" then
			controller.ripperHitsLeft = phase.slashes or 3
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			controller.velocity = Vector3.zero
		elseif phase.id == "refract" then
			controller.refractTimer = (controller.refractTimer or 0) + dt
			if controller.refractTimer >= (phase.interval or 0.35) then
				controller.refractTimer = 0
				SpecialVFX.refractPulse(controller.part.Position, phase.range or 7, move.color, folder)
