...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarCoronaLoop" then
		...
		elseif phase.id == "recover" then
			...
			SpecialVFX.spinRecoveryFlash(controller, color)
	...
	elseif move.id == "CrimsonRipperFang" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.interval or 0.18) then
				...
				local turn = CFrame.lookAt(Vector3.zero, controller.facing)
				controller.facing = (turn * CFrame.Angles(0, math.rad(120), 0)).LookVector
