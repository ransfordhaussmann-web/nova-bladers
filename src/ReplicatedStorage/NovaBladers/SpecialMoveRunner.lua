...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "feint" then
			...
			if controller.feintTimer >= 0.12 then
				controller.feintTimer = 0
				controller.feintZig = -(controller.feintZig or 1)
				local perp = Vector3.new(-controller.facing.Z, 0, controller.facing.X) * (controller.feintZig or 1)
				controller.velocity = (controller.facing + perp * 0.35).Unit * (phase.rushSpeed or move.rushSpeed or 80)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "strike" then
