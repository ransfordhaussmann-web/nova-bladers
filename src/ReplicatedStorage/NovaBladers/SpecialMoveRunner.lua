...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFangRush" then
		...
		elseif phase.id == "fangs" then
			controller.fangHitsLeft = phase.hits or 4
	...
	elseif move.id == "CrimsonFangRush" then
		...
		if phase.id == "fangs" then
			...
			if controller.fangTimer >= (phase.hitInterval or 0.16) then
				controller.fangTimer = 0
				local pos = controller.part.Position
				SpecialVFX.slashArc(controller.fangLastPos, pos, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
	...
	elseif move.id == "FrostHaloCrown" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.3) then
				controller.shatterTimer = 0
				SpecialVFX.frostBurst(controller.part.Position, phase.range or 7.5, move.color, folder)
