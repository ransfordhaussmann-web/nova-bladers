...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarCoronaLoop" then
		...
		elseif phase.id == "drift" then
			...
			controller.driftCoast = phase.coastMult or 1.3
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperFang" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.interval or 0.3) then
				controller.slashTimer = 0
				controller.slashCount = (controller.slashCount or 0) + 1
				if controller.slashCount % 2 == 0 then
	...
	elseif move.id == "SolarCoronaLoop" then
		...
		elseif phase.id == "corona" then
			...
			if controller.coronaTimer >= (phase.interval or 0.28) then
				controller.coronaTimer = 0
		...
		elseif phase.id == "drift" then
			local coast = controller.driftCoast or 1.3
			if controller.velocity.Magnitude < BeyConfig.BASE_SPEED * coast then
				controller.velocity = controller.facing * BeyConfig.BASE_SPEED * coast
			end
			SpecialVFX.solarDriftTrail(controller, move.color, folder)
		end
