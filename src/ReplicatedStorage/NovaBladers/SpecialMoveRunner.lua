...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.orbitRadius = phase.orbitRadius or move.orbitRadius or 10
			controller.orbitSpeed = phase.orbitSpeed or 9
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "afterimage" then
			...
			controller.afterimageHitsLeft = phase.hits or 3
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				controller.cycloneTimer = 0
				SpecialVFX.bladeArc(controller.part.Position, phase.range or 5.5, move.color, folder, controller.cycloneAngle)
	...
	elseif move.id == "SolarFlareDrift" then
		if phase.id == "drift" and controller.orbitCenter then
			controller.orbitAngle += (controller.orbitSpeed or 9) * dt
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "afterimage" then
			...
			if controller.afterimageTimer >= (phase.interval or 0.16) and (controller.afterimageHitsLeft or 0) > 0 then
				controller.afterimageTimer = 0
				controller.afterimageHitsLeft -= 1
				local offset = Vector3.new(math.random(-4, 4), 0, math.random(-4, 4))
				local hitPos = controller.part.Position + offset
				SpecialVFX.afterimageHit(hitPos, move.color, folder)
