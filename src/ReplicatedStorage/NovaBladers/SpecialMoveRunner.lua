...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "anchor" then
		if phase.id == "sink" then
			controller.velocity *= 0.3
			SpecialVFX.tidalWave(controller.part.Position, 4, color, folder)
		elseif phase.id == "anchor" then
			controller.guardReduction = move.damageReduction or 0.6
			controller.velocity = Vector3.zero
			SpecialVFX.anchorShield(controller, color, phase.duration)
		elseif phase.id == "tidal" then
			controller.tidalTimer = 0
		end
	elseif mode == "rend" then
		if phase.id == "stalk" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
		...
		elseif phase.id == "slash" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.rendSlash(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "bleed" then
			SpecialVFX.bleedBurst(controller.part.Position, color, folder)
		end
	elseif mode == "bastion" then
		if phase.id == "fortify" then
			controller.guardReduction = move.damageReduction or 0.65
			controller.velocity = Vector3.zero
			SpecialVFX.bastionRing(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lance" then
		...
		if phase.id == "barrage" then
			...
			if controller.lanceTimer >= (phase.hitInterval or 0.16) then
				controller.lanceTimer = 0
				local pos = controller.part.Position
				SpecialVFX.lanceStrike(controller.lanceLastPos or pos, pos, move.color, folder)
	...
	elseif mode == "anchor" then
		...
		elseif phase.id == "tidal" then
			...
			if controller.tidalTimer >= (phase.interval or 0.3) then
				controller.tidalTimer = 0
				SpecialVFX.tidalWave(controller.part.Position, phase.range or 7.5, move.color, folder)
				controller:areaHit(allControllers, phase.range or 7.5, phase.damage or 12, true)
			end
		end

	elseif mode == "rend" then
		if phase.id == "stalk" and controller.orbitCenter then
			...
			local pos = center + Vector3.new(math.cos(controller.orbitAngle) * r, 0, math.sin(controller.orbitAngle) * r)
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = (center - pos).Unit * 8
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "slash" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
	...
	elseif mode == "bastion" then
		...
		elseif phase.id == "shock" then
			...
			if controller.shockTimer >= (phase.interval or 0.35) then
				controller.shockTimer = 0
				SpecialVFX.graniteShock(controller.part.Position, phase.range or 6.5, move.color, folder)
				controller:areaHit(allControllers, phase.range or 6.5, phase.damage or 11, true)
			end
		elseif phase.id == "counter" then
