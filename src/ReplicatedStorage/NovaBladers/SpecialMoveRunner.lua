...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonSolarFlare" then
		if phase.id == "charge" then
		...
		elseif phase.id == "burn" then
			controller.burnHitsLeft = phase.hits or 4
			controller.burnTimer = 0
		end
	elseif move.id == "FrostGlacierLock" then
		...
		elseif phase.id == "anchor" then
			...
			SpecialVFX.glacierAnchor(controller, color, phase.duration)
