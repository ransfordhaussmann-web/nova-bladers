...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrystalShatterStorm" then
		if phase.id == "freeze" then
			SpecialVFX.freezeAura(controller, color, phase.duration)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "EmberPhoenixSpiral" then
		...
		elseif phase.id == "burst" then
			controller.velocity = Vector3.zero
			SpecialVFX.phoenixBurst(controller.part.Position, phase.range or 7, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrystalShatterStorm" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.range or 4) + controller.shardCount * 1.2
	...
	elseif move.id == "EmberPhoenixSpiral" then
		...
		elseif phase.id == "spiral" then
			...
			if controller.phoenixTimer >= (phase.interval or 0.22) then
				controller.phoenixTimer = 0
