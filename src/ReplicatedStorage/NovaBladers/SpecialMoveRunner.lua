...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFangStorm" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		...
		elseif phase.id == "finisher" then
			SpecialVFX.clawSlash(controller.part.Position, controller.facing, color, folder)
		end
	...
	elseif move.id == "PhantomPhaseStrike" then
		if phase.id == "phase" then
			...
			if dir.Magnitude > 1 then
				...
				controller.part.CFrame = CFrame.new(
					controller.part.Position + dir * math.min(10, dir.Magnitude * 0.6),
					controller.part.Position + dir
				)
				controller.facing = dir
			end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFangStorm" then
		...
		elseif phase.id == "rush" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.16) then
				controller.ripperTimer = 0
				SpecialVFX.clawSlash(controller.part.Position, controller.facing, move.color, folder)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "anchor" or phase.id == "wall" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
