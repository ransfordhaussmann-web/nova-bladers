...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CometInferno" then
		...
		elseif phase.id == "nova" then
			controller.novaHitsLeft = phase.hits or 3
			controller.novaTimer = 0
		end
	elseif move.id == "GlacierCrown" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CometInferno" then
		...
		elseif phase.id == "nova" then
			...
			if controller.novaTimer >= (phase.interval or 0.22) then
				controller.novaTimer = 0
				controller.novaHitsLeft = (controller.novaHitsLeft or 1) - 1
				SpecialVFX.infernoBurst(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "GlacierCrown" then
		...
		elseif phase.id == "spikes" then
			...
			if controller.frostTimer >= (phase.interval or 0.3) then
				controller.frostTimer = 0
				SpecialVFX.frostSpikes(controller.part.Position, phase.range or 7.5, move.color, folder)
