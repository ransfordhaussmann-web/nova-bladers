...
function SpecialMoveRunner.endMove(controller)
	...
	controller.underground = false
	if controller._blazeFire then
		controller._blazeFire:Destroy()
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.mode == "blaze" then
		if phase.id == "ignite" then
			controller.velocity = controller.facing * 32
		elseif phase.id == "orbit" and controller.orbitCenter then
		...
		elseif phase.id == "slam" then
			...
			if controller.part.Position.Y <= controller.floorY + 0.5 then
				SpecialVFX.blazeSlam(controller.part.Position, move.color, folder)
