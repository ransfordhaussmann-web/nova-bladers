...
local function onFortressPhaseStart(controller, move, phase)
	local theme = move.vfxTheme
	if phase.id == "burrow" then
		SpecialVFX.setUnderground(controller, true)
		SpecialVFX.burrowCloud(controller, move.color, theme)
	...
	elseif phase.id == "wall" then
		...
		SpecialVFX.wallRing(controller, move.color, phase.duration, theme)
...
local function onBlazePhaseStart(controller, move, phase, target)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	local mode = move.mode or move.id

	if mode == "meteor" then
		onMeteorPhaseStart(controller, move, phase, target)
	elseif mode == "fortress" then
		onFortressPhaseStart(controller, move, phase)
	elseif mode == "sonic" then
		onSonicPhaseStart(controller, move, phase, target)
	elseif mode == "eclipse" then
		onEclipsePhaseStart(controller, move, phase, target, folder)
	elseif mode == "blaze" then
		onBlazePhaseStart(controller, move, phase, target)
	end
end
...
local function updateMeteor(controller, move, phase, dt, folder, allControllers)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
...
local function updateFortress(controller, move, phase, dt, folder, allControllers)
	local theme = move.vfxTheme
	if phase.id == "burrow" then
		controller.velocity = Vector3.zero
		local pos = controller.part.Position
	...
	elseif phase.id == "pulse" then
		...
		if controller.pulseTimer >= (phase.interval or 0.35) then
			controller.pulseTimer = 0
			SpecialVFX.pulseWave(controller.part.Position, phase.range or 8, move.color, folder, theme)
			controller:areaHit(allControllers, phase.range or 8, phase.damage or 13, true)
		end
	end
end

local function updateSonic(controller, move, phase, dt, folder, allControllers)
	if phase.id == "charge" then
...
local function updateBlaze(controller, move, phase, dt, folder, allControllers)
	if phase.id == "ignite" then
		controller.velocity = Vector3.zero
	elseif phase.id == "rush" then
	...
	elseif phase.id == "inferno" then
		...
		if controller.infernoTimer >= (phase.interval or 0.22) then
			...
			local range = 3.5 + controller.infernoCount * 1.2
			SpecialVFX.blazeRing(controller.part.Position, range, move.color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if mode == "meteor" then
		updateMeteor(controller, move, phase, dt, folder, allControllers)
	elseif mode == "fortress" then
		updateFortress(controller, move, phase, dt, folder, allControllers)
	elseif mode == "sonic" then
		updateSonic(controller, move, phase, dt, folder, allControllers)
	elseif mode == "eclipse" then
		updateEclipse(controller, move, phase, allControllers)
	elseif mode == "blaze" then
		updateBlaze(controller, move, phase, dt, folder, allControllers)
	end
