...
local function onPhaseModeStart(controller, move, phase, target)
	...
	elseif phase.id == "blink" then
		...
		local dist = math.min(dir.Magnitude, phase.blinkRange or 14)
...
local function updatePhaseMode(controller, move, phase, allControllers)
	if phase.id == "fade" then
		controller.velocity = Vector3.zero
	elseif phase.id == "blink" then
		controller.velocity = Vector3.zero
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local handler = MODE_UPDATE_HANDLERS[move.mode]
	if handler then
