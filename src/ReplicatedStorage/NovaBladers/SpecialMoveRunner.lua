...
local function onCometPhaseStart(controller, move, phase, color, target, folder)
	...
	elseif phase.id == "flare" then
		SpecialVFX.flareBurst(controller.part.Position, color, folder, phase.range)
		controller.flareHitDone = false
	end
end

local function onGlacierPhaseStart(controller, move, phase, color)
	if phase.id == "freeze" then
		controller.guardReduction = move.damageReduction or 0.6
		SpecialVFX.frostArmor(controller, color, phase.duration)
		controller.velocity = Vector3.zero
	elseif phase.id == "shards" then
		controller.shardTimer = 0
	elseif phase.id == "avalanche" then
		controller.velocity = Vector3.zero
		controller.avalancheHitDone = false
	end
end
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if mode == "meteor" then
		onMeteorPhaseStart(controller, move, phase, color, target)
	elseif mode == "fortress" then
		onFortressPhaseStart(controller, move, phase, color)
	elseif mode == "sonic" then
		onSonicPhaseStart(controller, move, phase, color, target)
	elseif mode == "eclipse" then
		onEclipsePhaseStart(controller, move, phase, color, target, folder)
	elseif mode == "comet" then
		onCometPhaseStart(controller, move, phase, color, target, folder)
	elseif mode == "glacier" then
		onGlacierPhaseStart(controller, move, phase, color)
	end
end
...
local function updateComet(controller, move, phase, dt, allControllers, folder)
	if phase.id == "ignite" then
		controller.velocity = Vector3.zero
	elseif phase.id == "streak" then
...
local function updateGlacier(controller, move, phase, dt, allControllers, folder)
	if phase.id == "freeze" then
		controller.velocity = Vector3.zero
	elseif phase.id == "shards" then
		...
		if controller.shardTimer >= (phase.interval or 0.3) then
			...
			for i = 0, 2 do
				...
				SpecialVFX.iceShard(origin, dir, move.color, folder)
			end
			controller:areaHit(allControllers, phase.range or 7, phase.damage or 10, true)
		end
	elseif phase.id == "avalanche" then
		controller.velocity = Vector3.zero
		if not controller.avalancheHitDone then
			controller.avalancheHitDone = true
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if mode == "meteor" then
		updateMeteor(controller, move, phase, dt, allControllers, folder)
	elseif mode == "fortress" then
		updateFortress(controller, move, phase, dt, allControllers, folder)
	elseif mode == "sonic" then
		updateSonic(controller, move, phase, dt, allControllers, folder)
	elseif mode == "eclipse" then
		updateEclipse(controller, move, phase, allControllers)
	elseif mode == "comet" then
		updateComet(controller, move, phase, dt, allControllers, folder)
	elseif mode == "glacier" then
		updateGlacier(controller, move, phase, dt, allControllers, folder)
	end
