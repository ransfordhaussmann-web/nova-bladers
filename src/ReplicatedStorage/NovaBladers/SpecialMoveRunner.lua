...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "leap" then
			controller.verticalVelocity = phase.leapForce or move.leapForce or 55
			controller.airborne = true
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
		...
		elseif phase.id == "slam" then
			...
			controller.facing = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.velocity = dir * (phase.slamSpeed or move.slamSpeed or 70)
			controller.verticalVelocity = -(phase.slamSpeed or move.slamSpeed or 70) * 0.6
	...
	elseif move.id == "PrismCrystalShield" then
		...
		elseif phase.id == "shatter" then
			SpecialVFX.crystalShatter(controller.part.Position, color, folder, phase.shardCount or 6)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "slam" then
			controller.velocity = controller.facing * (phase.slamSpeed or move.slamSpeed or 70)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "shock" then
			controller.shockTimer = (controller.shockTimer or 0) + dt
			if controller.shockTimer >= (phase.interval or 0.2) then
				controller.shockTimer = 0
