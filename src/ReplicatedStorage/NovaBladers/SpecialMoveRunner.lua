...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "spiral" then
		...
		elseif phase.id == "finisher" then
			...
			SpecialVFX.clawSlash(controller.part.Position, dir, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "spiral" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "spiral" then
			controller.spiralAngle = (controller.spiralAngle or 0) + (controller.orbitSpeed or 22) * dt
			...
			if controller.spiralTimer >= (phase.interval or 0.2) then
				controller.spiralTimer = 0
				SpecialVFX.clawSlash(pos, (pos - center).Unit, move.color, folder)
