...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			local r = 2 + math.sin(controller.cycloneTimer * 6) * 1.5
			local offset = Vector3.new(math.cos(controller.cycloneAngle) * r, 0, math.sin(controller.cycloneAngle) * r)
			controller.velocity = offset * 12 + controller.facing * 20
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "dash" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "surge" then
			controller:areaHit(allControllers, phase.range or 7, phase.damage or 35, true)
		end
	end
