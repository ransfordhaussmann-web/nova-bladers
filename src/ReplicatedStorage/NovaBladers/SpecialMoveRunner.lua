...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomMirageCut" then
		if phase.id == "blink" then
			...
			if dir.Magnitude > 0.1 then
				local dist = math.min(dir.Magnitude, phase.blinkRange or 10)
				dir = dir.Unit
				local blinkPos = controller.part.Position + dir * dist
				SpecialVFX.mirageBlink(controller.part.Position, blinkPos, color, folder)
				controller.part.CFrame = CFrame.new(Vector3.new(blinkPos.X, controller.part.Position.Y, blinkPos.Z))
					* (controller.part.CFrame - controller.part.CFrame.Position)
				controller.facing = dir
			end
		elseif phase.id == "cut" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.slashArc(controller.part.Position, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "finish" then
			SpecialVFX.slashBurst(controller.part.Position, color, folder)
		end
	...
	elseif move.id == "CrimsonDriftSurge" then
		...
		elseif phase.id == "aftershock" then
			SpecialVFX.pulseWave(controller.part.Position, phase.range or 6, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperBarrage" then
		...
		if phase.id == "barrage" then
			...
			if controller.barrageTimer >= (phase.hitInterval or 0.14) then
				controller.barrageTimer = 0
				SpecialVFX.slashArc(controller.part.Position, controller.part.Position + controller.facing * 4, move.color, folder)
	...
	elseif move.id == "SolarCoronaLoop" then
		...
		elseif phase.id == "corona" then
			...
			if controller.coronaTimer >= (phase.interval or 0.25) then
				...
				local range = 3.5 + controller.coronaCount * 1.4
				SpecialVFX.sonicRing(controller.part.Position, range, move.color, folder)
	...
	elseif move.id == "FrostCrownShatter" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.22) then
				...
				local range = 3 + controller.shardCount * 1.2
				SpecialVFX.frostShard(controller.part.Position, range, move.color, folder)
				controller:areaHit(allControllers, range, phase.damage or 7, true)
			end
		elseif phase.id == "shatter" then
