...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostPillarLock" then
		...
		elseif phase.id == "pillars" then
			...
			SpecialVFX.frostPillars(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.24) then
				controller.cycloneTimer = 0
				controller.cycloneCount = (controller.cycloneCount or 0) + 1
				local range = 3.5 + controller.cycloneCount * 1.8
				SpecialVFX.cycloneSlash(controller.part.Position, range, move.color, folder)
