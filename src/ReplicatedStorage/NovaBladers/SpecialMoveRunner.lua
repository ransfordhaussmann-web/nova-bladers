...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoRush" then
		...
		elseif phase.id == "spiral" then
			...
			controller.velocity = (controller.facing + perp * math.sin(controller.spiralAngle) * 0.55).Unit * speed
			controller.spiralTimer = (controller.spiralTimer or 0) + dt
			if controller.spiralTimer >= (phase.hitInterval or 0.22) then
				controller.spiralTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameSpiral(pos, move.color, folder)
	...
	elseif move.id == "FrostCrownBarrage" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.3) then
				...
				local range = 3.5 + controller.shardCount * 1.4
				SpecialVFX.iceShardRing(controller.part.Position, range, move.color, folder)
