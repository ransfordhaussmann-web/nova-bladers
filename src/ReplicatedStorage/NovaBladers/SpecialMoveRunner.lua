...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		...
		elseif phase.id == "anchor" then
			controller.velocity = Vector3.zero
			SpecialVFX.tideRipple(controller.part.Position, color, folder)
		elseif phase.id == "crash" then
			SpecialVFX.anchorSlam(controller.part.Position, color, folder)
		end
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "rise" then
			SpecialVFX.stoneRise(controller, color, phase.duration)
		elseif phase.id == "wall" then
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "phase" then
			SpecialVFX.setPhase(controller, true)
		elseif phase.id == "dash" then
			...
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "slash" then
			SpecialVFX.setPhase(controller, false)
			SpecialVFX.ghostSlash(controller.part.Position, controller.facing, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeFeatherStorm" then
		...
		if phase.id == "storm" then
			...
			if controller.featherTimer >= (phase.hitInterval or 0.2) then
				controller.featherTimer = 0
				local pos = controller.part.Position
				SpecialVFX.featherBurst(controller.featherLastPos, pos, move.color, folder)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "pull" then
			...
			if controller.pullTimer >= (phase.interval or 0.15) then
				controller.pullTimer = 0
				local pullR = phase.pullRadius or move.pullRadius or 10
				SpecialVFX.tideRipple(controller.part.Position, move.color, folder)
				for _, other in allControllers do
					if other ~= controller and other.alive and not other.underground then
						...
						if dist > 0.5 and dist <= pullR then
							local pull = offset.Unit * (move.pullStrength or 28) * dt * 3
