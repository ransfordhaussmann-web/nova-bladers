...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "lungeA" or phase.id == "lungeB" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.lungeSlash(controller.part.Position, dir, color, folder)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "supernova" then
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "echo" then
			SpecialVFX.mirageEcho(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			if controller.flareTimer >= 0.22 then
				controller.flareTimer = 0
				SpecialVFX.solarFlare(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, 4.5, 8, true)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "supernova" then
			controller.velocity = Vector3.zero
