...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendingArc" then
		...
		elseif phase.id == "slash" then
			controller.slashSpin = (controller.slashSpin or 0) + dt * 14
			local spinDir = Vector3.new(math.cos(controller.slashSpin), 0, math.sin(controller.slashSpin))
			controller.facing = spinDir
			controller.velocity = spinDir * (phase.rushSpeed or 55)
			...
			if controller.slashTimer >= (phase.interval or 0.22) then
				...
				SpecialVFX.slashArc(controller.part.Position, controller.facing, move.color, folder, phase.hitRadius or 5)
