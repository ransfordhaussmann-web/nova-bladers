...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFlameBarrage" then
		...
		elseif phase.id == "barrage" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (move.rushSpeed or 74)
			controller.barrageTimer = 0
			controller.barrageHitsLeft = phase.hits or 5
		elseif phase.id == "finisher" then
			SpecialVFX.flameFinisher(controller.part.Position, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameBarrage" then
		...
		elseif phase.id == "barrage" then
			...
			if controller.barrageTimer >= (phase.interval or 0.17) then
				controller.barrageTimer = 0
				local pos = controller.part.Position
		...
		elseif phase.id == "finisher" then
			controller.velocity = controller.facing * (move.rushSpeed or 74) * 0.5
