	...
	elseif move.id == "CrimsonSlashBarrage" then
		...
		elseif phase.id == "barrage" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.15) then
				controller.slashTimer = 0
				local slashPos = controller.part.Position + controller.facing * 3
				SpecialVFX.slashArc(slashPos, controller.facing, move.color, folder)
	...
	elseif move.id == "GlacialHaloLock" then
		...
		elseif phase.id == "halo" then
			...
			if controller.haloTimer >= (phase.interval or 0.3) then
				controller.haloTimer = 0
				SpecialVFX.iceRing(controller.part.Position, phase.range or 7, move.color, folder)
