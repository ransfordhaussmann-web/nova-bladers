...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "dome" then
			...
			SpecialVFX.frostDome(controller, color, move.accentColor or color, phase.duration + 0.85)
		elseif phase.id == "hold" then
			controller.velocity = Vector3.zero
			SpecialVFX.auroraShimmer(controller, color, move.accentColor or color)
		elseif phase.id == "release" then
			SpecialVFX.auroraRelease(controller.part.Position, phase.range or 9, color, move.accentColor or color, folder)
		end
	end
end
