...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if mode == "meteor" or mode == "rend" then
		...
		elseif phase.id == "shower" or phase.id == "slash" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if mode == "meteor" or mode == "rend" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "launch" or phase.id == "rush" or phase.id == "shower" or phase.id == "slash" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
		end
		if phase.id == "shower" or phase.id == "slash" then
	...
	elseif mode == "shard" then
		if phase.id == "orbit" then
			...
			if controller.shardTimer >= (phase.interval or 0.2) then
				...
				local range = 3 + controller.shardCount
				SpecialVFX.sonicRing(controller.part.Position, range, move.color, folder)
