...
local function moveMode(move)
	return move.mode or move.id
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "eruption" then
		...
		elseif phase.id == "erupt" then
			...
			if controller.eruptTimer >= (phase.interval or 0.28) then
				controller.eruptTimer = 0
				controller.eruptCount = (controller.eruptCount or 0) + 1
				local range = (phase.range or 7) + controller.eruptCount
