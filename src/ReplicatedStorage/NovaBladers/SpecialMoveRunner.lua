...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostHaloCrown" then
		...
		elseif phase.id == "crown" then
			...
			controller.icePulseTimer = 0
		elseif phase.id == "shatter" then
			SpecialVFX.frostBurst(controller.part.Position, color, folder, phase.range or 7.5)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFangRush" then
		...
		if phase.id == "fangs" then
			...
			if controller.fangTimer >= (phase.hitInterval or 0.15) then
				controller.fangTimer = 0
				local pos = controller.part.Position
				SpecialVFX.fangSlash(controller.fangLastPos, pos, move.color, folder)
	...
	elseif move.id == "FrostHaloCrown" then
		...
		if phase.id == "crown" then
			controller.icePulseTimer = (controller.icePulseTimer or 0) + dt
			if controller.icePulseTimer >= (phase.interval or 0.3) then
				controller.icePulseTimer = 0
