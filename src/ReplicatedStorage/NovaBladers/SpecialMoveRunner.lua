...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "EmberCyclone" then
		...
		elseif phase.id == "cyclone" then
			SpecialVFX.emberCyclone(controller, color, phase.duration)
	...
	elseif move.id == "EmberCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.2) then
				controller.cycloneTimer = 0
				controller:areaHit(allControllers, phase.radius or 6, phase.damage or 12, true)
			end
			...
			for _, other in allControllers do
				if other ~= controller and other.alive and not other.underground then
					...
					if dist > 0.5 and dist <= (phase.radius or 6) + 2 then
	...
	elseif move.id == "GlacierShroud" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.25) then
				controller.shardTimer = 0
				SpecialVFX.glacierShards(controller.part.Position, phase.range or 9, move.color, folder)
