...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" then
			...
			SpecialVFX.auroraDome(controller, color, phase.duration + 0.65)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.14) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.crimsonRipSlash(controller.ripLastPos, pos, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		if phase.id == "hold" then
			...
			if controller.domePulseTimer >= (phase.interval or 0.3) then
				controller.domePulseTimer = 0
