...
local function setupOrbit(controller, target, move)
...
local PHASE_START = {
	...
	sonic = function(controller, move, phase, target, folder)
		...
		elseif phase.id == "orbit" then
			setupOrbit(controller, target, move)
		end
	end,
	...
	lunge = function(controller, move, phase, target, folder)
		local color = move.color
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
	...
	bastion = function(controller, move, phase, target, folder)
		local color = move.color
		if phase.id == "brace" then
			controller.guardReduction = move.damageReduction or 0.65
			SpecialVFX.chargeAura(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "fortify" then
			SpecialVFX.wallRing(controller, color, phase.duration)
			controller.velocity = Vector3.zero
	...
	solar = function(controller, move, phase, target, folder)
		local color = move.color
	...
	mirage = function(controller, move, phase, target, folder)
		...
		if phase.id == "split" then
			for i = 1, phase.clones or 3 do
				local offset = Vector3.new(math.cos(i * 2.1) * 3, 0, math.sin(i * 2.1) * 3)
				SpecialVFX.mirageClone(controller.part.Position + offset, color, folder)
			end
...
local PHASE_UPDATE = {
	meteor = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
	...
	fortress = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "burrow" then
			controller.velocity = Vector3.zero
			local pos = controller.part.Position
	...
	sonic = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "charge" then
	...
	eclipse = function(controller, move, phase, dt, allControllers, folder, target)
	...
	lunge = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "lunge" then
			local speed = phase.rushSpeed or move.rushSpeed or 85
		...
		elseif phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.22) then
				controller.ripperTimer = 0
	...
	bastion = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "brace" or phase.id == "fortify" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
	...
	solar = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "ignite" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarFlare(controller.part.Position, 2.5, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end
	end,
	mirage = function(controller, move, phase, dt, allControllers, folder, target)
		if phase.id == "dash" then
			controller.dashTimer = (controller.dashTimer or 0) + dt
			local dashInterval = (phase.duration or 0.55) / (phase.dashes or 3)
			if controller.dashTimer >= dashInterval then
				controller.dashTimer = 0
				controller.dashCount = (controller.dashCount or 0) + 1
				if target and target.part then
					aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
				end
				SpecialVFX.mirageClone(controller.part.Position, move.color, folder)
			end
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler then
		handler(controller, move, phase, controller.specialTarget, folder)
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if handler then
		handler(controller, move, phase, dt, allControllers, folder, controller.specialTarget)
	end
