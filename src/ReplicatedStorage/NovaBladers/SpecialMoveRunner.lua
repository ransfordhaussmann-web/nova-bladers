...
local function startOrbit(controller, target, move, phase)
	...
	controller.orbitRadius = phase.orbitRadius or move.orbitRadius or 6
	controller.orbitSpeed = phase.orbitSpeed or move.orbitSpeed or 16
end
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "sonic" then
		...
		elseif phase.id == "orbit" then
			startOrbit(controller, target, move, phase)
		end
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "ripper" then
			...
			controller.ripperHitsLeft = phase.hits or 3
		end
	elseif mode == "bastion" then
		...
		elseif phase.id == "bastion" then
			controller.velocity = Vector3.zero
			SpecialVFX.bastionShield(controller, color, phase.duration, phase.damageReduction or move.damageReduction)
	...
	elseif mode == "solar" then
		if phase.id == "flare" then
			...
			SpecialVFX.solarAura(controller, color, phase.duration)
		elseif phase.id == "drift" then
			startOrbit(controller, target, move, phase)
		elseif phase.id == "burst" then
			SpecialVFX.solarFlare(controller.part.Position, phase.range or 6, color, folder)
		end
	elseif mode == "mirage" then
		if phase.id == "split" then
			SpecialVFX.phantomClones(controller, color, phase.clones or 3, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "rush" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "ripper" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70) * 0.85
	...
	elseif mode == "bastion" then
		...
		elseif phase.id == "shock" then
			...
			if controller.shockTimer >= (phase.interval or 0.35) then
				controller.shockTimer = 0
				SpecialVFX.graniteShock(controller.part.Position, phase.range or 7.5, move.color, folder)
	...
	elseif mode == "mirage" then
		if phase.id == "dash" then
			...
			local dashInterval = phase.duration / ((phase.dashes or 2) + 1)
			if controller.dashTimer >= dashInterval then
				controller.dashTimer = 0
				controller.dashCount = (controller.dashCount or 0) + 1
				aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
				SpecialVFX.phantomStrike(controller.part.Position, move.color, folder)
				controller:checkCollisions(allControllers, true)
			end
		elseif phase.id == "strike" then
