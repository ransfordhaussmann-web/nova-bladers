...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "anchor" then
		...
		elseif phase.id == "tide" then
			...
			SpecialVFX.tidalWave(controller.part.Position, phase.pullRange or 14, color, folder)
		elseif phase.id == "crash" then
			...
			controller.facing = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.velocity = dir * (move.rushSpeed or 60)
	...
	elseif mode == "phase" then
		...
		elseif phase.id == "rewind" then
			SpecialVFX.phaseBurst(controller.part.Position, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "anchor" then
		...
		elseif phase.id == "tide" then
			...
			if controller.tideTimer >= 0.12 then
				controller.tideTimer = 0
				for _, other in allControllers do
					if other ~= controller and other.part and other.part.Parent then
						local delta = controller.part.Position - other.part.Position
						local dist = delta.Magnitude
						if dist > 0.5 and dist < (phase.pullRange or 14) then
							local pull = delta.Unit * (phase.pullStrength or 22) * dt
		...
		elseif phase.id == "crash" then
			...
			if controller.part.Position.Y <= controller.floorY + 0.5 then
				...
				controller:areaHit(allControllers, phase.range or 9, phase.damage or 38, true)
			end
		end
	...
	elseif mode == "phase" then
		...
		elseif phase.id == "slash" then
			...
			if controller.phaseStartPos then
				SpecialVFX.phaseSlash(controller.phaseStartPos, controller.part.Position, move.color, folder)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "rewind" then
