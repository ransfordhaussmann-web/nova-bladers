...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "EmberCyclone" then
		...
		elseif phase.id == "burst" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or 80)
		end
	elseif move.id == "GlacierShroud" then
		...
		elseif phase.id == "shroud" then
			SpecialVFX.glacierMist(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
	...
	elseif move.id == "GlacierShroud" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.3) then
				controller.shardTimer = 0
				SpecialVFX.iceShard(controller.part.Position, phase.range or 7, move.color, folder)
