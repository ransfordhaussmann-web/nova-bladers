...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "slam" then
			...
			controller.facing = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.velocity = controller.facing * (phase.slamSpeed or 70)
			controller.verticalVelocity = -(phase.slamSpeed or 70) * 0.85
			controller.airborne = true
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "slam" then
			controller.velocity = controller.facing * (phase.slamSpeed or move.rushSpeed or 70)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "shock" then
			controller.velocity = Vector3.zero
