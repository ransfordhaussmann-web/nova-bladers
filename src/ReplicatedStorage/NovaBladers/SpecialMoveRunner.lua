...
local function updateOrbit(controller, allControllers)
	if not controller.orbitCenter then
		return
	end
	controller.orbitAngle += (controller.orbitSpeed or 16) * (controller._lastDt or 0.016)
...
local PHASE_START = {
	meteor = function(controller, move, phase, folder, target)
	...
	fortress = function(controller, move, phase, folder, target)
	...
	sonic = function(controller, move, phase, folder, target)
	...
	eclipse = function(controller, move, phase, folder, target)
	...
	lunge = function(controller, move, phase, folder, target)
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, move.color, phase.duration)
		elseif phase.id == "lunge" then
			aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
			controller.lungeHitsLeft = phase.hits or 3
			controller.lungeTimer = 0
		elseif phase.id == "rip" then
			SpecialVFX.ripperSlash(controller.part.Position, move.color, folder)
		end
	end,
	bastion = function(controller, move, phase, folder, target)
		...
		elseif phase.id == "counter" then
			...
			aimAtTarget(controller, target, phase.rushSpeed or 55)
		end
	end,
	solar = function(controller, move, phase, folder, target)
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, move.color, phase.duration)
	...
	mirage = function(controller, move, phase, folder, target)
		if phase.id == "split" then
			SpecialVFX.phantomSplit(controller, move.color, folder)
		elseif phase.id == "feint" then
			aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
			SpecialVFX.diveTrail(controller, getTargetPos(controller, target), move.color, folder)
...
local PHASE_UPDATE = {
	...
	sonic = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "orbit" then
			controller._lastDt = dt
			updateOrbit(controller, allControllers)
		end
	end,
	...
	lunge = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "lunge" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller.lungeTimer = (controller.lungeTimer or 0) + dt
			if controller.lungeTimer >= (phase.hitInterval or 0.12) then
				controller.lungeTimer = 0
				SpecialVFX.ripperSlash(controller.part.Position, move.color, folder)
	...
	bastion = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "brace" then
			controller.velocity = Vector3.zero
		elseif phase.id == "bastion" then
			controller.velocity = Vector3.zero
	...
	mirage = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "feint" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 80)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "slash" then
			controller:areaHit(allControllers, phase.range or 7, phase.damage or 38, true)
		end
	end,
}
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler then
		handler(controller, move, phase, folder, controller.specialTarget)
	end
end
