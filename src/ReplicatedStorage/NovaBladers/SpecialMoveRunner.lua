...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrystalVeil" then
		if phase.id == "veil" then
			...
			SpecialVFX.crystalVeil(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeInfernoWheel" then
		...
		elseif phase.id == "inferno" then
			...
			if controller.infernoTimer >= (phase.interval or 0.2) then
				controller.infernoTimer = 0
				controller.infernoCount = (controller.infernoCount or 0) + 1
				local range = (phase.range or 7) + controller.infernoCount * 0.5
	...
	elseif move.id == "FrostCrystalVeil" then
		...
		elseif phase.id == "spikes" then
			...
			if controller.spikeTimer >= (phase.interval or 0.25) then
				controller.spikeTimer = 0
				SpecialVFX.iceSpike(controller.part.Position, controller.facing, phase.range or 5.5, move.color, folder)
