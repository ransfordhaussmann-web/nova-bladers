...
local PHASE_START = {
	...
	rend = function(controller, move, phase, target)
		...
		elseif phase.id == "slash1" or phase.id == "slash2" or phase.id == "slash3" then
			aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
			SpecialVFX.rendSlash(controller, move.color, SpecialVFX.ensureFolder(controller))
		end
	end,
	...
	phase = function(controller, move, phase, target)
		...
		elseif phase.id == "blink" then
			local targetPos = getTargetPos(controller, target)
			local blinkPos = targetPos - controller.facing * 3
...
local PHASE_UPDATE = {
	...
	anchor = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "slam" then
			if not controller.slamHit and controller.part.Position.Y <= controller.floorY + 0.5 and controller.verticalVelocity < 0 then
				controller.slamHit = true
				SpecialVFX.anchorSlam(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.range or 7, phase.slamDamage or 28, true)
