	...
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.15) then
				controller.ripperTimer = 0
				controller.ripperHits = (controller.ripperHits or 0) + 1
...
MODE_HANDLERS.bastion = {
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "slam" then
			...
			if controller.slamTimer >= (phase.interval or 0.4) then
				controller.slamTimer = 0
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "flare" then
			...
			if controller.flareTimer >= (phase.interval or 0.2) then
				...
				local range = 3 + controller.flareCount * 1.8
...
MODE_HANDLERS.mirage = {
	onStart = function(controller, move, phase, folder, target)
		if phase.id == "split" then
			for _ = 1, (phase.clones or 3) do
				SpecialVFX.mirageClone(controller, move.color, folder)
			end
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "strike" then
			controller.velocity = Vector3.zero
			controller.strikeTimer = (controller.strikeTimer or 0) + dt
			if controller.strikeTimer >= (phase.interval or 0.2) then
				controller.strikeTimer = 0
