...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			local accent = controller.beyData and controller.beyData.accentColor or color
			SpecialVFX.auroraDome(controller, color, accent, phase.duration)
		elseif phase.id == "burst" then
			local accent = controller.beyData and controller.beyData.accentColor or color
			SpecialVFX.auroraBurst(controller.part.Position, phase.range or 9, color, accent, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			if controller.lungeStartPos then
				SpecialVFX.crimsonLungeTrail(controller.lungeStartPos, controller.part.Position, move.color, folder)
				controller.lungeStartPos = controller.part.Position
			end
		elseif phase.id == "slash" then
