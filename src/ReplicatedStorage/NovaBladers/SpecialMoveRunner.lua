...
local function onPhaseStartMeteor(controller, move, phase, target)
...
local function onPhaseStartFortress(controller, move, phase)
	local color = move.color
...
local function onPhaseStartSonic(controller, move, phase, target)
	local color = move.color
...
local function onPhaseStartEclipse(controller, move, phase, target, folder)
	local color = move.color
...
local function onPhaseStartRiptide(controller, move, phase)
	local color = move.color
	if phase.id == "surge" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
	elseif phase.id == "whirl" then
		controller.whirlTimer = 0
		controller.whirlAngle = 0
		controller.whirlRadius = 4
	elseif phase.id == "rip" then
		local dir = controller.facing
		if controller.specialTarget and controller.specialTarget.part then
			dir = (controller.specialTarget.part.Position - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
		end
		controller.facing = dir
		controller.velocity = dir * (phase.rushSpeed or move.rushSpeed or 85)
		SpecialVFX.crimsonRipWave(controller.part.Position, dir, color, SpecialVFX.ensureFolder(controller))
	end
end

local function onPhaseStartFrost(controller, move, phase)
	local color = move.color
	if phase.id == "frostfield" then
		controller.guardReduction = move.damageReduction or 0.6
		SpecialVFX.frostField(controller, color, phase.duration, phase.range or 7)
...
local function onPhaseStartResonance(controller, move, phase)
	local color = move.color
	if phase.id == "tune" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
	elseif phase.id == "pulse" then
		controller.pulseTimer = 0
		controller.pulseCount = 0
	elseif phase.id == "resonance" then
		controller.resonanceDone = false
	end
end

local PHASE_START = {
	meteor = onPhaseStartMeteor,
	fortress = onPhaseStartFortress,
	sonic = onPhaseStartSonic,
	eclipse = onPhaseStartEclipse,
	riptide = onPhaseStartRiptide,
	frost = onPhaseStartFrost,
	resonance = onPhaseStartResonance,
}
...
function SpecialMoveRunner.endMove(controller)
	...
	controller.underground = false
	controller.slowMult = nil
	SpecialVFX.setUnderground(controller, false)
	SpecialVFX.cleanup(controller)
end

local function updateMeteor(controller, move, phase, dt, allControllers, folder)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
...
local function updateFortress(controller, move, phase, dt, allControllers, folder)
	if phase.id == "burrow" then
		controller.velocity = Vector3.zero
		local pos = controller.part.Position
...
local function updateSonic(controller, move, phase, dt, allControllers, folder)
	if phase.id == "charge" then
...
local function updateEclipse(controller, move, phase, allControllers)
...
local function updateRiptide(controller, move, phase, dt, allControllers, folder)
	if phase.id == "surge" then
		controller.velocity *= 0.85
	elseif phase.id == "whirl" then
		controller.whirlTimer = (controller.whirlTimer or 0) + dt
		controller.whirlAngle = (controller.whirlAngle or 0) + 9 * dt
		local r = controller.whirlRadius or 4
		local center = controller.part.Position
		local pos = center + Vector3.new(math.cos(controller.whirlAngle) * r, 0, math.sin(controller.whirlAngle) * r)
		controller.part.CFrame = CFrame.new(pos, center)
		controller.velocity = Vector3.zero
		if controller.whirlTimer >= (phase.interval or 0.22) then
			controller.whirlTimer = 0
			SpecialVFX.crimsonRipWave(pos, controller.facing, move.color, folder)
...
local function updateFrost(controller, move, phase, dt, allControllers, folder)
	if phase.id == "frostfield" then
		controller.velocity *= 0.7
		controller:areaHit(allControllers, phase.range or 7, 4, true)
	elseif phase.id == "crown" then
		controller.velocity = Vector3.zero
	elseif phase.id == "shatter" then
...
local function findNearestEnemy(controller, allControllers, range)
...
local function updateResonance(controller, move, phase, dt, allControllers, folder)
	if phase.id == "tune" then
		controller.velocity *= 0.9
	elseif phase.id == "pulse" then
		controller.pulseTimer = (controller.pulseTimer or 0) + dt
		if controller.pulseTimer >= (phase.interval or 0.28) then
			controller.pulseTimer = 0
			controller.pulseCount = (controller.pulseCount or 0) + 1
			local range = (phase.range or 5) + controller.pulseCount * 1.2
			SpecialVFX.neonPulseRing(controller.part.Position, range, move.color, folder)
			controller:areaHit(allControllers, range, phase.damage or 9, true)
		end
	elseif phase.id == "resonance" and not controller.resonanceDone then
		controller.resonanceDone = true
		local chainRange = phase.chainRange or 14
		local target = controller.specialTarget
		if not target or not target.alive then
			target = findNearestEnemy(controller, allControllers, chainRange)
		end
		if target and target.part then
			SpecialVFX.resonanceBeam(controller.part.Position, target.part.Position, move.color, folder)
			target:takeHit(controller, phase.damage or 22, move.spinLoss or 12, true)
		end
		controller:areaHit(allControllers, 5, 12, true)
	end
end
...
local MODE_UPDATE = {
	...
	sonic = updateSonic,
	eclipse = updateEclipse,
	riptide = updateRiptide,
	frost = updateFrost,
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local folder = SpecialVFX.ensureFolder(controller)
	local updater = MODE_UPDATE[move.mode]
	if updater then
		updater(controller, move, phase, dt, allControllers, folder)
	end
