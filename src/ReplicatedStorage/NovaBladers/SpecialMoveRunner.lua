...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFangRush" then
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.flameSlash(controller, color, folder)
	...
	elseif move.id == "FrostCrownShatter" then
		if phase.id == "crystal" then
			SpecialVFX.iceCrystal(controller, color, phase.duration)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "CrimsonFangRush" then
		...
		elseif phase.id == "flare" then
			...
			if controller.meteorTimer >= (phase.hitInterval or 0.2) then
				controller.meteorTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameFlare(pos, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 12, true)
			end
		end

	elseif move.id == "FrostCrownShatter" then
		if phase.id == "crystal" or phase.id == "barrier" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
