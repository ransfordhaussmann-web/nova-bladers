...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "fang" then
		if phase.id == "stance" then
			...
			SpecialVFX.fangStance(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		...
		elseif phase.id == "riposte" then
			SpecialVFX.riposteFlash(controller.part.Position, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "inferno" then
		...
		elseif phase.id == "flare" then
			...
			if controller.infernoTimer >= (phase.trailInterval or 0.12) then
				controller.infernoTimer = 0
				SpecialVFX.infernoFlare(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, 3.5, 8, true)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "crown" then
