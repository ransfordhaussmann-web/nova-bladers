...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		...
		elseif phase.id == "bastion" then
			...
			SpecialVFX.graniteBastion(controller, color, phase.duration)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirage" then
			SpecialVFX.phantomMirage(controller, color, phase.duration, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.17) then
				controller.ripperTimer = 0
	...
	elseif move.id == "GraniteBastion" then
		...
		elseif phase.id == "quake" then
			...
			if controller.quakeTimer >= (phase.interval or 0.38) then
				controller.quakeTimer = 0
				SpecialVFX.graniteQuake(controller.part.Position, phase.range or 7.5, move.color, folder)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarBurnTrail(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end
