...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "solar" then
		if phase.id == "flare" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "rays" then
			controller.solarTimer = 0
			controller.solarCount = 0
		elseif phase.id == "flareBurst" then
			SpecialVFX.solarFlare(controller.part.Position, phase.range or 10, color, folder)
		end
	elseif mode == "mirage" then
		if phase.id == "fade" then
			SpecialVFX.setMirageFade(controller, true)
		...
		elseif phase.id == "slash" then
			SpecialVFX.setMirageFade(controller, false)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.16) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.crimsonRip(pos, move.color, folder)
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "flareBurst" then
			controller:areaHit(allControllers, phase.range or 10, phase.damage or 30, true)
		end
