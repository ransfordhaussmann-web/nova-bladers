...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonVortexRush" then
		...
		elseif phase.id == "rush" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.vortexTrail(controller.part.Position, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonVortexRush" then
		...
		elseif phase.id == "vortex" then
			...
			local pullRange = phase.pullRange or 9
			local pullForce = phase.pullForce or 55
			local myPos = controller.part.Position
			for _, other in allControllers do
				if other ~= controller and other.alive and other.part then
					...
					if dist > 0.5 and dist <= pullRange then
						other.velocity = other.velocity + flat.Unit * pullForce * dt
					end
				end
			end
			if controller.vortexTimer >= (phase.interval or 0.2) then
				controller.vortexTimer = 0
				SpecialVFX.vortexRing(myPos, pullRange * 0.6, move.color, folder)
				controller:areaHit(allControllers, pullRange * 0.55, phase.damage or 8, true)
			end
		elseif phase.id == "rush" then
