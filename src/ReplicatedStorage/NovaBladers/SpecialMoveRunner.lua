...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "leap" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.verticalVelocity = 32
			controller.airborne = true
		elseif phase.id == "slam" then
			controller.verticalVelocity = -(phase.diveSpeed or 55)
			controller.velocity = controller.facing * (move.rushSpeed * 0.4)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "slam" then
			controller.velocity = controller.facing * (move.rushSpeed * 0.35)
			controller:checkCollisions(allControllers, true)
			if not controller.airborne then
				SpecialVFX.hammerSlamImpact(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, 5, move.damage or 38, true)
			end
		elseif phase.id == "shock" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
