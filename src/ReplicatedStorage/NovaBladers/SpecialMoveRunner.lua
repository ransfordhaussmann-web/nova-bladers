...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "prism" then
		...
		elseif phase.id == "refract" then
			controller.prismTimer = 0
		elseif phase.id == "finale" then
			SpecialVFX.prismBurst(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "prism" then
		...
		elseif phase.id == "refract" then
			controller.velocity = Vector3.zero
			controller.prismTimer = (controller.prismTimer or 0) + dt
			if controller.prismTimer >= (phase.interval or 0.3) then
				controller.prismTimer = 0
