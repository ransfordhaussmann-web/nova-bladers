...
local PHASE_START = {
	...
	tide = function(controller, move, phase)
		local color = move.color
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "pull" then
			controller.tidePullRadius = phase.pullRadius or 14
			controller.tidePullStrength = phase.pullStrength or 22
			SpecialVFX.tidePull(controller.part.Position, controller.tidePullRadius, color, folder)
		elseif phase.id == "sink" then
			SpecialVFX.setUnderground(controller, true)
			controller.velocity = Vector3.zero
		elseif phase.id == "crash" then
			SpecialVFX.setUnderground(controller, false)
			local pos = controller.part.Position
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, controller.floorY + 0.5, pos.Z))
				* (controller.part.CFrame - controller.part.CFrame.Position)
			SpecialVFX.anchorCrash(pos, color, folder)
	...
	phase = function(controller, move, phase, target)
		...
		elseif phase.id == "blink" then
			local targetPos = getTargetPos(controller, target)
			local behind = targetPos - (target and target.facing or controller.facing) * 3
			behind = Vector3.new(behind.X, controller.part.Position.Y, behind.Z)
			SpecialVFX.phaseBlink(controller, behind, color, folder)
			controller.part.CFrame = CFrame.new(behind, targetPos)
			local dir = (targetPos - behind)
			controller.facing = Vector3.new(dir.X, 0, dir.Z).Unit
			SpecialVFX.setUnderground(controller, false)
...
local PHASE_UPDATE = {
	...
	feather = function(controller, move, phase, dt, allControllers)
		...
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "spiral" or phase.id == "storm" then
	...
	tide = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "pull" then
			local radius = controller.tidePullRadius or 14
			local strength = controller.tidePullStrength or 22
			for _, other in allControllers do
				if other ~= controller and other.alive and other.part then
					...
					if dist > 0.5 and dist < radius then
						local pull = flat.Unit * strength * dt * (1 - dist / radius)
		...
		elseif phase.id == "sink" then
			controller.velocity = Vector3.zero
			local pos = controller.part.Position
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, controller.floorY - 0.8, pos.Z))
				* (controller.part.CFrame - controller.part.CFrame.Position)
		elseif phase.id == "crash" then
			controller.velocity = Vector3.zero
		elseif phase.id == "wave" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.pulseWave(controller.part.Position, phase.range or 10, move.color, folder)
				controller:areaHit(allControllers, phase.range or 10, phase.damage or 12, true)
			end
		end
	end,
	rend = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "rend" then
			controller.meteorTimer = (controller.meteorTimer or 0) + dt
			if controller.meteorTimer >= (phase.hitInterval or 0.14) then
				controller.meteorTimer = 0
				SpecialVFX.rendSlash(controller.part.Position, move.color, folder)
	...
	solar = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "flare" then
			controller.sonicTimer = (controller.sonicTimer or 0) + dt
			if controller.sonicTimer >= (phase.interval or 0.26) then
				controller.sonicTimer = 0
				controller.sonicCount = (controller.sonicCount or 0) + 1
