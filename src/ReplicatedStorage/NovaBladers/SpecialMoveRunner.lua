...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.mode == "lunge" then
		...
		elseif phase.id == "finish" then
			SpecialVFX.ripSlash(controller.part.Position, controller.facing, color, folder)
		end
	elseif move.mode == "bastion" then
		...
		elseif phase.id == "bastion" then
			...
			SpecialVFX.graniteBastionWall(controller, color, phase.duration)
	...
	elseif move.mode == "mirage" then
		...
		elseif phase.id == "feint" then
			...
			controller.feintDir = side.Unit
			controller.velocity = controller.feintDir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.mirageFeint(controller, color, folder)
		elseif phase.id == "strike" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.phantomStrike(controller, targetPos, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.mode == "lunge" then
		...
		if phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.13) then
				controller.ripTimer = 0
				SpecialVFX.ripSlash(controller.part.Position, controller.facing, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 12, true)
			end
		elseif phase.id == "finish" then
			controller:areaHit(allControllers, phase.range or 5.5, phase.damage or 24, true)
		end
	...
	elseif move.mode == "solar" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarOrbitRing(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burst" then
	...
	elseif move.mode == "mirage" then
		if phase.id == "feint" then
			controller.velocity = (controller.feintDir or controller.facing) * (phase.rushSpeed or move.rushSpeed or 75)
		elseif phase.id == "strike" then
