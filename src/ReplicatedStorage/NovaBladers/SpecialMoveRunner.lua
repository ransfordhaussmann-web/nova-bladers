...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		...
		elseif phase.id == "bastion" then
			...
			SpecialVFX.stoneBarrier(controller, color, phase.duration)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "mirage" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.phantomClones(controller, targetPos, color, phase.clones or 3, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "lunge" or phase.id == "slash" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		end
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "burn" then
			...
			if controller.burnTimer >= (phase.interval or 0.25) then
				controller.burnTimer = 0
				controller.burnCount = (controller.burnCount or 0) + 1
				local range = 3.5 + controller.burnCount * 1.4
