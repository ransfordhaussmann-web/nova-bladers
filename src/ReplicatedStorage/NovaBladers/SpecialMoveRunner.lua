...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeCorkscrew" then
		if phase.id == "ignite" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "corkscrew" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
			)
			controller.orbitRadius = phase.orbitRadius or move.orbitRadius or 5
			controller.orbitSpeed = phase.orbitSpeed or move.orbitSpeed or 24
	...
	elseif move.id == "GlacierLock" then
		...
		elseif phase.id == "freeze" then
			SpecialVFX.glacierFrostField(controller, color, phase.duration, phase.range or 9)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeCorkscrew" then
		...
		elseif phase.id == "corkscrew" and controller.orbitCenter then
			...
			if controller.corkscrewTimer >= (phase.hitInterval or 0.2) then
				controller.corkscrewTimer = 0
				SpecialVFX.blazeCorkscrewTrail(controller.part.Position, move.color, folder)
	...
	elseif move.id == "GlacierLock" then
		...
		elseif phase.id == "lock" then
			...
			if controller.pulseTimer >= (phase.interval or 0.35) then
				controller.pulseTimer = 0
				SpecialVFX.glacierIcePulse(controller.part.Position, phase.range or 7, move.color, folder)
