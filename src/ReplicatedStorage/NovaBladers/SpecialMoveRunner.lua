...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "StarfallBarrage" then
		if phase.id == "windup" then
			SpecialVFX.starCharge(controller, color, phase.duration)
		elseif phase.id == "launch" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
		...
		elseif phase.id == "barrage" then
			controller.starHitsLeft = phase.hits or 5
			controller.starTimer = 0
			controller.starLastPos = controller.part.Position
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "StarfallBarrage" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "launch" or phase.id == "barrage" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 82)
		end
		if phase.id == "barrage" then
			controller.starTimer = (controller.starTimer or 0) + dt
			if controller.starTimer >= (phase.hitInterval or 0.16) then
				controller.starTimer = 0
				local pos = controller.part.Position
				SpecialVFX.starTrail(controller.starLastPos, pos, move.color, folder)
				SpecialVFX.starImpact(pos, move.color, folder)
				controller.starLastPos = pos
