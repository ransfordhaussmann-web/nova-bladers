...
local phaseUpdateHandlers = {}

phaseStartHandlers.meteor = function(controller, move, phase)
...
phaseStartHandlers.sonic = function(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
...
phaseStartHandlers.eclipse = function(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
	...
	elseif phase.id == "dive" then
		...
		SpecialVFX.diveTrail(controller, targetPos, color, SpecialVFX.ensureFolder(controller))
	...
	elseif phase.id == "burst" then
		SpecialVFX.venomBurst(controller.part.Position, color, SpecialVFX.ensureFolder(controller))
	end
end

phaseStartHandlers.lunge = function(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
	...
	elseif phase.id == "rip" then
		...
		SpecialVFX.lungeSlash(controller.part.Position, controller.facing, color, folder)
	end
end
...
phaseStartHandlers.bastion = function(controller, move, phase)
	...
	elseif phase.id == "bastion" then
		...
		SpecialVFX.bastionDome(controller, color, phase.duration)
	elseif phase.id == "quake" then
		controller.pulseTimer = 0
	end
end

phaseStartHandlers.solar = function(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
	if phase.id == "charge" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
...
phaseStartHandlers.mirage = function(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "split" then
		SpecialVFX.mirageSplit(controller, color, folder)
	...
	elseif phase.id == "fade" then
		...
		SpecialVFX.mirageFade(controller.part.Position, color, folder)
	end
end

phaseUpdateHandlers.meteor = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
...
phaseUpdateHandlers.fortress = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "burrow" then
		controller.velocity = Vector3.zero
		local pos = controller.part.Position
...
phaseUpdateHandlers.sonic = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "charge" then
...
phaseUpdateHandlers.eclipse = function(controller, move, phase, dt, allControllers)
...
phaseUpdateHandlers.lunge = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	elseif phase.id == "lunge" then
...
phaseUpdateHandlers.bastion = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
...
phaseUpdateHandlers.solar = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "charge" then
...
phaseUpdateHandlers.mirage = function(controller, move, phase, dt, allControllers)
	if phase.id == "split" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local handler = phaseUpdateHandlers[move.mode]
	if handler then
