...
local PHASE_START = {
	...
	solar = function(controller, move, phase, folder, target)
		...
		elseif phase.id == "orbit" then
			setupOrbit(controller, target, move)
			controller.solarOrbitTimer = 0
		end
	end,
	mirage = function(controller, move, phase, folder, target)
		if phase.id == "split" then
			local count = phase.clones or 3
			for i = 1, count do
				local angle = (i / count) * math.pi * 2
				local offset = Vector3.new(math.cos(angle) * 4, 0, math.sin(angle) * 4)
				SpecialVFX.mirageClone(controller, move.color, offset, folder)
			end
		elseif phase.id == "feint" then
			controller.velocity = Vector3.zero
			local side = controller.facing:Cross(Vector3.yAxis)
			controller.part.CFrame = controller.part.CFrame + side * (math.random() > 0.5 and 2.5 or -2.5)
...
local PHASE_UPDATE = {
	...
	solar = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "orbit" then
			updateOrbit(controller, dt, allControllers)
			controller.solarOrbitTimer = (controller.solarOrbitTimer or 0) + dt
			if controller.solarOrbitTimer >= 0.35 then
				controller.solarOrbitTimer = 0
	...
	mirage = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "strike" then
			...
			controller:checkCollisions(allControllers, true)
			if phase.range then
				controller:areaHit(allControllers, phase.range, phase.damage or 38, true)
			end
		end
	end,
}
