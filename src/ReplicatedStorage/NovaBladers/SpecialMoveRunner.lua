...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "strike" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.phantomStrike(controller.part.Position, dir, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.22) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.crimsonRip(controller.ripLastPos, pos, move.color, folder)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "flare" then
			...
			if controller.solarTimer >= (phase.interval or 0.32) then
				controller.solarTimer = 0
