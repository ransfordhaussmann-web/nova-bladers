...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "fang" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.fangBurstPending = true
			SpecialVFX.spiralArc(controller.part.Position, controller.spiralAngle or 0, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "spiral" and controller.spiralCenter then
			...
			controller.spiralRadius = math.max(2, (controller.spiralRadius or 8) - (phase.spiralShrink or 0.35) * dt * 10)
			...
			if controller.spiralTimer >= (phase.interval or 0.22) then
				controller.spiralTimer = 0
				SpecialVFX.spiralArc(controller.part.Position, controller.spiralAngle, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 10, true)
			end
		elseif phase.id == "fang" then
			if controller.fangBurstPending then
				controller.fangBurstPending = false
