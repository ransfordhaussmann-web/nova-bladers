...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrownBarrage" then
		...
		elseif phase.id == "freeze" then
			...
			SpecialVFX.freezeBurst(controller.part.Position, phase.range or 7.5, color, folder)
		end
	end
end
	...
	elseif move.id == "FrostCrownBarrage" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.range or 7) + controller.shardCount * 0.4
				local angle = controller.shardCount * 0.9
				local shardDir = Vector3.new(math.cos(angle), 0, math.sin(angle))
				local shardEnd = controller.part.Position + shardDir * range
				SpecialVFX.iceShard(controller.part.Position, shardEnd, move.color, folder)
				controller:areaHit(allControllers, range, phase.damage or 9, true)
			end
		elseif phase.id == "freeze" then
			controller.velocity = Vector3.zero
