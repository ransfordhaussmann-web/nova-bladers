...
local PHASE_START = {
	...
	eclipse = {
		...
		dive = function(controller, move, phase, target, folder)
		...
		burst = function(controller, move, _, _, folder)
	...
	barrage = {
		...
		flurry = function(controller)
			controller.flurryTimer = 0
		end,
		ripper = function(controller, move, phase, target)
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
	...
	mirage = {
		vanish = function(controller, move, phase)
			SpecialVFX.mirageFade(controller, move.color, phase.duration)
		end,
		slash = function(controller, move, phase, target, folder)
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.diveTrail(controller, targetPos, move.color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
		...
		afterimage = function(controller, move, _, _, folder)
			SpecialVFX.meteorImpact(controller.part.Position, move.color, folder)
		end,
	},
	shatter = {
		frost = function(controller, move, phase)
			SpecialVFX.frostAura(controller, move.color, phase.duration)
			controller.guardReduction = 0.35
		end,
		crack = function(controller)
			controller.pulseTimer = 0
		end,
		burst = function(controller, move, _, _, folder)
			SpecialVFX.frostBurst(controller.part.Position, move.color, folder)
		end,
	},
	surge = {
		boost = function(controller, move, phase)
			SpecialVFX.chargeAura(controller, move.color, phase.duration)
		end,
		...
		echo = function(controller, move, _, _, folder)
			SpecialVFX.meteorImpact(controller.part.Position, move.color, folder)
		end,
	},
}
...
local PHASE_UPDATE = {
	...
	eclipse = function(controller, move, phase, _, allControllers)
	...
	barrage = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "charge" then
			controller.velocity = Vector3.zero
	...
	corona = function(controller, move, phase, dt, allControllers, folder)
	...
	mirage = function(controller, move, phase, _, allControllers)
	...
	surge = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "rush" then
			...
			if controller.surgeTimer >= (phase.hitInterval or 0.15) then
				controller.surgeTimer = 0
				local pos = controller.part.Position
				SpecialVFX.meteorTrail(controller.surgeLastPos or pos, pos, move.color, folder)
				controller.surgeLastPos = pos
				controller:areaHit(allControllers, phase.hitRadius or 4, phase.damage or 10, true)
			end
			controller:checkCollisions(allControllers, true)
