...
MODE_HANDLERS.meteor = {
	onPhaseStart = function(controller, move, phase, target, folder)
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "windup" then
	...
	onPhaseStart = function(controller, move, phase, target, folder)
		if phase.id == "mirage" then
			...
			for i = 0, 2 do
				...
				local offset = Vector3.new(math.cos(math.rad(angle)) * 5, 0, math.sin(math.rad(angle)) * 5)
				SpecialVFX.mirageClone(center + offset, move.color, folder)
			end
		...
		elseif phase.id == "finisher" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.phantomStrike(controller.part.Position, targetPos, move.color, folder)
		end
	end,
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		if phase.id == "strike" then
			...
			if controller.mirageTimer >= (phase.hitInterval or 0.18) then
				controller.mirageTimer = 0
				local targetPos = getTargetPos(controller, target)
				SpecialVFX.phantomStrike(controller.part.Position, targetPos, move.color, folder)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler and handler.onPhaseStart then
		handler.onPhaseStart(controller, move, phase, controller.specialTarget, folder)
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if handler and handler.update then
		handler.update(controller, move, phase, dt, allControllers, folder)
	end
