...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			SpecialVFX.cycloneRing(controller, color, phase.duration)
	...
	elseif move.id == "GraniteBastionPulse" then
		if phase.id == "fortify" then
			...
			SpecialVFX.stoneArmor(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "pulse" then
			controller.pulseTimer = 0
		elseif phase.id == "slam" then
			SpecialVFX.stoneSlam(controller.part.Position, phase.range or 10, color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			controller.phantomDashIdx = 0
			controller.phantomDashTimer = 0
			controller.phantomTargetPos = getTargetPos(controller, target)
		elseif phase.id == "fade" then
			SpecialVFX.setPhantom(controller, false)
			SpecialVFX.phantomFade(controller.part.Position, phase.range or 6, color, folder)
			controller.fadeHit = false
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				controller.cycloneTimer = 0
				SpecialVFX.cycloneSlice(controller.part.Position, phase.hitRadius or 7, move.color, folder)
				...
				for _, other in allControllers do
					if other ~= controller and other.alive and not other.underground then
						...
						if dist > 0.5 and dist <= (phase.hitRadius or 7) * 1.4 then
							other.velocity += flat.Unit * pull * dt * 3
						end
					end
				end
			end
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "slam" then
			...
			if not controller.slamHit then
				...
				controller:areaHit(allControllers, phase.range or 10, phase.damage or 32, true)
			end
		end

	elseif move.id == "SolarFlareDrift" then
		if phase.id == "drift" then
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				controller.solarTimer = 0
				SpecialVFX.burnTrail(controller.part.Position, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			local dashCount = phase.dashes or 3
			local dashInterval = phase.duration / dashCount
			controller.phantomDashTimer = (controller.phantomDashTimer or 0) + dt
			if controller.phantomDashTimer >= dashInterval then
				controller.phantomDashTimer = 0
				controller.phantomDashIdx = (controller.phantomDashIdx or 0) + 1
				if controller.phantomDashIdx <= dashCount then
					local targetPos = controller.phantomTargetPos
					if target and target.part then
					...
					dir = Vector3.new(dir.X, 0, dir.Z).Unit
					controller.facing = dir
					controller.velocity = dir * (phase.dashSpeed or 88)
					SpecialVFX.afterimage(fromPos, move.color, folder)
					controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 14, true)
				end
			end
		elseif phase.id == "fade" then
			if not controller.fadeHit then
				controller.fadeHit = true
