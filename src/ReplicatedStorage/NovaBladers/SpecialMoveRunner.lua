	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "pulse" or phase.id == "quake" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.groundQuake(controller.part.Position, phase.range or 8, move.color, folder)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "flare" then
			...
			if controller.flareTimer >= (phase.interval or 0.25) then
				controller.flareTimer = 0
				SpecialVFX.solarFlare(controller.part.Position, phase.range or 6, move.color, folder)
				controller:areaHit(allControllers, phase.range or 6, phase.damage or 9, true)
			end
		elseif phase.id == "bloom" then
			controller.velocity = Vector3.zero
			SpecialVFX.solarBloom(controller.part.Position, phase.range or 9, move.color, folder)
