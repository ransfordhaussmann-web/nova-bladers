...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrystalShardBarrage" then
		...
		elseif phase.id == "shards" then
			...
			controller.shardMax = phase.shardCount or 4
	...
	elseif move.id == "BlazeRipper" then
		...
		elseif phase.id == "rip" then
			...
			if controller.blazeTimer >= (phase.hitInterval or 0.14) then
				controller.blazeTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameSlash(controller.blazeLastPos, pos, move.color, folder)
	...
	elseif move.id == "CrystalShardBarrage" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.22) then
				...
				local angle = controller.shardCount * (math.pi * 2 / (phase.shardCount or 4))
				local offset = Vector3.new(math.cos(angle), 0.4, math.sin(angle)) * (phase.range or 5)
