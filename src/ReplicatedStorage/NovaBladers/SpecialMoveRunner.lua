...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "charge" then
			SpecialVFX.auroraShimmer(controller, move.color, move.accentColor or color, phase.duration)
		elseif phase.id == "dome" then
			controller.guardReduction = move.damageReduction or 0.6
			SpecialVFX.auroraDome(controller, move.color, move.accentColor or color, phase.duration)
			controller.pulseTimer = 0
		elseif phase.id == "nova" then
			SpecialVFX.auroraNova(controller.part.Position, move.color, move.accentColor or color, folder)
		end
	end
end
