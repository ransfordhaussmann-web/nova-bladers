...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastionPulse" then
		if phase.id == "anchor" then
			controller.velocity = Vector3.zero
			SpecialVFX.stoneAnchor(controller, color)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.orbitSpeed = phase.orbitSpeed or 15
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "edge" then
			SpecialVFX.edgeFinisher(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			controller.cycloneAngle = (controller.cycloneAngle or 0) + (phase.spinRate or 12) * dt
			local spinOffset = CFrame.Angles(0, controller.cycloneAngle, 0)
			controller.part.CFrame = CFrame.new(controller.part.Position) * spinOffset
				* (controller.part.CFrame - controller.part.CFrame.Position)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" and controller.orbitCenter then
			...
			local pos = center + Vector3.new(math.cos(controller.orbitAngle) * r, 0, math.sin(controller.orbitAngle) * r)
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = (pos - center).Unit * (move.driftSpeed or 55)
			controller.flareTimer = (controller.flareTimer or 0) + dt
			if controller.flareTimer >= (phase.hitInterval or 0.22) then
				controller.flareTimer = 0
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			...
			if controller.surgeDashesLeft and controller.surgeDashesLeft > 0
				and controller.surgeTimer >= 0.12 then
				controller.surgeTimer = 0
				...
				controller:checkCollisions(allControllers, true)
				controller:areaHit(allControllers, 5, phase.damage or 14, true)
			end
		elseif phase.id == "edge" then
