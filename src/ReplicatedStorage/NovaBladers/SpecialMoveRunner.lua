...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "ForgeEruption" then
		...
		elseif phase.id == "erupt" then
			controller.eruptHitsLeft = phase.hits or 3
			controller.eruptTimer = 0
			controller.eruptLastPos = controller.part.Position
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GlacierLock" then
		if phase.id == "freeze" or phase.id == "lock" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
	...
	elseif move.id == "ForgeEruption" then
		...
		elseif phase.id == "rush" or phase.id == "erupt" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 82)
		end
		if phase.id == "erupt" then
			controller.eruptTimer = (controller.eruptTimer or 0) + dt
			if controller.eruptTimer >= (phase.hitInterval or 0.22) then
				controller.eruptTimer = 0
				local pos = controller.part.Position
				SpecialVFX.eruptionTrail(controller.eruptLastPos, pos, move.color, folder)
				SpecialVFX.forgeBurst(pos, move.color, folder)
				controller.eruptLastPos = pos
