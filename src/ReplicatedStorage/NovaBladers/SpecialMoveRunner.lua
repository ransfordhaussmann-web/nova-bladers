...
MODE_HANDLERS.meteor = {
	onStart = function(controller, move, phase, target)
...
MODE_HANDLERS.fortress = {
	onStart = function(controller, move, phase)
...
MODE_HANDLERS.lunge = {
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.15) and (controller.slashHitsLeft or 0) > 0 then
				controller.slashTimer = 0
				controller.slashHitsLeft -= 1
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "slam" then
			...
			if controller.slamTimer >= (phase.interval or 0.35) then
				controller.slamTimer = 0
				SpecialVFX.stoneSlam(controller.part.Position, phase.range or 7, move.color, folder)
	...
	update = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "orbit" and controller.orbitCenter then
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				controller.solarTimer = 0
				SpecialVFX.sonicRing(controller.part.Position, 4, move.color, folder)
				controller:areaHit(allControllers, 4, phase.damage or 10, true)
			end
		elseif phase.id == "burst" then
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler and handler.onStart then
		handler.onStart(controller, move, phase, controller.specialTarget)
	end
end
