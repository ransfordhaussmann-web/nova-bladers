...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonForgeBreak" then
		...
		elseif phase.id == "slam" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "shrapnel" then
			SpecialVFX.forgeSlam(controller.part.Position, color, folder)
			controller.shrapnelHitsLeft = phase.hits or 3
			controller.shrapnelTimer = 0
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonForgeBreak" then
		...
		elseif phase.id == "slam" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "shrapnel" then
			controller.shrapnelTimer = (controller.shrapnelTimer or 0) + dt
			if controller.shrapnelTimer >= (phase.hitInterval or 0.22) then
				controller.shrapnelTimer = 0
				local pos = controller.part.Position
				SpecialVFX.shrapnelBurst(pos, move.color, folder)
