...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "deploy" then
			controller.velocity = Vector3.zero
			controller.guardReduction = move.damageReduction or 0.65
			SpecialVFX.bastionShield(controller, color, move.duration)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirage" then
			controller.mirageOffsets = SpecialVFX.phantomMirage(controller, color, phase.duration)
		...
		elseif phase.id == "echo" then
			SpecialVFX.phantomEcho(controller, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "deploy" or phase.id == "bastion" then
			controller.velocity = Vector3.zero
		end
		if phase.id == "bastion" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.bastionPulse(controller.part.Position, phase.range or 7, move.color, folder)
