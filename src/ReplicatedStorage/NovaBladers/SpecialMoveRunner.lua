...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrystalShatterStorm" then
		if phase.id == "freeze" then
			...
			controller.velocity *= phase.slowMult or 0.35
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "EmberPhoenixSpiral" then
		...
		elseif phase.id == "spiral" then
			...
			controller.spiralAngle = (controller.spiralAngle or 0) + (move.orbitSpeed or 20) * dt
			controller.spiralRadius = math.min(move.orbitRadius or 7, (controller.spiralRadius or 3) + dt * 5)
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			if controller.spiralTimer >= (phase.interval or 0.28) then
				controller.spiralTimer = 0
				SpecialVFX.phoenixTrail(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, r + 2, phase.damage or 9, true)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burst" then
			controller:areaHit(allControllers, phase.range or 7.5, phase.damage or 34, true)
		end
	end
