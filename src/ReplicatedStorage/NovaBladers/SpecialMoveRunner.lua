...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeSpiralRush" then
		...
		elseif phase.id == "erupt" then
			SpecialVFX.flameEruption(controller.part.Position, phase.range or 7, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeSpiralRush" then
		if phase.id == "ignite" then
			controller.velocity = Vector3.zero
		elseif phase.id == "spiral" then
			controller.spiralAngle = (controller.spiralAngle or 0) + (move.spiralSpeed or 22) * dt
			local r = move.spiralRadius or 3.5
			local forward = controller.facing * (phase.rushSpeed or move.rushSpeed or 80) * dt
			...
			controller.part.CFrame = CFrame.new(
				controller.part.Position + forward,
				controller.part.Position + forward + controller.facing
			) * CFrame.new(offset.X * 0.15, 0, offset.Z * 0.15)
