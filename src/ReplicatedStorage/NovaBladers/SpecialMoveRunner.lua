...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.11) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.clawSlash(pos, controller.facing, move.color, folder)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirrors" then
			controller.mirageTimer = (controller.mirageTimer or 0) + dt
			if controller.mirageTimer >= (phase.hitInterval or 0.14) then
				controller.mirageTimer = 0
				local angle = math.random() * math.pi * 2
				local offset = Vector3.new(math.cos(angle), 0, math.sin(angle)) * 3
				SpecialVFX.mirageBlade(controller.part.Position + offset, move.color, folder)
