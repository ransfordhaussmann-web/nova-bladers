...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteAvalanche" then
		...
		elseif phase.id == "crush" then
			SpecialVFX.avalancheBurst(controller.part.Position, color, folder)
		end
	elseif move.id == "EmberPulseWheel" then
		if phase.id == "ignite" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "pulse" then
			controller.emberPulseTimer = 0
			controller.emberPulseCount = 0
		elseif phase.id == "wheel" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrystalShatterStorm" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.range or 4) + controller.shatterCount * 1.3
				SpecialVFX.crystalShard(controller.part.Position, range, move.color, folder)
	...
	elseif move.id == "GraniteAvalanche" then
		...
		elseif phase.id == "avalanche" then
			...
			if controller.avalancheTimer >= (phase.interval or 0.35) then
				controller.avalancheTimer = 0
				SpecialVFX.pulseWave(controller.part.Position, phase.range or 8, move.color, folder)
	...
	elseif move.id == "EmberPulseWheel" then
		if phase.id == "ignite" then
			controller.velocity *= 0.9
		elseif phase.id == "pulse" then
			controller.emberPulseTimer = (controller.emberPulseTimer or 0) + dt
			if controller.emberPulseTimer >= (phase.interval or 0.26) then
				controller.emberPulseTimer = 0
				controller.emberPulseCount = (controller.emberPulseCount or 0) + 1
				local range = 3.5 + controller.emberPulseCount * 1.4
				SpecialVFX.emberRing(controller.part.Position, range, move.color, folder)
				controller:areaHit(allControllers, range, phase.damage or 9, true)
			end
		elseif phase.id == "wheel" and controller.orbitCenter then
