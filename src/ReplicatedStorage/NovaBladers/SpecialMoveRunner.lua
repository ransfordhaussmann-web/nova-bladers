...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			controller.spinBoost = 2.5
		elseif phase.id == "rush" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.spinBoost = 1
		end
	elseif move.id == "GraniteBastionPulse" then
		if phase.id == "anchor" then
			SpecialVFX.chargeAura(controller, color, phase.duration * 0.5)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			...
			controller.dashMax = phase.dashCount or 3
			controller.dashTimer = 0
			controller.dashFrom = controller.part.Position
			local targetPos = getTargetPos(controller, target)
			controller.dashTarget = targetPos
		elseif phase.id == "edge" then
			SpecialVFX.setPhantomFade(controller, false)
			SpecialVFX.edgeSlash(controller.part.Position, getTargetPos(controller, target), color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.hitInterval or 0.17) then
				controller.cycloneTimer = 0
				SpecialVFX.bladeCyclone(controller, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			local dashInterval = phase.duration / (controller.dashMax or 3)
			if controller.dashTimer >= dashInterval then
				...
				local to = controller.dashTarget or getTargetPos(controller, target)
				local dir = (to - from)
				dir = Vector3.new(dir.X, 0, dir.Z)
				if dir.Magnitude > 0.1 then
					dir = dir.Unit
					controller.facing = dir
					controller.part.CFrame = CFrame.new(from + dir * math.min(dir.Magnitude, 8), to)
					SpecialVFX.edgeSlash(from, controller.part.Position, move.color, folder)
					controller:checkCollisions(allControllers, true)
				end
			end
		elseif phase.id == "edge" then
