...
local PHASE_START = {
	...
	mirage = function(controller, move, phase, target)
		...
		elseif phase.id == "dash" then
			controller.mirageDashIdx = 0
			controller.mirageDashTimer = 0
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
		elseif phase.id == "converge" then
			SpecialVFX.mirageConverge(controller.part.Position, color, folder)
		end
	end,
}
...
local PHASE_UPDATE = {
	...
	bastion = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "root" or phase.id == "fortify" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
	...
	mirage = function(controller, move, phase, dt, allControllers)
		if phase.id == "dash" then
			...
			if controller.mirageDashTimer >= dashInterval then
				controller.mirageDashTimer = 0
				controller.mirageDashIdx = (controller.mirageDashIdx or 0) + 1
				if controller.mirageDashIdx <= dashCount then
					local angle = math.rad((controller.mirageDashIdx - 1) * (360 / dashCount))
					local dir = Vector3.new(math.cos(angle), 0, math.sin(angle))
					if controller.specialTarget and controller.specialTarget.part then
						dir = (controller.specialTarget.part.Position - controller.part.Position)
						dir = Vector3.new(dir.X, 0, dir.Z).Unit
						dir = CFrame.Angles(0, angle, 0) * dir
					end
					controller.facing = dir
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	local handler = PHASE_START[mode]
	if handler then
