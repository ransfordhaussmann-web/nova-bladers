...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeInfernoSlash" then
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.infernoTrail(controller.part.Position, dir, color, folder)
	...
	elseif move.id == "CrystalTidalSurge" then
		...
		elseif phase.id == "ripples" then
			...
			if controller.rippleTimer >= (phase.interval or 0.3) then
				controller.rippleTimer = 0
				SpecialVFX.frostRipple(controller.part.Position, phase.range or 5, move.color, folder)
