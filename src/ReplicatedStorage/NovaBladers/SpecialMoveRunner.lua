...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoRush" then
		...
		if phase.id == "blaze" then
			...
			if controller.blazeTimer >= (phase.hitInterval or 0.16) then
				controller.blazeTimer = 0
				local pos = controller.part.Position
	...
	elseif move.id == "FrostCrownBarrage" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.22) then
				...
				local targetPos = getTargetPos(controller, target)
				local angle = (controller.shardCount or 1) * 0.9
				local offset = Vector3.new(math.cos(angle), 0, math.sin(angle)) * (phase.range or 5)
				local hitPos = controller.part.Position + offset
				SpecialVFX.iceShard(controller.part.Position + Vector3.new(0, 0.5, 0), hitPos, move.color, folder)
