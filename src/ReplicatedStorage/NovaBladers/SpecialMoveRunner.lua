...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "rise" then
			SpecialVFX.barrierDome(controller, color, phase.duration + 1.2)
		elseif phase.id == "guard" then
			controller.guardReduction = move.damageReduction or 0.7
			controller.barrierTimer = 0
		elseif phase.id == "release" then
			controller.guardReduction = 0
			SpecialVFX.auroraBurst(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "guard" then
			controller.velocity = Vector3.zero
			controller.barrierTimer = (controller.barrierTimer or 0) + dt
			if controller.barrierTimer >= (phase.interval or 0.35) then
				controller.barrierTimer = 0
