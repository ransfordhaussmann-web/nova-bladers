...
local function startMeteor(controller, move, phase, target)
...
local function startFortress(controller, move, phase)
...
local function startSonic(controller, move, phase, target)
...
local function startEclipse(controller, move, phase, target)
...
local function startFeather(controller, move, phase, target)
	if phase.id == "windup" then
		SpecialVFX.chargeAura(controller, move.color, phase.duration)
...
local function startAnchor(controller, move, phase)
	if phase.id == "pull" then
		SpecialVFX.waterPull(controller, move.color, phase.duration)
		controller.velocity = Vector3.zero
...
local function startRend(controller, move, phase, target)
	...
	elseif phase.id == "slash" then
		...
		SpecialVFX.slashTrail(controller, move.color)
	elseif phase.id == "finish" then
		SpecialVFX.venomBurst(controller.part.Position, move.color, SpecialVFX.ensureFolder(controller))
	end
end

local function startRampart(controller, move, phase)
	if phase.id == "rise" then
		SpecialVFX.stoneWall(controller, move.color, phase.duration)
		controller.velocity = Vector3.zero
	elseif phase.id == "block" then
		controller.guardReduction = phase.damageReduction or move.damageReduction or 0.6
		controller.velocity = Vector3.zero
	elseif phase.id == "shatter" then
		SpecialVFX.stoneShatter(controller.part.Position, move.color, SpecialVFX.ensureFolder(controller))
	end
end

local function startSolar(controller, move, phase, target)
	if phase.id == "charge" then
		SpecialVFX.chargeAura(controller, move.color, phase.duration)
...
local function startPhase(controller, move, phase, target)
...
local PHASE_START = {
	meteor = startMeteor,
	fortress = startFortress,
	sonic = startSonic,
	eclipse = startEclipse,
	feather = startFeather,
	anchor = startAnchor,
	rend = startRend,
	rampart = startRampart,
	solar = startSolar,
	phase = startPhase,
}
...
local function updateFeather(controller, move, phase, dt, allControllers, folder)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	elseif phase.id == "rush" or phase.id == "storm" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
	end
...
local function updateAnchor(controller, move, phase, dt, allControllers, folder)
	local target = controller.specialTarget
	if phase.id == "pull" and target and target.part then
		controller.velocity = Vector3.zero
		local toSelf = controller.part.Position - target.part.Position
		local flat = Vector3.new(toSelf.X, 0, toSelf.Z)
		if flat.Magnitude > 2 then
			target.velocity = flat.Unit * (phase.pullStrength or move.pullStrength or 38)
		end
	elseif phase.id == "slam" then
		controller:checkCollisions(allControllers, true)
		if not controller.airborne then
			SpecialVFX.waterSplash(controller.part.Position, move.color, folder)
			controller:areaHit(allControllers, 5, phase.damage or 22, true)
		end
	elseif phase.id == "wave" then
		controller.pulseTimer = (controller.pulseTimer or 0) + dt
		if controller.pulseTimer >= (phase.interval or 0.35) then
			controller.pulseTimer = 0
			SpecialVFX.waterSplash(controller.part.Position, phase.range or 9, move.color, folder)
			controller:areaHit(allControllers, phase.range or 9, phase.damage or 12, true)
		end
	end
end

local function updateRend(controller, move, phase, dt, allControllers)
	if phase.id == "slash" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		controller:checkCollisions(allControllers, true)
	elseif phase.id == "finish" then
		controller:areaHit(allControllers, phase.range or 5, phase.damage or 38, true)
	end
end

local function updateRampart(controller, move, phase, dt, allControllers)
	if phase.id == "rise" or phase.id == "block" then
		controller.velocity = Vector3.zero
	elseif phase.id == "shatter" then
		controller:areaHit(allControllers, phase.range or 7, phase.damage or 15, true)
	end
end

local function updateSolar(controller, move, phase, dt, allControllers, folder)
	if phase.id == "charge" then
...
local function updatePhase(controller, move, phase, dt, allControllers)
	if phase.id == "dash" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 90)
		controller:checkCollisions(allControllers, true)
	elseif phase.id == "slash" then
		controller:areaHit(allControllers, phase.range or 6, phase.damage or 34, true)
	end
end
...
local MODE_UPDATE = {
	...
	feather = updateFeather,
	anchor = updateAnchor,
	rend = updateRend,
	rampart = updateRampart,
	solar = updateSolar,
	phase = updatePhase,
}
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local handler = MODE_UPDATE[move.mode]
	if handler then
