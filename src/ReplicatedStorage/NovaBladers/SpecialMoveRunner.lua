...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		if phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.interval or 0.12) then
				controller.slashTimer = 0
				controller.slashCount = (controller.slashCount or 0) + 1
				local offset = (controller.slashCount - 2) * 35
				SpecialVFX.crimsonSlash(controller, move.color, offset)
				controller:areaHit(allControllers, 4.5, 10, true)
			end
		elseif phase.id == "lunge" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "release" then
			...
			if controller.releaseTimer >= (phase.interval or 0.25) then
				controller.releaseTimer = 0
