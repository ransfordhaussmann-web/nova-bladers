...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "nova" then
		...
		elseif phase.id == "burst" then
			...
			if controller.novaTimer >= (phase.interval or 0.12) then
				controller.novaTimer = 0
				controller.novaWaves = (controller.novaWaves or 0) + 1
				local range = (phase.range or 5) + controller.novaWaves * 1.2
				SpecialVFX.novaBurst(controller.part.Position, range, move.color, folder)
