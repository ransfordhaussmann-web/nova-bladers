...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "EmberCyclone" then
		...
		elseif phase.id == "erupt" then
			SpecialVFX.emberErupt(controller.part.Position, phase.range or 7.5, color, folder)
		end
	elseif move.id == "GlacierShroud" then
		...
		elseif phase.id == "shatter" then
			...
			SpecialVFX.glacierShatter(controller.part.Position, phase.range or 8, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "EmberCyclone" then
		...
		elseif phase.id == "whirl" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.range or 5.5) + controller.cycloneCount * 0.4
				SpecialVFX.fireRing(controller.part.Position, range, move.color, folder)
			...
			for _, other in allControllers do
				if other ~= controller and other.alive and not other.underground then
					...
					if dist > 0.5 and dist <= (phase.range or 5.5) + 2 then
	...
	elseif move.id == "GlacierShroud" then
		if phase.id == "frost" or phase.id == "shroud" then
			controller.velocity = Vector3.zero
		end
		if phase.id == "shroud" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.frostPulse(controller.part.Position, phase.range or 6, move.color, folder)
