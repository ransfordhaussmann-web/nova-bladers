...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "rend" then
			SpecialVFX.rendBurst(controller.part.Position, color, folder)
		end
	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "dash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.diveTrail(controller, getTargetPos(controller, target), color, folder)
		elseif phase.id == "slash" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "dash" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "slash" then
			controller:areaHit(allControllers, phase.range or 6.2, phase.damage or 34, true)
		end
	end
