...
local MODE_ON_START = {
	...
	lunge = function(controller, move, phase, target)
		...
		elseif phase.id == "rip" then
			controller.ripHitsLeft = phase.hits or 2
	...
	bastion = function(controller, move, phase)
		if phase.id == "anchor" then
			controller.velocity = Vector3.zero
			SpecialVFX.bastionAnchor(controller, move.color, phase.duration)
		elseif phase.id == "bastion" then
			controller.guardReduction = phase.damageReduction or move.damageReduction or 0.7
			controller.reflectKnockback = move.reflectKnockback or 1.4
	...
	mirage = function(controller, move, phase, target)
		if phase.id == "split" then
			SpecialVFX.mirageClones(controller, phase.clones or 2, move.color)
		elseif phase.id == "dash" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.mirageDash(controller, move.color)
		elseif phase.id == "strike" then
			SpecialVFX.mirageStrike(controller.part.Position, move.color, SpecialVFX.ensureFolder(controller))
		end
	end,
}

local MODE_ON_UPDATE = {
	...
	eclipse = function(controller, move, phase, _dt, allControllers)
	...
	lunge = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.22) then
				controller.ripTimer = 0
				controller.ripHitsLeft = (controller.ripHitsLeft or 1) - 1
				SpecialVFX.lungeRip(controller.part.Position, controller.facing, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 6, phase.damage or 22, true)
			end
		end
	end,
	bastion = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "counter" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.bastionCounter(controller.part.Position, phase.range or 7, move.color, folder)
	...
	mirage = function(controller, move, phase, _dt, allControllers)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	local handler = MODE_ON_START[move.mode]
	if handler then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local handler = MODE_ON_UPDATE[move.mode]
	if handler then
