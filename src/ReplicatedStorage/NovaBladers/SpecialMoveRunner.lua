...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "crest" then
			SpecialVFX.crestBurst(controller.part.Position, color, folder)
		end
	end
end
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "reflect" then
			...
			if controller.reflectTimer >= (phase.interval or 0.35) then
				controller.reflectTimer = 0
				SpecialVFX.reflectWave(controller.part.Position, phase.range or 7, move.color, folder)
