...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "brace" or phase.id == "fortress" then
			controller.velocity = Vector3.zero
		elseif phase.id == "counter" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
			if controller.pulseTimer >= (phase.interval or 0.38) then
				controller.pulseTimer = 0
				SpecialVFX.stoneCounter(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "SolarFlareOrbit" then
		if phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarOrbitSpark(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burst" then
			controller:areaHit(allControllers, phase.range or 8, phase.damage or 40, true)
		end
