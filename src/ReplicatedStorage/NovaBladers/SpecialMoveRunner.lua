...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "spiral" then
		...
		elseif phase.id == "slash" then
			SpecialVFX.spiralSlash(controller.part.Position, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "spiral" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "spiral" then
			controller.spiralAngle = (controller.spiralAngle or 0) + dt * 12
			local spiralDir = Vector3.new(
				math.cos(controller.spiralAngle) * controller.facing.X - math.sin(controller.spiralAngle) * controller.facing.Z,
				0,
				math.sin(controller.spiralAngle) * controller.facing.X + math.cos(controller.spiralAngle) * controller.facing.Z
			).Unit
			controller.velocity = spiralDir * (phase.rushSpeed or move.rushSpeed or 85)
			controller.spiralTimer = (controller.spiralTimer or 0) + dt
			if controller.spiralTimer >= (phase.hitInterval or 0.2) then
				controller.spiralTimer = 0
				SpecialVFX.spiralMark(controller.part.Position, move.color, folder, controller.spiralAngle)
