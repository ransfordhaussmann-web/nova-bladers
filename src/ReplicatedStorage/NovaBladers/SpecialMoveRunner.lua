	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "spiral" then
			...
			elseif not center then
				center = controller.part.Position
			end
			controller.orbitAngle += (controller.orbitSpeed or 20) * dt
			local r = math.max(2.5, (controller.orbitRadius or 7) - (controller.spiralTimer or 0) * 3)
			local y = controller.part.Position.Y
			local pos = center + Vector3.new(math.cos(controller.orbitAngle) * r, 0, math.sin(controller.orbitAngle) * r)
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
				...
				for _, other in allControllers do
					if other ~= controller and other.alive and not other.underground then
						...
						if dist <= (phase.range or 7) then
							...
							other.speedDebuffUntil = os.clock() + (phase.slowDuration or 1.2)
							other.speedDebuffMult = phase.slowMult or 0.5
						end
					end
				end
			end
