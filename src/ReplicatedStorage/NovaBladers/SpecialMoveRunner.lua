...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "mirage" then
			...
			controller.mirageHitsLeft = phase.hits or 4
			controller.mirageAngle = 0
		elseif phase.id == "edge" then
			SpecialVFX.bladeEdgeBurst(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.interval or 0.15) then
				controller.ripTimer = 0
				controller.ripHitsLeft = (controller.ripHitsLeft or 1) - 1
				local pos = controller.part.Position
				SpecialVFX.bladeRip(pos, controller.facing, move.color, folder)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirage" then
			...
			if controller.mirageTimer >= (phase.interval or 0.12) then
				controller.mirageTimer = 0
				controller.mirageHitsLeft = (controller.mirageHitsLeft or 1) - 1
				controller.mirageAngle = (controller.mirageAngle or 0) + math.pi / 2
				local offset = Vector3.new(math.cos(controller.mirageAngle), 0, math.sin(controller.mirageAngle)) * 3
				local ghostPos = controller.part.Position + offset
				SpecialVFX.phantomAfterimage(ghostPos, move.color, folder)
				controller.part.CFrame = CFrame.new(ghostPos, ghostPos + controller.facing)
				controller:areaHit(allControllers, phase.hitRadius or 4, phase.damage or 10, true)
			end
		elseif phase.id == "edge" then
			controller:areaHit(allControllers, phase.range or 5.5, phase.damage or 30, true)
		end
	end
