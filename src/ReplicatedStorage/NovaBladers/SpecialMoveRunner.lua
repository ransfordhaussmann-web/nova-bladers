...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" then
			...
			SpecialVFX.auroraDome(controller, move.color, move.accentColor or color, phase.duration + 0.95)
		elseif phase.id == "shimmer" then
			controller.shimmerTimer = 0
		elseif phase.id == "burst" then
			SpecialVFX.auroraBurst(controller.part.Position, move.color, move.accentColor or color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" or phase.id == "shimmer" then
		...
		if phase.id == "shimmer" then
			...
			if controller.shimmerTimer >= (phase.interval or 0.3) then
				controller.shimmerTimer = 0
				SpecialVFX.auroraShimmer(controller.part.Position, phase.range or 7, move.color, move.accentColor or move.color, folder)
