...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.lungeSlash(controller, dir, color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "burst" then
			SpecialVFX.domeBurst(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.15) then
				...
				SpecialVFX.ripFlash(controller.part.Position, move.color, folder)
