...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "split" then
			SpecialVFX.phantomMirage(controller, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastion" then
		...
		elseif phase.id == "quake" then
			...
			if controller.quakeTimer >= (phase.interval or 0.38) then
				controller.quakeTimer = 0
				SpecialVFX.graniteQuake(controller.part.Position, phase.range or 8.5, move.color, folder)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirage" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 78)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "strike" then
