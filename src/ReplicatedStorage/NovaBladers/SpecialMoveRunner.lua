...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperFang" then
		...
		elseif phase.id == "fang" then
			SpecialVFX.crimsonBiteBurst(controller.part.Position, color, folder)
		end
	elseif move.id == "FrostHaloGlacier" then
		...
		elseif phase.id == "halo" then
			...
			SpecialVFX.frostHaloRing(controller, color, phase.duration)
	...
	elseif move.id == "CrimsonRipperFang" then
		...
		elseif phase.id == "rush" then
			...
			if controller.slashTimer >= (phase.slashInterval or 0.14) then
				controller.slashTimer = 0
				SpecialVFX.crimsonSlashArc(controller.part.Position, controller.facing, move.color, folder)
				controller:areaHit(allControllers, 4.5, phase.slashDamage or 10, true)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "fang" then
