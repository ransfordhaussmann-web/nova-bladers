...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipper" then
		...
		elseif phase.id == "slash1" or phase.id == "slash2" or phase.id == "slash3" then
			...
			if not controller.slashHitDone then
				controller.slashHitDone = true
				SpecialVFX.slashArc(controller.part.Position, controller.facing, move.color, folder)
				controller:areaHit(allControllers, phase.range or 4.5, phase.damage or 12, true)
			end
		end
