...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "rush" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
			local pos = controller.part.Position
			SpecialVFX.meteorTrail(controller.meteorLastPos or pos, pos, move.color, folder)
			controller.meteorLastPos = pos
		elseif phase.id == "shatter" then
			controller.shatterTimer = (controller.shatterTimer or 0) + dt
			if controller.shatterTimer >= (phase.interval or 0.2) then
				controller.shatterTimer = 0
				controller.shatterWaves = (controller.shatterWaves or 0) + 1
				local range = (phase.range or 5) + controller.shatterWaves * 1.5
				SpecialVFX.crystalShatter(controller.part.Position, range, move.color, folder)
				controller:areaHit(allControllers, range, phase.damage or 13, true)
			end
		end
	...
	elseif mode == "blaze" then
		if phase.id == "spiral" and controller.orbitCenter then
			...
			if controller.blazeTimer >= (phase.interval or 0.22) then
				controller.blazeTimer = 0
				controller.blazeCount = (controller.blazeCount or 0) + 1
				local range = 3 + controller.blazeCount * 1.2
				SpecialVFX.blazeSpiral(controller.part.Position, range, move.color, folder)
				controller:areaHit(allControllers, range, phase.damage or 9, true)
			end
			local r = controller.orbitRadius or 4
			local center = controller.orbitCenter
			local y = controller.part.Position.Y
			local pos = center + Vector3.new(math.cos(controller.orbitAngle) * r, 0, math.sin(controller.orbitAngle) * r)
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
		...
		elseif phase.id == "flare" then
			...
			SpecialVFX.blazeFlare(controller.part.Position, move.color, folder)
