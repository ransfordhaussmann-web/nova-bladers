	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.22) then
				controller.ripTimer = 0
				controller.ripAngle = (controller.ripAngle or 0) + math.pi * 0.66
				local pos = controller.part.Position
				SpecialVFX.ripSlash(pos, controller.ripAngle, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "pulse" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.auroraPulse(controller.part.Position, phase.range or 7.5, move.color, folder)
