...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "fortress" then
		if phase.id == "burrow" then
			...
			if move.vfxStyle == "stone" then
				SpecialVFX.stoneBurrow(controller, color)
			else
	...
	elseif mode == "phase" then
		if phase.id == "vanish" then
			SpecialVFX.phaseVanish(controller, color, phase.duration)
		elseif phase.id == "blink" then
			local blinkPos = getTargetPos(controller, target)
			if target and target.part then
				local behind = target.part.Position - target.facing * 4
				blinkPos = Vector3.new(behind.X, controller.part.Position.Y, behind.Z)
			end
			SpecialVFX.phaseBlink(controller, blinkPos, color, folder)
			controller.part.CFrame = CFrame.new(blinkPos, getTargetPos(controller, target))
			local dir = (getTargetPos(controller, target) - blinkPos)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
		elseif phase.id == "slash" then
			SpecialVFX.slashArc(controller.part.Position, controller.facing, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local target = controller.specialTarget
	local mode = move.mode
	local useFeatherVfx = move.vfxStyle == "feather"
	local useFlareVfx = move.vfxStyle == "flare"
	...
	elseif mode == "fortress" then
		...
		elseif phase.id == "pulse" then
			...
			if controller.pulseTimer >= (phase.interval or 0.35) then
				controller.pulseTimer = 0
				if move.vfxStyle == "stone" then
					SpecialVFX.stonePulse(controller.part.Position, phase.range or 8, move.color, folder)
				else
	...
	elseif mode == "anchor" then
		...
		elseif phase.id == "slam" then
			...
			if not controller.airborne then
				...
				controller:areaHit(allControllers, 6, phase.slamDamage or 26, true)
			end
		elseif phase.id == "wave" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
	...
	elseif mode == "phase" then
		if phase.id == "slash" then
			controller.velocity = controller.facing * (move.rushSpeed or 90)
			controller:areaHit(allControllers, phase.range or 8, phase.damage or 38, true)
		end
	end
