...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GlacierCrownLock" then
	...
	elseif move.id == "CrimsonRageLunge" then
		if phase.id == "rage" then
			SpecialVFX.rageAura(controller, color, phase.duration)
		elseif phase.id == "lunge" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "combo" then
			controller.comboHitsLeft = phase.hits or 3
			controller.comboTimer = 0
			controller.comboLastPos = controller.part.Position
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GlacierCrownLock" then
		if phase.id == "frost" or phase.id == "crown" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
			controller.shatterTimer = (controller.shatterTimer or 0) + dt
			if controller.shatterTimer >= (phase.interval or 0.35) then
				controller.shatterTimer = 0
				SpecialVFX.iceShatterWave(controller.part.Position, phase.range or 7.5, move.color, folder)
	...
	elseif move.id == "CrimsonRageLunge" then
		...
		elseif phase.id == "combo" then
			...
			controller.comboTimer = (controller.comboTimer or 0) + dt
			if controller.comboTimer >= (phase.hitInterval or 0.16) then
				controller.comboTimer = 0
				local pos = controller.part.Position
				SpecialVFX.fangSlash(controller.comboLastPos, pos, move.color, folder)
				controller.comboLastPos = pos
