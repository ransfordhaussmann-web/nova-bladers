...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastionPulse" then
		if phase.id == "anchor" then
			controller.velocity = Vector3.zero
			SpecialVFX.stoneAnchor(controller, color, phase.duration, folder)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.velocity = dir * (phase.driftSpeed or move.rushSpeed or 68)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "vanish" then
			SpecialVFX.setGhost(controller, true)
			SpecialVFX.ghostAura(controller, color, phase.duration)
		...
		elseif phase.id == "afterimage" then
			SpecialVFX.setGhost(controller, false)
			SpecialVFX.afterimageSlash(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				...
				controller:pullNearby(allControllers, phase.hitRadius or 5.5, phase.pullStrength or 18)
			end
		elseif phase.id == "rush" then
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.velocity = controller.facing * (phase.driftSpeed or move.rushSpeed or 68)
			if controller.driftTimer >= (phase.interval or 0.28) then
				controller.driftTimer = 0
				SpecialVFX.solarBurnTrail(controller.part.Position, move.color, folder)
