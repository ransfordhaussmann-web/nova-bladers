...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TidalPrismSurge" then
		if phase.id == "ripple" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
			SpecialVFX.tidalRipple(controller.part.Position, 4, color, folder)
		elseif phase.id == "surge" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed or 64)
			controller.guardReduction = move.damageReduction or 0.4
			SpecialVFX.prismShield(controller, color, phase.duration)
	...
	elseif move.id == "CoreFlareRush" then
		...
		elseif phase.id == "rush" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed or 86)
	...
	elseif move.id == "TidalPrismSurge" then
		...
		elseif phase.id == "prism" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.tidalRipple(controller.part.Position, phase.range or 7, move.color, folder)
