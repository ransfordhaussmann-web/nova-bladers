...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "fortify" then
			SpecialVFX.granitePillar(controller, color, phase.duration)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "SolarFlareOrbit" then
		if phase.id == "flare" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
			SpecialVFX.solarFlare(controller.part.Position, 5, color, folder)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "split" then
			local count = phase.mirages or 3
			for i = 1, count do
				local offset = controller.facing * (i * 2.5) + Vector3.new(math.random(-2, 2), 0, math.random(-2, 2))
				SpecialVFX.phantomAfterimage(controller.part.Position + offset, color, folder)
			end
		...
		elseif phase.id == "slash" then
			SpecialVFX.sonicRing(controller.part.Position, phase.range or 6.5, color, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			if controller.lungeTimer >= (phase.hitInterval or 0.12) then
				controller.lungeTimer = 0
				controller.lungeHitsLeft = (controller.lungeHitsLeft or 1) - 1
				SpecialVFX.meteorTrail(controller.part.Position - controller.facing * 2, controller.part.Position, move.color, folder)
				controller:checkCollisions(allControllers, true)
			end
		elseif phase.id == "rip" then
			controller.velocity = Vector3.zero
