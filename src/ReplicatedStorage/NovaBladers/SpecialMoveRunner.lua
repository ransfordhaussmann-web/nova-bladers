...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "dome" then
			...
			SpecialVFX.auroraDome(controller, color, phase.duration + 0.75)
		elseif phase.id == "hold" then
			controller.pulseTimer = 0
		elseif phase.id == "release" then
			SpecialVFX.auroraBurst(controller.part.Position, color, folder)
			controller.auroraReleaseHit = true
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.14) then
				controller.ripperTimer = 0
				local pos = controller.part.Position
				SpecialVFX.fangRip(pos, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "release" and controller.auroraReleaseHit then
			controller.auroraReleaseHit = false
