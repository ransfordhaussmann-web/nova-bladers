...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "ripper" then
			...
			controller.ripperLastPos = controller.part.Position
		end
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "solar" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
	...
	elseif mode == "mirage" then
		if phase.id == "split" then
			SpecialVFX.mirageShards(controller, color, phase.clones or 3)
		elseif phase.id == "phantom" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.mirageTrail(controller, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "ripper" then
			controller.velocity = controller.facing * (move.rushSpeed or 70) * 0.85
			controller.ripperTimer = (controller.ripperTimer or 0) + dt
			if controller.ripperTimer >= (phase.hitInterval or 0.16) then
				controller.ripperTimer = 0
				local pos = controller.part.Position
				SpecialVFX.crimsonSlash(controller.ripperLastPos, pos, move.color, folder)
				controller.ripperLastPos = pos
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "solar" and controller.orbitCenter then
