...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonInfernoSpiral" then
	...
	elseif move.id == "GlacierRampart" then
		...
		elseif phase.id == "rampart" then
			...
			SpecialVFX.iceRampart(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoSpiral" then
		...
		elseif phase.id == "spiral" then
			...
			local radius = 4 + (controller.spiralHitsLeft or 5) * 0.3
	...
	elseif move.id == "GlacierRampart" then
		if phase.id == "freeze" or phase.id == "rampart" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
