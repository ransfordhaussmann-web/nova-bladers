...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if move.id == "NovaMeteorShower" or move.mode == "meteor" then
	...
	elseif move.id == "VoltSonicTempest" or move.mode == "sonic" then
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "counter" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or 75)
		end
	...
	elseif move.id == "PhantomMirageCut" then
		if phase.id == "fade" then
			SpecialVFX.mirageFade(controller, true)
		elseif phase.id == "blink" then
			SpecialVFX.mirageFade(controller, false)
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.diveTrail(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "counter" then
			controller.guardReduction = 0
			controller.velocity = controller.facing * (phase.rushSpeed or 75)
			controller:checkCollisions(allControllers, true)
		end
