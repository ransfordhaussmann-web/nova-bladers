...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFlameLance" then
		if phase.id == "forge" then
			SpecialVFX.flameForge(controller, color, phase.duration)
		...
		elseif phase.id == "pierce" then
			SpecialVFX.flamePierce(controller.part.Position, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameLance" then
		...
		elseif phase.id == "lunge" then
			...
			local pos = controller.part.Position
			SpecialVFX.flameTrail(controller.lanceLastPos or pos, pos, move.color, folder)
	...
	elseif move.id == "FrostCrystalAegis" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.28) then
				...
				if controller.shatterCount == 1 then
