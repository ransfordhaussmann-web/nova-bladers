...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "burst" then
			SpecialVFX.crystalShards(controller.part.Position, phase.range or 7, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.hitRadius or 5) + controller.shardCount * 0.8
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			...
			local targetPos = center + offset + controller.facing * (controller.blazeTimer * (phase.rushSpeed or move.rushSpeed or 80) * 0.3)
			local dir = (targetPos - controller.part.Position)
			...
			if controller.blazeTimer >= (phase.interval or 0.15) then
				controller.blazeTimer = 0
