...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "veil" then
		if phase.id == "vanish" then
			...
			if dir.Magnitude > 0.1 then
				local step = dir.Unit * math.min(dir.Magnitude * 0.45, 10)
				local pos = controller.part.Position + step
				controller.part.CFrame = CFrame.new(Vector3.new(pos.X, controller.part.Position.Y, pos.Z))
					* (controller.part.CFrame - controller.part.CFrame.Position)
			end
		elseif phase.id == "slash" then
			SpecialVFX.setVeil(controller, false)
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.veilSlash(controller.part.Position, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "after" then
			SpecialVFX.veilAfterBurst(controller.part.Position, color, folder)
		end
	elseif mode == "inferno" then
		...
		elseif phase.id == "flare" then
			SpecialVFX.infernoFlare(controller.part.Position, color, folder)
		end
	end
end
	...
	elseif mode == "inferno" then
		...
		elseif phase.id == "burst" then
			...
			if controller.infernoTimer >= (phase.interval or 0.22) then
				...
				local range = 3.5 + controller.infernoCount * 1.8
		...
		elseif phase.id == "flare" then
			controller:areaHit(allControllers, phase.range or 8, phase.damage or 42, true)
		end
	end
