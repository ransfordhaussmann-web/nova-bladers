...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.crimsonTrail(controller, color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "rise" then
			SpecialVFX.auroraRise(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "dome" then
			...
			controller.domeTimer = 0
		elseif phase.id == "nova" then
			controller.guardReduction = 0
			SpecialVFX.auroraNova(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			controller.domeTimer = (controller.domeTimer or 0) + dt
			if controller.domeTimer >= (phase.interval or 0.35) then
				controller.domeTimer = 0
