...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "rip" then
			SpecialVFX.ripImpact(controller.part.Position, color, folder)
		end
	elseif mode == "bastion" then
		if phase.id == "fortify" then
			controller.guardReduction = move.damageReduction or 0.65
			SpecialVFX.bastionShield(controller, color, phase.duration + 0.65)
	...
	elseif mode == "solar" then
		if phase.id == "glow" then
			SpecialVFX.solarGlow(controller, color, phase.duration)
		elseif phase.id == "flares" or phase.id == "sustain" then
	...
	elseif mode == "mirage" then
		...
		elseif phase.id == "dash1" or phase.id == "dash2" then
			...
			if target and target.part then
				dir = (getTargetPos(controller, target) - controller.part.Position)
				dir = Vector3.new(dir.X, 0, dir.Z).Unit
				controller.facing = dir
			end
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.phantomDash(controller.part.Position, dir, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "rip" then
			...
			controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 42, true)
		end
	...
	elseif mode == "bastion" then
		...
		elseif phase.id == "counter" then
			...
			if controller.pulseTimer >= (phase.interval or 0.2) then
				controller.pulseTimer = 0
				SpecialVFX.bastionCounter(controller.part.Position, phase.range or 7.5, move.color, folder)
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "flares" or phase.id == "sustain" then
			...
			if controller.solarTimer >= (phase.interval or 0.24) then
				controller.solarTimer = 0
