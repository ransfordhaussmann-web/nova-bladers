...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "voidslash" then
		...
		elseif phase.id == "slash" then
			...
			local targetPos = getTargetPos(controller, target)
			local startPos = controller.part.Position
			SpecialVFX.voidSlashTrail(startPos, targetPos, color, folder)
			local dir = (targetPos - startPos)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "echo" then
			SpecialVFX.voidEcho(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "maelstrom" then
		...
		elseif phase.id == "spiral" then
			...
			local speed = phase.rushSpeed or move.rushSpeed or 74
			local forward = controller.facing
			local right = Vector3.new(-forward.Z, 0, forward.X)
			local spiralDir = (forward + right * math.sin(controller.maelstromAngle) * 0.6).Unit
			controller.velocity = spiralDir * speed
			controller.facing = spiralDir

			controller.maelstromTimer = (controller.maelstromTimer or 0) + dt
			if controller.maelstromTimer >= (phase.hitInterval or 0.2) then
				controller.maelstromTimer = 0
				SpecialVFX.spiralSlash(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5.2, phase.damage or 10, true)
			end
		end
