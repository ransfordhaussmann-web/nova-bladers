...
MODE_HANDLERS.bastion = {
	onStart = function(controller, move, phase)
		if phase.id == "slam" then
			SpecialVFX.stoneSlam(controller.part.Position, move.color, SpecialVFX.ensureFolder(controller))
			controller.velocity = Vector3.zero
	...
	update = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "slam" or phase.id == "bastion" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
...
MODE_HANDLERS.solar = {
	onStart = function(controller, move, phase, target)
		...
		elseif phase.id == "burst" then
			SpecialVFX.solarBurst(controller.part.Position, phase.range or 8, move.color, SpecialVFX.ensureFolder(controller))
		end
	end,
	update = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				controller.solarTimer = 0
				SpecialVFX.sonicRing(controller.part.Position, 4.5, move.color, folder)
...
MODE_HANDLERS.mirage = {
	onStart = function(controller, move, phase, target)
		if phase.id == "split" then
			SpecialVFX.mirageClone(controller, move.color, -60)
			SpecialVFX.mirageClone(controller, move.color, 60)
			SpecialVFX.chargeAura(controller, move.color, phase.duration)
		...
		elseif phase.id == "reveal" then
			SpecialVFX.phantomStrike(controller.part.Position, move.color, SpecialVFX.ensureFolder(controller))
		end
	end,
	update = function(controller, move, phase, _dt, allControllers)
		if phase.id == "split" then
