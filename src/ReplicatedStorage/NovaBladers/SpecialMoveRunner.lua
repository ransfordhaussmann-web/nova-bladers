...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonInfernoLance" then
	...
	elseif move.id == "FrostCrownShatter" then
		if phase.id == "freeze" then
			...
			SpecialVFX.iceArmor(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoLance" then
		...
		elseif phase.id == "eruption" then
			...
			if controller.eruptionTimer >= (phase.interval or 0.22) then
				controller.eruptionTimer = 0
				SpecialVFX.fireBurst(controller.part.Position, phase.range or 7, move.color, folder)
