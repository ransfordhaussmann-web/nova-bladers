...
local function onFortressPhaseStart(controller, move, phase)
	...
	if phase.id == "burrow" or phase.id == "freeze" then
		if phase.id == "freeze" then
			SpecialVFX.glacierFreeze(controller, color, phase.duration)
		else
...
local function onEclipsePhaseStart(controller, move, phase, target)
	local color = move.color
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if mode == "meteor" or mode == "solar" then
		onMeteorPhaseStart(controller, move, phase, target)
	elseif mode == "fortress" or mode == "glacier" then
	...
	elseif mode == "eclipse" then
		onEclipsePhaseStart(controller, move, phase, target)
	end
end
...
local function updateMeteor(controller, move, phase, dt, allControllers)
	...
	if phase.id == "shower" then
		...
		if controller.meteorTimer >= (phase.hitInterval or 0.18) then
			...
			if isSolar then
				SpecialVFX.solarTrail(controller.meteorLastPos, pos, move.color, folder)
				SpecialVFX.solarImpact(pos, move.color, folder)
			else
...
local function updateFortress(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	local isGlacier = move.mode == "glacier"
	...
	elseif phase.id == "pulse" then
		...
		if controller.pulseTimer >= (phase.interval or 0.35) then
			controller.pulseTimer = 0
			if isGlacier then
				SpecialVFX.icePulse(controller.part.Position, phase.range or 8, move.color, folder)
			else
