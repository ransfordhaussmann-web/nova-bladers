...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.hitRadius or 5) + controller.shatterCount * 1.2
				SpecialVFX.crystalRing(controller.part.Position, range, move.color, folder)
	...
	elseif mode == "blaze" then
		if phase.id == "spiral" then
			...
			controller.spiralRadius = math.min((controller.spiralRadius or 3) + 4 * dt, 8)
			...
			local forward = controller.facing * (phase.rushSpeed or move.rushSpeed or 65)
			controller.velocity = forward + offset * 2
