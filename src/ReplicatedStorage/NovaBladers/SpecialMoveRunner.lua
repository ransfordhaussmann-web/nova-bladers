...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "rippers" then
			controller.ripperHitsLeft = phase.hits or 3
			controller.ripperTimer = 0
		end
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "charge" then
			SpecialVFX.auroraCharge(controller, color, move.accentColor or color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			controller.velocity = Vector3.zero
		elseif phase.id == "reflect" then
			if not controller.reflectFired then
				controller.reflectFired = true
				SpecialVFX.auroraReflect(controller.part.Position, phase.range or 9, move.color, move.accentColor, folder)
