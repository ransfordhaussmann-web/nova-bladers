...
local function onPhaseStartMeteor(controller, move, phase)
...
local function onPhaseStartSonic(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
...
local function onPhaseStartEclipse(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
...
local function onPhaseStartSolar(controller, move, phase)
	local color = move.color
	local target = controller.specialTarget
...
local function updateSolar(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	...
	elseif phase.id == "flare" then
		...
		if controller.solarTimer >= (phase.interval or 0.2) then
			...
			local range = 3.5 + controller.solarCount * 2
	...
	elseif phase.id == "rush" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		local pos = controller.part.Position
		SpecialVFX.solarRushTrail(controller.solarLastPos or pos, pos, move.color, folder)
		controller.solarLastPos = pos
		controller:checkCollisions(allControllers, true)
	end
end

local function updateGlacier(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)

	if phase.id == "anchor" or phase.id == "glacier" then
		controller.velocity = Vector3.zero
	elseif phase.id == "shatter" then
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	local handler = PHASE_START_BY_MODE[move.mode]
	if handler then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local handler = UPDATE_BY_MODE[move.mode]
	if handler then
