...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" then
			...
			SpecialVFX.barrierDome(controller, color, move.duration - phase.duration)
		elseif phase.id == "sustain" then
			controller.auroraTimer = 0
		elseif phase.id == "flare" then
			SpecialVFX.auroraFlare(controller.part.Position, phase.range or 8, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "sustain" then
			controller.velocity *= 0.85
			controller.auroraTimer = (controller.auroraTimer or 0) + dt
			if controller.auroraTimer >= (phase.interval or 0.38) then
				controller.auroraTimer = 0
