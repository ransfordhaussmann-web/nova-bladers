...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "EmberCoreMeltdown" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "EmberCoreMeltdown" then
		...
		elseif phase.id == "meltdown" then
			...
			if controller.meltdownTimer >= (phase.interval or 0.28) then
				controller.meltdownTimer = 0
				SpecialVFX.meltdownWave(controller.part.Position, phase.range or 7.5, move.color, folder)
