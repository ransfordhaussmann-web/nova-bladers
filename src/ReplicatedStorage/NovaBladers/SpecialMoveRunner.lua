	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "blink" then
			...
			if target and target.part then
				...
				behind = targetPos + Vector3.new(tFacing.X, 0, tFacing.Z).Unit * 4
			end
			...
			controller.facing = (targetPos - controller.part.Position)
			controller.facing = Vector3.new(controller.facing.X, 0, controller.facing.Z).Unit
		elseif phase.id == "cross" then
			SpecialVFX.phantomCrossSlash(controller.part.Position, color, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonFangRend" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.16) then
				controller.slashTimer = 0
				controller.slashHitsLeft = (controller.slashHitsLeft or 3) - 1
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "quake" then
			...
			if controller.quakeTimer >= (phase.interval or 0.3) then
				controller.quakeTimer = 0
				SpecialVFX.stoneQuake(controller.part.Position, phase.range or 9, move.color, folder)
