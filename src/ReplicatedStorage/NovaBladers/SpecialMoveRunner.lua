...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "brace" then
			...
			SpecialVFX.wallRing(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "spike" or phase.id == "fortress" then
			controller.pulseTimer = 0
		end
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "vanish" then
			...
			if dist > 0.5 then
				dir = dir.Unit
				local blinkTo = fromPos + dir * dist
				SpecialVFX.phaseBlink(fromPos, blinkTo, color, folder)
				controller.part.CFrame = CFrame.new(blinkTo, blinkTo + dir)
				controller.facing = dir
			end
			controller.velocity = Vector3.zero
		...
		elseif phase.id == "afterimage" then
			SpecialVFX.phaseBlink(controller.part.Position, controller.part.Position + controller.facing * 3, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "finisher" then
			controller.velocity = controller.facing * (move.rushSpeed or 85) * 0.5
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "flare" then
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				controller.solarTimer = 0
				local range = 3 + (controller.solarCount or 0) * 1.2
