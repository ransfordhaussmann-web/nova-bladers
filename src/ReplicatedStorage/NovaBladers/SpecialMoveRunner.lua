...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "spiral" then
			...
			controller.orbitSpeed = phase.orbitSpeed or 24
		...
		elseif phase.id == "fang" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.spiralTrail(controller.part.Position, targetPos, color, folder)
		end
	elseif move.id == "NeonPulseSurge" then
		...
		elseif phase.id == "blast" then
			SpecialVFX.neonBlast(controller.part.Position, phase.range or 11, color, folder)
			controller.blastDone = false
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			controller.orbitRadius = math.max(2.5, (controller.orbitRadius or 5) - 3 * dt)
			local center = controller.orbitCenter
			if controller.specialTarget and controller.specialTarget.part then
				center = controller.specialTarget.part.Position
				controller.orbitCenter = center
			...
			if controller.spiralTimer >= (phase.interval or 0.22) then
				controller.spiralTimer = 0
				SpecialVFX.spiralRing(controller.part.Position, phase.hitRadius or 5, move.color, folder)
	...
	elseif move.id == "NeonPulseSurge" then
		...
		elseif phase.id == "blast" and not controller.blastDone then
			controller.blastDone = true
			controller:areaHit(allControllers, phase.range or 11, phase.damage or 28, true)
		end
	end
