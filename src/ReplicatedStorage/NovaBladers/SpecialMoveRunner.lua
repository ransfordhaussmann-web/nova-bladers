...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.hitInterval or 0.14) then
				controller.shatterTimer = 0
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			...
			if controller.blazeTimer >= (phase.interval or 0.22) then
				...
				local range = 3.5 + controller.blazeCount * 1.2
				SpecialVFX.flameSpiral(controller.part.Position, range, move.color, folder)
