...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRendCascade" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "rush" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "cascade" then
			controller.cascadeHitsLeft = phase.hits or 4
			controller.cascadeTimer = 0
		end
	elseif move.id == "FrostShardMaelstrom" then
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendCascade" then
		...
		elseif phase.id == "cascade" then
			...
			controller.cascadeTimer = (controller.cascadeTimer or 0) + dt
			if controller.cascadeTimer >= (phase.hitInterval or 0.16) then
				controller.cascadeTimer = 0
				local pos = controller.part.Position
	...
	elseif move.id == "FrostShardMaelstrom" then
		...
		elseif phase.id == "shards" then
			...
			if controller.frostTimer >= (phase.interval or 0.26) then
				controller.frostTimer = 0
				controller.frostCount = (controller.frostCount or 0) + 1
				local range = 3.5 + controller.frostCount * 1.4
