...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostAegisSpiral" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendingClaw" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "rush" or phase.id == "rend" then
		...
		if phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.16) then
				controller.rendTimer = 0
				local pos = controller.part.Position
				SpecialVFX.fangSlash(controller.rendLastPos, pos, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
	...
	elseif move.id == "FrostAegisSpiral" then
		...
		elseif phase.id == "spiral" then
			...
			if controller.frostTimer >= (phase.interval or 0.3) then
				...
				local range = 4 + controller.frostCount * 1.2
