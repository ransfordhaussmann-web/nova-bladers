...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarCoronaLoop" then
		...
		elseif phase.id == "recover" then
			...
			SpecialVFX.spinRecoveryGlow(controller, color, phase.duration)
	...
	elseif move.id == "PhantomPhaseCut" then
		if phase.id == "phase" then
			...
			controller.guardReduction = 0.8
		elseif phase.id == "dash" then
			local targetPos = getTargetPos(controller, target)
			controller.dashStart = controller.part.Position
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperFang" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.interval or 0.2) then
				controller.slashTimer = 0
				controller.slashCount = (controller.slashCount or 0) + 1
				local angle = (controller.slashCount - 1) * 120
				SpecialVFX.slashArc(controller.part.Position, controller.facing, move.color, folder, angle)
	...
	elseif move.id == "SolarCoronaLoop" then
		if phase.id == "corona" then
			...
			if controller.coronaTimer >= (phase.interval or 0.22) then
				...
				local range = 3 + controller.coronaCount * 1.8
		...
		elseif phase.id == "drift" then
			controller.velocity = controller.facing * (phase.driftSpeed or move.driftSpeed or 68)
			controller:checkCollisions(allControllers, true)
		end
	...
	elseif move.id == "PhantomPhaseCut" then
		if phase.id == "dash" then
			...
			if controller.dashStart then
				SpecialVFX.ghostTrail(controller.dashStart, controller.part.Position, move.color, folder)
				controller.dashStart = controller.part.Position
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "cut" then
