...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFlameLance" then
		...
		elseif phase.id == "lance" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.flameLance(controller, color, folder)
	...
	elseif move.id == "GlacierAegis" then
		if phase.id == "freeze" then
			SpecialVFX.freezeBurst(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "aegis" then
			controller.guardReduction = move.damageReduction or 0.6
			SpecialVFX.aegisShield(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameLance" then
		if phase.id == "ignite" then
			controller.velocity = Vector3.zero
		elseif phase.id == "lance" or phase.id == "burn" then
		...
		if phase.id == "burn" then
			...
			if controller.burnTimer >= (phase.hitInterval or 0.2) then
				controller.burnTimer = 0
				local pos = controller.part.Position
				SpecialVFX.burnTrail(controller.burnLastPos, pos, move.color, folder)
	...
	elseif move.id == "GlacierAegis" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.3) then
				controller.shardTimer = 0
				SpecialVFX.iceShardPulse(controller.part.Position, phase.range or 7, move.color, folder)
