...
local function onPhaseStartEclipse(controller, move, phase, target)
...
local function onPhaseStartSpiral(controller, move, phase, target)
	if phase.id == "windup" then
		SpecialVFX.chargeAura(controller, move.color, phase.duration)
	elseif phase.id == "spiral" and target and target.part then
		...
		controller.orbitRadius = phase.orbitRadius or 9
		controller.orbitSpeed = phase.orbitSpeed or 14
...
local function onPhaseStartPrism(controller, move, phase)
	if phase.id == "freeze" then
		SpecialVFX.chargeAura(controller, move.color, phase.duration)
		controller.guardReduction = (move.damageReduction or 0.6) * 0.4
		controller.velocity *= 0.3
	elseif phase.id == "prism" then
		controller.guardReduction = move.damageReduction or 0.6
		SpecialVFX.frostPrism(controller, move.color, phase.duration)
...
local PHASE_START = {
	...
	spiral = onPhaseStartSpiral,
	prism = onPhaseStartPrism,
}
...
local function updateSpiral(controller, move, phase, dt, allControllers, folder)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	elseif phase.id == "spiral" and controller.orbitCenter then
		controller.orbitAngle += (controller.orbitSpeed or 14) * dt
		controller.orbitRadius = math.max(3, (controller.orbitRadius or 9) - 6 * dt)
		local center = controller.orbitCenter
		if controller.specialTarget and controller.specialTarget.part then
			center = controller.specialTarget.part.Position
			controller.orbitCenter = center
		...
		if controller.spiralTimer >= (phase.interval or 0.22) then
			controller.spiralTimer = 0
			SpecialVFX.spiralSlash(controller.part.Position, move.color, folder, controller.orbitAngle)
...
local function updatePrism(controller, move, phase, dt, allControllers, folder)
	if phase.id == "freeze" then
