...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "slash" then
			SpecialVFX.bladeSlash(controller.part.Position, controller.facing, color, folder)
		end
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "shatter" then
			SpecialVFX.stoneShatter(controller.part.Position, color, folder)
		end
	elseif move.id == "SolarFlareDrift" then
		if phase.id == "flare_up" then
			SpecialVFX.solarFlare(controller, color, phase.duration)
			controller.verticalVelocity = 14
			controller.airborne = true
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "edge_cut" then
			SpecialVFX.edgeCrossSlash(controller.part.Position, controller.facing, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "slash" then
			controller.velocity = controller.facing * (move.rushSpeed or 82) * 0.5
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			...
			if controller.surgeTimer >= dashInterval then
				...
				dir = Vector3.new(dir.X, 0, dir.Z).Unit
				controller.facing = dir
				controller.velocity = dir * (phase.rushSpeed or move.rushSpeed or 105)
