...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "anchor" then
		if phase.id == "tether" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
			controller.anchorTarget = getTargetPos(controller, target)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "anchor" then
		if phase.id == "tether" and controller.anchorTarget then
			local toTarget = controller.anchorTarget - controller.part.Position
			toTarget = Vector3.new(toTarget.X, 0, toTarget.Z)
			if toTarget.Magnitude > 1 then
				controller.velocity = toTarget.Unit * (phase.pullSpeed or 28)
