...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRendingArc" then
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.slashSweepAngle = phase.sweepAngle or 140
			controller.slashStartFacing = controller.facing
		elseif phase.id == "cleave" then
			SpecialVFX.slashCleave(controller.part.Position, controller.facing, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendingArc" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "slash" then
			local sweep = controller.slashSweepAngle or 140
			local startAngle = math.atan2(controller.slashStartFacing.Z, controller.slashStartFacing.X)
			local elapsed = phase.duration - (controller.specialPhaseEnd - now)
			local t = math.clamp(elapsed / phase.duration, 0, 1)
			local angle = startAngle + math.rad(sweep) * t
			controller.facing = Vector3.new(math.cos(angle), 0, math.sin(angle))
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			SpecialVFX.slashTrail(controller.part.Position, controller.facing, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "cleave" then
	...
	elseif move.id == "GlacialFrostBarrier" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.25) then
				controller.shardTimer = 0
