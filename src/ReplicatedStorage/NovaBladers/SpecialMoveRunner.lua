...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "orbit" and target and target.part then
			...
			controller.spin = math.min(BeyConfig.MAX_SPIN, controller.spin + (move.spinHeal or 0))
		end
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "feint" then
			local side = controller.facing:Cross(Vector3.new(0, 1, 0))
			if math.random() > 0.5 then
				side = -side
			end
			controller.facing = side.Unit
			controller.velocity = side * (phase.sideSpeed or 55)
		elseif phase.id == "blade" then
			local targetPos = getTargetPos(controller, target)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
...
function SpecialMoveRunner.endMove(controller)
	...
	controller.underground = false
	controller.crumblePending = false
	controller.bladeHitDone = false
	SpecialVFX.setUnderground(controller, false)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.14) then
				controller.ripperTimer = 0
				local pos = controller.part.Position
				SpecialVFX.slashArc(pos, controller.facing, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 4.8, phase.damage or 10, true)
			end
		end

	elseif move.id == "GraniteBastion" then
		if phase.id == "settle" or phase.id == "bastion" then
			controller.velocity = Vector3.zero
		end
		...
		elseif phase.id == "crumble" then
			controller.velocity = Vector3.zero
			if controller.crumblePending then
				controller.crumblePending = false
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "feint" then
			controller.velocity = controller.facing * (phase.sideSpeed or 55)
		elseif phase.id == "blade" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 90)
			controller:checkCollisions(allControllers, true)
			if not controller.bladeHitDone then
				controller.bladeHitDone = true
