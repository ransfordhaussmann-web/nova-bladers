...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipper" then
		...
		elseif phase.id == "rush" or phase.id == "rip" then
		...
		if phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.14) then
				...
				local pos = controller.part.Position
				SpecialVFX.slashRip(pos, controller.facing, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 10, true)
			end
		end

	elseif move.id == "GlacierBind" then
		if phase.id == "frost" or phase.id == "shell" then
			controller.velocity = Vector3.zero
		elseif phase.id == "bind" then
			controller.bindTimer = (controller.bindTimer or 0) + dt
			if controller.bindTimer >= (phase.interval or 0.32) then
				controller.bindTimer = 0
				SpecialVFX.bindPulse(controller.part.Position, phase.range or 7.5, move.color, folder)
