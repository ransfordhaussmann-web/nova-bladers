...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "anchor" then
			SpecialVFX.anchorDrop(controller, color)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "rend" then
			SpecialVFX.crimsonSlash(controller.part.Position, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "anchor" or phase.id == "tide" then
			controller.velocity = Vector3.zero
		elseif phase.id == "crash" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "flare" then
			...
			if controller.sonicTimer >= (phase.interval or 0.26) then
				controller.sonicTimer = 0
				controller.sonicCount = (controller.sonicCount or 0) + 1
				local range = 4 + controller.sonicCount * 1.6
