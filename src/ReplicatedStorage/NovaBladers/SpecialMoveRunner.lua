...
local function onEclipsePhaseStart(controller, move, phase, color, target)
...
local function onBlazePhaseStart(controller, move, phase, color, target)
	if phase.id == "windup" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
	elseif phase.id == "launch" then
		local dir = (getTargetPos(controller, target) - controller.part.Position)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "eclipse" then
		onEclipsePhaseStart(controller, move, phase, color, target)
	elseif mode == "blaze" then
		onBlazePhaseStart(controller, move, phase, color, target)
	end
end
...
local function updateBlaze(controller, move, phase, dt, allControllers, folder)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	elseif phase.id == "launch" or phase.id == "inferno" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 80)
	end
	if phase.id == "inferno" then
		controller.meteorTimer = (controller.meteorTimer or 0) + dt
		if controller.meteorTimer >= (phase.hitInterval or 0.2) then
			controller.meteorTimer = 0
			local pos = controller.part.Position
			SpecialVFX.fireTrail(controller.meteorLastPos, pos, move.color, folder)
			SpecialVFX.fireImpact(pos, move.color, folder)
			controller.meteorLastPos = pos
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "eclipse" then
		updateEclipse(controller, move, phase, dt, allControllers)
	elseif mode == "blaze" then
		updateBlaze(controller, move, phase, dt, allControllers, folder)
	end
