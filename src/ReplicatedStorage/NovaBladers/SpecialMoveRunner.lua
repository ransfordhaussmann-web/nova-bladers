...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "slam" then
		...
		elseif phase.id == "leap" then
			controller.verticalVelocity = phase.leapSpeed or 42
			controller.airborne = true
			SpecialVFX.slamLeap(controller, color, folder)
		elseif phase.id == "crash" then
			controller.verticalVelocity = -(phase.crashSpeed or 55)
	...
	elseif mode == "phase" then
		...
		elseif phase.id == "slash" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.phaseSlash(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
	...
	elseif mode == "slam" then
		...
		elseif phase.id == "crash" then
			...
			if controller.part.Position.Y <= controller.floorY + 1.5 and controller.verticalVelocity <= 0 then
				SpecialVFX.slamImpact(controller.part.Position, phase.range or 7, move.color, folder)
				controller:areaHit(allControllers, phase.range or 7, phase.damage or 38, true)
				controller.verticalVelocity = 0
				controller.airborne = false
				controller.landingSlam = false
			end
		end
