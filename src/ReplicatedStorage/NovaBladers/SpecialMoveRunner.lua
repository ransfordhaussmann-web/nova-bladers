...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonVortexRush" then
		...
		elseif phase.id == "vortex" then
			controller.vortexHitsLeft = phase.hits or 5
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonVortexRush" then
		...
		elseif phase.id == "vortex" then
			...
			controller.vortexAngle = (controller.vortexAngle or 0) + dt * 14
			local orbitR = 2.5
			local offset = Vector3.new(math.cos(controller.vortexAngle) * orbitR, 0, math.sin(controller.vortexAngle) * orbitR)
			controller.velocity = controller.facing * 30 + offset * 8
			if controller.vortexTimer >= (phase.hitInterval or 0.15) then
				controller.vortexTimer = 0
				SpecialVFX.vortexBlade(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 10, true)
			end
		end

	elseif move.id == "GlacierFrostLock" then
		if phase.id == "frost" or phase.id == "ring" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
