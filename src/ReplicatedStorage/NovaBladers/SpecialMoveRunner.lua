...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFlameBarrage" then
		...
		elseif phase.id == "barrage" then
			controller.flameHitsLeft = phase.hits or 5
	...
	elseif move.id == "FrostCrownLock" then
		if phase.id == "chill" then
			SpecialVFX.iceChill(controller, color, phase.duration)
			controller.velocity = Vector3.zero
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameBarrage" then
		...
		if phase.id == "barrage" then
			...
			if controller.flameTimer >= (phase.hitInterval or 0.15) then
				controller.flameTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameTrail(controller.flameLastPos, pos, move.color, folder)
				SpecialVFX.flameBurst(pos, move.color, folder)
	...
	elseif move.id == "FrostCrownLock" then
		if phase.id == "chill" then
			controller.velocity = Vector3.zero
			controller:areaHit(allControllers, 7, 6, true)
		elseif phase.id == "crown" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
