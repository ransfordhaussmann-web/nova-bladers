...
local PHASE_START = {
	...
	barrage = function(controller, move, phase, target)
		...
		elseif phase.id == "barrage" then
			...
			controller.barrageHitsLeft = phase.hits or 6
		end
	end,
	...
	corona = function(controller, move, phase, target)
		...
		elseif phase.id == "flare" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
	...
	mirage = function(controller, move, phase, target)
		...
		elseif phase.id == "dash" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.mirageAfterimage(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "cut" then
			SpecialVFX.spectralCut(controller.part.Position, controller.facing, color, folder)
		end
	end,
	...
	frost = function(controller, move, phase)
		...
		elseif phase.id == "freeze" then
			SpecialVFX.frostRing(controller.part.Position, phase.range or 6, color, folder)
	...
	surge = function(controller, move, phase, target)
		...
		if phase.id == "boost" then
			SpecialVFX.surgeBoost(controller, color, phase.duration)
		...
		elseif phase.id == "impact" then
			SpecialVFX.surgeImpact(controller.part.Position, color, SpecialVFX.ensureFolder(controller))
		end
	end,
}
...
local PHASE_UPDATE = {
	...
	barrage = function(controller, move, phase, dt, allControllers)
		...
		if phase.id == "barrage" then
			...
			if controller.barrageTimer >= (phase.hitInterval or 0.12) then
				controller.barrageTimer = 0
	...
	rampart = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "quake" then
			...
			if controller.quakeTimer >= (phase.interval or 0.35) then
				controller.quakeTimer = 0
				SpecialVFX.quakeWave(controller.part.Position, phase.range or 9, move.color, folder)
	...
	corona = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "flare" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarFlare(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end
	end,
	...
	frost = function(controller, move, phase, dt, allControllers)
		if phase.id == "chill" then
	...
	surge = function(controller, move, phase, dt, allControllers)
		...
		if phase.id == "surge" then
			...
			SpecialVFX.surgeTrail(controller.part.Position, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "impact" then
