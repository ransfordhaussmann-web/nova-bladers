...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRendingClaw" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
	...
	elseif move.id == "FrostCrownGlaciate" then
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendingClaw" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.22) then
				...
				local angle = (3 - (controller.slashHitsLeft or 0)) * math.rad(120)
				SpecialVFX.clawSlash(controller.part.Position, controller.facing, angle, move.color, folder)
	...
	elseif move.id == "FrostCrownGlaciate" then
		...
		elseif phase.id == "shard" then
			...
			if controller.shardTimer >= (phase.interval or 0.33) then
				controller.shardTimer = 0
