...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "slam" then
			...
			if not controller.slamLanded and controller.verticalVelocity <= 0 and controller.inBowl then
	...
	elseif move.id == "TidalSurgeSpiral" then
		...
		elseif phase.id == "surge" then
			...
			if controller.surgeTimer >= (phase.interval or 0.25) then
				...
				local range = 3.5 + controller.surgeCount * 1.8
				SpecialVFX.sonicRing(controller.part.Position, range, move.color, folder)
