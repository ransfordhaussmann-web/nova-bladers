...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "slam" then
		if phase.id == "rise" then
			...
			controller.airborne = true
			controller.velocity = Vector3.zero
		elseif phase.id == "hang" then
			controller.verticalVelocity = 0
			controller.velocity = Vector3.zero
		elseif phase.id == "crash" then
			aimAtTarget(controller, target, phase.slamSpeed or 50)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if mode == "meteor" or mode == "feather" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "launch" or phase.id == "scatter" or phase.id == "shower" or phase.id == "storm" then
	...
	elseif mode == "slam" then
		if phase.id == "rise" or phase.id == "hang" then
			controller.velocity = Vector3.zero
		elseif phase.id == "crash" then
