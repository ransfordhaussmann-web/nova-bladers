...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonCrossSlash" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.interval or 0.14) then
				controller.slashTimer = 0
				controller.slashCount = (controller.slashCount or 0) + 1
				SpecialVFX.crossSlash(controller.part.Position, controller.facing, move.color, folder, controller.slashCount)
