...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "InfernoSawRush" then
		...
		elseif phase.id == "rush" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			local pos = controller.part.Position
			SpecialVFX.meteorTrail(controller.infernoLastPos or pos, pos, move.color, folder)
			controller.infernoLastPos = pos
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "saw" then
			controller.velocity = Vector3.zero
			controller.sawTimer = (controller.sawTimer or 0) + dt
			if controller.sawTimer >= (phase.interval or 0.22) then
				controller.sawTimer = 0
				SpecialVFX.sonicRing(controller.part.Position, phase.range or 5.5, move.color, folder)
