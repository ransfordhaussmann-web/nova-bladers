...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CometTailSlash" then
		...
		elseif phase.id == "slash" then
			controller.cometHitsLeft = phase.hits or 3
			controller.cometTimer = 0
		end
	elseif move.id == "GlacialLockdown" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CometTailSlash" then
		...
		if phase.id == "slash" then
			...
			if controller.cometTimer >= (phase.hitInterval or 0.2) then
				controller.cometTimer = 0
				local pos = controller.part.Position
				SpecialVFX.cometSlash(pos, move.color, folder)
