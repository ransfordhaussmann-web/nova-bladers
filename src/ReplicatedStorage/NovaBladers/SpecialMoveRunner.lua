...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "tidal" then
		if phase.id == "anchor" then
			SpecialVFX.tidalAnchor(controller, color, phase.duration)
			controller.velocity = Vector3.zero
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lance" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "volley" then
			controller.velocity = controller.facing * (move.rushSpeed or 65) * 0.4
			controller.lanceTimer = (controller.lanceTimer or 0) + dt
			if controller.lanceTimer >= (phase.hitInterval or 0.15) then
				controller.lanceTimer = 0
				local pos = controller.part.Position + controller.facing * 3
				SpecialVFX.emberLance(pos, controller.facing, move.color, folder)
		...
		elseif phase.id == "finisher" then
			...
			controller:checkCollisions(allControllers, true)
			if phase.range then
				controller:areaHit(allControllers, phase.range, phase.damage or 28, true)
			end
		end

	elseif mode == "tidal" then
		if phase.id == "anchor" or phase.id == "lock" then
			controller.velocity = Vector3.zero
		elseif phase.id == "surge" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
