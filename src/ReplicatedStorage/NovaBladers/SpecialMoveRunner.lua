...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "mirage" then
		if phase.id == "vanish" then
			SpecialVFX.phantomClone(controller, color, folder)
		...
		elseif phase.id == "mirage" then
			...
			controller.mirageDashesLeft = phase.dashCount or 2
			controller.mirageDashTimer = 0
			controller.mirageFromPos = controller.part.Position
		elseif phase.id == "storm" then
			SpecialVFX.bladeStorm(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lunge" then
		...
		if phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.15) then
				controller.ripperTimer = 0
				local pos = controller.part.Position
				SpecialVFX.lungeSlash(pos, controller.facing, move.color, folder)
	...
	elseif mode == "mirage" then
		if phase.id == "vanish" then
			controller.velocity = Vector3.zero
		elseif phase.id == "mirage" then
			controller.mirageDashTimer = (controller.mirageDashTimer or 0) + dt
			if controller.mirageDashTimer >= 0.22 then
				controller.mirageDashTimer = 0
				local targetPos = getTargetPos(controller, target)
				local fromPos = controller.part.Position
				SpecialVFX.mirageDash(fromPos, targetPos, move.color, folder)
				...
				controller.part.CFrame = CFrame.new(
					...
					Vector3.new(targetPos.X, controller.part.Position.Y, targetPos.Z) + dir
				)
				controller:checkCollisions(allControllers, true)
				controller.mirageDashesLeft = (controller.mirageDashesLeft or 1) - 1
				if controller.mirageDashesLeft <= 0 then
