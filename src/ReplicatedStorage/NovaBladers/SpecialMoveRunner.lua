...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "TideAnchorCrash" then
		...
		elseif phase.id == "pull" then
			...
			if controller.pullTimer >= (phase.interval or 0.2) then
				controller.pullTimer = 0
				SpecialVFX.pulseWave(controller.part.Position, phase.range or 10, move.color, folder)
				controller:areaHit(allControllers, phase.range or 10, phase.damage or 6, true)
			end
		elseif phase.id == "crash" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
			controller:checkCollisions(allControllers, true)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "slash" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.12) then
				controller.rendTimer = 0
				SpecialVFX.slashBurst(controller.part.Position, move.color, folder)
				controller:checkCollisions(allControllers, true)
				controller:areaHit(allControllers, 3.5, phase.damage or 9, true)
			end
		elseif phase.id == "finish" then
