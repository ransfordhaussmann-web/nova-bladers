...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeSpiralDrive" then
		...
		elseif phase.id == "spiral" then
			...
			controller.spiralRadius = (controller.part.Position - targetPos).Magnitude
			controller.spiralRadius = math.clamp(controller.spiralRadius, 4, 14)
			controller.spiralTimer = 0
		elseif phase.id == "ignite" then
			SpecialVFX.blazeIgnite(controller.part.Position, color, folder)
		end
	elseif move.id == "FrostTidalBind" then
		if phase.id == "anchor" then
			controller.guardReduction = move.damageReduction or 0.45
			SpecialVFX.frostAnchorRing(controller, color, phase.duration)
			controller.velocity = Vector3.zero
		elseif phase.id == "tidal" then
			controller.tidalTimer = 0
		elseif phase.id == "bind" then
			SpecialVFX.frostBindBurst(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeSpiralDrive" then
		...
		elseif phase.id == "spiral" and controller.spiralCenter then
			...
			controller.spiralAngle += 9 * dt
			local shrink = 1 - controller.spiralTimer / (phase.duration or 0.85)
			local r = math.max(2, (controller.spiralRadius or 8) * shrink)
	...
	elseif move.id == "FrostTidalBind" then
		...
		elseif phase.id == "tidal" then
			...
			if controller.tidalTimer >= (phase.interval or 0.3) then
				controller.tidalTimer = 0
				SpecialVFX.frostWave(controller.part.Position, phase.range or 7.5, move.color, folder)
