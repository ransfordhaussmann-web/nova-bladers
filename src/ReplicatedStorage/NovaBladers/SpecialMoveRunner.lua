...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrownShatter" then
		...
		elseif phase.id == "shatter" then
			SpecialVFX.frostShatter(controller.part.Position, phase.range or 7, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoRoar" then
		...
		elseif phase.id == "rush" or phase.id == "burn" then
		...
		if phase.id == "burn" then
			...
			if controller.burnTimer >= (phase.interval or 0.22) then
				controller.burnTimer = 0
				local pos = controller.part.Position
				SpecialVFX.fireBurst(pos, move.color, folder)
	...
	elseif move.id == "FrostCrownShatter" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.25) then
				...
				local angle = controller.shardCount * (math.pi * 2 / (phase.shardCount or 6))
				local dir = Vector3.new(math.cos(angle), 0, math.sin(angle))
				local pos = controller.part.Position + dir * 3
				SpecialVFX.iceShard(controller.part.Position, pos, move.color, folder)
