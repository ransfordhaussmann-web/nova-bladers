...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" then
			controller.velocity = Vector3.zero
			SpecialVFX.domeRaise(controller, color, phase.duration)
		...
		elseif phase.id == "release" then
			...
			SpecialVFX.domeRelease(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "shield" then
			...
			if controller.domePulseTimer >= (phase.interval or 0.35) then
				controller.domePulseTimer = 0
				SpecialVFX.domePulse(controller.part.Position, phase.range or 7, move.color, folder)
