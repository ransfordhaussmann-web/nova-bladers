...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "raise" then
			SpecialVFX.rampartWall(controller, color, phase.duration + 0.7)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.slashDashesLeft = phase.dashes or 2
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRend" then
		...
		if phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.16) then
				controller.rendTimer = 0
				local pos = controller.part.Position
				SpecialVFX.rendSplash(pos, move.color, folder)
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "slash" then
			...
			controller.slashDashTimer = (controller.slashDashTimer or 0) + dt
			if controller.slashDashTimer >= 0.18 then
				controller.slashDashTimer = 0
				SpecialVFX.slashArc(controller.part.Position, controller.facing, move.color, folder)
				controller:checkCollisions(allControllers, true)
			end
		elseif phase.id == "echo" then
