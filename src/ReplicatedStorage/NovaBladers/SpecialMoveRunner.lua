...
local PHASE_START = {
	...
	lunge = function(controller, move, phase, folder, target)
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, move.color, phase.duration)
		elseif phase.id == "lunge" or phase.id == "finish" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
	...
	solar = function(controller, move, phase, folder, target)
		...
		elseif phase.id == "orbit" then
			local center = target and target.part and target.part.Position or controller.part.Position
	...
	mirage = function(controller, move, phase, folder, target)
		if phase.id == "split" then
			SpecialVFX.mirageGhosts(controller, move.color, folder)
		elseif phase.id == "rush" or phase.id == "converge" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.diveTrail(controller, targetPos, move.color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
...
local PHASE_UPDATE = {
	...
	bastion = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "collapse" then
			controller:areaHit(allControllers, phase.range or 9, phase.damage or 20, true)
		end
	end,
	solar = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "flare" then
			...
			if controller.solarTimer >= (phase.interval or 0.3) then
				controller.solarTimer = 0
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarFlare(controller.part.Position, 3, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end
	end,
	mirage = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "rush" or phase.id == "converge" then
