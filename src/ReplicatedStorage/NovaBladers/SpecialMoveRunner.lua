...
MODE_HANDLERS.bastion = MODE_HANDLERS.fortress
...
MODE_HANDLERS.solar = {
	...
	update = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.solarFlare(pos, 3, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end
	end,
}
...
MODE_HANDLERS.lunge = {
	...
	update = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "lunge" then
			...
			SpecialVFX.lungeTrail(controller.lungeStartPos or controller.part.Position, controller.part.Position, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "rip" then
...
MODE_HANDLERS.mirage = {
	onPhaseStart = function(controller, move, phase)
		...
		if phase.id == "mirage" then
			SpecialVFX.mirageDecoys(controller, phase.decoys or 3, color, folder)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler and handler.onPhaseStart then
		handler.onPhaseStart(controller, move, phase)
	end
end
