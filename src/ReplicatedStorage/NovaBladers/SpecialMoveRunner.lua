...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "sink" then
			SpecialVFX.setUnderground(controller, true)
			SpecialVFX.tidalSplash(controller, color)
			controller.velocity = Vector3.zero
		elseif phase.id == "anchor" then
			SpecialVFX.setUnderground(controller, false)
			controller.guardReduction = move.damageReduction or 0.5
			SpecialVFX.anchorRing(controller, color, phase.duration)
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "phase" then
			SpecialVFX.phaseShift(controller, color, phase.duration)
		elseif phase.id == "slash" then
			SpecialVFX.endPhaseShift(controller)
			local targetPos = getTargetPos(controller, target)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "sink" then
			controller.velocity = Vector3.zero
			local pos = controller.part.Position
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, controller.floorY - 1.0, pos.Z))
				* (controller.part.CFrame - controller.part.CFrame.Position)
		elseif phase.id == "anchor" then
			controller.velocity = Vector3.zero
		elseif phase.id == "crash" then
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "burrow" then
			controller.velocity = Vector3.zero
			local pos = controller.part.Position
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, controller.floorY - 1.4, pos.Z))
				* (controller.part.CFrame - controller.part.CFrame.Position)
		elseif phase.id == "wall" then
			controller.velocity = Vector3.zero
		elseif phase.id == "pulse" then
