	...
	elseif move.id == "CrimsonVortexRush" then
		...
		elseif phase.id == "spiral" then
			...
			if controller.vortexTimer >= (phase.hitInterval or 0.2) then
				controller.vortexTimer = 0
				local pos = controller.part.Position
				local range = (phase.hitRadius or 5) + (controller.vortexHitsLeft or 4) * 0.4
				SpecialVFX.vortexSpiral(pos, range, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
				controller.vortexHitsLeft = (controller.vortexHitsLeft or 4) - 1
