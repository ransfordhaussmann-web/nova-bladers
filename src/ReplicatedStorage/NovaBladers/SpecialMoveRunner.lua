...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" then
			...
			SpecialVFX.auroraDome(controller, move.color, move.accentColor, phase.duration + 0.9)
			controller.velocity = Vector3.zero
		elseif phase.id == "sustain" then
			controller.velocity = Vector3.zero
		elseif phase.id == "pulse" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.interval or 0.16) then
				controller.ripTimer = 0
				controller.ripHitsLeft = (controller.ripHitsLeft or 1) - 1
				local angle = math.random() * math.pi * 2
				local slashDir = Vector3.new(math.cos(angle), 0, math.sin(angle))
				SpecialVFX.ripperSlash(controller.part.Position, slashDir, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 12, true)
			end
		end

	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" or phase.id == "sustain" then
			controller.velocity = Vector3.zero
		elseif phase.id == "pulse" then
			controller.auroraPulseTimer = (controller.auroraPulseTimer or 0) + dt
			if controller.auroraPulseTimer >= (phase.interval or 0.3) then
				controller.auroraPulseTimer = 0
				local accent = move.accentColor or move.color
				SpecialVFX.auroraPulse(controller.part.Position, phase.range or 9, move.color, accent, folder)
