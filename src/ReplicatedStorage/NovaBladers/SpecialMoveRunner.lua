...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		...
		elseif phase.id == "crash" then
			...
			SpecialVFX.diveTrail(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, -0.35, dir.Z).Unit
			controller.facing = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.velocity = dir * (phase.rushSpeed or 55)
			controller.verticalVelocity = -(phase.diveSpeed or 35)
			controller.airborne = true
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "burst" then
			SpecialVFX.sonicRing(controller.part.Position, phase.range or 6, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "roots" or phase.id == "rampart" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
