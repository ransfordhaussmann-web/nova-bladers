...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.fangLungeTrail(controller, color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "raise" then
			...
			SpecialVFX.auroraDome(controller, color, phase.duration + (move.phases[2] and move.phases[2].duration or 0.7))
		elseif phase.id == "hold" then
			controller.pulseTimer = 0
		elseif phase.id == "shatter" then
			controller.guardReduction = 0
			SpecialVFX.auroraShatter(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
