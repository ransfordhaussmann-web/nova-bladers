	...
	elseif move.id == "BlazeSpiral" then
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			if controller.spiralTimer >= (phase.interval or 0.22) then
				controller.spiralTimer = 0
				SpecialVFX.fireSpiral(controller.part.Position, phase.range or 5.5, move.color, folder)
		...
		elseif phase.id == "eruption" then
			...
			controller:checkCollisions(allControllers, true)
			if not controller._eruptionVFX then
				controller._eruptionVFX = true
				SpecialVFX.blazeEruption(controller.part.Position, move.color, folder)
			end
		end
	...
	elseif move.id == "GlacierHold" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.glacierBurst(controller.part.Position, move.color, folder)
