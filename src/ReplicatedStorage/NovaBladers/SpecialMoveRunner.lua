...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "fortress" or mode == "frost" then
		if phase.id == "burrow" or phase.id == "freeze" then
			if mode == "frost" then
				SpecialVFX.frostCrystal(controller, color)
			else
		...
		elseif phase.id == "pulse" or phase.id == "shards" then
			controller.pulseTimer = 0
		end
	elseif mode == "sonic" or mode == "spiral" then
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "sonic" or phase.id == "nova" then
			controller.sonicTimer = 0
			controller.sonicCount = 0
		elseif phase.id == "orbit" or phase.id == "spiral" then
			if target and target.part then
	...
	elseif mode == "spiral" then
		...
		elseif phase.id == "nova" then
			...
			if controller.sonicTimer >= (phase.interval or 0.25) then
				controller.sonicTimer = 0
				controller.sonicCount = (controller.sonicCount or 0) + 1
				local range = 3 + controller.sonicCount * 1.8
				SpecialVFX.solarNova(controller.part.Position, range, move.color, folder)
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			local shrink = controller.spiralShrink or 0.12
			controller.orbitRadius = math.max(2.5, (controller.orbitRadius or 7) - shrink * dt * 10)
