...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		if phase.id == "windup" then
			SpecialVFX.crimsonCharge(controller, color, phase.duration)
	...
	elseif move.id == "AuroraBarrierDome" then
		if phase.id == "rise" then
			SpecialVFX.auroraRise(controller, color, move.accentColor, phase.duration)
			controller.verticalVelocity = 10
			controller.airborne = true
		elseif phase.id == "dome" then
			controller.guardReduction = move.damageReduction or 0.5
			SpecialVFX.auroraDome(controller, color, move.accentColor, phase.duration)
			controller.pulseTimer = 0
		elseif phase.id == "burst" then
			controller.guardReduction = 0
			SpecialVFX.auroraBurst(controller.part.Position, color, move.accentColor, folder)
		end
	end
end
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.auroraPulse(controller.part.Position, phase.range or 7, move.color, move.accentColor, folder)
