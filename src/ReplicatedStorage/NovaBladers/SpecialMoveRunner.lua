...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GlacierFrostLock" then
		if phase.id == "frost" then
			controller:areaStatus(allControllers, phase.range or 9, {
				slowDuration = phase.slowDuration or move.slowDuration or 2.5,
				slowMult = phase.slowMult or move.slowMult or 0.45,
			})
		elseif phase.id == "shield" then
		...
		elseif phase.id == "freeze" then
			...
			if controller.freezeTimer >= (phase.interval or 0.35) then
				...
				controller:areaStatus(allControllers, phase.range or 8, {
					damage = phase.damage or 14,
					spinLoss = BeyConfig.SPECIAL_SPIN_LOSS,
					isSpecial = true,
					freezeDuration = phase.freezeDuration or move.freezeDuration or 1.2,
				})
			end
		end
	end
