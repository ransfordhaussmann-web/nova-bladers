...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		...
		elseif phase.id == "rise" then
			controller.verticalVelocity = phase.liftSpeed or 22
			controller.airborne = true
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "slash1" or phase.id == "slash2" then
			...
			SpecialVFX.slashMark(fromPos, targetPos, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "TideAnchorCrash" then
		...
		elseif phase.id == "crash" then
			controller.velocity = Vector3.zero
			if controller.part.Position.Y <= controller.floorY + 0.5 and controller.verticalVelocity < -10 then
				SpecialVFX.waterSplash(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.range or 9, phase.slamDamage or 32, true)
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "quake" then
			...
			if controller.quakeTimer >= (phase.interval or 0.34) then
				controller.quakeTimer = 0
