...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "burst" then
			SpecialVFX.solarFlare(controller.part.Position, phase.range or 9, color, folder)
		end
	elseif mode == "mirage" then
		...
		elseif phase.id == "mirage" then
			controller.mirageStrikesLeft = phase.strikes or 3
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.15) then
				controller.ripTimer = 0
				SpecialVFX.ripSlash(controller.part.Position, move.color, folder)
	...
	elseif mode == "solar" then
		...
		elseif phase.id == "rings" then
			...
			if controller.solarTimer >= (phase.interval or 0.2) then
				...
				local range = 3 + controller.solarCount * 1.8
	...
	elseif mode == "mirage" then
		if phase.id == "mirage" then
			...
			if controller.mirageTimer >= (phase.interval or 0.18) then
				controller.mirageTimer = 0
				local targetPos = getTargetPos(controller, target)
				local offset = Vector3.new(math.random(-4, 4), 0, math.random(-4, 4))
				local strikePos = targetPos + offset
				SpecialVFX.phantomStrike(strikePos, move.color, folder)
				controller.part.CFrame = CFrame.new(strikePos, targetPos)
