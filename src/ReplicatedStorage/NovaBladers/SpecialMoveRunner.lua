...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "veil" then
		if phase.id == "veil" then
			SpecialVFX.veilBlur(controller, color, phase.duration)
		elseif phase.id == "slash" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.slashTrail(controller, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "finisher" then
			SpecialVFX.veilBurst(controller.part.Position, color, folder)
		end
	elseif mode == "inferno" then
		if phase.id == "crown" then
			SpecialVFX.infernoCrown(controller, color, phase.duration)
...
local function updateMeteor(controller, move, phase, dt, folder)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	...
	if phase.id == "shower" then
		...
		if controller.meteorTimer >= (phase.hitInterval or 0.18) then
			...
			controller:areaHit(controller._allControllers, phase.hitRadius or 5, phase.damage or 11, true)
		end
	end
end

local function updateFortress(controller, move, phase, dt, folder)
	if phase.id == "burrow" then
		controller.velocity = Vector3.zero
		local pos = controller.part.Position
	...
	elseif phase.id == "pulse" then
		...
		if controller.pulseTimer >= (phase.interval or 0.35) then
			...
			controller:areaHit(controller._allControllers, phase.range or 8, phase.damage or 13, true)
		end
	end
end

local function updateSonic(controller, move, phase, dt, folder)
	if phase.id == "charge" then
	...
	elseif phase.id == "sonic" then
		...
		if controller.sonicTimer >= (phase.interval or 0.28) then
			...
			controller:areaHit(controller._allControllers, range, phase.damage or 9, true)
		end
	elseif phase.id == "orbit" and controller.orbitCenter then
		...
		controller.velocity = Vector3.zero
		controller:checkCollisions(controller._allControllers, true)
	end
end

local function updateEclipse(controller, move, phase, dt)
	if phase.id == "dive" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		controller:checkCollisions(controller._allControllers, true)
	elseif phase.id == "burst" then
		controller:areaHit(controller._allControllers, phase.range or 6, phase.damage or 38, true)
	end
end

local function updateRiposte(controller, move, phase, dt, folder)
	if phase.id == "brace" then
		controller.velocity = Vector3.zero
	elseif phase.id == "lunge" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
	...
	elseif phase.id == "fang" then
		...
		if controller.fangTimer >= (phase.hitInterval or 0.15) then
			controller.fangTimer = 0
			local pos = controller.part.Position
			SpecialVFX.riposteStrike(pos, move.color, folder)
			controller:areaHit(controller._allControllers, phase.hitRadius or 4.5, phase.damage or 12, true)
		end
	end
end

local function updateVeil(controller, move, phase, dt)
	if phase.id == "veil" then
	...
	elseif phase.id == "slash" then
		...
		controller:checkCollisions(controller._allControllers, true)
	elseif phase.id == "finisher" then
		controller:areaHit(controller._allControllers, phase.range or 5.5, phase.damage or 32, true)
	end
end

local function updateInferno(controller, move, phase, dt)
	if phase.id == "crown" then
		controller.velocity *= 0.9
	elseif phase.id == "rush" then
		...
		controller:checkCollisions(controller._allControllers, true)
	elseif phase.id == "burst" then
		controller:areaHit(controller._allControllers, phase.range or 7, phase.damage or 36, true)
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if mode == "meteor" then
		updateMeteor(controller, move, phase, dt, folder)
	elseif mode == "fortress" then
		updateFortress(controller, move, phase, dt, folder)
	elseif mode == "sonic" then
		updateSonic(controller, move, phase, dt, folder)
	elseif mode == "eclipse" then
		updateEclipse(controller, move, phase, dt)
	elseif mode == "riposte" then
		updateRiposte(controller, move, phase, dt, folder)
	elseif mode == "veil" then
		updateVeil(controller, move, phase, dt)
	elseif mode == "inferno" then
		updateInferno(controller, move, phase, dt)
	end
