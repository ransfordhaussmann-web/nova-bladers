...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonInfernoClaw" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoClaw" then
		if phase.id == "ignite" then
			controller.velocity = Vector3.zero
		elseif phase.id == "rush" or phase.id == "claw" then
		...
		if phase.id == "claw" then
			...
			if controller.clawTimer >= (phase.hitInterval or 0.2) then
				controller.clawTimer = 0
				local pos = controller.part.Position
				SpecialVFX.infernoClaw(pos, controller.facing, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
