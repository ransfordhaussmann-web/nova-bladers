...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipTide" then
		...
		elseif phase.id == "tide" then
			controller.tideHitsLeft = phase.hits or 3
	...
	elseif move.id == "NeonPulseResonance" then
		if phase.id == "sync" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "pulse" then
			controller.pulseResonanceTimer = 0
			controller.pulseResonanceCount = 0
		elseif phase.id == "resonate" then
			SpecialVFX.neonResonanceBurst(
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipTide" then
		...
		elseif phase.id == "tide" then
			...
			if controller.tideTimer >= (phase.interval or 0.22) then
				controller.tideTimer = 0
	...
	elseif move.id == "NeonPulseResonance" then
		if phase.id == "sync" then
			controller.velocity *= 0.9
		elseif phase.id == "pulse" then
			controller.pulseResonanceTimer = (controller.pulseResonanceTimer or 0) + dt
			if controller.pulseResonanceTimer >= (phase.interval or 0.3) then
				controller.pulseResonanceTimer = 0
				controller.pulseResonanceCount = (controller.pulseResonanceCount or 0) + 1
				local range = (phase.range or 5) + controller.pulseResonanceCount * 1.2
				SpecialVFX.neonResonanceRing(
					controller.part.Position,
					range,
					move.color,
					move.accentColor,
