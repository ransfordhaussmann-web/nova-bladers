...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GlacierVaultLock" then
		if phase.id == "freeze" then
			SpecialVFX.frostBurst(controller, color, phase.duration)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "CrimsonRendBarrage" then
		...
		elseif phase.id == "barrage" then
			...
			if controller.barrageTimer >= (phase.hitInterval or 0.12) then
				controller.barrageTimer = 0
				local pos = controller.part.Position
				SpecialVFX.crimsonSlash(controller.barrageLastPos, pos, move.color, folder)
				SpecialVFX.rendImpact(pos, move.color, folder)
