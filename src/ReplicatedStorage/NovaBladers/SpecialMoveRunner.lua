...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			...
			controller.phantomDashTarget = getTargetPos(controller, target)
		elseif phase.id == "slash" then
			SpecialVFX.ghostSlash(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			controller.velocity = controller.velocity * 0.85
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				controller.cycloneTimer = 0
				controller.cycloneCount = (controller.cycloneCount or 0) + 1
				SpecialVFX.bladeCyclone(controller.part.Position, phase.range or 5, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			if controller.phantomDashTimer >= dashInterval then
				...
				local targetPos = controller.phantomDashTarget or getTargetPos(controller, target)
				local dir = (targetPos - controller.part.Position)
				dir = Vector3.new(dir.X, 0, dir.Z).Unit
				controller.facing = dir
				controller.velocity = dir * (phase.dashSpeed or move.rushSpeed or 88)
				SpecialVFX.phantomDash(controller, targetPos, color, folder)
				controller:checkCollisions(allControllers, true)
			end
		elseif phase.id == "slash" then
