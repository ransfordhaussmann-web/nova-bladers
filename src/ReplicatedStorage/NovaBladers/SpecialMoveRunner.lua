...
local PHASE_START = {
	...
	feather = function(controller, move, phase)
		local color = move.color
	...
	anchor = function(controller, move, phase)
		...
		elseif phase.id == "pull" then
			SpecialVFX.anchorChain(controller.part.Position, color, folder)
			controller.pullRange = move.pullRange or 14
		elseif phase.id == "crash" then
			controller.verticalVelocity = -40
			controller.airborne = true
		end
	end,
	rend = function(controller, move, phase, target)
		...
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id:sub(1, 5) == "slash" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
...
local PHASE_UPDATE = {
	...
	anchor = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "pull" then
			...
			local pullRange = controller.pullRange or 14
			for _, other in allControllers do
				if other ~= controller and other.alive and other.part then
					...
					if dist > 0.5 and dist <= pullRange then
						local pull = -flat.Unit * (phase.pullForce or 28) * dt
	...
	rend = function(controller, move, phase, dt, allControllers)
		if phase.id:sub(1, 5) == "slash" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
			if phase.range then
				controller:areaHit(allControllers, phase.range, phase.damage or 18, true)
			end
		end
	end,
