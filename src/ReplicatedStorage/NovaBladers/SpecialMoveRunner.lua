...
local function onMeteorPhaseStart(controller, move, phase, color, target)
...
local function onFortressPhaseStart(controller, move, phase, color)
...
local function onSonicPhaseStart(controller, move, phase, color, target)
...
local function onEclipsePhaseStart(controller, move, phase, color, target, folder)
...
local function onLungePhaseStart(controller, move, phase, color, target)
	if phase.id == "windup" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
	elseif phase.id == "dash" then
		...
		controller.facing = dir
		controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		controller.lungeLastPos = controller.part.Position
...
local function onBastionPhaseStart(controller, move, phase, color)
...
local function onSolarPhaseStart(controller, move, phase, color)
	if phase.id == "charge" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
	...
	elseif phase.id == "drift" then
		controller.driftBoost = phase.speedBoost or move.driftSpeedMult or 1.35
	end
end

local function onMiragePhaseStart(controller, move, phase, color, target, folder)
	if phase.id == "split" then
		SpecialVFX.mirageClone(controller, color, folder)
	elseif phase.id == "feint" then
		local targetPos = getTargetPos(controller, target)
		local dir = (targetPos - controller.part.Position)
		dir = Vector3.new(dir.X, 0, dir.Z).Unit
		controller.facing = dir
...
local PHASE_START = {
	meteor = onMeteorPhaseStart,
	fortress = onFortressPhaseStart,
	sonic = onSonicPhaseStart,
	eclipse = onEclipsePhaseStart,
	lunge = onLungePhaseStart,
	bastion = onBastionPhaseStart,
	solar = onSolarPhaseStart,
	mirage = onMiragePhaseStart,
}
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler then
		handler(controller, move, phase, color, target, folder)
	end
end
...
local function updateLunge(controller, move, phase, dt, allControllers, folder)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	elseif phase.id == "dash" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 88)
		local pos = controller.part.Position
		SpecialVFX.lungeTrail(controller.lungeLastPos or pos, pos, move.color, folder)
		controller.lungeLastPos = pos
		controller:checkCollisions(allControllers, true)
	elseif phase.id == "rip" then
...
local function updateBastion(controller, move, phase, dt, allControllers, folder)
	if phase.id == "root" or phase.id == "shield" then
		controller.velocity = Vector3.zero
	elseif phase.id == "counter" then
		controller.pulseTimer = (controller.pulseTimer or 0) + dt
		if controller.pulseTimer >= (phase.interval or 0.35) then
...
local function updateSolar(controller, move, phase, dt, allControllers, folder)
	...
	elseif phase.id == "drift" then
		local boost = controller.driftBoost or 1.35
		controller.velocity = controller.facing * BeyConfig.BASE_SPEED * boost
	end
end

local function updateMirage(controller, move, phase, dt, allControllers)
