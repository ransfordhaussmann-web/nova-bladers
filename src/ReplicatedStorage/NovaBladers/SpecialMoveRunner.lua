...
local function onMeteorPhaseStart(controller, move, phase, target)
...
local function onFortressPhaseStart(controller, move, phase)
	local color = move.color
...
local function onSonicPhaseStart(controller, move, phase, target)
	local color = move.color
...
local function onEclipsePhaseStart(controller, move, phase, target, folder)
	local color = move.color
...
local function onFangPhaseStart(controller, move, phase, target)
	local color = move.color
	if phase.id == "brace" then
		controller.guardReduction = move.damageReduction or 0.45
		controller.velocity = Vector3.zero
		SpecialVFX.wallRing(controller, color, phase.duration)
	elseif phase.id == "riposte" then
		controller.guardReduction = 0
		local dir = (getTargetPos(controller, target) - controller.part.Position)
		dir = Vector3.new(dir.X, 0, dir.Z).Unit
		controller.facing = dir
		controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
	elseif phase.id == "fangs" then
		controller.fangTimer = 0
		controller.fangHitsLeft = phase.hits or 3
	end
end

local function onInfernoPhaseStart(controller, move, phase, folder)
	local color = move.color
...
local PHASE_START = {
	meteor = function(controller, move, phase)
		onMeteorPhaseStart(controller, move, phase, controller.specialTarget)
	end,
	fortress = function(controller, move, phase)
		onFortressPhaseStart(controller, move, phase)
	end,
	sonic = function(controller, move, phase)
		onSonicPhaseStart(controller, move, phase, controller.specialTarget)
	end,
	eclipse = function(controller, move, phase)
		local folder = SpecialVFX.ensureFolder(controller)
		onEclipsePhaseStart(controller, move, phase, controller.specialTarget, folder)
	end,
	fang = function(controller, move, phase)
		onFangPhaseStart(controller, move, phase, controller.specialTarget)
	end,
	inferno = function(controller, move, phase)
		local folder = SpecialVFX.ensureFolder(controller)
		onInfernoPhaseStart(controller, move, phase, folder)
	end,
}
...
local function updateFang(controller, move, phase, dt, allControllers, folder)
	if phase.id == "brace" then
		controller.velocity = Vector3.zero
	elseif phase.id == "riposte" then
		...
		controller:checkCollisions(allControllers, true)
	elseif phase.id == "fangs" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70) * 0.6
		controller.fangTimer = (controller.fangTimer or 0) + dt
		if controller.fangTimer >= (phase.hitInterval or 0.16) then
			controller.fangTimer = 0
...
local function updateInferno(controller, move, phase, dt, allControllers, folder)
	if phase.id == "ignite" then
	...
	elseif phase.id == "blaze" then
		...
		if controller.blazeTimer >= (phase.hitInterval or 0.2) then
			controller.blazeTimer = 0
			local pos = controller.part.Position
			SpecialVFX.blazeTrail(pos - controller.facing * 2, pos, move.color, folder)
