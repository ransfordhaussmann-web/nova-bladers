...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			controller.domePart = SpecialVFX.auroraDome(controller, color, phase.duration)
			controller.pulseTimer = 0
...
function SpecialMoveRunner.endMove(controller)
	...
	controller.underground = false
	if controller.domePart and controller.domePart.Parent then
		controller.domePart:Destroy()
	end
	controller.domePart = nil
	SpecialVFX.setUnderground(controller, false)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.22) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.fangSlash(controller.ripLastPos, pos, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			if controller.domePart and controller.domePart.Parent then
				controller.domePart.CFrame = CFrame.new(controller.part.Position)
			end
		elseif phase.id == "shatter" then
