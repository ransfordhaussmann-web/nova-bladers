	...
	elseif move.id == "EmberCyclone" then
		...
		elseif phase.id == "spin" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.22) then
				controller.cycloneTimer = 0
				controller.cycloneRing = (controller.cycloneRing or 3) + (phase.ringGrowth or 1.2)
				local range = controller.cycloneRing
				SpecialVFX.fireCycloneRing(controller.part.Position, range, move.color, folder)
