...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "fang" then
		...
		elseif phase.id == "riposte" then
			controller.riposteHitsLeft = phase.hits or 5
			controller.riposteTimer = 0
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "fang" then
		...
		elseif phase.id == "riposte" then
			...
			controller.riposteTimer = (controller.riposteTimer or 0) + dt
			if controller.riposteTimer >= (phase.hitInterval or 0.12) then
				controller.riposteTimer = 0
