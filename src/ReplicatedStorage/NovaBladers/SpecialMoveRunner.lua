...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "root" then
			SpecialVFX.rockAura(controller, color, phase.duration)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "vanish" then
			SpecialVFX.phantomVanish(controller.part.Position, color, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			if controller.zigzagTimer >= (phase.zigzagInterval or 0.12) then
				controller.zigzagTimer = 0
				local sign = controller.zigzagSign or 1
				local angle = math.rad(phase.zigzagAngle or 55) * sign
				local flat = Vector3.new(controller.facing.X, 0, controller.facing.Z)
				local rotated = CFrame.fromAxisAngle(Vector3.yAxis, angle):VectorToWorldSpace(flat)
				controller.facing = rotated.Unit
				controller.zigzagSign = -sign
				SpecialVFX.zigzagSlash(controller.part.Position, controller.facing, move.color, folder)
			end
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				controller.solarTimer = 0
				SpecialVFX.solarBurn(controller.part.Position, move.color, folder)
				controller:checkCollisions(allControllers, true)
