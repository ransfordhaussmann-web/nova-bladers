...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "pull" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
			controller.tidePullActive = true
		elseif phase.id == "rise" then
			controller.tidePullActive = false
	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "blink" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			local blinkPos = controller.part.Position + dir * 8
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeFeatherStorm" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "launch" or phase.id == "volley" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
		end
		if phase.id == "volley" then
			controller.meteorTimer = (controller.meteorTimer or 0) + dt
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "pull" and controller.tidePullActive then
			for _, other in allControllers do
				if other ~= controller and other.part and not other.bursted then
					local diff = controller.part.Position - other.part.Position
					local flat = Vector3.new(diff.X, 0, diff.Z)
					local dist = flat.Magnitude
					if dist > 0.5 and dist <= (phase.range or 12) then
						other.velocity += flat.Unit * (phase.pullForce or 40) * dt
					end
				end
			end
