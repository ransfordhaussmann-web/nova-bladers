...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "fang" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.fangBurstPlayed = false
		end
	...
	elseif move.id == "CrimsonSpiralFang" then
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			if controller.spiralTimer >= (phase.interval or 0.22) then
				controller.spiralTimer = 0
				SpecialVFX.spiralSlash(controller.part.Position, center, move.color, folder)
				controller:areaHit(allControllers, phase.range or 5, phase.damage or 10, true)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "fang" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			if not controller.fangBurstPlayed then
				SpecialVFX.spiralFangBurst(controller.part.Position, move.color, folder)
				controller.fangBurstPlayed = true
			end
			controller:checkCollisions(allControllers, true)
		end
