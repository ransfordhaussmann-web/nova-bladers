...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FlameClawRampage" then
		if phase.id == "ember" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "pounce" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "claw" then
			controller.clawHitsLeft = phase.hits or 4
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "FlameClawRampage" then
		...
		elseif phase.id == "pounce" or phase.id == "claw" then
		...
		if phase.id == "claw" then
			...
			if controller.clawTimer >= (phase.hitInterval or 0.21) then
				controller.clawTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameClawSwipe(pos, controller.facing, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5.2, phase.damage or 10, true)
			end
		end

	elseif move.id == "GlacierCrownLock" then
		if phase.id == "chill" or phase.id == "crown" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shards" then
