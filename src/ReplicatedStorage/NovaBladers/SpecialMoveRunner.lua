...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "slash" then
		...
		elseif phase.id == "rend" then
			...
			controller.rendHitsLeft = phase.hits or 2
		end

	elseif mode == "phase" then
		if phase.id == "phase" then
			SpecialVFX.setPhaseGhost(controller, true, color)
			controller.velocity = Vector3.zero
		...
		elseif phase.id == "slash" then
			SpecialVFX.setPhaseGhost(controller, false, color)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "wave" then
		if phase.id == "surge" then
			...
			if controller.waveTimer >= (phase.interval or 0.2) then
				controller.waveTimer = 0
				SpecialVFX.tidalWave(controller.part.Position, phase.range or 6, move.color, folder)
