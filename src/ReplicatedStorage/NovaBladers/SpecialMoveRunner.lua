...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			if controller.lungeStartPos then
				SpecialVFX.ripperLungeTrail(controller.lungeStartPos, controller.part.Position, move.color, folder)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "ripper" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 88) * 0.35
