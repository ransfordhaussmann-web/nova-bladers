...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "rampart" then
			SpecialVFX.graniteRampart(controller, color, phase.duration)
		elseif phase.id == "tremor" then
			controller.tremorTimer = 0
		end
	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "reappear" then
			...
			SpecialVFX.phaseReappear(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "anchor" or phase.id == "rampart" then
			controller.velocity = Vector3.zero
		elseif phase.id == "tremor" then
			controller.tremorTimer = (controller.tremorTimer or 0) + dt
			if controller.tremorTimer >= (phase.interval or 0.3) then
				controller.tremorTimer = 0
				SpecialVFX.tremorWave(controller.part.Position, phase.range or 9, move.color, folder)
