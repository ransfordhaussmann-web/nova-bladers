...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeSpiralDrive" then
		...
		elseif phase.id == "eruption" then
			SpecialVFX.fireEruption(controller.part.Position, phase.range or 7, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeSpiralDrive" then
		...
		elseif phase.id == "spiral" and controller.spiralCenter then
			...
			controller.spiralRadius = math.max(2, controller.spiralRadius - dt * 5.5)
			...
			if controller.spiralTimer >= (phase.hitInterval or 0.22) then
				controller.spiralTimer = 0
				SpecialVFX.spiralFlame(pos, controller.spiralAngle, controller.spiralRadius, move.color, folder)
	...
	elseif move.id == "CrystalBloomLock" then
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.35) then
				controller.shardTimer = 0
				SpecialVFX.crystalShards(controller.part.Position, phase.range or 9, move.color, folder)
