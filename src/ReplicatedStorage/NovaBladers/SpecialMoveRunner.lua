	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.16) then
				controller.ripperTimer = 0
				controller.ripperHitsLeft = (controller.ripperHitsLeft or 3) - 1
				local angle = (controller.ripperHitsLeft or 0) * math.rad(60)
				SpecialVFX.ripperSlash(controller.part.Position, controller.facing, angle, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "reflect" then
			...
			if controller.reflectTimer >= (phase.interval or 0.3) then
				controller.reflectTimer = 0
				SpecialVFX.reflectPulse(controller.part.Position, phase.range or 7, move.color, folder)
