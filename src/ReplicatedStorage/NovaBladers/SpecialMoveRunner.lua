...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "root" then
			controller.velocity = Vector3.zero
			SpecialVFX.stoneWall(controller, color, phase.duration + 0.7)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "dash" then
			controller.zigzagIdx = 0
			controller.zigzagTotal = phase.zigzags or 3
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "vanish" then
			...
			SpecialVFX.mirageBurst(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.14) then
				controller.ripTimer = 0
				local pos = controller.part.Position
				SpecialVFX.bloodTrail(controller.ripLastPos, pos, move.color, folder)
				SpecialVFX.ripSlash(pos, move.color, folder)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "root" or phase.id == "reflect" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "drift" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			SpecialVFX.burnTrail(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "dash" then
			...
			if controller.zigzagTimer >= 0.16 then
				controller.zigzagTimer = 0
				controller.zigzagIdx = (controller.zigzagIdx or 0) + 1
				local side = (controller.zigzagIdx % 2 == 0) and 1 or -1
				local right = controller.facing:Cross(Vector3.new(0, 1, 0)).Unit
				controller.facing = (controller.facing + right * side * 0.6).Unit
