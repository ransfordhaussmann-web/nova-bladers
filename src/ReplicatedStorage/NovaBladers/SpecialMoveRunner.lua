...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "finisher" then
			SpecialVFX.rendSlash(controller.part.Position, color, folder)
		end
	elseif move.id == "GraniteRampart" then
		if phase.id == "rise" then
			SpecialVFX.stoneRise(controller, color)
			controller.velocity = Vector3.zero
		elseif phase.id == "wall" then
			controller.guardReduction = move.damageReduction or 0.6
			SpecialVFX.wallRing(controller, color, phase.duration)
		elseif phase.id == "boulder" then
			controller.pulseTimer = 0
		end
	...
	elseif move.id == "PhantomPhaseSlash" then
		...
		elseif phase.id == "slash" then
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.phaseTrail(controller.part.Position, targetPos, color, folder)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
