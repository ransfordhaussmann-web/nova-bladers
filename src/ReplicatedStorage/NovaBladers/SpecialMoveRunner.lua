...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "quake" then
			SpecialVFX.quakeBurst(controller.part.Position, phase.range or 11, color, folder)
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "quake" then
			controller.velocity = Vector3.zero
			if not controller.quakeHit then
				controller.quakeHit = true
				controller:areaHit(allControllers, phase.range or 11, phase.damage or 20, true)
			end
		end
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			controller.velocity = controller.facing * (move.rushSpeed or 62)
