...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrystalBastion" then
		...
		elseif phase.id == "bastion" then
			...
			SpecialVFX.crystalBastion(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonCrossSlash" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.18) then
				controller.slashTimer = 0
				controller.slashHitsLeft = (controller.slashHitsLeft or 3) - 1
				local angle = (3 - (controller.slashHitsLeft or 0)) * 90
				SpecialVFX.slashArc(controller.part.Position, controller.facing, angle, move.color, folder)
