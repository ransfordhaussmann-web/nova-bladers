...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "IronVaultLock" or move.id == "PolarOrbitLock" then
		if phase.id == "burrow" then
			if move.id == "PolarOrbitLock" then
	...
	elseif move.id == "CoreBlazeRush" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "spiral" then
			controller.spiralAngle = math.atan2(controller.facing.Z, controller.facing.X)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CoreBlazeRush" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "spiral" then
			controller.spiralAngle = (controller.spiralAngle or 0) + dt * 13
			controller.facing = Vector3.new(math.cos(controller.spiralAngle), 0, math.sin(controller.spiralAngle))
			local speed = phase.rushSpeed or move.rushSpeed or 70
			...
			if controller.spiralTimer >= (phase.hitInterval or 0.2) then
				controller.spiralTimer = 0
				local pos = controller.part.Position
				SpecialVFX.meteorTrail(pos - controller.facing * 2, pos, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 5.2, phase.damage or 10, true)
			end
		elseif phase.id == "finale" then
			controller.velocity *= 0.85
			controller:areaHit(allControllers, phase.range or 6.8, phase.damage or 30, true)
		end
	end
