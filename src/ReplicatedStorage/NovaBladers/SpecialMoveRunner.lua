...
local function setupOrbit(controller, move, target)
...
local PHASE_START = {
	...
	sonic = {
		...
		orbit = function(controller, move, _, target)
			setupOrbit(controller, move, target)
		end,
	},
	eclipse = {
	...
	lunge = {
		...
		ripper = function(controller)
			controller.ripperTimer = 0
		end,
	},
	bastion = {
		root = function(controller, move, phase)
			SpecialVFX.chargeAura(controller, move.color, phase.duration)
			controller.velocity = Vector3.zero
		end,
		...
		collapse = function(controller, move, phase)
			SpecialVFX.pulseWave(controller.part.Position, phase.range or 9, move.color, SpecialVFX.ensureFolder(controller))
		end,
	},
	solar = {
		charge = function(controller, move, phase)
	...
	mirage = {
		vanish = function(controller, move, phase)
			SpecialVFX.phantomVanish(controller, phase.duration)
		end,
		mirage = function(controller, move, phase, target)
			local targetPos = getTargetPos(controller, target)
			SpecialVFX.mirageClones(controller, targetPos, move.color, SpecialVFX.ensureFolder(controller))
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		...
		slash = function(controller, move)
			SpecialVFX.bladeSlash(controller.part.Position, controller.facing, move.color, SpecialVFX.ensureFolder(controller))
		end,
	},
}
...
local PHASE_UPDATE = {
	meteor = {
		...
		launch = function(controller, phase, move)
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
		end,
		shower = function(controller, dt, move, phase, allControllers, folder)
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
	...
	fortress = {
		...
		pulse = function(controller, dt, move, phase, allControllers, folder)
	...
	sonic = {
		...
		sonic = function(controller, dt, move, phase, allControllers, folder)
		...
		orbit = function(controller, dt, _, __, allControllers)
	...
	eclipse = {
		dive = function(controller, phase, move, _, allControllers)
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
		end,
		burst = function(controller, phase, _, _, allControllers)
	...
	lunge = {
		...
		lunge = function(controller, phase, move, _, allControllers, folder)
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 95)
			SpecialVFX.lungeTrail(controller, move.color, folder)
			controller:checkCollisions(allControllers, true)
		end,
		ripper = function(controller, dt, move, phase, allControllers, folder)
			controller.velocity = controller.facing * (move.rushSpeed or 70) * 0.6
			controller.ripperTimer = (controller.ripperTimer or 0) + dt
			if controller.ripperTimer >= (phase.hitInterval or 0.15) then
				controller.ripperTimer = 0
	...
	bastion = {
		...
		bastion = function(controller, dt, move, phase, allControllers, folder)
			controller.velocity = Vector3.zero
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
			if controller.pulseTimer >= (phase.interval or 0.4) then
				controller.pulseTimer = 0
		...
		collapse = function(controller, phase, _, _, allControllers)
	...
	solar = {
		...
		flare = function(controller, dt, move, phase, allControllers, folder)
	...
	mirage = {
		mirage = function(controller, phase, move, _, allControllers)
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 88)
			controller:checkCollisions(allControllers, true)
		end,
		slash = function(controller, phase, _, _, allControllers)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if modeHandlers then
		local handler = modeHandlers[phase.id]
		if handler then
			handler(controller, dt, move, phase, allControllers, folder)
		end
	end
