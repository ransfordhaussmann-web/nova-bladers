...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "SolarFlareDrift" then
		if phase.id == "drift" then
			...
			if controller.driftTimer >= (phase.interval or 0.25) then
				...
				controller:areaHit(allControllers, phase.trailRadius or 4, phase.damage or 8, true)
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "burst" then
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			if controller.surgeTimer >= 0.12 and (controller.surgeDashesLeft or 0) > 0 then
				controller.surgeTimer = 0
				controller.surgeDashesLeft -= 1
				local targetPos = controller.surgeTarget or controller.part.Position
				local start = controller.part.Position
				local dashPos = start:Lerp(targetPos, (3 - controller.surgeDashesLeft) / 3)
				SpecialVFX.phantomDash(start, dashPos, move.color, folder)
				controller.part.CFrame = CFrame.new(Vector3.new(dashPos.X, controller.part.Position.Y, dashPos.Z))
					* (controller.part.CFrame - controller.part.CFrame.Position)
				controller:checkCollisions(allControllers, true)
			end
		elseif phase.id == "afterimage" then
			controller.afterimageTimer = (controller.afterimageTimer or 0) + dt
			if controller.afterimageTimer >= (phase.interval or 0.2) then
				controller.afterimageTimer = 0
				SpecialVFX.afterimageStrike(controller.part.Position, move.color, folder)
