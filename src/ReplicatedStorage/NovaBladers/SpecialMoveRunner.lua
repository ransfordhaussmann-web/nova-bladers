...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "inferno" then
		if phase.id == "ignite" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "claw_rush" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "inferno_burst" then
	...
	elseif mode == "frost" then
		...
		elseif phase.id == "citadel" then
			...
			SpecialVFX.iceCitadelRing(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "inferno" then
		...
		elseif phase.id == "inferno_burst" then
			...
			if controller.infernoTimer >= (phase.hitInterval or 0.22) then
				controller.infernoTimer = 0
				local pos = controller.part.Position
				SpecialVFX.infernoClawSlash(pos, move.color, folder)
	...
	elseif mode == "frost" then
		if phase.id == "freeze" or phase.id == "citadel" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
