...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "raise" then
			controller.velocity = Vector3.zero
			SpecialVFX.stoneBarrier(controller, color, phase.duration + 0.7)
		...
		elseif phase.id == "slam" then
			...
			SpecialVFX.earthquakeSlam(controller.part.Position, phase.range or 9, color, folder)
		end
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "split" then
			for i = 1, phase.copies or 3 do
				local angle = (i / (phase.copies or 3)) * math.pi * 2
				local offset = Vector3.new(math.cos(angle) * 4, 0, math.sin(angle) * 4)
				SpecialVFX.phantomCopy(controller, color, offset)
			end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller:checkCollisions(allControllers, true)
			local pos = controller.part.Position
			SpecialVFX.crimsonSlash(controller.lungeLastPos or pos, pos, move.color, folder)
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.14) then
				...
				local pos = controller.part.Position
				SpecialVFX.crimsonSlash(pos - controller.facing * 2, pos + controller.facing * 2, move.color, folder)
