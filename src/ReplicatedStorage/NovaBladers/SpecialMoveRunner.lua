...
local PHASE_START = {
	meteor = function(controller, move, phase, folder, color, target)
	...
	fortress = function(controller, move, phase, folder, color, target)
	...
	sonic = function(controller, move, phase, folder, color, target)
	...
	eclipse = function(controller, move, phase, folder, color, target)
	...
	lunge = function(controller, move, phase, folder, color, target)
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.lungeTrail(controller.part.Position, targetPos, color, folder)
		elseif phase.id == "ripper" then
			controller.ripperTimer = 0
			controller.ripperHitsLeft = phase.hits or 5
		end
	end,
	bastion = function(controller, move, phase, folder, color, target)
	...
	solar = function(controller, move, phase, folder, color, target)
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		...
		elseif phase.id == "drift" then
			...
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed or 55)
		end
	end,
	mirage = function(controller, move, phase, folder, color, target)
		if phase.id == "split" then
			SpecialVFX.mirageClones(controller, color, folder)
		...
		elseif phase.id == "fade" then
			SpecialVFX.phantomFade(controller.part.Position, color, folder)
		end
	end,
}

local PHASE_UPDATE = {
	meteor = function(controller, move, phase, dt, allControllers, folder, color)
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
	...
	fortress = function(controller, move, phase, dt, allControllers, folder, color)
		if phase.id == "burrow" then
			controller.velocity = Vector3.zero
			local pos = controller.part.Position
	...
	sonic = function(controller, move, phase, dt, allControllers, folder, color)
		if phase.id == "charge" then
	...
	eclipse = function(controller, move, phase, dt, allControllers, folder, color)
	...
	lunge = function(controller, move, phase, dt, allControllers, folder, color)
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "lunge" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "ripper" then
			controller.velocity = controller.facing * (move.rushSpeed or 70) * 0.65
			controller.ripperTimer = (controller.ripperTimer or 0) + dt
			if controller.ripperTimer >= (phase.hitInterval or 0.13) then
				controller.ripperTimer = 0
				SpecialVFX.ripperSlash(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 4.5, phase.damage or 10, true)
			end
		end
	end,
	bastion = function(controller, move, phase, dt, allControllers, folder, color)
		if phase.id == "anchor" or phase.id == "bastion" then
			controller.velocity = Vector3.zero
		elseif phase.id == "slam" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
			if controller.pulseTimer >= 0.15 then
	...
	solar = function(controller, move, phase, dt, allControllers, folder, color)
		if phase.id == "charge" then
		...
		elseif phase.id == "flare" then
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				...
				local range = 3.5 + controller.solarCount * 1.2
		...
		elseif phase.id == "drift" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 55)
			controller:checkCollisions(allControllers, true)
		end
	end,
	mirage = function(controller, move, phase, dt, allControllers, folder, color)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if handler then
		handler(controller, move, phase, folder, color, target)
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	if handler then
		handler(controller, move, phase, dt, allControllers, folder, move.color)
	end
