...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "anchor" then
			SpecialVFX.chargeAura(controller, color, phase.duration * 0.6)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "mirage" then
			SpecialVFX.phantomMirage(controller, color, phase.cloneCount or 3, folder)
		...
		elseif phase.id == "feint" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed) * 0.85
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "rip" then
			controller.velocity = controller.facing * (move.rushSpeed or 95) * 0.7
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "feint" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 88) * 0.85
		elseif phase.id == "strike" then
