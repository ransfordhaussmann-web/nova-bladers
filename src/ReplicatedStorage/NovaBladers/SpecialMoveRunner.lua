...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonVortexRush" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "rush" or phase.id == "cyclone" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		end
		if phase.id == "cyclone" then
			controller.cycloneTimer = (controller.cycloneTimer or 0) + dt
			if controller.cycloneTimer >= (phase.hitInterval or 0.2) then
				controller.cycloneTimer = 0
				local pos = controller.part.Position
				SpecialVFX.vortexCyclone(pos, phase.hitRadius or 6, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 6, phase.damage or 10, true)
			end
			local pull = phase.pullStrength or move.pullStrength or 18
			for _, other in allControllers do
				if other ~= controller and other.alive and not other.underground then
					...
					if dist > 0.5 and dist <= (phase.hitRadius or 6) + 2 then
				...
				for _, other in allControllers do
					if other ~= controller and other.alive and not other.underground then
						...
						if dist <= (phase.range or 9) then
							...
							other.slowUntil = os.clock() + 1.4
							other.slowMult = move.slowMult or 0.45
						end
					end
				end
			end
