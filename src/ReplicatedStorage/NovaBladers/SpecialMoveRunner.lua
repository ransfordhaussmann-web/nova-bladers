...
local function aimAtTarget(controller, target, rushSpeed)
	...
	controller.velocity = dir * (rushSpeed or 70)
end
...
local PHASE_START = {
	...
	barrage = function(controller, move, phase, target)
		local color = move.color
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "dash" then
			...
			controller.barrageLastPos = controller.part.Position
		elseif phase.id == "finale" then
			controller.velocity = Vector3.zero
		end
	end,
	corona = function(controller, move, phase, target)
		local color = move.color
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
	...
	mirage = function(controller, move, phase, target)
		...
		if phase.id == "vanish" then
			SpecialVFX.setMirage(controller, true)
			controller.velocity = Vector3.zero
		elseif phase.id == "slash" then
			SpecialVFX.setMirage(controller, false)
			aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
			controller.mirageHitsLeft = phase.hits or 3
			controller.mirageTimer = 0
		elseif phase.id == "burst" then
			SpecialVFX.slashMark(controller.part.Position, color, folder)
		end
	end,
	...
	surge = function(controller, move, phase, target)
		local color = move.color
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
...
local PHASE_UPDATE = {
	...
	barrage = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "dash" then
			...
			if controller.barrageTimer >= (phase.hitInterval or 0.22) then
				controller.barrageTimer = 0
				local pos = controller.part.Position
				SpecialVFX.surgeTrail(controller.barrageLastPos or pos, pos, move.color, folder)
				controller.barrageLastPos = pos
				controller:checkCollisions(allControllers, true)
	...
	corona = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "charge" then
			controller.velocity *= 0.9
		elseif phase.id == "corona" then
			controller.coronaTimer = (controller.coronaTimer or 0) + dt
			if controller.coronaTimer >= (phase.interval or 0.25) then
				controller.coronaTimer = 0
				controller.coronaCount = (controller.coronaCount or 0) + 1
				local range = 3.5 + controller.coronaCount * 1.8
	...
	mirage = function(controller, move, phase, dt, allControllers)
		...
		if phase.id == "slash" then
			...
			controller.mirageTimer = (controller.mirageTimer or 0) + dt
			if controller.mirageTimer >= (phase.hitInterval or 0.25) then
				controller.mirageTimer = 0
				SpecialVFX.slashMark(controller.part.Position, move.color, folder)
				controller:checkCollisions(allControllers, true)
				controller:areaHit(allControllers, 5, phase.damage or 12, true)
			end
		elseif phase.id == "burst" then
	...
	shatter = function(controller, move, phase, dt, allControllers)
		...
		elseif phase.id == "shards" then
			...
			if controller.shardTimer >= (phase.interval or 0.28) then
				...
				SpecialVFX.pulseWave(pos, phase.range or 7, move.color, folder)
				controller:areaHit(allControllers, phase.range or 7, phase.damage or 10, true)
			end
		elseif phase.id == "freeze" then
			SpecialVFX.iceShard(controller.part.Position, move.color, folder)
			controller:areaHit(allControllers, phase.range or 8, phase.damage or 20, true)
		end
	end,
	surge = function(controller, move, phase, dt, allControllers)
		local folder = SpecialVFX.ensureFolder(controller)
		if phase.id == "charge" then
		...
		elseif phase.id == "surge" then
			...
			local pos = controller.part.Position
			SpecialVFX.surgeTrail(controller.surgeLastPos or pos, pos, move.color, folder)
