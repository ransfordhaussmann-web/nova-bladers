...
local function updateOrbit(controller, dt, allControllers, hitOnOrbit)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "lunge" then
		...
		elseif phase.id == "rip" then
			SpecialVFX.ripClaws(controller.part.Position, color, folder)
		end
	...
	elseif mode == "solar" then
		if phase.id == "flare_charge" then
			SpecialVFX.solarFlare(controller, color, phase.duration)
		elseif phase.id == "flare_rings" then
			controller.sonicTimer = 0
			controller.sonicCount = 0
		elseif phase.id == "orbit_burn" then
			setupOrbit(controller, move, target)
		end
	elseif mode == "mirage" then
		if phase.id == "split" then
			SpecialVFX.mirageSplit(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "sonic" then
		...
		elseif phase.id == "orbit" then
			updateOrbit(controller, dt, allControllers, true)
		end
