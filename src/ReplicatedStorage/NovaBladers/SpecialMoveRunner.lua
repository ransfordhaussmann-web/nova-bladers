...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "leap" then
			...
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.verticalVelocity = phase.leapForce or move.leapForce or 38
			controller.airborne = true
		elseif phase.id == "slam" then
			controller.verticalVelocity = -(phase.slamSpeed or move.slamSpeed or 55)
	...
	elseif move.id == "PrismCrystalShield" then
		...
		elseif phase.id == "shatter" then
			...
			SpecialVFX.crystalShards(controller.part.Position, color, folder, phase.shards or 6)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "ForgeHammerSlam" then
		...
		elseif phase.id == "slam" then
			controller.velocity = controller.facing * 25
			if not controller.airborne then
				SpecialVFX.slamImpact(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, 5, phase.damage or 28, true)
			end
		elseif phase.id == "shock" then
			controller.velocity = Vector3.zero
			controller.shockTimer = (controller.shockTimer or 0) + dt
			if controller.shockTimer >= (phase.interval or 0.25) then
				controller.shockTimer = 0
