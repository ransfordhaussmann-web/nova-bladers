...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonInfernoDrive" then
		...
		elseif phase.id == "scorch" then
			controller.scorchHitsLeft = phase.hits or 3
			controller.scorchTimer = 0
			controller.scorchLastPos = controller.part.Position
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoDrive" then
		if phase.id == "ignite" then
			controller.velocity = Vector3.zero
		elseif phase.id == "blaze" or phase.id == "scorch" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 80)
		end
		if phase.id == "scorch" then
			controller.scorchTimer = (controller.scorchTimer or 0) + dt
			if controller.scorchTimer >= (phase.hitInterval or 0.16) then
				controller.scorchTimer = 0
				local pos = controller.part.Position
				SpecialVFX.meteorTrail(controller.scorchLastPos, pos, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
				controller.scorchLastPos = pos
