...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "stomp" then
			SpecialVFX.stompShock(controller.part.Position, color, folder)
			controller.velocity = Vector3.zero
	...
	elseif move.id == "PhantomMirageCut" then
		...
		elseif phase.id == "cut" then
			SpecialVFX.mirageCut(controller.part.Position, phase.range or 7, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "SolarCoronaLoop" then
		...
		elseif phase.id == "corona" then
			...
			if controller.coronaTimer >= (phase.interval or 0.32) then
				controller.coronaTimer = 0
