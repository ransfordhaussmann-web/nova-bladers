...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeInfernoWheel" then
		...
		elseif phase.id == "wheel" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller.flameTrailTimer = (controller.flameTrailTimer or 0) + dt
			if controller.flameTrailTimer >= (phase.trailInterval or 0.12) then
				controller.flameTrailTimer = 0
	...
	elseif move.id == "FrostCrystalVeil" then
		if phase.id == "freeze" or phase.id == "veil" then
