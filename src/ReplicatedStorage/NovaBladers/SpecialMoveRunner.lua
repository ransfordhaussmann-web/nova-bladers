...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "pull" then
			SpecialVFX.tidalPull(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "rise" or phase.id == "fortify" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
			if controller.pulseTimer >= (phase.interval or 0.35) then
				controller.pulseTimer = 0
				SpecialVFX.quakePulse(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "slash" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "afterimage" then
			controller:areaHit(allControllers, phase.range or 5, phase.damage or 18, true)
		end
	end
