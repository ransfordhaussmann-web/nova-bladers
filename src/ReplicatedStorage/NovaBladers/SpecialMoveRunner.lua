...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FangSpiralRush" then
		...
		elseif phase.id == "rend" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.spiralSlash(controller.part.Position, dir, color, folder)
		end
	...
	elseif move.id == "FangSpiralRush" then
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			if controller.spiralTimer >= (phase.interval or 0.22) then
				controller.spiralTimer = 0
				SpecialVFX.spiralSlash(pos, controller.facing, move.color, folder)
		...
		elseif phase.id == "rend" then
			...
			if now >= controller.specialPhaseEnd - 0.05 then
				SpecialVFX.fangRend(controller.part.Position, move.color, folder)
