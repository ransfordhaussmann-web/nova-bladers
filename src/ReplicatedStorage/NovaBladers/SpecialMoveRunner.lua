...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRendStrike" then
		...
		elseif phase.id == "cleave" then
			controller.cleaveHitsLeft = phase.hits or 2
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRendStrike" then
		...
		elseif phase.id == "cleave" then
			...
			if controller.cleaveTimer >= (phase.interval or 0.2) then
				controller.cleaveTimer = 0
				controller.cleaveHitsLeft = (controller.cleaveHitsLeft or 1) - 1
				local pos = controller.part.Position
				SpecialVFX.meteorImpact(pos, move.color, folder)
