...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "eclipse" then
		...
		elseif phase.id == "burst" then
			if move.id == "ShadowEclipseFang" then
				SpecialVFX.venomBurst(controller.part.Position, color, folder)
			else
				SpecialVFX.shardBurst(controller.part.Position, color, folder, 8)
			end
		end
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			controller.shatterLastPos = controller.part.Position
		end
	elseif mode == "blaze" then
		if phase.id == "charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
			SpecialVFX.blazeCrown(controller, color, phase.duration + 0.5)
		...
		elseif phase.id == "flare" then
			SpecialVFX.shardBurst(controller.part.Position, color, folder, 10)
		end
	end
end
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.hitInterval or 0.16) then
				controller.shatterTimer = 0
				local pos = controller.part.Position
				SpecialVFX.shardBurst(pos, move.color, folder, 5)
				SpecialVFX.blazeTrail(controller.shatterLastPos or pos, pos, move.color, folder)
				controller.shatterLastPos = pos
				controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 11, true)
			end
		end
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			if controller.blazeTimer >= (phase.interval or 0.22) then
				controller.blazeTimer = 0
				local trailPos = controller.part.Position
				SpecialVFX.blazeTrail(controller.blazeLastPos or trailPos, trailPos, move.color, folder)
				controller.blazeLastPos = trailPos
