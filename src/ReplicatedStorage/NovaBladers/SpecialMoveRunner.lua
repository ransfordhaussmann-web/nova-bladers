...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "dive" then
			SpecialVFX.setUnderground(controller, true)
			SpecialVFX.waterSplash(controller, color)
			controller.velocity = Vector3.zero
		elseif phase.id == "crash" then
			SpecialVFX.setUnderground(controller, false)
			SpecialVFX.anchorCrash(controller.part.Position, color, folder)
	...
	elseif move.id == "GraniteRampart" then
		if phase.id == "raise" then
			...
			SpecialVFX.stoneWall(controller, color, phase.duration + 0.6)
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "phase" then
			SpecialVFX.phaseGlow(controller, color, phase.duration)
		...
		elseif phase.id == "slash" then
			...
			SpecialVFX.phaseSlash(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeFeatherStorm" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "launch" or phase.id == "scatter" then
		...
		if phase.id == "scatter" then
			...
			if controller.featherTimer >= (phase.hitInterval or 0.16) then
				controller.featherTimer = 0
				local pos = controller.part.Position
				SpecialVFX.featherScatter(controller.featherLastPos, pos, move.color, folder)
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.18) then
				controller.rendTimer = 0
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "flare" then
			...
			if controller.flareTimer >= (phase.interval or 0.26) then
				...
				local range = 4 + controller.flareCount * 1.4
