...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.crimsonRipTrail(controller, color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			SpecialVFX.auroraDome(controller, color, Color3.fromRGB(175, 115, 255), phase.duration)
		elseif phase.id == "pulse" then
			controller.pulseTimer = 0
		end
	end
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.ripTimer >= (phase.hitInterval or 0.15) then
				...
				SpecialVFX.crimsonRipSlash(controller.part.Position, controller.facing, move.color, folder)
