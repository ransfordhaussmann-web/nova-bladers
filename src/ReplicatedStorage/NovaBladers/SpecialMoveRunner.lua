...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "finish" then
			SpecialVFX.bladeFinish(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "brace" or phase.id == "pillar" then
			controller.velocity = Vector3.zero
		elseif phase.id == "reflect" then
	...
	elseif move.id == "SolarFlareOrbit" then
		if phase.id == "orbit" and controller.orbitCenter then
			...
			if controller.burnTimer >= (phase.interval or 0.3) then
				controller.burnTimer = 0
