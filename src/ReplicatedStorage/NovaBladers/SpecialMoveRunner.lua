...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrownLock" then
		...
		elseif phase.id == "lock" then
			SpecialVFX.frostLock(controller.part.Position, color, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonForgeBreak" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.3) then
				controller.shatterTimer = 0
				SpecialVFX.forgeShatter(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "FrostCrownLock" then
		...
		elseif phase.id == "freeze" then
			...
			if controller.freezeTimer >= (phase.interval or 0.25) then
				controller.freezeTimer = 0
				SpecialVFX.frostShard(controller.part.Position, phase.range or 6, move.color, folder)
