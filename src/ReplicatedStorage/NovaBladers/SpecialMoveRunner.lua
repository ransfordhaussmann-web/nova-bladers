...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "riptide" then
		...
		elseif phase.id == "riptide" then
			...
			controller.riptideLastPos = controller.part.Position
		elseif phase.id == "tidal_end" then
			SpecialVFX.tidalBurst(controller.part.Position, color, folder)
		end
	elseif mode == "frost" then
		if phase.id == "frost_spread" then
			SpecialVFX.frostAura(controller, color, phase.duration)
			controller.velocity *= 0.5
		elseif phase.id == "crown_wall" then
	...
	elseif mode == "resonance" then
		if phase.id == "pulse_charge" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "resonance" then
			controller.resonanceTimer = 0
		elseif phase.id == "spike" then
			SpecialVFX.resonanceSpike(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "riptide" then
		...
		elseif phase.id == "riptide" then
			...
			if controller.riptideTimer >= (phase.hitInterval or 0.22) then
				controller.riptideTimer = 0
				local pos = controller.part.Position
				SpecialVFX.tideWave(controller.riptideLastPos, pos, move.color, folder)
				controller.riptideLastPos = pos
	...
	elseif mode == "frost" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.frostTimer >= (phase.interval or 0.3) then
				controller.frostTimer = 0
				SpecialVFX.frostShards(controller.part.Position, move.color, folder)
	...
	elseif mode == "resonance" then
		...
		elseif phase.id == "resonance" then
			...
			if controller.resonanceTimer >= (phase.interval or 0.3) then
				controller.resonanceTimer = 0
				local range = phase.range or 9
				SpecialVFX.resonanceRing(controller.part.Position, range, move.color, folder)
				controller:areaPull(allControllers, range, phase.pullStrength or 22)
				controller:areaHit(allControllers, range * 0.55, phase.damage or 8, true)
			end
		elseif phase.id == "spike" then
