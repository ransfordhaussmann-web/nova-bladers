...
local PHASE_START = {
	...
	bastion = function(controller, move, phase)
		...
		elseif phase.id == "counterpulse" then
			controller.pulseTimer = 0
		end
	end,
	...
	mirage = function(controller, move, phase, target, folder)
		if phase.id == "mirror" then
			SpecialVFX.mirageClones(controller, move.color, phase.clones or 3, folder)
		elseif phase.id == "feint" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed) * 0.6
			SpecialVFX.mirageFeint(controller, move.color, folder)
		elseif phase.id == "strike" then
			local targetPos = getTargetPos(controller, target)
			local dir = (targetPos - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (move.rushSpeed or 75)
		end
	end,
}
...
local PHASE_UPDATE = {
	...
	solar = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "flare" then
			...
			if controller.solarTimer >= (phase.interval or 0.2) then
				...
				local range = 3 + controller.solarCount * 1.2
	...
	mirage = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "feint" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 75) * 0.6
		elseif phase.id == "strike" then
