...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "loop" then
			...
			controller.loopLastPos = controller.part.Position
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRend" then
		...
		if phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.28) then
				controller.rendTimer = 0
				SpecialVFX.slashArc(controller.part.Position, controller.facing, move.color, folder, 5.5)
	...
	elseif move.id == "SolarFlareLoop" then
		...
		elseif phase.id == "loop" and controller.orbitCenter then
			...
			local y = controller.part.Position.Y
			local pos = center + Vector3.new(math.cos(controller.orbitAngle) * r, 0, math.sin(controller.orbitAngle) * r)
			local lastPos = controller.loopLastPos or pos
			SpecialVFX.burnTrail(lastPos, pos, move.color, folder)
			controller.loopLastPos = pos
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			controller:checkCollisions(allControllers, true)
		end
