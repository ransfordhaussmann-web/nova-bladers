...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "EmberNovaBurst" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "FrostCrownShatter" then
		if phase.id == "frostfield" or phase.id == "freeze" then
	...
	elseif move.id == "EmberNovaBurst" then
		...
		elseif phase.id == "nova" then
			...
			if controller.novaTimer >= (phase.interval or 0.22) then
				...
				local range = 3.5 + controller.novaCount * 2.2
				SpecialVFX.emberNovaRing(controller.part.Position, range, move.color, folder)
