...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.facing = dir
			controller.velocity = dir * (move.driftSpeed or 68)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			...
			if controller.cycloneTimer >= (phase.interval or 0.2) then
				...
				SpecialVFX.bladeCyclone(controller.part.Position, phase.range or 6, move.color, folder)
	...
	elseif move.id == "SolarFlareDrift" then
		if phase.id == "drift" then
			controller.velocity = controller.facing * (move.driftSpeed or 68)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			local dashInterval = (phase.duration or 0.8) / (phase.dashes or 3)
			if controller.surgeTimer >= dashInterval and (controller.surgeDashLeft or 0) > 0 then
				controller.surgeTimer = 0
