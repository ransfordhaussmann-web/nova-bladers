...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "release" then
			controller.pendingReleaseHit = true
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "rip" then
			...
			if controller.slashTimer >= (phase.interval or 0.15) then
				controller.slashTimer = 0
				controller.slashCount = (controller.slashCount or 0) + 1
				local angle = controller.slashCount * 120
				SpecialVFX.slashArc(controller.part.Position, angle, phase.range or 5.5, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "release" then
			if controller.pendingReleaseHit then
				controller.pendingReleaseHit = false
