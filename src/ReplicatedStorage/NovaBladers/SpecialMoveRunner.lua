...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrownShatter" then
		...
		elseif phase.id == "crown" then
			SpecialVFX.crownAura(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonInfernoRush" then
		if phase.id == "ignite" then
			controller.velocity = Vector3.zero
		elseif phase.id == "blaze" or phase.id == "ember" then
		...
		if phase.id == "ember" then
			...
			if controller.emberTimer >= (phase.hitInterval or 0.2) then
				controller.emberTimer = 0
				local pos = controller.part.Position
				SpecialVFX.blazeTrail(controller.emberLastPos, pos, move.color, folder)
