...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "leap" then
			controller.verticalVelocity = phase.leapHeight or 6
			controller.airborne = true
		elseif phase.id == "shatter" then
			controller.shatterTimer = 0
			controller.shatterWave = 0
		end
	elseif mode == "blaze" then
		if phase.id == "ignite" then
			SpecialVFX.blazeIgnite(controller, color, phase.duration)
		elseif phase.id == "spiral" then
			controller.spiralAngle = 0
			controller.spiralRadius = phase.spiralRadius or 8
			controller.spiralCenter = getTargetPos(controller, target)
			local dir = (controller.spiralCenter - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.22) then
				controller.shatterTimer = 0
				controller.shatterWave = (controller.shatterWave or 0) + 1
				local range = (phase.range or 5) + controller.shatterWave * 1.8
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			...
			local r = (controller.spiralRadius or 8) * (1 - controller.spiralAngle / 12)
			r = math.max(r, 2)
			local pos = center + Vector3.new(math.cos(controller.spiralAngle) * r, 0, math.sin(controller.spiralAngle) * r)
			local y = controller.part.Position.Y
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = (center - controller.part.Position).Unit * (phase.rushSpeed or move.rushSpeed or 70)
