...
local PHASE_START = {
	...
	mirage = function(controller, move, phase, target, folder)
		...
		elseif phase.id == "cut" or phase.id == "cut2" then
			controller.part.Transparency = 0.35
			aimAtTarget(controller, target, phase.rushSpeed or move.rushSpeed)
	...
	frost = function(controller, move, phase, folder)
...
local PHASE_UPDATE = {
	...
	barrage = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "rush" then
			...
			if controller.barrageTimer >= (phase.hitInterval or 0.12) then
				controller.barrageTimer = 0
				controller.barrageHits = (controller.barrageHits or 0) + 1
				SpecialVFX.meteorImpact(controller.part.Position, move.color, folder)
	...
	mirage = function(controller, move, phase, dt, allControllers)
		if phase.id == "cut" or phase.id == "cut2" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:checkCollisions(allControllers, true)
			controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 16, true)
		elseif phase.id == "fade" then
			controller:areaHit(allControllers, phase.range or 5.5, phase.damage or 22, true)
		end
	end,
	frost = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "freeze" then
