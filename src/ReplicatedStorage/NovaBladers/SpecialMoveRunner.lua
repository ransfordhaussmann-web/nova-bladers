...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeCometLoop" then
		...
		elseif phase.id == "slam" then
			...
			controller.verticalVelocity = -35
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeCometLoop" then
		...
		elseif phase.id == "orbit" and controller.orbitCenter then
			...
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center)
			controller.velocity = Vector3.zero
			controller.orbitHitTimer = (controller.orbitHitTimer or 0) + dt
			if controller.orbitHitTimer >= (phase.hitInterval or 0.22) then
				controller.orbitHitTimer = 0
				SpecialVFX.fireOrbitTrail(pos, move.color, folder)
		...
		elseif phase.id == "slam" then
			...
			if controller.part.Position.Y <= controller.floorY + 0.5 then
				controller:areaHit(allControllers, phase.slamRange or 7, phase.damage or 32, true)
			end
		end
