...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "ForgeEruption" then
		...
		elseif phase.id == "splash" then
			controller.splashHitsLeft = phase.hits or 3
			controller.splashTimer = 0
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "ForgeEruption" then
		...
		elseif phase.id == "erupt" or phase.id == "splash" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 82)
		end
		if phase.id == "splash" then
			controller.splashTimer = (controller.splashTimer or 0) + dt
			if controller.splashTimer >= (phase.hitInterval or 0.2) then
				controller.splashTimer = 0
				local pos = controller.part.Position
				SpecialVFX.moltenSplash(pos, move.color, folder)
