...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			if controller.lungeTimer >= (phase.hitInterval or 0.1) then
				controller.lungeTimer = 0
		...
		elseif phase.id == "ripper" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			controller:areaHit(allControllers, phase.range or 5, phase.damage or 35, true)
		elseif phase.id == "finisher" then
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "reflect" then
			...
			if controller.reflectTimer >= (phase.interval or 0.22) then
				controller.reflectTimer = 0
				SpecialVFX.reflectPulse(controller.part.Position, phase.range or 9, move.color, folder)
