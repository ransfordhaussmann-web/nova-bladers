...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonShredSpiral" then
		...
		elseif phase.id == "spiral" and target and target.part then
			...
			SpecialVFX.shredTrail(controller, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonShredSpiral" then
		...
		elseif phase.id == "spiral" and controller.orbitCenter then
			...
			if controller.shredTimer >= (phase.interval or 0.22) then
				controller.shredTimer = 0
				SpecialVFX.shredSlash(controller.part.Position, move.color, folder)
	...
	elseif move.id == "FrostBastionDome" then
		...
		elseif phase.id == "dome" then
			...
			if controller.frostTimer >= (phase.interval or 0.38) then
				controller.frostTimer = 0
