...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFangRush" then
		...
		elseif phase.id == "lunge" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
		elseif phase.id == "frenzy" then
			controller.fangTimer = 0
			controller.fangHitsLeft = phase.hits or 5
		end
	elseif move.id == "FrostRingShatter" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "FrostRingShatter" then
		if phase.id == "frost" or phase.id == "cage" then
			controller.velocity = Vector3.zero
		end
		if phase.id == "cage" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
