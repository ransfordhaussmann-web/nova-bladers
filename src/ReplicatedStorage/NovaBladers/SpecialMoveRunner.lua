...
local MODE_PHASE_START = {
	...
	anchor = function(controller, move, phase)
		if phase.id == "rise" then
			controller.verticalVelocity = phase.riseSpeed or 22
			controller.airborne = true
		...
		elseif phase.id == "wave" then
			controller.pulseTimer = 0
			SpecialVFX.waterSlam(controller.part.Position, move.color, SpecialVFX.ensureFolder(controller))
		end
	end,
	...
	rampart = function(controller, move, phase)
		...
		elseif phase.id == "barrier" then
			...
			SpecialVFX.stoneBarrier(controller, move.color, phase.duration)
	...
	phase = function(controller, move, phase, target)
		if phase.id == "vanish" then
			SpecialVFX.phaseFade(controller, true)
		elseif phase.id == "blink" then
			...
			if dir.Magnitude > 0.1 then
				dir = dir.Unit
				controller.part.CFrame = CFrame.new(targetPos - dir * 3, targetPos)
				controller.facing = dir
			end
			SpecialVFX.phaseFade(controller, false)
		end
	end,
}

local MODE_PHASE_UPDATE = {
	...
	anchor = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "wave" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.waterSlam(controller.part.Position, move.color, folder)
	...
	rend = function(controller, move, phase, dt, allControllers, folder)
		...
		elseif phase.id == "slash" then
			if not controller._slashHit then
				controller._slashHit = true
	...
	rampart = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "brace" or phase.id == "barrier" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shock" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
			if controller.pulseTimer >= (phase.interval or 0.35) then
	...
	flare = function(controller, move, phase, dt, allControllers, folder)
		if phase.id == "charge" then
			controller.velocity *= 0.9
		elseif phase.id == "rings" then
			controller.sonicTimer = (controller.sonicTimer or 0) + dt
			if controller.sonicTimer >= (phase.interval or 0.25) then
				...
				SpecialVFX.solarFlare(controller.part.Position, move.color, folder)
				SpecialVFX.sonicRing(controller.part.Position, range, move.color, folder)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	local handler = MODE_PHASE_START[move.mode]
	if handler then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local updater = MODE_PHASE_UPDATE[move.mode]
	if updater then
