...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "split" then
			SpecialVFX.phantomClone(controller, color, phase.duration)
		elseif phase.id == "feint" then
			local perp = Vector3.new(-controller.facing.Z, 0, controller.facing.X)
			if math.random() > 0.5 then
				perp = -perp
			end
			controller.facing = perp
			controller.velocity = perp * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.mirageDash(controller, color, folder)
	...
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.2) then
				...
				SpecialVFX.phantomSlash(controller.part.Position, slashDir, move.color, folder)
