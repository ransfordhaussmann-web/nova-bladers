...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			SpecialVFX.auroraDome(controller, color, controller.beyData.accentColor or color, phase.duration)
		elseif phase.id == "reflect" then
			controller.guardReduction = 0
			SpecialVFX.auroraDomeBurst(controller.part.Position, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "lunge" then
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "ripper" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 50) * 0.6
			controller.ripperTimer = (controller.ripperTimer or 0) + dt
			if controller.ripperTimer >= (phase.hitInterval or 0.14) then
				controller.ripperTimer = 0
				controller.ripperAngle = (controller.ripperAngle or 0) + math.rad(72)
				local pos = controller.part.Position
				local slashDir = Vector3.new(math.sin(controller.ripperAngle), 0, math.cos(controller.ripperAngle))
				SpecialVFX.ripperSlash(pos, slashDir, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "reflect" then
			...
			if controller.reflectTimer >= (phase.interval or 0.3) then
				controller.reflectTimer = 0
				SpecialVFX.auroraReflectPulse(controller.part.Position, phase.range or 7, move.color, folder)
