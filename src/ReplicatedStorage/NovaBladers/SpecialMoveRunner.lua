...
local function startMeteorPhase(controller, move, phase, target)
...
local function startFortressPhase(controller, move, phase)
...
local function startSonicPhase(controller, move, phase, target)
...
local function startEclipsePhase(controller, move, phase, target)
...
local function startSlamPhase(controller, move, phase, target)
	if phase.id == "charge" then
		SpecialVFX.chargeAura(controller, move.color, phase.duration)
...
local function startPhaseSlash(controller, move, phase, target)
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	if mode == "meteor" then
		startMeteorPhase(controller, move, phase, target)
	elseif mode == "fortress" then
		startFortressPhase(controller, move, phase)
	elseif mode == "sonic" then
		startSonicPhase(controller, move, phase, target)
	elseif mode == "eclipse" then
		startEclipsePhase(controller, move, phase, target)
	elseif mode == "slam" then
		startSlamPhase(controller, move, phase, target)
	elseif mode == "phase" then
		startPhaseSlash(controller, move, phase, target)
	end
end
...
local function updateMeteor(controller, move, phase, dt, allControllers)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
...
local function updateFortress(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "burrow" then
		controller.velocity = Vector3.zero
		local pos = controller.part.Position
...
local function updateSonic(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "charge" then
...
local function updateSlam(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "charge" then
...
local function updatePhaseSlash(controller, move, phase, dt, allControllers)
	if phase.id == "vanish" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local mode = move.mode
	if mode == "meteor" then
		updateMeteor(controller, move, phase, dt, allControllers)
	elseif mode == "fortress" then
		updateFortress(controller, move, phase, dt, allControllers)
	elseif mode == "sonic" then
		updateSonic(controller, move, phase, dt, allControllers)
	elseif mode == "eclipse" then
		updateEclipse(controller, move, phase, dt, allControllers)
	elseif mode == "slam" then
		updateSlam(controller, move, phase, dt, allControllers)
	elseif mode == "phase" then
		updatePhaseSlash(controller, move, phase, dt, allControllers)
	end
