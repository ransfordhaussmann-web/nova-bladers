...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "phase" then
			SpecialVFX.phaseOut(controller, color, phase.duration)
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.phaseSlash(controller, color, folder)
		elseif phase.id == "echo" then
			SpecialVFX.phaseEcho(controller.part.Position, color, folder)
		end
	end
end
	...
	elseif move.id == "CrimsonRend" then
		...
		if phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.14) then
				controller.rendTimer = 0
				local pos = controller.part.Position
				SpecialVFX.slashArc(controller.rendLastPos, pos, move.color, folder)
