...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		if phase.id == "windup" then
			SpecialVFX.chargeAura(controller, color, phase.duration)
		elseif phase.id == "cyclone" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
			)
			controller.orbitRadius = phase.orbitRadius or 7
		...
		elseif phase.id == "slash" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.bladeCycloneRing(controller.part.Position, color, folder)
		end
	...
	elseif move.id == "SolarFlareDrift" then
		if phase.id == "flare" then
			SpecialVFX.solarFlareCharge(controller, color, phase.duration)
		elseif phase.id == "drift" and target and target.part then
			controller.orbitCenter = target.part.Position
			controller.orbitAngle = math.atan2(
				controller.part.Position.Z - target.part.Position.Z,
				controller.part.Position.X - target.part.Position.X
			...
			controller.orbitSpeed = move.orbitSpeed or 12
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "fade" then
			SpecialVFX.phantomFade(controller, true)
		elseif phase.id == "surge" then
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			SpecialVFX.phantomTrail(controller, color, folder)
		elseif phase.id == "edge" then
			SpecialVFX.phantomFade(controller, false)
			SpecialVFX.edgeSurgeBurst(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		if phase.id == "windup" then
			controller.velocity = Vector3.zero
		elseif phase.id == "cyclone" and controller.orbitCenter then
			...
			if controller.cycloneTimer >= (phase.hitInterval or 0.22) then
				controller.cycloneTimer = 0
				SpecialVFX.bladeCycloneRing(controller.part.Position, move.color, folder)
		...
		elseif phase.id == "slash" then
			...
			controller:checkCollisions(allControllers, true)
			controller:areaHit(allControllers, phase.range or 5, phase.damage or 32, true)
		end
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "quake" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.quakePulse(controller.part.Position, phase.range or 9, move.color, folder)
				controller:areaHit(allControllers, phase.range or 9, phase.damage or 12, true)
			end
		end

	elseif move.id == "SolarFlareDrift" then
		if phase.id == "drift" and controller.orbitCenter then
			controller.orbitAngle += (controller.orbitSpeed or 12) * dt
			...
			if controller.solarTimer >= (phase.interval or 0.25) then
				controller.solarTimer = 0
				SpecialVFX.solarFlareRing(controller.part.Position, r, move.color, folder)
				controller:areaHit(allControllers, 5, phase.damage or 8, true)
			end
		elseif phase.id == "burn" then
			controller.burnTimer = (controller.burnTimer or 0) + dt
			if controller.burnTimer >= (phase.hitInterval or 0.15) then
				controller.burnTimer = 0
				SpecialVFX.burnTrail(controller.part.Position, move.color, folder)
				controller:areaHit(allControllers, phase.range or 5, phase.damage or 14, true)
			end
		end
