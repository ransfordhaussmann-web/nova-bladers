...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteBastionSurge" then
	...
	elseif move.id == "SolarFlareSpiral" then
	...
	elseif move.id == "PhantomEdgeSlice" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "slash" then
			...
			if controller.slashCount < (phase.slashes or 3)
				and controller.slashTimer >= (phase.interval or 0.2)
			then
				controller.slashTimer = 0
				controller.slashCount = (controller.slashCount or 0) + 1
				local angle = (controller.slashCount - 1) * (math.pi * 2 / (phase.slashes or 3))
	...
	elseif move.id == "GraniteBastionSurge" then
		...
		elseif phase.id == "pillars" then
			...
			local maxPillars = phase.pillars or 4
			if controller.pillarCount < maxPillars and controller.pillarTimer >= 0.15 then
				controller.pillarTimer = 0
				controller.pillarCount = (controller.pillarCount or 0) + 1
				local angle = (controller.pillarCount - 1) * (math.pi * 2 / maxPillars)
				local offset = Vector3.new(math.cos(angle), 0, math.sin(angle)) * (phase.range or 7) * 0.55
				local pillarPos = controller.part.Position + offset
				SpecialVFX.rockPillar(pillarPos, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSlice" then
		if phase.id == "blink" then
			controller.blinkTimer = (controller.blinkTimer or 0) + dt
			if controller.blinkCount < (phase.blinks or 3)
				and controller.blinkTimer >= (phase.interval or 0.18)
			then
				controller.blinkTimer = 0
				controller.blinkCount = (controller.blinkCount or 0) + 1
				local targetPos = getTargetPos(controller, target)
				local dir = (targetPos - controller.part.Position)
				dir = Vector3.new(dir.X, 0, dir.Z).Unit
				controller.facing = dir
