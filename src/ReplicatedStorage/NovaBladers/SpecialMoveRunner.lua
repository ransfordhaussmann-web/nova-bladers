...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "whirl" then
			...
			SpecialVFX.bladeWhirl(controller, color, phase.duration)
	...
	elseif move.id == "GraniteBastionPulse" then
		...
		elseif phase.id == "pillars" then
			SpecialVFX.stonePillars(controller, color, phase.duration)
		elseif phase.id == "slam" then
			controller.slamTimer = 0
		end
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.trailLastPos = controller.part.Position
			if target and target.part then
				controller.orbitCenter = target.part.Position
				controller.orbitAngle = math.atan2(
					controller.part.Position.Z - target.part.Position.Z,
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "surge" then
			controller.surgeDash = 0
			controller.surgeTimer = 0
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
		elseif phase.id == "salvo" then
			controller.salvoTimer = 0
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "whirl" then
			...
			if controller.whirlTimer >= (phase.interval or 0.14) then
				controller.whirlTimer = 0
		...
		elseif phase.id == "rush" then
			...
			controller:checkCollisions(allControllers, true)
			controller:areaHit(allControllers, phase.hitRadius or 6, phase.damage or 14, true)
		end

	elseif move.id == "GraniteBastionPulse" then
		if phase.id == "anchor" or phase.id == "pillars" then
			controller.velocity = Vector3.zero
		elseif phase.id == "slam" then
			controller.slamTimer = (controller.slamTimer or 0) + dt
			if controller.slamTimer >= (phase.interval or 0.35) then
				controller.slamTimer = 0
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" and controller.orbitCenter then
			...
			if controller.trailTimer >= (phase.trailInterval or 0.12) then
				controller.trailTimer = 0
				SpecialVFX.sunTrail(controller.trailLastPos, pos, move.color, folder)
				controller.trailLastPos = pos
				controller:areaHit(allControllers, 4, phase.trailDamage or 6, true)
			end
		elseif phase.id == "flare" then
			controller.flareTimer = (controller.flareTimer or 0) + dt
			if controller.flareTimer >= (phase.interval or 0.25) then
				controller.flareTimer = 0
				SpecialVFX.flareBurst(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			if controller.surgeTimer >= dashInterval then
				controller.surgeTimer = 0
				controller.surgeDash = (controller.surgeDash or 0) + 1
		...
		elseif phase.id == "salvo" then
			controller.salvoTimer = (controller.salvoTimer or 0) + dt
			if controller.salvoTimer >= (phase.interval or 0.15) then
				controller.salvoTimer = 0
				SpecialVFX.edgeSalvo(controller.part.Position, phase.range or 5.5, move.color, folder)
