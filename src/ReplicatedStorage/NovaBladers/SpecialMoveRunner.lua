...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "GraniteAvalanche" then
		if phase.id == "slam" then
			controller.velocity = Vector3.zero
			SpecialVFX.groundSlam(controller, color, folder)
		...
		elseif phase.id == "debris" then
			controller.debrisTimer = 0
		end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrystalShatterStorm" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.interval or 0.22) then
				...
				local range = (phase.range or 5) + controller.shatterCount * 1.2
	...
	elseif move.id == "GraniteAvalanche" then
		...
		elseif phase.id == "wave" then
			...
			SpecialVFX.avalancheWave(controller.part.Position, controller.facing, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "debris" then
			controller.debrisTimer = (controller.debrisTimer or 0) + dt
			if controller.debrisTimer >= (phase.interval or 0.3) then
				controller.debrisTimer = 0
				SpecialVFX.debrisRing(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "EmberPulseWheel" then
		...
		elseif phase.id == "pulse" then
			...
			if controller.emberPulseTimer >= (phase.interval or 0.25) then
				controller.emberPulseTimer = 0
