...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "FrostCrownBloom" then
		...
		elseif phase.id == "bloom" then
			...
			SpecialVFX.frostBloom(controller.part.Position, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameSpiral" then
		...
		elseif phase.id == "spiral" then
			...
			if controller.flameTimer >= (phase.hitInterval or 0.12) then
				controller.flameTimer = 0
				local pos = controller.part.Position
				SpecialVFX.meteorTrail(controller.flameLastPos or pos, pos, move.color, folder)
	...
	elseif move.id == "FrostCrownBloom" then
		if phase.id == "crown" or phase.id == "bloom" then
