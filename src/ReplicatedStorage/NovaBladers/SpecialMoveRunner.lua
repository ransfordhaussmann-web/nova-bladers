...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonForgeOverdrive" then
		...
		elseif phase.id == "rush" then
			...
			controller.facing = dir
			controller.velocity = dir * (phase.rushSpeed or move.rushSpeed)
			controller.forgeLastPos = controller.part.Position
		elseif phase.id == "slam" then
			controller.guardReduction = 0
			controller.velocity = Vector3.zero
			SpecialVFX.moltenSlam(controller.part.Position, phase.range or 7, color, folder)
		end
	elseif move.id == "CrystalTideBarrier" then
		...
		elseif phase.id == "shatter" then
			controller.guardReduction = 0
			SpecialVFX.shatterBurst(controller.part.Position, phase.range or 8.5, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonForgeOverdrive" then
		...
		elseif phase.id == "rush" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			local pos = controller.part.Position
			local last = controller.forgeLastPos or pos
			if (pos - last).Magnitude > 1.5 then
				SpecialVFX.flameTrail(last, pos, move.color, folder)
				controller.forgeLastPos = pos
			end
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "slam" then
			controller.velocity = Vector3.zero
	...
	elseif move.id == "CrystalTideBarrier" then
		...
		elseif phase.id == "waves" then
			...
			if controller.waveTimer >= (phase.interval or 0.28) then
				controller.waveTimer = 0
				SpecialVFX.iceWave(controller.part.Position, phase.range or 7, move.color, folder)
				controller:areaHit(allControllers, phase.range or 7, phase.damage or 10, true)
			end
		elseif phase.id == "shatter" then
			controller:areaHit(allControllers, phase.range or 8.5, phase.damage or 35, true)
		end
	end
