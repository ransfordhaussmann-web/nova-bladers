...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "flares" then
			...
			controller.flareAngle = 0
			controller.flareCount = phase.orbitCount or 3
		elseif phase.id == "burst" then
			SpecialVFX.solarBurst(controller.part.Position, phase.range or 8, color, folder)
		end
	elseif move.id == "PhantomBladeMirage" then
		...
		elseif phase.id == "dash" then
			controller.dashIdx = 0
			controller.dashTimer = 0
			controller.phantomPositions = SpecialVFX.getPhantomPositions(controller, phase.dashCount or 3)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		if phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.14) then
				controller.ripperTimer = 0
	...
	elseif move.id == "GraniteBastion" then
		...
		elseif phase.id == "slam" then
			controller.velocity = Vector3.zero
			SpecialVFX.stoneSlam(controller.part.Position, phase.range or 9, move.color, folder)
	...
	elseif move.id == "SolarFlareOrbit" then
		...
		elseif phase.id == "flares" then
			controller.flareAngle = (controller.flareAngle or 0) + (move.orbitSpeed or 14) * dt
			controller.flareTimer = (controller.flareTimer or 0) + dt
			local count = controller.flareCount or 3
			local radius = move.orbitRadius or 5.5
			local center = controller.part.Position
			for i = 1, count do
				local angle = controller.flareAngle + (i - 1) * (math.pi * 2 / count)
				local flarePos = center + Vector3.new(math.cos(angle) * radius, 0.5, math.sin(angle) * radius)
				SpecialVFX.solarFlare(flarePos, move.color, folder)
			end
			if controller.flareTimer >= (phase.interval or 0.25) then
				controller.flareTimer = 0
				controller:areaHit(allControllers, radius + 2, phase.damage or 8, true)
			end
		elseif phase.id == "burst" then
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "dash" then
			...
			local dashInterval = phase.duration / (phase.dashCount or 3)
			if controller.dashTimer >= dashInterval then
				controller.dashTimer = 0
				controller.dashIdx = (controller.dashIdx or 0) + 1
				local positions = controller.phantomPositions or {}
				local pos = positions[controller.dashIdx]
				if pos then
					SpecialVFX.phantomDash(controller.part.Position, pos, move.color, folder)
					controller.part.CFrame = CFrame.new(Vector3.new(pos.X, controller.part.Position.Y, pos.Z), controller.part.Position)
