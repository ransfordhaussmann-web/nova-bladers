...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "split" then
			SpecialVFX.phantomClones(controller, color, phase.duration)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "GraniteBastion" then
		if phase.id == "anchor" or phase.id == "fortify" then
			controller.velocity = Vector3.zero
		elseif phase.id == "quake" then
	...
	elseif move.id == "PhantomBladeMirage" then
		if phase.id == "dash" then
			controller.mirageZigzag = (controller.mirageZigzag or 0) + dt
			local zig = math.sin(controller.mirageZigzag * 18) * 0.35
			local right = controller.facing:Cross(Vector3.yAxis).Unit
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85) + right * zig * 30
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "converge" then
