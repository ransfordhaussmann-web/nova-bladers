...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "edge" then
			SpecialVFX.phantomEdgeBurst(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonBladeCyclone" then
		...
		elseif phase.id == "cyclone" then
			controller.velocity = controller.facing * (move.rushSpeed or 75) * 0.6
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			if controller.driftTimer >= (phase.trailInterval or 0.15) then
				controller.driftTimer = 0
				local pos = controller.part.Position
				SpecialVFX.solarFlareTrail(controller.driftLastPos or pos, pos, move.color, folder)
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			local dashInterval = phase.duration / math.max(phase.dashes or 3, 1)
			if controller.dashTimer >= dashInterval then
				...
				SpecialVFX.phantomBlink(controller, targetPos, move.color, folder)
				local offset = targetPos - controller.part.Position
				offset = Vector3.new(offset.X, 0, offset.Z)
				local dist = offset.Magnitude
				if dist > 0.5 then
					local unit = offset / dist
					controller.facing = unit
					controller.velocity = unit * (phase.dashSpeed or move.rushSpeed or 90)
					controller.part.CFrame = CFrame.new(
						controller.part.Position + unit * math.min(dist * 0.45, 6),
						targetPos
					)
				end
				controller:checkCollisions(allControllers, true)
