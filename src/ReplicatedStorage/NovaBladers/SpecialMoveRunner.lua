...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "pull" then
			SpecialVFX.tideRipple(controller.part.Position, phase.range or 10, color, folder)
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "TideAnchorCrash" then
		if phase.id == "pull" then
			...
			if controller.pullTimer >= 0.08 then
				controller.pullTimer = 0
				local origin = controller.part.Position
				local pullRange = phase.range or 10
				for _, other in allControllers do
					if other ~= controller and other.alive and other.part then
						...
						if dist > 0.5 and dist < pullRange then
							local pull = flat.Unit * (phase.pullStrength or 28) * dt
		...
		elseif phase.id == "wave" then
			...
			if controller.pulseTimer >= (phase.interval or 0.3) then
				controller.pulseTimer = 0
				SpecialVFX.tideRipple(controller.part.Position, phase.range or 8, move.color, folder)
				controller:areaHit(allControllers, phase.range or 8, phase.damage or 11, true)
			end
		end
	...
	elseif move.id == "CrimsonRend" then
		...
		elseif phase.id == "rend" then
			...
			if controller.rendTimer >= (phase.hitInterval or 0.15) then
				controller.rendTimer = 0
	...
	elseif move.id == "GraniteRampart" then
		...
		elseif phase.id == "quake" then
			...
			if controller.pulseTimer >= (phase.interval or 0.35) then
				controller.pulseTimer = 0
				SpecialVFX.stoneQuake(controller.part.Position, phase.range or 8.5, move.color, folder)
	...
	elseif move.id == "PhantomPhaseSlash" then
		if phase.id == "slash" then
			...
			if controller.slashTimer >= (phase.hitInterval or 0.12) then
				controller.slashTimer = 0
			...
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "emerge" then
			controller:areaHit(allControllers, phase.range or 6, phase.damage or 26, true)
		end
	end
