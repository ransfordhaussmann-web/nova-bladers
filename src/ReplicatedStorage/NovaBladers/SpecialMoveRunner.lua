...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonForgeBreak" then
		if phase.id == "heat" then
			controller.velocity = Vector3.zero
		elseif phase.id == "slam" or phase.id == "shatter" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
		end
		if phase.id == "shatter" then
			...
			if controller.meteorTimer >= (phase.hitInterval or 0.2) then
				controller.meteorTimer = 0
				local pos = controller.part.Position
				SpecialVFX.forgeShatter(pos, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 6, phase.damage or 14, true)
			end
		end

	elseif move.id == "FrostCrownLock" then
		if phase.id == "mist" or phase.id == "crown" then
			controller.velocity = Vector3.zero
		elseif phase.id == "freeze" then
			controller.pulseTimer = (controller.pulseTimer or 0) + dt
			if controller.pulseTimer >= (phase.interval or 0.35) then
