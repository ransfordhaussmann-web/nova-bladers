...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonVortexRush" then
		...
		elseif phase.id == "vortex" then
			...
			controller:pullNearby(allControllers, phase.range or 7, (phase.pullStrength or 28) * dt)
			if controller.vortexTimer >= (phase.interval or 0.22) then
				controller.vortexTimer = 0
				SpecialVFX.vortexSlice(controller.part.Position, phase.range or 7, move.color, folder)
	...
	elseif move.id == "GlacierFrostLock" then
		if phase.id == "chill" then
			...
			if not controller._chillApplied then
				controller._chillApplied = true
				controller:applyChillToArea(allControllers, phase.range or 9, phase.slowDuration or 2)
			end
		elseif phase.id == "lock" then
			controller.velocity = Vector3.zero
		elseif phase.id == "burst" then
			...
			if controller.frostTimer >= (phase.interval or 0.35) then
				controller.frostTimer = 0
				SpecialVFX.frostBurst(controller.part.Position, phase.range or 8, move.color, folder)
				controller:applyFrostBurst(allControllers, phase.range or 8, phase.damage or 12, phase.freezeDuration or 0.85)
			end
		end
	end
