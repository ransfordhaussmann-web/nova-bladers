...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonFlameVortex" then
		...
		elseif phase.id == "spiral" then
			...
			controller.flameHitsLeft = phase.hits or 3
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonFlameVortex" then
		...
		elseif phase.id == "spiral" then
			...
			local wobble = perp * math.sin(controller.spiralAngle) * 3
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85) + wobble
