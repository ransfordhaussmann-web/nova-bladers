...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "finisher" then
			SpecialVFX.ripperFinisher(controller.part.Position, color, folder)
		end
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			...
			SpecialVFX.barrierDome(controller, color, phase.duration)
			controller.domePulseTimer = 0
		elseif phase.id == "release" then
			controller.guardReduction = 0
			SpecialVFX.frostPulse(controller.part.Position, phase.range or 9, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.14) then
				controller.ripperTimer = 0
				local pos = controller.part.Position
				SpecialVFX.ripperSlash(controller.ripperLastPos or pos, pos, move.color, folder)
				SpecialVFX.meteorImpact(pos, move.color, folder)
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "dome" then
			controller.velocity = Vector3.zero
			controller.domePulseTimer = (controller.domePulseTimer or 0) + dt
			if controller.domePulseTimer >= (phase.interval or 0.4) then
				controller.domePulseTimer = 0
