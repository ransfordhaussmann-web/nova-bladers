	...
	elseif move.id == "CrimsonRipperLunge" then
		...
		elseif phase.id == "ripper" then
			...
			if controller.ripperTimer >= (phase.hitInterval or 0.16) then
				controller.ripperTimer = 0
				controller.ripperAngle = (controller.ripperAngle or 0) + math.rad(120)
				local slashDir = CFrame.Angles(0, controller.ripperAngle, 0).LookVector
	...
	elseif move.id == "AuroraBarrierDome" then
		...
		elseif phase.id == "pulse" then
			...
			if controller.auroraPulseTimer >= (phase.interval or 0.3) then
				controller.auroraPulseTimer = 0
				SpecialVFX.auroraPulse(controller.part.Position, phase.range or 9, move.color, folder)
