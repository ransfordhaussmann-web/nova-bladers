...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			controller.velocity = dir * (move.driftSpeed or 55)
	...
	elseif move.id == "PhantomEdgeSurge" then
		...
		elseif phase.id == "finish" then
			SpecialVFX.edgeFinish(controller.part.Position, color, folder)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "SolarFlareDrift" then
		...
		elseif phase.id == "drift" then
			...
			if controller.flareTimer >= (phase.interval or 0.25) then
				...
				local range = (phase.range or 5) + controller.flareCount * 0.8
			...
			controller.velocity = controller.facing * (move.driftSpeed or 55)
		elseif phase.id == "flare" then
	...
	elseif move.id == "PhantomEdgeSurge" then
		if phase.id == "surge" then
			...
			if controller.surgeTimer >= slashInterval then
				controller.surgeTimer = 0
				controller.surgeSlash = (controller.surgeSlash or 0) + 1
				SpecialVFX.edgeSlash(controller.part.Position, controller.facing, move.color, folder)
