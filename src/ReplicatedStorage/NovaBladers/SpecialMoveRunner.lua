...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "comet" then
		...
		elseif phase.id == "flare" then
			SpecialVFX.cometFlare(controller.part.Position, color, folder)
		end
	elseif mode == "glacier" then
		if phase.id == "freeze" then
			...
			controller.velocity = Vector3.zero
		elseif phase.id == "spire" then
			controller.spireTimer = 0
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "comet" then
		...
		elseif phase.id == "streak" then
			...
			local pos = controller.part.Position
			SpecialVFX.cometStreak(controller.cometLastPos, pos, move.color, folder)
	...
	elseif mode == "glacier" then
		if phase.id == "freeze" then
			controller.velocity = Vector3.zero
		elseif phase.id == "spire" then
			controller.spireTimer = (controller.spireTimer or 0) + dt
			if controller.spireTimer >= (phase.interval or 0.3) then
				controller.spireTimer = 0
				SpecialVFX.frostSpire(controller.part.Position, phase.range or 7, move.color, folder)
