...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "BlazeInfernoWheel" then
		...
		elseif phase.id == "wheel" then
			controller.wheelHitsLeft = phase.hits or 3
			controller.wheelTimer = 0
		end
	elseif move.id == "GlacierCitadel" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "BlazeInfernoWheel" then
		if phase.id == "ignite" then
			controller.velocity = Vector3.zero
		elseif phase.id == "rush" or phase.id == "wheel" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 80)
		end
		if phase.id == "wheel" then
			controller.wheelTimer = (controller.wheelTimer or 0) + dt
			if controller.wheelTimer >= (phase.hitInterval or 0.2) then
				controller.wheelTimer = 0
				local pos = controller.part.Position
				SpecialVFX.flameWheel(pos, move.color, folder)
				controller:areaHit(allControllers, phase.hitRadius or 6, phase.damage or 12, true)
			end
		end

	elseif move.id == "GlacierCitadel" then
		if phase.id == "crystallize" or phase.id == "citadel" then
			controller.velocity = Vector3.zero
		elseif phase.id == "shatter" then
