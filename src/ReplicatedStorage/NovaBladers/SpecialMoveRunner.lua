...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif move.id == "InfernoSawRush" then
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif move.id == "InfernoSawRush" then
		...
		elseif phase.id == "rush" then
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
			SpecialVFX.infernoTrail(controller.part.Position, controller.facing, move.color, folder)
		elseif phase.id == "saw" then
			controller.sawTimer = (controller.sawTimer or 0) + dt
			if controller.sawTimer >= (phase.hitInterval or 0.2) then
				controller.sawTimer = 0
				local pos = controller.part.Position
				SpecialVFX.sawStrike(pos, move.color, folder)
