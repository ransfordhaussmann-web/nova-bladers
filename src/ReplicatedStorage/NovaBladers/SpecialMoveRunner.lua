...
local PhaseStartHandlers = {}

PhaseStartHandlers.meteor = function(controller, move, phase, target)
...
PhaseStartHandlers.fortress = function(controller, move, phase)
	local color = move.color
...
PhaseStartHandlers.sonic = function(controller, move, phase, target)
	local color = move.color
...
PhaseStartHandlers.eclipse = function(controller, move, phase, target)
	local color = move.color
...
PhaseStartHandlers.lunge = function(controller, move, phase, target)
	local color = move.color
	if phase.id == "windup" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
...
PhaseStartHandlers.bastion = function(controller, move, phase)
	local color = move.color
...
PhaseStartHandlers.solar = function(controller, move, phase, target)
	local color = move.color
	if phase.id == "charge" then
		SpecialVFX.chargeAura(controller, color, phase.duration)
...
PhaseStartHandlers.mirage = function(controller, move, phase, target)
	local color = move.color
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "split" then
		local offset = controller.facing * 5
		controller.miragePos = controller.part.Position - offset
		SpecialVFX.mirageGhost(controller.miragePos, color, folder)
	...
	elseif phase.id == "fade" then
		SpecialVFX.mirageGhost(controller.part.Position, color, folder)
		if controller.miragePos then
			SpecialVFX.mirageGhost(controller.miragePos, color, folder)
		end
	end
end
...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	local handler = PhaseStartHandlers[move.mode]
	if handler then
...
UpdateHandlers.meteor = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
...
UpdateHandlers.fortress = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "burrow" then
		controller.velocity = Vector3.zero
		local pos = controller.part.Position
...
UpdateHandlers.sonic = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "charge" then
...
UpdateHandlers.eclipse = function(controller, move, phase, dt, allControllers)
...
UpdateHandlers.lunge = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "windup" then
		controller.velocity = Vector3.zero
	elseif phase.id == "lunge" then
		...
		controller:checkCollisions(allControllers, true)
	elseif phase.id == "rip" then
		controller.velocity = controller.facing * (move.rushSpeed or 70) * 0.4
		controller.ripTimer = (controller.ripTimer or 0) + dt
		if controller.ripTimer >= (phase.hitInterval or 0.16) then
			controller.ripTimer = 0
			SpecialVFX.lungeSlash(controller.part.Position, controller.facing, move.color, folder)
			controller:areaHit(allControllers, phase.hitRadius or 5, phase.damage or 12, true)
		end
	end
end

UpdateHandlers.bastion = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
...
UpdateHandlers.solar = function(controller, move, phase, dt, allControllers)
	local folder = SpecialVFX.ensureFolder(controller)
	if phase.id == "charge" then
		controller.velocity *= 0.92
	elseif phase.id == "flare" then
		controller.solarTimer = (controller.solarTimer or 0) + dt
		if controller.solarTimer >= (phase.interval or 0.25) then
			controller.solarTimer = 0
			controller.solarCount = (controller.solarCount or 0) + 1
			local range = 3.5 + controller.solarCount * 1.8
	...
	elseif phase.id == "drift" and controller.orbitCenter then
		...
		controller.solarTimer = (controller.solarTimer or 0) + dt
		if controller.solarTimer >= (phase.interval or 0.2) then
			controller.solarTimer = 0
			SpecialVFX.solarFlare(controller.part.Position, 4, move.color, folder)
			controller:areaHit(allControllers, 4, phase.damage or 7, true)
		end
	end
end

UpdateHandlers.mirage = function(controller, move, phase, dt, allControllers)
	if phase.id == "strike" then
		controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 85)
		controller:checkCollisions(allControllers, true)
	elseif phase.id == "fade" then
		controller:areaHit(allControllers, phase.range or 7, phase.damage or 28, true)
		if controller.miragePos then
			local savedPos = controller.part.Position
			controller.part.CFrame = CFrame.new(controller.miragePos) * (controller.part.CFrame - controller.part.CFrame.Position)
			controller:areaHit(allControllers, phase.range or 7, (phase.damage or 28) * 0.7, true)
			controller.part.CFrame = CFrame.new(savedPos) * (controller.part.CFrame - controller.part.CFrame.Position)
		end
	end
end
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	local handler = UpdateHandlers[move.mode]
	if handler then
