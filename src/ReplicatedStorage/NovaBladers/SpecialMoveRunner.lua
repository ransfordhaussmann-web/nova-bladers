...
function SpecialMoveRunner.onPhaseStart(controller, move, phase)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			controller.shatterHitsLeft = phase.hits or 3
		end
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			...
			controller.spiralSpeed = phase.spiralSpeed or 12
			local dir = (getTargetPos(controller, target) - controller.part.Position)
			dir = Vector3.new(dir.X, 0, dir.Z).Unit
			controller.facing = dir
...
function SpecialMoveRunner.update(controller, dt, allControllers)
	...
	elseif mode == "shatter" then
		...
		elseif phase.id == "shatter" then
			...
			if controller.shatterTimer >= (phase.hitInterval or 0.15) then
				controller.shatterTimer = 0
				local pos = controller.part.Position
				SpecialVFX.crystalShardBurst(pos, move.color, folder)
	...
	elseif mode == "blaze" then
		...
		elseif phase.id == "spiral" then
			controller.spiralAngle = (controller.spiralAngle or 0) + (controller.spiralSpeed or 12) * dt
			...
			local y = controller.part.Position.Y
			controller.part.CFrame = CFrame.new(Vector3.new(pos.X, y, pos.Z), center + controller.facing * 3)
			controller.velocity = controller.facing * (phase.rushSpeed or move.rushSpeed or 70)
			SpecialVFX.blazeSpiralTrail(pos, move.color, folder)
			controller:checkCollisions(allControllers, true)
		elseif phase.id == "crown" then
