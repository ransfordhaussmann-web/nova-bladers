local BeyCatalog = {
	{
		id = "NovaStriker",
		name = "Nova Striker",
		beyType = "Attack",
		color = Color3.fromRGB(80, 140, 255),
		accentColor = Color3.fromRGB(120, 210, 255),
		stats = { Attack = 8, Defense = 4, Speed = 7, Stamina = 5 },
		special = "Nova Meteor Shower",
		specialId = "NovaMeteorShower",
		desc = "Attack-Typ: Multi-Hit Meteor-Rush aus der Luft.",
		modelRef = {
			sketchfabId = "6bd1a9f1864a46dba4632307ce6c2660",
			sketchfabUrl = "https://sketchfab.com/models/6bd1a9f1864a46dba4632307ce6c2660",
			referenceName = "Storm Pegasus 105 RF",
			credit = "IcaroAndradeOliveira1",
			studioModelName = "NovaStriker",
		},
	},
	{
		id = "IronShell",
		name = "Iron Shell",
		beyType = "Defense",
		color = Color3.fromRGB(80, 180, 110),
		accentColor = Color3.fromRGB(130, 255, 170),
		stats = { Attack = 4, Defense = 8, Speed = 5, Stamina = 8 },
		special = "Iron Vault Lock",
		specialId = "IronVaultLock",
		desc = "Defense-Typ: Burrow, Schutzmauer und Schockwellen.",
	},
	{
		id = "VoltDash",
		name = "Volt Dash",
		beyType = "Stamina",
		color = Color3.fromRGB(255, 200, 60),
		accentColor = Color3.fromRGB(255, 240, 120),
		stats = { Attack = 6, Defense = 5, Speed = 9, Stamina = 9, SpinDecayMult = 0.65 },
		special = "Volt Sonic Tempest",
		specialId = "VoltSonicTempest",
		desc = "Stamina-Typ: Sonic-Ringe und Orbit-Angriff.",
	},
	{
		id = "ShadowBite",
		name = "Shadow Bite",
		beyType = "Balance",
		color = Color3.fromRGB(140, 80, 220),
		accentColor = Color3.fromRGB(200, 100, 255),
		stats = { Attack = 7, Defense = 6, Speed = 6, Stamina = 6 },
		special = "Shadow Eclipse Fang",
		specialId = "ShadowEclipseFang",
		desc = "Balance-Typ: Dark-Aura, Dive und Venom-Burst.",
	},
	{
		id = "CrimsonFang",
		name = "Crimson Fang",
		beyType = "Attack",
		color = Color3.fromRGB(200, 45, 55),
		accentColor = Color3.fromRGB(255, 90, 70),
		stats = { Attack = 9, Defense = 3, Speed = 8, Stamina = 4 },
		special = "Crimson Fang Rush",
		specialId = "CrimsonFangRush",
		desc = "Attack-Typ: Blutrote Klingen-Serie mit Meteor-Rush.",
		creatorStore = {
			searchTerms = { "spinning top", "attack bey", "red top" },
			hint = "Toolbox → Creator Store → flache Attack-Top mit roten Klingen",
		},
	},
	{
		id = "GraniteFort",
		name = "Granite Fort",
		beyType = "Defense",
		color = Color3.fromRGB(110, 105, 95),
		accentColor = Color3.fromRGB(160, 150, 130),
		stats = { Attack = 3, Defense = 9, Speed = 4, Stamina = 9 },
		special = "Granite Bastion",
		specialId = "GraniteBastion",
		desc = "Defense-Typ: Steinmauer, Burrow und Schockpulse.",
		creatorStore = {
			searchTerms = { "defense top", "heavy bey", "stone top" },
			hint = "Toolbox → Creator Store → breiter Defense-Top mit Steinring",
		},
	},
	{
		id = "SolarDrift",
		name = "Solar Drift",
		beyType = "Stamina",
		color = Color3.fromRGB(255, 170, 50),
		accentColor = Color3.fromRGB(255, 230, 120),
		stats = { Attack = 5, Defense = 6, Speed = 8, Stamina = 10, SpinDecayMult = 0.6 },
		special = "Solar Wind Spiral",
		specialId = "SolarWindSpiral",
		desc = "Stamina-Typ: Sonnenwind-Ringe und Orbit-Spirale.",
		creatorStore = {
			searchTerms = { "stamina top", "flat bey", "gold top" },
			hint = "Toolbox → Creator Store → flacher Stamina-Top in Gold/Orange",
		},
	},
	{
		id = "PhantomEdge",
		name = "Phantom Edge",
		beyType = "Balance",
		color = Color3.fromRGB(70, 90, 180),
		accentColor = Color3.fromRGB(120, 160, 255),
		stats = { Attack = 6, Defense = 5, Speed = 7, Stamina = 7 },
		special = "Phantom Mirage",
		specialId = "PhantomMirage",
		desc = "Balance-Typ: Geister-Aura, Dive und Phantom-Burst.",
		creatorStore = {
			searchTerms = { "balance top", "blue bey", "phantom top" },
			hint = "Toolbox → Creator Store → schlanker Balance-Top in Blau/Violett",
		},
	},
	{
		id = "CrystalEdge",
		name = "Crystal Edge",
		beyType = "Attack",
		color = Color3.fromRGB(120, 220, 240),
		accentColor = Color3.fromRGB(200, 255, 255),
		stats = { Attack = 8, Defense = 5, Speed = 7, Stamina = 5 },
		special = "Crystal Shard Rain",
		specialId = "CrystalShardRain",
		desc = "Attack-Typ: Kristall-Splitter-Regen mit schnellem Rush.",
		creatorStore = {
			searchTerms = { "crystal top", "ice bey", "sharp top" },
			hint = "Toolbox → Creator Store → kantiger Attack-Top mit Kristall-Look",
		},
	},
	{
		id = "BlazeCrown",
		name = "Blaze Crown",
		beyType = "Balance",
		color = Color3.fromRGB(255, 120, 40),
		accentColor = Color3.fromRGB(255, 200, 60),
		stats = { Attack = 7, Defense = 5, Speed = 7, Stamina = 6 },
		special = "Blaze Inferno Ring",
		specialId = "BlazeInfernoRing",
		desc = "Balance-Typ: Feuer-Ringe und Inferno-Orbit.",
		creatorStore = {
			searchTerms = { "fire top", "flame bey", "crown top" },
			hint = "Toolbox → Creator Store → kronenförmiger Top mit Flammen-Akzenten",
		},
	},
}

return BeyCatalog
