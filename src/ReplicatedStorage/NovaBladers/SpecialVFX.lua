...
function SpecialVFX.iceField(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local field = Instance.new("Part")
	field.Shape = Enum.PartType.Cylinder
	field.Size = Vector3.new(0.15, 8, 8)
	field.Anchored = true
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 3.5, 0.4, math.sin(angle) * 3.5))
		shard.Parent = folder
	...
	Debris:AddItem(field, duration + 0.15)
end
...
function SpecialVFX.iceCrownRing(controller, color, duration)
	...
	local spikes = Instance.new("Part")
	spikes.Shape = Enum.PartType.Ball
	spikes.Size = Vector3.new(4, 4, 4)
	spikes.Anchored = true
	spikes.CanCollide = false
	spikes.Material = Enum.Material.Neon
	spikes.Color = Color3.fromRGB(200, 240, 255)
	spikes.Transparency = 0.5
	spikes.CFrame = CFrame.new(controller.part.Position + Vector3.new(0, 1.5, 0))
	spikes.Parent = folder
...
function SpecialVFX.forgeTrail(position, color, folder)
...
function SpecialVFX.lavaBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
