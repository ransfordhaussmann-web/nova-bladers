...
function SpecialVFX.crimsonVortex(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	...
	TweenService:Create(ring, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
		Orientation = Vector3.new(0, 0, 720),
		Transparency = 0.85,
	}):Play()
	Debris:AddItem(ring, duration + 0.1)
end
...
function SpecialVFX.crimsonSlash(position, color, folder)
	...
	slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.random() * math.pi * 2, math.rad(25))
	slash.Parent = folder
...
function SpecialVFX.stoneRampart(controller, color, duration)
	...
	for i = 0, 5 do
		...
		TweenService:Create(pillar, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			Size = Vector3.new(1.4, 3.5, 1.4),
		}):Play()
...
function SpecialVFX.voidBlink(fromPos, toPos, color, folder)
	...
	TweenService:Create(trail, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
function SpecialVFX.voidBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
