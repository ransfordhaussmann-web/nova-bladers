...
function SpecialVFX.fireAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		flame.Size = Vector3.new(0.6, 1.2, 0.6)
		flame.Anchored = true
		...
		flame.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.2, 0.4, math.sin(angle) * 2.2))
		flame.Parent = folder
...
function SpecialVFX.fireTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(1.4, 0.8, 1.4)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
...
function SpecialVFX.infernoRoar(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	core.Size = Vector3.new(3, 3, 3)
	core.Anchored = true
...
function SpecialVFX.iceShard(origin, direction, color, folder)
	...
	local endPos = origin + direction * 10
