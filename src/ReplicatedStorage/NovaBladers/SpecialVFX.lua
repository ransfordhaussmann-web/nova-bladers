...
function SpecialVFX.flameLanceTrail(position, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(1.4, 0.6, 2.8)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
...
function SpecialVFX.flameLanceImpact(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostCrystalRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FrostCrystalRing"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.frostShatterWave(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
	...
	local sparkles = Instance.new("Sparkles")
	sparkles.SparkleColor = Color3.fromRGB(180, 230, 255)
