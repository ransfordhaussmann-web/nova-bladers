...
function SpecialVFX.infernoTrail(controller, color, folder)
	local pos = controller.part.Position
	local trail = Instance.new("Part")
	...
	trail.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	trail.Parent = folder
...
function SpecialVFX.frostCrownRing(controller, color, duration)
	...
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(0.5, 5.5, 5.5)
	ring.Anchored = true
	ring.CanCollide = false
	...
	spikes.Size = Vector3.new(0.35, 4.8, 4.8)
	spikes.Anchored = true
	spikes.CanCollide = false
	spikes.Material = Enum.Material.Neon
	spikes.Color = Color3.fromRGB(220, 245, 255)
	spikes.Transparency = 0.4
...
function SpecialVFX.iceShardBurst(origin, range, color, folder)
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(origin + dir * 2) * CFrame.lookAt(origin + dir * 2, origin + dir * 4)
		shard.Parent = folder
