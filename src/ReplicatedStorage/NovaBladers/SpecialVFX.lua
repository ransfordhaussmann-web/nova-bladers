...
function SpecialVFX.ripperSlash(controller, targetPos, color, folder)
	local start = controller.part.Position
	local dir = (targetPos - start).Unit
	...
	slash.CFrame = CFrame.new((start + targetPos) / 2, targetPos) * CFrame.Angles(math.rad(90), 0, 0)
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 4, 8),
		Transparency = 1,
	}):Play()
	...
	for i = 1, 3 do
		...
		mark.CanCollide = false
		mark.Material = Enum.Material.Neon
		mark.Color = Color3.fromRGB(255, 120, 80)
		mark.CFrame = CFrame.new(start + dir * (3 * i))
		mark.Parent = folder
...
function SpecialVFX.stoneSlam(position, color, folder)
	...
	slab.CFrame = CFrame.new(position) * CFrame.Angles(0, 0, math.rad(90))
	slab.Parent = folder

	TweenService:Create(slab, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
		Size = Vector3.new(1.2, 7, 7),
		Transparency = 0.6,
	}):Play()
	Debris:AddItem(slab, 0.5)
end
...
function SpecialVFX.stoneBastion(controller, color, duration)
	...
	inner.Color = Color3.fromRGB(200, 190, 170)
...
function SpecialVFX.mirageClone(position, color, folder)
	...
	TweenService:Create(clone, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.4, 4.5, 4.5),
	}):Play()
	Debris:AddItem(clone, 0.65)
end

function SpecialVFX.phantomStrike(position, color, folder)
	local strike = Instance.new("Part")
	strike.Size = Vector3.new(0.2, 3, 3)
	strike.Anchored = true
	strike.CanCollide = false
	strike.Material = Enum.Material.Neon
	strike.Color = color
	strike.Transparency = 0.25
	strike.CFrame = CFrame.new(position) * CFrame.Angles(0, math.random() * math.pi * 2, math.rad(75))
	strike.Parent = folder

	TweenService:Create(strike, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 5, 5),
		Transparency = 1,
	}):Play()
	Debris:AddItem(strike, 0.3)
end
