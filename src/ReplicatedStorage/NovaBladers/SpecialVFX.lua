...
function SpecialVFX.cycloneRing(origin, angle, color, folder)
	local radius = 4.5
	for i = 0, 2 do
		local a = angle + i * (math.pi * 2 / 3)
		local pos = origin + Vector3.new(math.cos(a) * radius, 0.6, math.sin(a) * radius)
		local spark = Instance.new("Part")
		spark.Size = Vector3.new(0.7, 1.2, 0.7)
		spark.Anchored = true
		spark.CanCollide = false
		spark.Material = Enum.Material.Neon
		spark.Color = color
...
function SpecialVFX.frostShield(controller, color, duration)
	...
	task.delay(duration, function()
		if shield.Parent then
			TweenService:Create(shield, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Transparency = 1,
				Size = Vector3.new(7, 7, 7),
			}):Play()
...
function SpecialVFX.glacierShatter(position, color, folder)
	for i = 0, 5 do
		...
		shard.Size = Vector3.new(0.6, 1.4, 0.3)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.15
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(35)) * CFrame.new(0, 0, 3)
		shard.Parent = folder
