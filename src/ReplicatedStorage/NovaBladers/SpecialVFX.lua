...
function SpecialVFX.crimsonSlash(position, slashDir, color, folder)
	...
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.6, 0), position + slashDir)
	slash.Parent = folder
...
function SpecialVFX.auroraFlare(origin, range, color, accentColor, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
