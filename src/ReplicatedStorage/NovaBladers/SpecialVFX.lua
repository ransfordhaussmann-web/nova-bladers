...
function SpecialVFX.forgeShatter(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	...
	for i = 1, 6 do
		...
		shard.Color = Color3.fromRGB(255, 180, 60)
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + Vector3.new(math.cos(angle) * range * 0.5, 1, math.sin(angle) * range * 0.5),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.5, 0.5, math.sin(angle) * 2.5))
...
function SpecialVFX.iceBarrier(controller, color, duration)
	...
	crown.Size = Vector3.new(0.5, 4, 4)
	crown.Anchored = true
	crown.CanCollide = false
	crown.Material = Enum.Material.Neon
	crown.Color = Color3.fromRGB(200, 240, 255)
	crown.Transparency = 0.4
	crown.CFrame = ring.CFrame
	crown.Parent = folder
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 4 do
		...
		local endPos = origin + Vector3.new(math.cos(angle) * dist, 0.2, math.sin(angle) * dist)
		shard.CFrame = CFrame.new(origin, endPos) * CFrame.Angles(math.rad(90), 0, 0)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos) * CFrame.Angles(math.rad(90), angle, 0),
			Transparency = 1,
		}):Play()
