...
function SpecialVFX.bladeCyclone(controller, color, duration)
	...
	TweenService:Create(ring, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
		Orientation = Vector3.new(0, 0, 90 + duration * 720),
	}):Play()
	Debris:AddItem(ring, duration + 0.1)
end
...
function SpecialVFX.crimsonBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.quakeRumble(position, color, folder)
	for i = 1, 3 do
		local wave = Instance.new("Part")
		wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.heatTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(1, 0.4, 1)
	trail.Shape = Enum.PartType.Ball
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
...
function SpecialVFX.phantomTrail(startPos, endPos, color, folder)
	local dir = (endPos - startPos).Unit
	for i = 1, 4 do
		local t = i / 4
		local p = startPos + dir * (10 * t)
		local mark = Instance.new("Part")
...
function SpecialVFX.phantomCut(position, color, folder)
	for i = 0, 2 do
		...
		cut.Size = Vector3.new(0.2, 0.2, 5)
		cut.Anchored = true
		cut.CanCollide = false
		cut.Material = Enum.Material.Neon
		cut.Color = color
		cut.Transparency = 0.15
		cut.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 60), math.rad(80))
		cut.Parent = folder

		TweenService:Create(cut, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
		}):Play()
		Debris:AddItem(cut, 0.3)
	end
end
