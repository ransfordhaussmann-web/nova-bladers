...
function SpecialVFX.shredTrail(controller, color, folder)
	local trail = Instance.new("Part")
	trail.Shape = Enum.PartType.Cylinder
	trail.Size = Vector3.new(0.12, 3.5, 3.5)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
...
function SpecialVFX.shredSlash(position, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.3, 0.15, 3.5)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.2
	slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(math.random(0, 180)), math.rad(35))
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.15, 0.08, 5),
	}):Play()
...
function SpecialVFX.frostCharge(controller, color, duration)
	...
	for i = 1, 6 do
		...
		local angle = i * 60
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 3, 0.5, math.sin(math.rad(angle)) * 3))
		shard.Parent = folder
...
function SpecialVFX.frostShatter(position, color, folder)
	for i = 1, 8 do
		...
		local angle = i * 45
		shard.CFrame = CFrame.new(position) * CFrame.Angles(math.rad(20), math.rad(angle), 0)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(math.cos(math.rad(angle)) * 6, 2, math.sin(math.rad(angle)) * 6),
			Transparency = 1,
		}):Play()
