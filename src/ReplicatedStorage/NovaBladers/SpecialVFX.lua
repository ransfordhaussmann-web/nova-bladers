...
function SpecialVFX.bladeCyclone(controller, color, duration)
	...
	for i = 0, 2 do
		...
		blade.Size = Vector3.new(0.3, 0.8, 2.5)
		blade.Anchored = true
		blade.CanCollide = false
		blade.Material = Enum.Material.Neon
		blade.Color = color
		blade.Transparency = 0.2
		blade.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(i * 120), math.rad(70))
		blade.Parent = folder
...
function SpecialVFX.cycloneSlash(position, color, folder)
	...
	slash.Size = Vector3.new(0.25, 0.6, 3.5)
	slash.Anchored = true
...
function SpecialVFX.stoneFortress(controller, color, duration)
...
function SpecialVFX.solarFlareBurst(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	core.Size = Vector3.new(3, 3, 3)
	core.Anchored = true
...
function SpecialVFX.phantomBlink(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	controller._savedTransparency = controller._savedTransparency or controller.part.Transparency
...
function SpecialVFX.afterimageBurst(position, color, folder)
	for i = 1, 4 do
		...
		local ghost = Instance.new("Part")
		ghost.Size = Vector3.new(1.2, 0.4, 2.2)
		ghost.Anchored = true
		ghost.CanCollide = false
		ghost.Material = Enum.Material.ForceField
		ghost.Color = color
		ghost.Transparency = 0.3
		ghost.CFrame = CFrame.new(position + offset) * CFrame.Angles(0, math.rad(angle), math.rad(60))
		ghost.Parent = folder
