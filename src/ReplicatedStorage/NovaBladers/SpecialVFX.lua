...
function SpecialVFX.crystalAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 1.2, 1.2, math.sin(angle) * 1.2))
...
function SpecialVFX.crystalShatter(position, color, folder, shardCount)
	...
	for i = 1, shardCount do
		...
		local endPos = position + Vector3.new(math.cos(angle) * 8, 1.5, math.sin(angle) * 8)
