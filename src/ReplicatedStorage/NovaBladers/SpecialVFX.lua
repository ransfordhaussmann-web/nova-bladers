...
function SpecialVFX.frostFreeze(controller, color, duration)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2), 0.3, math.random(-2, 2)))
			* CFrame.Angles(math.rad(math.random(-20, 20)), math.rad(i * 60), math.rad(math.random(-15, 15)))
		shard.Parent = folder

		local tween = TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.iceBarrier(controller, color, duration)
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(1.2, 5, 5)
	inner.Anchored = true
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 4 do
		...
		local target = origin + Vector3.new(math.cos(angle) * range * 0.5, 0.3, math.sin(angle) * range * 0.5)
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(target) * CFrame.Angles(0, angle, math.rad(35)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.clawTrail(controller, targetPos, color, folder)
	SpecialVFX.diveTrail(controller, targetPos, color, folder)
	local start = controller.part.Position
	local dir = (targetPos - start).Unit
	for i = 1, 3 do
		local mark = Instance.new("Part")
		mark.Size = Vector3.new(1.2, 0.3, 0.8)
		mark.Anchored = true
		mark.CanCollide = false
		mark.Material = Enum.Material.Neon
		mark.Color = color
		mark.Transparency = 0.2
		mark.CFrame = CFrame.new(start + dir * (4 * i)) * CFrame.Angles(0, math.atan2(dir.Z, dir.X), math.rad(25))
		mark.Parent = folder
		Debris:AddItem(mark, 0.35)
	end
end
