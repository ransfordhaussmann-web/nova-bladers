...
function SpecialVFX.cometTrail(position, color, folder)
...
function SpecialVFX.iceCrystallize(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	for i = 1, 6 do
		local angle = (i - 1) * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0.5, 2.2)
		...
		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos) * offset * CFrame.new(0, 0, -1.5),
			Transparency = 0.6,
		}):Play()
...
function SpecialVFX.iceVaultRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "IceVaultRing"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.frostPulse(origin, range, color, folder)
	...
	for i = 1, 4 do
		local angle = i * 90
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.25, 0.8, 0.4)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = Color3.fromRGB(220, 250, 255)
		shard.Transparency = 0.15
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0.4, 1.5)
		shard.Parent = folder
