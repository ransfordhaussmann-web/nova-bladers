...
function SpecialVFX.bladeSwipe(position, color, folder)
	...
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0)) * CFrame.Angles(0, math.random() * math.pi * 2, 0)
	slash.Parent = folder
	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(7, 0.1, 2),
	}):Play()
...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	shield.Size = Vector3.new(1.2, 5, 5)
...
function SpecialVFX.stoneShards(origin, range, color, folder)
	for i = 1, 4 do
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos) * CFrame.Angles(0, angle, math.rad(30)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.quakePulse(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.solarTrail(controller, color, folder)
	...
	trail.CFrame = CFrame.new(controller.part.Position)
	trail.Parent = folder
...
function SpecialVFX.phantomSlash(position, color, folder)
	for i = 1, 3 do
		...
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(6, 0.2, 0.8)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.2
		slash.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0)) * CFrame.Angles(0, angle, math.rad(20))
		slash.Parent = folder
		TweenService:Create(slash, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(8, 0.1, 1.2),
		}):Play()
