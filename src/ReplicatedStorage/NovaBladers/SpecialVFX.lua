...
function SpecialVFX.afterburnBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2, 0.5, math.sin(angle) * 2))
		shard.Parent = folder
...
function SpecialVFX.iceShardBurst(origin, range, color, folder)
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * 1.5, 0.6, math.sin(angle) * 1.5))
