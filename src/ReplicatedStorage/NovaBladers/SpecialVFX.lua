...
function SpecialVFX.vortexTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.vortexFinisher(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 3, 1, math.sin(angle) * 3))
...
function SpecialVFX.slowField(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.freezeBurst(position, color, folder)
	...
	TweenService:Create(spikes, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(10, 10, 10),
		Transparency = 1,
	}):Play()
	Debris:AddItem(core, 0.5)
	Debris:AddItem(spikes, 0.5)
end
