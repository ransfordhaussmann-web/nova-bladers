...
function SpecialVFX.flameAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 6 do
		...
		flame.Color = color
		flame.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2), 0.3, math.random(-2, 2)))
		flame.Parent = folder
...
function SpecialVFX.spiralTrail(position, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.7, 0.7, 0.7)
	trail.Shape = Enum.PartType.Ball
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	...
	trail.CFrame = CFrame.new(position + Vector3.new(0, 0.4, 0))
	trail.Parent = folder
...
function SpecialVFX.flameFlare(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.chillShield(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "ChillShield"
	ring.Shape = Enum.PartType.Cylinder
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.8, 4.2, 4.2)
	inner.Anchored = true
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
...
function SpecialVFX.iceShard(fromPos, toPos, color, folder)
	...
	shard.CFrame = CFrame.new(fromPos, toPos)
	shard.Parent = folder
	...
	TweenService:Create(shard, TweenInfo.new(0.18, Enum.EasingStyle.Linear), {
		CFrame = CFrame.new(toPos),
		Transparency = 1,
	}):Play()
	Debris:AddItem(shard, 0.2)

	local impact = Instance.new("Part")
	impact.Shape = Enum.PartType.Ball
	impact.Size = Vector3.new(1.2, 1.2, 1.2)
	impact.Anchored = true
	impact.CanCollide = false
	impact.Material = Enum.Material.Neon
	impact.Color = color
	impact.Transparency = 0.4
	impact.CFrame = CFrame.new(toPos)
...
function SpecialVFX.iceShatter(position, range, color, folder)
	...
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, range * 0.5)
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			CFrame = shard.CFrame * CFrame.new(0, 0, range * 0.4),
		}):Play()
