...
function SpecialVFX.flameSlash(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(2.5, 0.25, 0.8)
	slash.Anchored = true
...
function SpecialVFX.iceShard(position, color, folder)
	...
	shard.CFrame = CFrame.new(position) * CFrame.Angles(math.random(), math.random(), math.random())
	shard.Parent = folder
