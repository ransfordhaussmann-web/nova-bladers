...
function SpecialVFX.crystalizeAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 6 do
		...
		local angle = i * 60
		shard.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 1.5, 2)
		shard.Parent = folder
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(position + offset) * CFrame.Angles(0, math.rad(angle), math.rad(45))
		shard.Parent = folder
