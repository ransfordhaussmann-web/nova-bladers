...
function SpecialVFX.bladeSlash(position, angle, color, folder)
	...
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0)) * CFrame.Angles(0, angle, math.rad(70))
	slash.Parent = folder
...
function SpecialVFX.stoneBastion(controller, color, duration)
	...
	glow.Color = Color3.fromRGB(180, 170, 150)
...
function SpecialVFX.phantomDash(position, dir, color, folder)
	for i = 1, 4 do
		local t = i / 4
		local p = position - dir * (3 * t)
		local ghost = Instance.new("Part")
		ghost.Shape = Enum.PartType.Ball
		ghost.Size = Vector3.new(1.5 - t * 0.3, 1.5 - t * 0.3, 1.5 - t * 0.3)
		ghost.Anchored = true
...
function SpecialVFX.edgeBurst(position, color, folder)
	for i = 0, 2 do
		local angle = i * 120
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.15, 0.6, 5)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
