...
function SpecialVFX.spiralSlash(origin, color, folder)
	...
	slash.CFrame = CFrame.new(origin) * CFrame.Angles(0, math.rad(math.random(0, 360)), math.rad(35))
	slash.Parent = folder
...
function SpecialVFX.frostCoil(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local coil = Instance.new("Part")
	coil.Shape = Enum.PartType.Cylinder
	coil.Size = Vector3.new(1.2, 5, 5)
	coil.Anchored = true
	coil.CanCollide = false
	coil.Material = Enum.Material.Glass
	coil.Color = color
	coil.Transparency = 0.35
	coil.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	coil.Parent = folder

	TweenService:Create(coil, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.8, 7, 7),
		Transparency = 0.75,
	}):Play()
	Debris:AddItem(coil, duration + 0.1)
end
