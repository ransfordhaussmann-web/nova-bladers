...
function SpecialVFX.vortexSpiral(controller, color, folder)
	local pos = controller.part.Position
	for i = 1, 6 do
		...
		spark.Material = Enum.Material.Neon
		spark.Color = color
		spark.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * r, 0.4, math.sin(angle) * r))
		spark.Parent = folder
...
function SpecialVFX.eruptionBurst(origin, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 8 do
		...
		shard.Color = color
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * 1.5, 0.8, math.sin(angle) * 1.5))
...
function SpecialVFX.statusFrost(controller, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	...
	frost.Color = Color3.fromRGB(180, 230, 255)
