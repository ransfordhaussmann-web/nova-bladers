...
function SpecialVFX.infernoSpiralTrail(position, color, folder)
	local trail = Instance.new("Part")
...
function SpecialVFX.infernoFlareBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostChillShield(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
...
function SpecialVFX.frostShard(fromPos, toPos, color, folder)
	...
	local mid = fromPos + dir * 4 + Vector3.new(0, 1.2, 0)
	...
	shard.CFrame = CFrame.new(mid, mid + dir)
	shard.Parent = folder
