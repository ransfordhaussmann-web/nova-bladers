...
function SpecialVFX.lungeSlash(origin, facing, color, folder)
	...
	slash.CFrame = CFrame.new(origin + facing * 2) * CFrame.lookAt(Vector3.zero, facing) * CFrame.Angles(0, math.rad(90), math.rad(35))
	slash.Parent = folder
...
function SpecialVFX.solarFlare(origin, range, color, folder)
	...
	flare.Size = Vector3.new(0.25, 3, 3)
	flare.Anchored = true
...
function SpecialVFX.mirageSplit(controller, color, folder)
	...
	for i = 1, 3 do
		...
		local offset = Vector3.new(math.cos(angle) * 3, 0.3, math.sin(angle) * 3)
		...
		TweenService:Create(ghost, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 0.9,
			CFrame = CFrame.new(pos + offset * 0.4) * CFrame.Angles(0, 0, math.rad(90)),
		}):Play()
