...
function SpecialVFX.shardBurst(position, range, color, folder)
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + offset * 2.2 + Vector3.new(0, 1.5, 0)) * CFrame.Angles(0, angle, math.rad(60)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.blazeErupt(position, color, folder)
	...
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = Color3.fromRGB(255, 180, 50)
