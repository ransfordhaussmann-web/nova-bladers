...
function SpecialVFX.crossSlash(origin, facing, color, folder)
	...
	for _, dir in ipairs({ facing, -facing, perp, -perp }) do
		...
		slash.CFrame = CFrame.lookAt(origin, origin + dir) * CFrame.new(0, 0, -3)
		slash.Parent = folder
...
function SpecialVFX.iceCrystalBurst(origin, range, color, folder)
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(origin + dir * 2) * CFrame.Angles(0, angle, math.rad(25))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + dir * range) * CFrame.Angles(0, angle, math.rad(25)),
			Transparency = 1,
		}):Play()
