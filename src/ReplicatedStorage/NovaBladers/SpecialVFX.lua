...
function SpecialVFX.flameClawSwipe(position, facing, color, folder)
	for i = -1, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 35), 0) * CFrame.new(0, 0, 2.2)
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.25, 1.8, 2.8)
		slash.Anchored = true
...
function SpecialVFX.iceShardBurst(origin, range, color, folder)
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, angle, math.rad(25)) * CFrame.new(0, 0.3, range * 0.45)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.1, 0.1, 1.8),
		}):Play()
