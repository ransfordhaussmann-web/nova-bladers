...
function SpecialVFX.cycloneBlade(position, color, folder)
	for i = 0, 2 do
		local angle = i * 120
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 3.5)
		...
		blade.CFrame = CFrame.new(position) * offset
		blade.Parent = folder
...
function SpecialVFX.stoneShatter(position, range, color, folder)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), math.rad(20)) * CFrame.new(0, 0, range * 0.5)
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			CFrame = CFrame.new(position) * offset * CFrame.new(0, 2, range * 0.3),
		}):Play()
...
function SpecialVFX.edgeSlash(position, facing, range, color, folder)
	for i = -1, 1 do
		local angle = i * 35
		local dir = (CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(facing.X, 0, facing.Z)).Position
		...
		slash.Size = Vector3.new(0.2, 0.8, range)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.15
		slash.CFrame = CFrame.new(position + dir * range * 0.4, position + dir * range * 0.4 + Vector3.new(0, 0.5, 0))
		slash.Parent = folder
