...
function SpecialVFX.blazeSpiralTrail(position, color, folder)
	local trail = Instance.new("Part")
	trail.Shape = Enum.PartType.Ball
...
function SpecialVFX.blazeIgnite(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostAnchorRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FrostAnchor"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.frostBindBurst(position, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	...
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(math.cos(angle) * 4, 0, math.sin(angle) * 4),
			Transparency = 1,
		}):Play()
