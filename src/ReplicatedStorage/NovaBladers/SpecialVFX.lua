...
function SpecialVFX.frostVeil(controller, color, duration)
	...
	veil.Size = Vector3.new(1.8, 6.5, 6.5)
	veil.Anchored = true
...
function SpecialVFX.iceShardBurst(position, color, folder)
	for i = 1, 6 do
		...
		local endPos = position + Vector3.new(math.cos(angle) * 5, 0.5, math.sin(angle) * 5)
...
function SpecialVFX.frostShockwave(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
