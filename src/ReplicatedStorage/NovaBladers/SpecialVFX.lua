...
function SpecialVFX.forgeSparks(controller, color, duration)
	...
	for i = 1, 6 do
		...
		TweenService:Create(spark, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(math.random(-1, 1), 2.5, math.random(-1, 1))),
			Transparency = 1,
		}):Play()
		Debris:AddItem(spark, duration + 0.1)
	end
...
function SpecialVFX.shockRing(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
	wave.Size = Vector3.new(0.35, 3, 3)
	wave.Anchored = true
	wave.CanCollide = false
	wave.Material = Enum.Material.Neon
	wave.Color = color
...
function SpecialVFX.crystalShards(controller, color, duration)
	...
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(angle), math.rad(70)) * CFrame.new(0, 0.5, 2.2)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(0, 1.5, 0.5),
			Transparency = 0.5,
		}):Play()
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 10 do
		...
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 0.6, 0.2)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.1
		shard.CFrame = CFrame.new(position) * CFrame.Angles(math.random(-30, 30), angle, math.random(-30, 30))
		shard.Parent = folder
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos) * shard.CFrame.Rotation,
			Transparency = 1,
		}):Play()
