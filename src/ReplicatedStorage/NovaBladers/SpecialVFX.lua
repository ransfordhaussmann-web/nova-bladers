...
function SpecialVFX.featherTrail(fromPos, toPos, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.6, 0.3, 1.2)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	...
	trail.CFrame = CFrame.new(mid, mid + dir) * CFrame.Angles(0, math.rad(90), 0)
	trail.Parent = folder
...
function SpecialVFX.featherBurst(position, color, folder)
	for i = 1, 8 do
		...
		feather.Size = Vector3.new(0.3, 0.5, 1.4)
		...
		feather.CFrame = CFrame.new(position + offset, position)
		feather.Parent = folder
		TweenService:Create(feather, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.1, 0.1, 0.3),
		}):Play()
		Debris:AddItem(feather, 0.45)
	end
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
...
function SpecialVFX.anchorSlam(position, color, folder)
	local anchor = Instance.new("Part")
	anchor.Size = Vector3.new(1.2, 2.5, 0.8)
	anchor.Anchored = true
	anchor.CanCollide = false
	anchor.Material = Enum.Material.Metal
	anchor.Color = Color3.fromRGB(80, 80, 90)
	anchor.CFrame = CFrame.new(position + Vector3.new(0, 3, 0)) * CFrame.Angles(math.rad(30), 0, 0)
	anchor.Parent = folder
	TweenService:Create(anchor, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
		CFrame = CFrame.new(position) * CFrame.Angles(math.rad(30), 0, 0),
	}):Play()
	Debris:AddItem(anchor, 0.35)
	SpecialVFX.pulseWave(position, 6, color, folder)
end

function SpecialVFX.slashArc(origin, facing, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.15, 4, 0.8)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.2
	local pos = origin + facing * 2
	slash.CFrame = CFrame.new(pos, pos + facing) * CFrame.Angles(0, math.rad(90), math.rad(45))
	slash.Parent = folder
	TweenService:Create(slash, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 6, 0.2),
		Transparency = 1,
	}):Play()
	Debris:AddItem(slash, 0.25)
end

function SpecialVFX.rampartWall(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "RampartWall"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.phantomSlash(origin, facing, color, folder)
	for i = 1, 3 do
		local offset = facing * (i * 1.5) + Vector3.new(0, 0.3, 0)
		local mark = Instance.new("Part")
		mark.Size = Vector3.new(0.1, 3, 1.5)
		mark.Anchored = true
		mark.CanCollide = false
		mark.Material = Enum.Material.Neon
		mark.Color = color
		...
		mark.CFrame = CFrame.new(pos, pos + facing) * CFrame.Angles(0, math.rad(90), math.rad(30))
		mark.Parent = folder
