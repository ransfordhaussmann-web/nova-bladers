...
function SpecialVFX.flameFlare(origin, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.iceShard(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.Color = color
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * range * 0.5, 0.4, math.sin(angle) * range * 0.5))
...
function SpecialVFX.frostShatter(origin, range, color, folder)
	...
	for i = 1, 8 do
		...
		chip.Color = Color3.fromRGB(220, 250, 255)
		chip.Transparency = 0.2
		chip.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * 1.5, 0.5, math.sin(angle) * 1.5))
		chip.Parent = folder

		TweenService:Create(chip, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * range * 0.8, 1.2, math.sin(angle) * range * 0.8)),
			Transparency = 1,
		}):Play()
