...
function SpecialVFX.flameSpiral(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.shatterBurst(position, color, folder)
	...
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.4, 1.2, 0.3)
		shard.Anchored = true
		...
		local offset = Vector3.new(math.cos(math.rad(angle)) * 5, 0.5, math.sin(math.rad(angle)) * 5)
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + offset) * CFrame.Angles(0, math.rad(angle), math.rad(60)),
			Transparency = 1,
		}):Play()
