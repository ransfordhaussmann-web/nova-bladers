...
function SpecialVFX.frostCrystal(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, angle, math.rad(30))
...
function SpecialVFX.frostShard(origin, color, folder)
	...
	shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * dist, 1.5, math.sin(angle) * dist))
		* CFrame.Angles(math.rad(math.random(-40, 40)), angle, math.rad(math.random(-40, 40)))
	shard.Parent = folder
