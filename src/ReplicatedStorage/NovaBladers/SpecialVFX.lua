...
function SpecialVFX.prismShatter(position, color, folder)
	for i = 0, 5 do
		local angle = i * 60
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.6, 1.2, 0.15)
		shard.Anchored = true
		...
		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(math.cos(math.rad(angle)) * 5, 2, math.sin(math.rad(angle)) * 5),
			Transparency = 1,
		}):Play()
