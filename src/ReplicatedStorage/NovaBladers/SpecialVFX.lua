...
function SpecialVFX.fangBurst(position, color, folder)
	...
	spikes.Size = Vector3.new(0.5, 3, 3)
	spikes.Anchored = true
	spikes.CanCollide = false
	spikes.Material = Enum.Material.Neon
	spikes.Color = Color3.fromRGB(255, 140, 80)
	spikes.Transparency = 0.3
	spikes.CFrame = CFrame.new(position) * CFrame.Angles(0, 0, math.rad(90))
	spikes.Parent = folder
	...
	TweenService:Create(spikes, TweenInfo.new(0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, 10, 10),
		Transparency = 1,
	}):Play()
	Debris:AddItem(burst, 0.42)
	Debris:AddItem(spikes, 0.42)
end
...
function SpecialVFX.iceBastion(controller, color, duration)
	...
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
	inner.Color = Color3.fromRGB(230, 245, 255)
...
function SpecialVFX.iceShardPulse(origin, range, color, folder)
	...
	shard.CFrame = CFrame.new(origin + Vector3.new(0, 0.6, 0))
	shard.Parent = folder
