...
function SpecialVFX.bladeCyclone(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local cyclone = Instance.new("Part")
	cyclone.Name = "BladeCyclone"
	cyclone.Shape = Enum.PartType.Cylinder
	cyclone.Size = Vector3.new(0.4, 4, 4)
	cyclone.Anchored = true
	cyclone.CanCollide = false
	cyclone.Material = Enum.Material.Neon
	cyclone.Color = color
	cyclone.Transparency = 0.35
	cyclone.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	cyclone.Parent = folder

	for i = 0, 2 do
		local blade = Instance.new("Part")
		blade.Size = Vector3.new(0.3, 0.5, 2.5)
		blade.Anchored = true
		blade.CanCollide = false
		blade.Material = Enum.Material.Neon
		blade.Color = color
		blade.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(i * 120), math.rad(35)) * CFrame.new(0, 0, 2.2)
		blade.Parent = folder
		Debris:AddItem(blade, duration + 0.1)
	end

	TweenService:Create(cyclone, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
		Size = Vector3.new(0.4, 7, 7),
		Transparency = 0.7,
	}):Play()
	Debris:AddItem(cyclone, duration + 0.15)
end

function SpecialVFX.bastionShield(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local shield = Instance.new("Part")
	shield.Name = "BastionShield"
	shield.Shape = Enum.PartType.Cylinder
	shield.Size = Vector3.new(1.8, 5.5, 5.5)
	shield.Anchored = true
	shield.CanCollide = false
	shield.Material = Enum.Material.Slate
	shield.Color = color
	shield.Transparency = 0.3
	shield.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	shield.Parent = folder

	local glow = Instance.new("Part")
	glow.Shape = Enum.PartType.Cylinder
	glow.Size = Vector3.new(0.4, 4.5, 4.5)
	glow.Anchored = true
	glow.CanCollide = false
	glow.Material = Enum.Material.Neon
	glow.Color = color
	glow.Transparency = 0.5
	glow.CFrame = shield.CFrame
	glow.Parent = folder
...
function SpecialVFX.stoneShatter(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	...
	for i = 0, 5 do
		...
		shard.Size = Vector3.new(0.6, 0.6, 0.6)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Concrete
		shard.Color = color
		local angle = i * 60
		shard.CFrame = CFrame.new(position + Vector3.new(math.cos(math.rad(angle)) * 2, 0.5, math.sin(math.rad(angle)) * 2))
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + Vector3.new(math.cos(math.rad(angle)) * range, 1.5, math.sin(math.rad(angle)) * range)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.solarOrbit(origin, radius, color, folder)
	local orbit = Instance.new("Part")
	orbit.Shape = Enum.PartType.Cylinder
	orbit.Size = Vector3.new(0.12, radius * 2, radius * 2)
	orbit.Anchored = true
	orbit.CanCollide = false
	orbit.Material = Enum.Material.Neon
	orbit.Color = color
	orbit.Transparency = 0.4
	orbit.CFrame = CFrame.new(origin + Vector3.new(0, 0.3, 0)) * CFrame.Angles(0, 0, math.rad(90))
	orbit.Parent = folder
	Debris:AddItem(orbit, 0.35)
end

function SpecialVFX.solarFlare(position, range, color, folder)
	...
	flare.Color = color
	flare.Transparency = 0.1
	flare.CFrame = CFrame.new(position + Vector3.new(0, 1, 0))
	flare.Parent = folder
	...
	TweenService:Create(flare, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(range * 1.8, range * 1.8, range * 1.8),
		Transparency = 1,
	}):Play()
	Debris:AddItem(flare, 0.45)
end

function SpecialVFX.phantomVanish(controller, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	controller._savedTransparency = controller._savedTransparency or controller.part.Transparency
	controller._savedRingTransparency = controller._savedRingTransparency or controller.spinRing.Transparency
	...
	mist.Color = Color3.fromRGB(180, 230, 255)
	...
	task.delay(duration, function()
		if controller.part and controller.part.Parent then
			controller.part.Transparency = controller._savedTransparency or 0
			controller.spinRing.Transparency = controller._savedRingTransparency or 0.4
		end
	end)
...
function SpecialVFX.edgeSlash(position, facing, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.2, 0.8, 5)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.15
	slash.CFrame = CFrame.new(position + facing * 2.5, position + facing * 2.5 + Vector3.new(0, 0.5, 0))
	slash.Parent = folder

	local arc = Instance.new("Part")
	arc.Shape = Enum.PartType.Cylinder
	arc.Size = Vector3.new(0.15, 6, 6)
	arc.Anchored = true
	...
	TweenService:Create(slash, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 0.4, 8),
		Transparency = 1,
	}):Play()
	TweenService:Create(arc, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
