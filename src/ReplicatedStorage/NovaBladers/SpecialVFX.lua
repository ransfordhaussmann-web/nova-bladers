...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		local offset = Vector3.new(math.cos(i) * 2.5, 0.6, math.sin(i) * 2.5)
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	for i = 1, 8 do
		...
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 0.4, 1.0)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Ice
		shard.Color = Color3.fromRGB(200, 235, 255)
		shard.Transparency = 0.1
		shard.CFrame = CFrame.new(position + Vector3.new(math.cos(angle) * 3, 0.5, math.sin(angle) * 3))
		shard.Parent = folder
