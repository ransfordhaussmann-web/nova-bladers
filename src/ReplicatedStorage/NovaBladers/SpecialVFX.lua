...
function SpecialVFX.bladeSlash(position, direction, color, folder)
	...
	slash.Size = Vector3.new(0.4, 0.4, 6)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.2
	slash.CFrame = CFrame.new(position + direction * 2, position + direction * 5)
	slash.Parent = folder
...
function SpecialVFX.stonePillars(controller, color, duration)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 4)
...
function SpecialVFX.edgeSlash(position, color, folder)
	for i = -1, 1, 2 do
		...
		slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 35), math.rad(20))
		slash.Parent = folder
