...
function SpecialVFX.flameTrail(position, color, folder)
	...
	flame.CFrame = CFrame.new(position + Vector3.new(0, 0.4, 0))
	flame.Parent = folder
...
function SpecialVFX.iceHalo(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local halo = Instance.new("Part")
	halo.Shape = Enum.PartType.Cylinder
	halo.Size = Vector3.new(0.15, 3.5, 3.5)
	halo.Anchored = true
	halo.CanCollide = false
	halo.Material = Enum.Material.Glass
	halo.Color = color
	halo.Transparency = 0.35
	halo.CFrame = CFrame.new(pos + Vector3.new(0, 1.2, 0)) * CFrame.Angles(0, 0, math.rad(90))
	halo.Parent = folder

	TweenService:Create(halo, TweenInfo.new(duration, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 6, 6),
		Transparency = 0.75,
	}):Play()
	Debris:AddItem(halo, duration + 0.1)
end
...
function SpecialVFX.frostShield(controller, color, duration)
	...
	shield.Size = Vector3.new(4.5, 4.5, 4.5)
...
function SpecialVFX.crystalShatter(position, color, shardCount, folder)
	for i = 1, shardCount do
		...
		shard.CFrame = CFrame.new(position) * CFrame.lookAt(position, position + dir)
		shard.Parent = folder

		local endPos = position + dir * 6
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos) * CFrame.lookAt(endPos, endPos + dir),
			Transparency = 1,
		}):Play()
