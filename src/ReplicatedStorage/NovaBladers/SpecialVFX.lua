...
function SpecialVFX.flameCyclone(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local cyclone = Instance.new("Part")
	cyclone.Name = "FlameCyclone"
	cyclone.Shape = Enum.PartType.Cylinder
	cyclone.Size = Vector3.new(duration * 8, 4, 4)
	...
	TweenService:Create(cyclone, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(duration * 8, 7, 7),
		Transparency = 0.85,
	}):Play()
	Debris:AddItem(cyclone, duration + 0.1)
end

function SpecialVFX.emberBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostVeil(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FrostVeil"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.iceShard(origin, color, folder)
	local shard = Instance.new("Part")
	shard.Size = Vector3.new(0.4, 1.2, 0.4)
	shard.Anchored = true
	shard.CanCollide = false
	shard.Material = Enum.Material.Glass
	shard.Color = color
	shard.Transparency = 0.15
	shard.CFrame = CFrame.new(origin + Vector3.new(math.random(-2, 2), 1.5, math.random(-2, 2)))
	...
	TweenService:Create(shard, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		CFrame = shard.CFrame * CFrame.new(0, -2, 0),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	for i = 1, 6 do
		...
		frag.Size = Vector3.new(0.5, 0.5, 1.5)
		frag.Anchored = true
		frag.CanCollide = false
		frag.Material = Enum.Material.Ice
		frag.Color = Color3.fromRGB(200, 240, 255)
		frag.Transparency = 0.1
		frag.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(45)) * CFrame.new(0, 0, 2)
		frag.Parent = folder
		TweenService:Create(frag, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = frag.CFrame * CFrame.new(0, 0, 4),
			Transparency = 1,
		}):Play()
		Debris:AddItem(frag, 0.5)
	end
