...
function SpecialVFX.ripperSlash(controller, color, folder)
	local pos = controller.part.Position
	for i = 0, 2 do
		local angle = i * 120 + math.random(-15, 15)
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.15, 0.8, 5)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.2
		slash.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(angle), math.rad(35))
		slash.Parent = folder
...
function SpecialVFX.stoneSlam(controller, color, folder)
	local pos = controller.part.Position
	local slab = Instance.new("Part")
	slab.Shape = Enum.PartType.Block
	slab.Size = Vector3.new(5, 0.6, 5)
	slab.Anchored = true
	slab.CanCollide = false
	slab.Material = Enum.Material.Slate
	slab.Color = color
	slab.Transparency = 0.1
	slab.CFrame = CFrame.new(pos - Vector3.new(0, 0.5, 0))
	slab.Parent = folder
	...
	dust.CFrame = CFrame.new(pos - Vector3.new(0, 0.3, 0))
	dust.Parent = folder
...
function SpecialVFX.solarFlare(controllerOrPos, color, durationOrFolder)
	local folder, pos, duration
	if typeof(controllerOrPos) == "Instance" and controllerOrPos.part then
		folder = SpecialVFX.ensureFolder(controllerOrPos)
		pos = controllerOrPos.part.Position
		duration = color
		color = durationOrFolder
	else
		pos = controllerOrPos
		folder = durationOrFolder
		duration = 0.4
	end
	...
	flare.CFrame = CFrame.new(pos + Vector3.new(0, 0.5, 0))
	flare.Parent = folder
	...
	TweenService:Create(flare, TweenInfo.new(duration or 0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(9, 9, 9),
		Transparency = 1,
	}):Play()
	Debris:AddItem(flare, (duration or 0.4) + 0.1)
end

function SpecialVFX.mirageClone(controller, color, folder)
	local pos = controller.part.Position
	for i = 1, 3 do
		local angle = (i - 1) * 120
...
function SpecialVFX.phantomStrike(controller, color, folder)
	local pos = controller.part.Position
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.5, 0.5, 6)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.35
	trail.CFrame = CFrame.new(pos, pos + controller.facing)
	trail.Parent = folder
