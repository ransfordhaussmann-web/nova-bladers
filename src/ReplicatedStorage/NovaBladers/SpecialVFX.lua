...
function SpecialVFX.cometFlare(position, range, color, folder)
	local burst = Instance.new("Part")
...
function SpecialVFX.glacierAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Position = pos + Vector3.new(math.cos(angle) * 3.5, 1.5, math.sin(angle) * 3.5),
			Transparency = 0.7,
		}):Play()
...
function SpecialVFX.glacierShatter(position, range, color, folder)
	for i = 1, 8 do
		...
		local offset = Vector3.new(math.cos(angle) * 1.5, 0.3, math.sin(angle) * 1.5)
		shard.CFrame = CFrame.new(position + offset) * CFrame.Angles(0, angle, math.rad(-20))
		shard.Parent = folder

		local target = position + Vector3.new(math.cos(angle) * range, 0.5, math.sin(angle) * range)
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(target) * CFrame.Angles(0, angle, math.rad(-20)),
			Transparency = 1,
		}):Play()
