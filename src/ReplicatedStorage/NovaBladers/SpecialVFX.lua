...
function SpecialVFX.fireWheelTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.snowPulse(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.frostShatter(position, color, folder)
	for i = 1, 6 do
		...
		local angle = i * 60
		shard.CFrame = CFrame.new(position) * CFrame.Angles(math.rad(25), math.rad(angle), 0) * CFrame.new(0, 0, 2)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(0, 1.5, 1.5),
			Transparency = 1,
		}):Play()
