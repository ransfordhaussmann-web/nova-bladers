...
function SpecialVFX.tidalSwell(controller, color, duration)
	...
	pool.Size = Vector3.new(0.15, 4, 4)
	pool.Anchored = true
	pool.CanCollide = false
	pool.Material = Enum.Material.Glass
	pool.Color = color
	pool.Transparency = 0.45
	pool.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	pool.Parent = folder

	TweenService:Create(pool, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 7, 7),
		Transparency = 0.75,
	}):Play()
	Debris:AddItem(pool, duration + 0.1)
end
...
function SpecialVFX.tsunamiBurst(position, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.fireIgnite(controller, color, duration)
	...
	for i = 1, 6 do
		local spark = Instance.new("Part")
		spark.Size = Vector3.new(0.5, 0.8, 0.5)
		spark.Anchored = true
		spark.CanCollide = false
		spark.Material = Enum.Material.Neon
		spark.Color = color
		spark.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2) * 0.5, 0.8, math.random(-2, 2) * 0.5))
		spark.Parent = folder
