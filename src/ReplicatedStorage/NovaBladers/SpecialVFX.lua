...
function SpecialVFX.cyclonePulse(origin, range, color, folder)
...
function SpecialVFX.fireFlare(position, color, folder)
...
function SpecialVFX.crystalVeil(controller, color, duration)
	...
	veil.Name = "CrystalVeil"
	veil.Shape = Enum.PartType.Cylinder
	veil.Size = Vector3.new(1.2, 5.5, 5.5)
	veil.Anchored = true
	veil.CanCollide = false
	veil.Material = Enum.Material.Glass
	veil.Color = color
	veil.Transparency = 0.35
	veil.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	veil.Parent = folder
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	for i = 1, shardCount do
		...
		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + offset * 1.8 + Vector3.new(0, 1.5, 0)),
			Transparency = 1,
		}):Play()
