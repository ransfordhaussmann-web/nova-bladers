...
function SpecialVFX.crossSlash(position, facing, color, folder, slashIndex)
	local angle = (slashIndex or 1) * 45
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.25, 0.15, 7)
	slash.Anchored = true
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	crown.Size = Vector3.new(0.5, 4.5, 4.5)
	crown.Anchored = true
	crown.CanCollide = false
	crown.Material = Enum.Material.Glass
	crown.Color = color
	crown.Transparency = 0.35
	crown.CFrame = CFrame.new(pos + Vector3.new(0, 0.8, 0)) * CFrame.Angles(0, 0, math.rad(90))
	crown.Parent = folder
	...
	crystals.CFrame = CFrame.new(pos + Vector3.new(0, 1.2, 0))
...
function SpecialVFX.frostShatter(position, color, folder)
	for i = 1, 8 do
		...
		local endPos = position + Vector3.new(math.cos(angle) * 5, 1.5, math.sin(angle) * 5)
