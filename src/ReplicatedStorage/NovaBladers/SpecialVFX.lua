...
function SpecialVFX.stonePillars(controller, color, duration)
	...
	shield.Size = Vector3.new(0.4, 5, 5)
...
function SpecialVFX.voidSlashTrail(fromPos, toPos, color, folder)
	...
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.2, 0.2, dir.Magnitude)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
...
function SpecialVFX.voidEcho(position, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(0.15, 3, 3)
	ring.Anchored = true
