...
function SpecialVFX.frostField(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, math.rad(angle), math.rad(15))
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.4, 0.8, 0.3)
		shard.Anchored = true
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + Vector3.new(math.cos(math.rad(angle)) * 5, 0.5, math.sin(math.rad(angle)) * 5)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.fireCyclone(controller, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	...
	ring.Color = color
	ring.Transparency = 0.25
	ring.CFrame = CFrame.new(controller.part.Position + Vector3.new(0, 0.4, 0)) * CFrame.Angles(0, 0, math.rad(90))
	ring.Parent = folder
