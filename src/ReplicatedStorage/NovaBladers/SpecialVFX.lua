...
function SpecialVFX.iceChill(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local mist = Instance.new("Part")
	mist.Shape = Enum.PartType.Ball
	mist.Size = Vector3.new(6, 3, 6)
	mist.Anchored = true
...
function SpecialVFX.iceCrown(controller, color, duration)
	...
	spikes.Size = Vector3.new(0.8, 4, 4)
	spikes.Anchored = true
	spikes.CanCollide = false
	spikes.Material = Enum.Material.Neon
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	for i = 1, 4 do
		...
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, math.rad(angle), math.rad(40)) * CFrame.new(0, 0, range * 0.6)
		shard.Parent = folder
