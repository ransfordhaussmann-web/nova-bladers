...
function SpecialVFX.slashArc(position, facing, color, folder)
	...
	arc.CFrame = CFrame.new(position + facing * 2.5, position + facing * 2.5 + Vector3.new(0, 1, 0))
	arc.Parent = folder
...
function SpecialVFX.slashBurst(position, facing, color, folder)
	for i = -1, 1 do
		local offset = CFrame.Angles(0, math.rad(i * 35), 0) * facing
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.4, 1, 8)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.1
		slash.CFrame = CFrame.new(position + offset * 3, position + offset * 3 + Vector3.new(0, 1, 0))
		slash.Parent = folder
...
function SpecialVFX.frostCrystals(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.6, math.sin(math.rad(angle)) * 2.5))
...
function SpecialVFX.iceShatter(position, color, folder)
	for i = 1, 8 do
		...
		shard.Size = Vector3.new(0.5, 0.5, 1.5)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.15
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(30)) * CFrame.new(0, 0, 3)
		shard.Parent = folder
