...
function SpecialVFX.ripperSlash(origin, facing, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.3, 1.2, 4.5)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.15
	slash.CFrame = CFrame.new(origin, origin + facing) * CFrame.new(0, 0, -2.2)
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, 0.6, 6),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.shatterBurst(origin, range, color, folder)
	...
	for i = 0, 5 do
		...
		shard.Color = color
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle), 0.3, math.sin(angle)) * 1.5)
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * range * 0.5, 0.8, math.sin(angle) * range * 0.5)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.heatRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.phantomClones(controller, color, folder)
	local pos = controller.part.Position
	for i = 1, 3 do
		local angle = i * 120
		local clone = Instance.new("Part")
		clone.Shape = Enum.PartType.Cylinder
		clone.Size = Vector3.new(0.5, 3.2, 3.2)
		clone.Anchored = true
		clone.CanCollide = false
		clone.Material = Enum.Material.ForceField
		clone.Color = color
		clone.Transparency = 0.55
		clone.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 3, 0, math.sin(math.rad(angle)) * 3))
...
function SpecialVFX.blinkFlash(position, color, folder)
...
function SpecialVFX.edgeSlash(position, facing, color, folder)
	for i = -1, 1 do
		...
		slash.CFrame = CFrame.new(position, position + facing)
			* CFrame.Angles(0, math.rad(i * 35), 0)
			* CFrame.new(0, 0, -2.5)
		slash.Parent = folder
