...
function SpecialVFX.mirageClones(controller, count, color, folder)
	local clones = {}
	local pos = controller.part.Position
	for i = 1, count do
		local angle = (i / count) * math.pi * 2
		local offset = Vector3.new(math.cos(angle) * 4, 0.3, math.sin(angle) * 4)
		...
		TweenService:Create(ghost, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
			Transparency = 0.9,
...
function SpecialVFX.mirageStrike(position, color, folder)
	...
	blade.CFrame = CFrame.new(position + Vector3.new(0, 0.8, 0))
	blade.Parent = folder
