...
function SpecialVFX.featherImpact(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.tidalPull(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	...
	TweenService:Create(ring, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
function SpecialVFX.slashBurst(position, color, folder)
	for i = 0, 2 do
		...
		slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 60), math.rad(15))
		slash.Parent = folder

		TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = Vector3.new(0.15, 0.05, 10),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.stoneRise(controller, color, duration)
	...
	for i = 0, 3 do
		...
		pillar.Size = Vector3.new(0.8, 0.5, 0.8)
		...
		TweenService:Create(pillar, TweenInfo.new(duration * 0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			CFrame = pillar.CFrame + Vector3.new(0, 2, 0),
		}):Play()
...
function SpecialVFX.phaseFade(controller, faded)
	controller._savedTransparency = controller._savedTransparency or controller.part.Transparency
	controller._savedRingTransparency = controller._savedRingTransparency or controller.spinRing.Transparency
...
function SpecialVFX.phaseBlink(controller, targetPos, color, folder)
	...
	ghost.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	ghost.Parent = folder
	...
	trail.CFrame = CFrame.new(targetPos)
	trail.Parent = folder
...
function SpecialVFX.afterimageSlash(position, color, folder)
	for i = 1, 3 do
		...
		img.Size = Vector3.new(0.2, 0.5, 3)
		img.Anchored = true
		img.CanCollide = false
		img.Material = Enum.Material.Neon
		img.Color = color
		img.Transparency = 0.35 + i * 0.15
		img.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 40), math.rad(20))
		img.Parent = folder
