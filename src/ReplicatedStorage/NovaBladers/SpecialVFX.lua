...
function SpecialVFX.mirageClone(controller, color, duration)
	...
	clone.Size = Vector3.new(1.0, 3.2, 3.2)
	clone.Anchored = true
	...
	TweenService:Create(clone, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		CFrame = clone.CFrame * CFrame.new(math.random(-4, 4), 0, math.random(-4, 4)),
	}):Play()
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	crown.Size = Vector3.new(1.2, 5, 5)
	crown.Anchored = true
	crown.CanCollide = false
	crown.Material = Enum.Material.Ice
	crown.Color = color
	crown.Transparency = 0.3
	crown.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	crown.Parent = folder
...
function SpecialVFX.frostShatter(position, color, folder)
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.4, 0.8, 0.2)
		shard.Anchored = true
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(math.cos(angle) * 5, 1, math.sin(angle) * 5),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.driftTrail(fromPos, toPos, color, folder)
	local trail = Instance.new("Part")
	...
	fire.Heat = 5
	fire.Color = color
	fire.SecondaryColor = Color3.fromRGB(255, 120, 80)
	fire.Parent = trail
