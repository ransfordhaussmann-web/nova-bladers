...
function SpecialVFX.slashArc(position, facing, color, folder)
	...
	arc.Size = Vector3.new(0.25, 3.5, 0.8)
	arc.Anchored = true
	...
	arc.CFrame = CFrame.new(position + flatFacing * 1.5, position + flatFacing * 4)
		* CFrame.Angles(0, math.atan2(flatFacing.X, flatFacing.Z), math.rad(70))
	arc.Parent = folder

	TweenService:Create(arc, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 5.5, 0.2),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.frostCharge(controller, color, duration)
	...
	for i = 1, 6 do
		...
		flake.CFrame = CFrame.new(pos + Vector3.new(math.random(-4, 4), 1.5, math.random(-4, 4)))
		flake.Parent = folder
