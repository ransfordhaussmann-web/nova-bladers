...
function SpecialVFX.bladeSlash(origin, angle, radius, color, folder)
	for i = 0, 2 do
		local a = angle + i * (math.pi * 2 / 3)
		local tip = origin + Vector3.new(math.cos(a) * radius, 0.5, math.sin(a) * radius)
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.3, 0.8, radius * 0.6)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.2
		slash.CFrame = CFrame.new(tip, origin)
		slash.Parent = folder
...
function SpecialVFX.bastionAnchor(controller, color)
	local folder = SpecialVFX.ensureFolder(controller)
	local dust = Instance.new("Part")
	dust.Shape = Enum.PartType.Cylinder
	dust.Size = Vector3.new(0.2, 4, 4)
	dust.Anchored = true
	dust.CanCollide = false
	dust.Material = Enum.Material.Slate
	dust.Color = color
	dust.Transparency = 0.4
	dust.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	dust.Parent = folder
...
function SpecialVFX.bastionDome(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
...
function SpecialVFX.seismicWave(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.solarTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.8, 0.8, 0.8)
	trail.Shape = Enum.PartType.Ball
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
...
function SpecialVFX.setPhased(controller, phased, color)
	...
	if phased then
		...
		SpecialVFX.chargeAura(controller, color, 0.3)
	else
		controller.part.Transparency = controller._savedTransparency
		controller.spinRing.Transparency = controller._savedRingTransparency
	end
