...
function SpecialVFX.crimsonSlash(position, facing, color, folder)
	...
	slash.CFrame = CFrame.new(position + flatFacing * 1.5, position + flatFacing * 3)
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.08, 3.5, 6),
		Transparency = 1,
	}):Play()
	...
	for i = 1, 3 do
		...
		spark.CFrame = CFrame.new(position + flatFacing * (i * 1.2))
		spark.Parent = folder
...
function SpecialVFX.auroraShimmer(controller, color, duration)
	...
	for i = 1, 6 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0.8, 2.5)
