...
function SpecialVFX.spiralSlash(origin, range, color, folder)
	local slash = Instance.new("Part")
	slash.Shape = Enum.PartType.Cylinder
	slash.Size = Vector3.new(0.12, 2.5, 2.5)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.15
	slash.CFrame = CFrame.new(origin + Vector3.new(0, 0.4, 0)) * CFrame.Angles(0, 0, math.rad(90))
	slash.Parent = folder
...
function SpecialVFX.spiralFangBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2), 1 + i * 0.3, math.random(-2, 2)))
		shard.Parent = folder
		Debris:AddItem(shard, duration + 0.2)
	end
end

function SpecialVFX.iceBastionRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "IceBastion"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 4 do
		local angle = (i - 1) * 90 + math.random(-15, 15)
		local shard = Instance.new("Part")
		...
		local target = origin + Vector3.new(math.cos(math.rad(angle)) * range, 0.3, math.sin(math.rad(angle)) * range)
