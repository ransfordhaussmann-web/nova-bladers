...
function SpecialVFX.lungeTrail(fromPos, toPos, color, folder)
	...
	trail.CFrame = CFrame.lookAt(fromPos, toPos) * CFrame.new(0, 0, -trail.Size.Z / 2)
	trail.Parent = folder
...
function SpecialVFX.fangSlash(position, color, folder)
	...
	slash.Size = Vector3.new(3, 0.15, 1.5)
	slash.Anchored = true
...
function SpecialVFX.mirageSplit(controller, color, duration)
	...
	for i = -1, 1, 2 do
		...
		TweenService:Create(clone, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			CFrame = clone.CFrame + Vector3.new(i * 2, 1.5, 0),
		}):Play()
...
function SpecialVFX.bladeVanish(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
