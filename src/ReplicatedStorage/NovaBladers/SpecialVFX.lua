...
function SpecialVFX.bladeArc(origin, facing, color, folder, range)
	local dir = Vector3.new(facing.X, 0, facing.Z).Unit
	local right = dir:Cross(Vector3.yAxis).Unit
	for side = -1, 1, 2 do
		local arc = Instance.new("Part")
		arc.Size = Vector3.new(0.25, 0.8, range or 4)
		arc.Anchored = true
		arc.CanCollide = false
		arc.Material = Enum.Material.Neon
		arc.Color = color
		arc.Transparency = 0.25
		arc.CFrame = CFrame.new(origin + dir * ((range or 4) * 0.45) + right * side * 1.2, origin + dir * (range or 4))
			* CFrame.Angles(0, math.atan2(dir.X, dir.Z), math.rad(side * 35))
		arc.Parent = folder

		TweenService:Create(arc, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.1, 0.2, (range or 4) * 1.2),
		}):Play()
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.25, 3.5, 3.5)
	inner.Anchored = true
	inner.CanCollide = false
...
function SpecialVFX.frostShatter(position, color, folder)
	for i = 1, 8 do
		...
		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + Vector3.new(math.cos(angle) * 5, 3, math.sin(angle) * 5),
			Transparency = 1,
		}):Play()
