...
function SpecialVFX.moltenHeat(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	SpecialVFX.chargeAura(controller, color, duration)

	local lava = Instance.new("Part")
	lava.Shape = Enum.PartType.Cylinder
	lava.Size = Vector3.new(0.25, 4, 4)
	lava.Anchored = true
	lava.CanCollide = false
	lava.Material = Enum.Material.Neon
	lava.Color = Color3.fromRGB(255, 140, 30)
	lava.Transparency = 0.35
	lava.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	lava.Parent = folder
	...
	TweenService:Create(lava, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, 6, 6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(lava, duration + 0.15)
end

function SpecialVFX.moltenSlamImpact(position, color, folder)
...
function SpecialVFX.prismAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	TweenService:Create(crystal, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
...
function SpecialVFX.iceShardBurst(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.35, 0.8, 0.35)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Ice
		shard.Color = color
		shard.Transparency = 0.1
		local offset = Vector3.new(math.cos(angle) * 1.5, 0.6, math.sin(angle) * 1.5)
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	ring.CanCollide = false
	ring.Material = Enum.Material.Neon
	ring.Color = Color3.fromRGB(200, 240, 255)
