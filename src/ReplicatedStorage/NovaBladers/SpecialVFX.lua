...
function SpecialVFX.vortexSpin(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 3 do
		local ring = Instance.new("Part")
		ring.Shape = Enum.PartType.Cylinder
		ring.Size = Vector3.new(0.12, 2.5 + i * 1.2, 2.5 + i * 1.2)
		ring.Anchored = true
		ring.CanCollide = false
		ring.Material = Enum.Material.Neon
		ring.Color = color
		...
		TweenService:Create(ring, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
			Orientation = Vector3.new(0, i * 120, 90),
		}):Play()
		Debris:AddItem(ring, duration + 0.1)
	end
end

function SpecialVFX.vortexPull(controller, color, range, folder)
	...
	TweenService:Create(ring, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
function SpecialVFX.crimsonSlam(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
