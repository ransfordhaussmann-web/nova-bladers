...
function SpecialVFX.flameSpiral(origin, radius, color, folder)
	...
	arc.Size = Vector3.new(0.2, radius * 1.6, radius * 1.6)
	arc.Anchored = true
...
function SpecialVFX.frostVeil(controller, color, duration)
	...
	veil.Shape = Enum.PartType.Ball
	veil.Size = Vector3.new(4.5, 4.5, 4.5)
	veil.Anchored = true
	veil.CanCollide = false
	veil.Material = Enum.Material.Glass
	veil.Color = color
	veil.Transparency = 0.55
	veil.CFrame = CFrame.new(pos)
	veil.Parent = folder
	...
	TweenService:Create(veil, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
		Size = Vector3.new(6, 6, 6),
		Transparency = 0.85,
	}):Play()
	Debris:AddItem(veil, duration + 0.1)
end
