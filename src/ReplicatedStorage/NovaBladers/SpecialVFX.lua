...
function SpecialVFX.vortexSwirl(controller, color, radius)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	for i = 1, 6 do
		local angle = math.rad(i * 60 + math.random(-15, 15))
		local dist = radius * (0.4 + math.random() * 0.5)
		local spark = Instance.new("Part")
		...
		spark.Material = Enum.Material.Neon
		spark.Color = color
		spark.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * dist, 0.4, math.sin(angle) * dist))
		spark.Parent = folder

		TweenService:Create(spark, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 10 do
		...
		crystal.Size = Vector3.new(0.25, 0.6, 0.25)
