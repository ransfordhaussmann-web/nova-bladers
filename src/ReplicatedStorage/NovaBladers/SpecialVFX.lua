...
function SpecialVFX.ripperSlash(position, facing, color, folder)
	...
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0), position + facing) * CFrame.Angles(0, math.rad(90), 0)
	slash.Parent = folder
...
function SpecialVFX.ripperBurst(position, color, folder)
	...
	for i = 0, 2 do
		...
		spike.Size = Vector3.new(0.3, 0.3, 2.5)
		spike.Anchored = true
		spike.CanCollide = false
		spike.Material = Enum.Material.Neon
		spike.Color = Color3.fromRGB(255, 120, 80)
		spike.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 120), math.rad(25)) * CFrame.new(0, 0, 2)
		spike.Parent = folder
...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(1.4, 5.5, 5.5)
	inner.Anchored = true
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
	inner.Color = Color3.fromRGB(200, 195, 180)
	inner.Transparency = 0.55
	inner.CFrame = shield.CFrame
	inner.Parent = folder
...
function SpecialVFX.mirageAfterimage(position, color, folder)
...
function SpecialVFX.bladeBurst(position, facing, color, folder)
	for i = -1, 1 do
		local blade = Instance.new("Part")
		blade.Size = Vector3.new(0.15, 0.6, 4.5)
		blade.Anchored = true
		blade.CanCollide = false
		blade.Material = Enum.Material.Glass
		blade.Color = color
		blade.Transparency = 0.15
		blade.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0), position + facing)
			* CFrame.Angles(0, math.rad(90 + i * 35), 0)
		blade.Parent = folder

		TweenService:Create(blade, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.08, 0.3, 6.5),
		}):Play()
