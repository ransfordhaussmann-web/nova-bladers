...
function SpecialVFX.flameSpiral(origin, color, folder)
	local spiral = Instance.new("Part")
	spiral.Shape = Enum.PartType.Ball
	spiral.Size = Vector3.new(1.5, 1.5, 1.5)
	spiral.Anchored = true
	spiral.CanCollide = false
	spiral.Material = Enum.Material.Neon
	spiral.Color = color
	spiral.Transparency = 0.25
	spiral.CFrame = CFrame.new(origin)
	...
	TweenService:Create(spiral, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.corkscrewTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.glacierRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "GlacierRing"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.iceShard(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, angle, math.rad(35))
			* CFrame.new(math.cos(angle) * 1.5, 0.5, math.sin(angle) * 1.5)
		shard.Parent = folder

		local target = origin + Vector3.new(math.cos(angle) * range, 0, math.sin(angle) * range)
