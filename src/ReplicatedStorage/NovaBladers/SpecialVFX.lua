...
function SpecialVFX.vortexSpiral(origin, color, folder, radius)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	...
	TweenService:Create(ring, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, radius * 1.8, radius * 1.8),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.iceLock(targetPart, color, duration, folder)
	local lock = Instance.new("Part")
	lock.Shape = Enum.PartType.Ball
	lock.Size = Vector3.new(4.5, 4.5, 4.5)
	lock.Anchored = true
	lock.CanCollide = false
	lock.Material = Enum.Material.Glass
	lock.Color = color
	lock.Transparency = 0.4
	lock.CFrame = CFrame.new(targetPart.Position)
	lock.Parent = folder

	task.spawn(function()
		local elapsed = 0
		while elapsed < duration and targetPart.Parent and lock.Parent do
			lock.CFrame = CFrame.new(targetPart.Position)
			elapsed += 0.05
			task.wait(0.05)
		end
