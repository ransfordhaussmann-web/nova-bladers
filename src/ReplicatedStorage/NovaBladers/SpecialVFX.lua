...
function SpecialVFX.crimsonSlash(position, facing, color, folder)
	...
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0), position + facing) * CFrame.Angles(0, 0, math.rad(35))
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 1.2, 5),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.stonePillar(position, color, folder)
	...
	pillar.CFrame = CFrame.new(position - Vector3.new(0, 0.5, 0))
