...
function SpecialVFX.crystalRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + offset) * CFrame.Angles(0, angle, math.rad(90)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.magmaTrail(position, color, folder)
