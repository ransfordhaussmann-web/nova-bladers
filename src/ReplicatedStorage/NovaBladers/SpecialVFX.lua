...
function SpecialVFX.ripSlash(position, facing, color, folder)
	...
	slash.Size = Vector3.new(0.3, 0.8, 4.5)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.15
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.4, 0), position + facing) * CFrame.Angles(0, 0, math.rad(25))
	slash.Parent = folder
...
function SpecialVFX.stoneBastion(controller, color, duration)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 3.2)
	...
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(1.0, 5.5, 5.5)
	ring.Anchored = true
	ring.CanCollide = false
	ring.Material = Enum.Material.Neon
	ring.Color = color
...
function SpecialVFX.phantomMirage(controller, color, cloneCount, folder)
	local pos = controller.part.Position
	for i = 1, cloneCount do
		local angle = (i / cloneCount) * math.pi * 2
		...
		TweenService:Create(ghost, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + offset * 0.3) * CFrame.Angles(0, 0, math.rad(90)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.phantomStrike(position, color, folder)
	...
	blade.Size = Vector3.new(0.4, 0.4, 5)
	blade.Anchored = true
	blade.CanCollide = false
	blade.Material = Enum.Material.Neon
	blade.Color = color
	blade.Transparency = 0.1
	blade.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0))
	blade.Parent = folder
