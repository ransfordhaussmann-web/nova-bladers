...
function SpecialVFX.crystalShard(position, color, folder)
	...
	shard.CFrame = CFrame.new(position + Vector3.new(0, 1.2, 0))
	shard.Parent = folder
...
function SpecialVFX.phoenixIgnite(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local firePart = Instance.new("Part")
	firePart.Shape = Enum.PartType.Cylinder
	firePart.Size = Vector3.new(0.3, 3, 3)
	firePart.Anchored = true
	firePart.CanCollide = false
	firePart.Material = Enum.Material.Neon
	firePart.Color = color
	firePart.Transparency = 0.25
	firePart.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	...
	flame.Heat = 10
	flame.Color = color
	flame.SecondaryColor = Color3.fromRGB(255, 240, 120)
	flame.Parent = firePart

	TweenService:Create(firePart, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, 6, 6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(firePart, duration + 0.1)
end

function SpecialVFX.emberTrail(position, color, folder)
	...
	ember.Color = color
	ember.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0))
	ember.Parent = folder
...
function SpecialVFX.phoenixFlare(position, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	core.Size = Vector3.new(3, 3, 3)
	core.Anchored = true
	...
	wings.Size = Vector3.new(10, 0.3, 4)
	wings.Anchored = true
	wings.CanCollide = false
	wings.Material = Enum.Material.Neon
	wings.Color = color
	wings.Transparency = 0.3
	wings.CFrame = CFrame.new(position + Vector3.new(0, 2, 0))
	wings.Parent = folder
	...
	TweenService:Create(wings, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(16, 0.2, 8),
		Transparency = 1,
	}):Play()
	Debris:AddItem(core, 0.5)
	Debris:AddItem(wings, 0.5)
end
