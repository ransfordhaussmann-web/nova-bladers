...
function SpecialVFX.slashArc(origin, direction, color, tiltDeg, folder)
	local perp = Vector3.new(-direction.Z, 0, direction.X)
	for i = 1, 4 do
		local t = i / 4
		local pos = origin + direction * (3 * t) + perp * math.sin(t * math.pi) * 2
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.3, 2.5, 0.15)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.15 + t * 0.3
		slash.CFrame = CFrame.new(pos, pos + direction) * CFrame.Angles(math.rad(tiltDeg), 0, 0)
		slash.Parent = folder
		Debris:AddItem(slash, 0.35)
	end
end

function SpecialVFX.crossImpact(position, color, folder)
	local h = Instance.new("Part")
	h.Size = Vector3.new(8, 0.2, 0.8)
	h.Anchored = true
	h.CanCollide = false
	...
	local v = h:Clone()
	v.Size = Vector3.new(0.8, 0.2, 8)
	v.Parent = folder

	TweenService:Create(h, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(12, 0.2, 1.2),
	}):Play()
	TweenService:Create(v, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(1.2, 0.2, 12),
	}):Play()
	Debris:AddItem(h, 0.4)
	Debris:AddItem(v, 0.4)
end

function SpecialVFX.frostCrystals(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	for i = 1, 6 do
		local angle = i * 60
		local offset = Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.8, math.sin(math.rad(angle)) * 2.5)
		...
		crystal.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, math.rad(angle), math.rad(20))
...
function SpecialVFX.iceShatter(position, color, range, folder)
	for i = 1, 8 do
		local angle = i * 45
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.5, 0.8, 0.15)
		shard.Anchored = true
		...
		shard.CFrame = CFrame.new(position + dir * 1.5, position + dir * 3)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + dir * (range * 0.6), position + dir * (range * 0.6 + 2)),
			Transparency = 1,
		}):Play()
