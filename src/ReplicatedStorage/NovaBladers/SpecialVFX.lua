...
function SpecialVFX.crystalize(controller, color, duration)
	...
	TweenService:Create(shell, TweenInfo.new(duration * 0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 8 do
		...
		local target = position + Vector3.new(math.cos(angle) * 6, 0.5, math.sin(angle) * 6)
		TweenService:Create(shard, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(target) * CFrame.Angles(0, angle, math.rad(80)),
			Transparency = 1,
		}):Play()
