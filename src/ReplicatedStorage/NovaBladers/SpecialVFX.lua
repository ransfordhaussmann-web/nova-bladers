...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2), 0.5, math.random(-2, 2)))
...
function SpecialVFX.iceCrownRing(controller, color, duration)
	...
	glow.Size = Vector3.new(0.8, 4.2, 4.2)
	glow.Anchored = true
...
function SpecialVFX.iceShatterWave(origin, range, color, folder)
...
function SpecialVFX.fangSlash(fromPos, toPos, color, folder)
	...
	slash.CFrame = CFrame.new(mid, mid + dir) * CFrame.Angles(0, math.rad(90), math.rad(35))
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.1, 2.2, 3.5),
	}):Play()
