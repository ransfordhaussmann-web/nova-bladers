...
function SpecialVFX.phaseTrail(controller, color, folder)
	local mark = Instance.new("Part")
	mark.Size = Vector3.new(1.5, 0.3, 4)
	mark.Anchored = true
	mark.CanCollide = false
	mark.Material = Enum.Material.ForceField
	mark.Color = color
	mark.Transparency = 0.4
	mark.CFrame = controller.part.CFrame
	mark.Parent = folder
	Debris:AddItem(mark, 0.3)
end

function SpecialVFX.phaseSlashBurst(position, color, folder)
