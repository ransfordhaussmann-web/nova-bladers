...
function SpecialVFX.crimsonSlash(controller, color, angleOffset)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	local rot = CFrame.lookAt(pos, pos + facing) * CFrame.Angles(math.rad(70 + (angleOffset or 0)), 0, 0)
	slash.CFrame = rot
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 5.5, 1.4),
		Transparency = 1,
		CFrame = rot * CFrame.new(right * 1.5),
	}):Play()
...
function SpecialVFX.auroraDome(controller, color, duration)
	...
	shimmer.Size = Vector3.new(0.1, 4, 4)
	...
	TweenService:Create(shimmer, TweenInfo.new(duration, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
		Transparency = 0.75,
	}):Play()
...
function SpecialVFX.auroraRelease(position, range, color, folder)
	...
	sparkles.SparkleColor = Color3.fromRGB(200, 180, 255)
