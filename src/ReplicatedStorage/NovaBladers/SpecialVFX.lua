...
function SpecialVFX.vortexSlash(origin, range, color, folder)
	...
	slash.Size = Vector3.new(0.2, 2.5, 2.5)
	slash.Anchored = true
	...
	for i = 1, 3 do
		...
		spark.CanCollide = false
		spark.Material = Enum.Material.Neon
		spark.Color = Color3.fromRGB(255, 120, 100)
		spark.CFrame = CFrame.new(origin + Vector3.new(math.cos(i * 2.1) * range * 0.5, 0.6, math.sin(i * 2.1) * range * 0.5))
		spark.Parent = folder
...
function SpecialVFX.freezeAura(controller, color, duration)
	...
	frost.Size = NumberSequence.new(0.4)
	...
	Debris:AddItem(aura, duration + 0.15)
end
...
function SpecialVFX.frostBurst(origin, range, color, folder)
	...
	burst.Anchored = true
	burst.CanCollide = false
	burst.Material = Enum.Material.Ice
	burst.Color = color
	...
	shards.Size = Vector3.new(0.15, 3, 3)
	...
	shards.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(90))
