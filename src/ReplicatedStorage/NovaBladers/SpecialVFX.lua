...
function SpecialVFX.crystalShard(position, color, folder)
	for i = 1, 4 do
		...
		shard.CFrame = CFrame.new(position + Vector3.new(math.cos(angle) * dist, 0.5, math.sin(angle) * dist))
...
function SpecialVFX.crystalBurst(position, color, folder)
	...
	for i = 1, 8 do
		...
		shard.Color = color
		shard.CFrame = CFrame.new(position + Vector3.new(math.cos(angle) * 3, 0.8, math.sin(angle) * 3))
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + Vector3.new(math.cos(angle) * 8, 2, math.sin(angle) * 8)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.blazeEruption(position, color, folder)
	...
	pillar.CFrame = CFrame.new(position + Vector3.new(0, 3, 0))
