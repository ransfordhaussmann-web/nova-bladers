...
function SpecialVFX.slashArc(fromPos, toPos, facing, color, folder)
	local mid = (fromPos + toPos) / 2
	local arc = Instance.new("Part")
	arc.Size = Vector3.new(2.8, 0.15, 0.8)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = 0.2
	arc.CFrame = CFrame.new(mid, mid + facing) * CFrame.Angles(0, math.rad(90), math.rad(35))
	arc.Parent = folder

	TweenService:Create(arc, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(4.2, 0.1, 0.4),
	}):Play()
