...
function SpecialVFX.flameSlash(position, facing, color, folder)
	...
	slash.CFrame = CFrame.new(position, position + facing) * CFrame.new(0, 0.5, -2)
	slash.Parent = folder
...
function SpecialVFX.frostPrison(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FrostPrison"
	ring.Shape = Enum.PartType.Cylinder
