...
function SpecialVFX.tidePool(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pool = Instance.new("Part")
	pool.Shape = Enum.PartType.Cylinder
	pool.Size = Vector3.new(0.2, 5, 5)
	pool.Anchored = true
	pool.CanCollide = false
	pool.Material = Enum.Material.Glass
	pool.Color = color
	pool.Transparency = 0.45
	pool.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
	pool.Parent = folder
...
function SpecialVFX.surgeWave(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.undertowVortex(origin, range, color, folder)
	...
	vortex.Size = Vector3.new(0.4, range * 1.5, range * 1.5)
