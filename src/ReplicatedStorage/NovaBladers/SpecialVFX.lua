...
function SpecialVFX.slashArc(position, angle, color, folder)
	...
	arc.Size = Vector3.new(4.5, 0.15, 1.2)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = 0.15
	arc.CFrame = CFrame.new(position + Vector3.new(0, 0.6, 0)) * CFrame.Angles(0, angle, math.rad(35))
	arc.Parent = folder
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		local offset = Vector3.new(math.cos(i) * 2.5, 0.5, math.sin(i) * 2.5)
		crystal.CFrame = CFrame.new(pos + offset)
...
function SpecialVFX.iceHalo(controller, color, duration)
	...
	ring.Name = "IceHalo"
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(0.4, 6.5, 6.5)
	ring.Anchored = true
	ring.CanCollide = false
	...
	glow.Size = Vector3.new(0.25, 5, 5)
	glow.Anchored = true
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 8 do
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos, origin),
			Transparency = 1,
		}):Play()
