...
function SpecialVFX.bladeCyclone(controller, color, duration)
	...
	TweenService:Create(ring, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
		Orientation = Vector3.new(0, 0, 360 * 3),
	}):Play()
	Debris:AddItem(ring, duration + 0.1)
end
...
function SpecialVFX.phantomFade(controller, color, duration)
	...
	SpecialVFX.darkAura(controller, color, duration)
end
...
function SpecialVFX.edgeStrike(position, color, folder)
	...
	strike.Size = Vector3.new(0.4, 0.4, 5)
	strike.Anchored = true
	strike.CanCollide = false
	strike.Material = Enum.Material.Neon
	strike.Color = color
	strike.Transparency = 0.1
	strike.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(45), math.rad(90))
	strike.Parent = folder

	local cross = strike:Clone()
	cross.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(-45), math.rad(90))
	cross.Parent = folder

	TweenService:Create(strike, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, 0.2, 8),
		Transparency = 1,
	}):Play()
