...
function SpecialVFX.cycloneIgnite(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
...
function SpecialVFX.cycloneFlare(position, color, folder, range)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.glacierAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
...
function SpecialVFX.glacierShards(origin, range, color, folder)
	for i = 0, 5 do
		local angle = i * 60
...
function SpecialVFX.glacierShroud(position, color, folder, range)
