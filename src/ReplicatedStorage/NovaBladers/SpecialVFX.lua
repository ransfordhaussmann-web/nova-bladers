...
function SpecialVFX.bladeArc(position, color, folder)
	...
	TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(4, 0.15, 1.2),
	}):Play()
	Debris:AddItem(slash, 0.3)
end

function SpecialVFX.bladeSlash(controller, dir, color, folder)
	local pos = controller.part.Position
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(5, 0.4, 1.2)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.15
	slash.CFrame = CFrame.new(pos + dir * 2, pos + dir * 5)
	slash.Parent = folder
	TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(8, 0.2, 2),
	}):Play()
...
function SpecialVFX.bastionWall(controller, color, duration)
	...
	for i = 0, 3 do
		...
		TweenService:Create(pillar, TweenInfo.new(duration * 0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
		}):Play()
...
function SpecialVFX.phantomSplit(controller, color, folder)
	...
	for i = 1, 3 do
		...
		local offset = Vector3.new(math.cos(i * 2.1) * 2, 0, math.sin(i * 2.1) * 2)
...
function SpecialVFX.phantomDash(fromPos, dir, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.6, 0.6, 3)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.35
	trail.CFrame = CFrame.new(fromPos + dir * 1.5, fromPos + dir * 4)
	trail.Parent = folder
