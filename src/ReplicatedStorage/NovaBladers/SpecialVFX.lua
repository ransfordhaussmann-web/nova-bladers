...
function SpecialVFX.forgeGlow(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
...
function SpecialVFX.hammerImpact(position, color, folder)
	...
	for i = 1, 6 do
		...
		crack.Size = Vector3.new(0.3, 0.3, 3)
		crack.Anchored = true
		crack.CanCollide = false
		crack.Material = Enum.Material.Neon
		crack.Color = Color3.fromRGB(255, 180, 60)
		crack.Transparency = 0.2
		crack.CFrame = CFrame.new(position) * CFrame.Angles(0, angle, math.rad(75))
		crack.Parent = folder
...
function SpecialVFX.crystalRise(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 5 do
		...
		crystal.Size = Vector3.new(0.5, 1.8, 0.5)
		crystal.Anchored = true
		crystal.CanCollide = false
		crystal.Material = Enum.Material.Glass
		crystal.Color = color
		crystal.Transparency = 0.1
		crystal.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2, -0.5, math.sin(angle) * 2))
			* CFrame.Angles(0, angle, math.rad(15))
		crystal.Parent = folder

		TweenService:Create(crystal, TweenInfo.new(duration, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.5, 1.2, math.sin(angle) * 2.5))
				* CFrame.Angles(0, angle, math.rad(15)),
		}):Play()
		Debris:AddItem(crystal, duration + 0.15)
	end
end

function SpecialVFX.prismShield(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "PrismShield"
	ring.Shape = Enum.PartType.Cylinder
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(1.4, 4.2, 4.2)
	inner.Anchored = true
	inner.CanCollide = false
	inner.Material = Enum.Material.Neon
	inner.Color = Color3.fromRGB(200, 245, 255)
...
function SpecialVFX.frostMist(origin, range, color, folder)
	local mist = Instance.new("Part")
	mist.Shape = Enum.PartType.Ball
	mist.Size = Vector3.new(range, 2, range)
	mist.Anchored = true
	mist.CanCollide = false
	mist.Material = Enum.Material.Glass
	mist.Color = color
	mist.Transparency = 0.55
	mist.CFrame = CFrame.new(origin)
	mist.Parent = folder

	TweenService:Create(mist, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(range * 1.4, 3, range * 1.4),
		Transparency = 1,
	}):Play()
	Debris:AddItem(mist, 0.55)
end
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 8 do
		...
		shard.Size = Vector3.new(0.4, 0.8, 0.15)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.05
		shard.CFrame = CFrame.new(position) * CFrame.Angles(math.random() * 0.5, angle, math.random() * 0.5)
		shard.Parent = folder

		local outward = Vector3.new(math.cos(angle) * 5, 2 + math.random(), math.sin(angle) * 5)
		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + outward) * shard.CFrame.Rotation,
			Transparency = 1,
		}):Play()
	...
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = Color3.fromRGB(220, 250, 255)
	burst.Transparency = 0.2
	burst.CFrame = CFrame.new(position)
	burst.Parent = folder
