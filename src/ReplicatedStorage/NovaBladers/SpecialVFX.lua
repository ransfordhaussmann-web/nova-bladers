...
function SpecialVFX.vortexTrail(position, color, folder)
	...
	swirl.Size = Vector3.new(0.12, 4, 4)
	swirl.Anchored = true
...
function SpecialVFX.crimsonFlare(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.iceShatter(position, range, color, folder)
	...
	for i = 1, 8 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + Vector3.new(math.cos(angle) * (range * 0.8), 1.5, math.sin(angle) * (range * 0.8))),
			Transparency = 1,
		}):Play()
