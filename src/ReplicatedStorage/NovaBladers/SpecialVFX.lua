...
function SpecialVFX.fireSpiral(origin, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.iceWallRing(controller, color, duration)
	...
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
	inner.Color = Color3.fromRGB(210, 240, 255)
...
function SpecialVFX.iceShardBurst(position, color, folder)
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(25)) * CFrame.new(0, 0.5, 3)
		shard.Parent = folder
	...
	core.CanCollide = false
	core.Material = Enum.Material.Neon
	core.Color = Color3.fromRGB(200, 235, 255)
