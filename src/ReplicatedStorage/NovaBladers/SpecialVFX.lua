...
function SpecialVFX.cycloneSlash(position, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.3, 0.3, 4.5)
	slash.Anchored = true
...
function SpecialVFX.cycloneBurst(position, color, folder)
	local burst = Instance.new("Part")
...
function SpecialVFX.stoneAnchor(controller, color, duration)
	...
	pillar.Size = Vector3.new(1.2, 3, 3)
...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	shield.Material = Enum.Material.Rock
	shield.Color = color
...
function SpecialVFX.solarRay(position, color, folder)
	local ray = Instance.new("Part")
	ray.Size = Vector3.new(0.25, 0.25, 3)
	ray.Anchored = true
	ray.CanCollide = false
	ray.Material = Enum.Material.Neon
	ray.Color = color
	ray.Transparency = 0.15
	ray.CFrame = CFrame.new(position) * CFrame.Angles(0, math.random() * math.pi * 2, 0)
	ray.Parent = folder
	TweenService:Create(ray, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 0.1, 5),
		Transparency = 1,
	}):Play()
	Debris:AddItem(ray, 0.35)
end
...
function SpecialVFX.phantomVanish(controller, color, duration)
	...
	shimmer.Size = Vector3.new(3, 3, 3)
...
function SpecialVFX.phantomTrail(controller, targetPos, color, folder)
	...
	for i = 1, 4 do
		...
		ghost.Size = Vector3.new(1.2, 0.4, 1.2)
...
function SpecialVFX.edgeSlash(position, facing, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.2, 0.2, 7)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	...
	slash.CFrame = CFrame.new(position, position + flatFacing) * CFrame.Angles(0, 0, math.rad(60))
	slash.Parent = folder
	TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 0.1, 10),
		Transparency = 1,
	}):Play()
