...
function SpecialVFX.bladeCyclone(controller, color, duration)
	...
	for i = 0, 5 do
		...
		blade.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(angle), math.rad(20))
		blade.Parent = folder

		TweenService:Create(blade, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
			CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(angle + 360), math.rad(20)),
		}):Play()
...
function SpecialVFX.cycloneHit(position, color, folder)
	...
	slash.Size = Vector3.new(3, 0.3, 3)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.25
	slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(math.random(0, 180)), math.rad(90))
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(6, 0.15, 6),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.stoneBastion(controller, color, duration)
	...
	for i = 0, 5 do
		...
		pillar.Size = Vector3.new(0.8, 2.5, 0.8)
		pillar.Anchored = true
		pillar.CanCollide = false
		pillar.Material = Enum.Material.Slate
		pillar.Color = color
		pillar.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.5, math.sin(math.rad(angle)) * 2.5))
		pillar.Parent = folder

		TweenService:Create(pillar, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			Size = Vector3.new(1.0, 3.5, 1.0),
		}):Play()
	...
	dome.CFrame = CFrame.new(pos + Vector3.new(0, 1, 0))
	dome.Parent = folder
	Debris:AddItem(dome, duration + 0.1)
end

function SpecialVFX.groundSlam(position, range, color, folder)
	local crack = Instance.new("Part")
	crack.Shape = Enum.PartType.Cylinder
	crack.Size = Vector3.new(0.2, 3, 3)
	crack.Anchored = true
	...
	local dust = Instance.new("Part")
	dust.Shape = Enum.PartType.Ball
	dust.Size = Vector3.new(4, 1.5, 4)
	dust.Anchored = true
	dust.CanCollide = false
	dust.Material = Enum.Material.SmoothPlastic
	dust.Color = Color3.fromRGB(90, 85, 75)
	dust.Transparency = 0.4
	dust.CFrame = CFrame.new(position)
	dust.Parent = folder
	TweenService:Create(dust, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(8, 2, 8),
		Transparency = 1,
	}):Play()
	Debris:AddItem(dust, 0.45)
end
...
function SpecialVFX.solarDriftTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.phantomSurge(fromPos, toPos, color, folder)
	local dir = (toPos - fromPos).Unit
	for i = 1, 4 do
		local t = i / 4
		local p = fromPos + dir * ((toPos - fromPos).Magnitude * t)
		local mark = Instance.new("Part")
		mark.Size = Vector3.new(0.6, 0.6, 2.5)
		mark.Anchored = true
		mark.CanCollide = false
		mark.Material = Enum.Material.Neon
		mark.Color = color
		mark.Transparency = 0.2 + t * 0.5
		mark.CFrame = CFrame.new(p, p + dir)
		mark.Parent = folder
		Debris:AddItem(mark, 0.3)
	end
end

function SpecialVFX.phantomBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
