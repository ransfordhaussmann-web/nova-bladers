...
function SpecialVFX.flameAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		ember.Size = Vector3.new(0.5, 0.8, 0.5)
		ember.Anchored = true
...
function SpecialVFX.blazeImpact(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.chillAura(controller, color, duration)
	...
	for i = 1, 5 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.random(-3, 3), 0.5, math.random(-3, 3)))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(duration * 0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.iceShard(origin, angle, range, color, folder)
	local dir = Vector3.new(math.cos(angle), 0, math.sin(angle))
	local endPos = origin + dir * range
	local shard = Instance.new("Part")
	shard.Size = Vector3.new(0.4, 0.15, 1.2)
	shard.Anchored = true
	shard.CanCollide = false
	shard.Material = Enum.Material.Ice
	shard.Color = color
	shard.CFrame = CFrame.lookAt(origin, endPos) * CFrame.Angles(0, math.rad(90), 0)
	shard.Parent = folder

	TweenService:Create(shard, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		CFrame = CFrame.lookAt(endPos, endPos + dir) * CFrame.Angles(0, math.rad(90), 0),
		Transparency = 1,
	}):Play()
	Debris:AddItem(shard, 0.35)
end

function SpecialVFX.frostBurst(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	core.Size = Vector3.new(3, 3, 3)
	core.Anchored = true
