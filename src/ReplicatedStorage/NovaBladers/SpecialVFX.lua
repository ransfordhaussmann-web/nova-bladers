...
function SpecialVFX.spiralTrail(position, color, folder)
	...
	arc.Size = Vector3.new(0.12, 2.5, 2.5)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = 0.25
	arc.CFrame = CFrame.new(position + Vector3.new(0, 0.4, 0)) * CFrame.Angles(0, 0, math.rad(90))
	arc.Parent = folder
...
function SpecialVFX.vortexBurst(position, color, folder)
	...
	ring.CanCollide = false
	ring.Material = Enum.Material.Neon
	ring.Color = Color3.fromRGB(255, 120, 80)
	ring.Transparency = 0.2
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	Debris:AddItem(mist, duration + 0.15)
end
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 6 do
		...
		local start = origin + Vector3.new(math.cos(angle) * 1.5, 0.6, math.sin(angle) * 1.5)
		local finish = origin + Vector3.new(math.cos(angle) * range, 0.4, math.sin(angle) * range)
		shard.CFrame = CFrame.new(start, finish)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(finish, finish + Vector3.new(math.cos(angle), 0, math.sin(angle))),
			Transparency = 1,
		}):Play()
