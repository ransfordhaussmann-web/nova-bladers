...
function SpecialVFX.starfallTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.starfallBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	...
	local spark = Instance.new("Sparkles")
	spark.SparkleColor = Color3.fromRGB(255, 220, 120)
	spark.Parent = burst
...
function SpecialVFX.glacierCrystal(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
