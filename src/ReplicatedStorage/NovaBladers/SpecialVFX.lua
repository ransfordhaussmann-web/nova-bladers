...
function SpecialVFX.infernoIgnite(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
...
function SpecialVFX.infernoTrail(fromPos, toPos, color, folder)
	...
	fire.Heat = 14
	fire.Color = color
	fire.SecondaryColor = Color3.fromRGB(255, 180, 40)
	fire.Parent = trail

	Debris:AddItem(trail, 0.4)
end

function SpecialVFX.infernoRoar(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	for i = 0, 5 do
		...
		local offset = Vector3.new(math.cos(angle) * 2.2, 0.6, math.sin(angle) * 2.2)
		...
		shard.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, angle, math.rad(20))
		shard.Parent = folder
...
function SpecialVFX.frostGuard(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
...
function SpecialVFX.frostShardBurst(origin, range, color, folder, shardIndex)
	local angle = (shardIndex or 0) * (math.pi * 2 / 6)
	...
	shard.CFrame = CFrame.new(origin + dir * 1.5, origin + dir * 4)
	shard.Parent = folder

	local target = origin + dir * range
