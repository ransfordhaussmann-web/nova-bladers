...
function SpecialVFX.vortexSpiral(controller, color, folder)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, math.rad(angle), math.rad(45))
		shard.Parent = folder
		Debris:AddItem(shard, 0.35)
	end
end

function SpecialVFX.vortexSlashTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
...
function SpecialVFX.iceCitadel(controller, color, duration)
	...
	spikes.Size = Vector3.new(0.4, 5, 5)
	spikes.Anchored = true
	spikes.CanCollide = false
	spikes.Material = Enum.Material.Neon
