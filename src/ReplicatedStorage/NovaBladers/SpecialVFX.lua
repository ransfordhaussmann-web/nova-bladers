...
function SpecialVFX.infernoTrail(position, color, folder)
	local spark = Instance.new("Part")
	...
	local trailFire = Instance.new("Fire")
	trailFire.Size = 2
	trailFire.Heat = 6
	trailFire.Color = color
	trailFire.SecondaryColor = Color3.fromRGB(255, 180, 60)
	trailFire.Parent = spark
...
function SpecialVFX.iceShard(fromPos, toPos, color, folder)
	...
	shard.Size = Vector3.new(0.3, 0.8, 1.2)
	shard.Anchored = true
	shard.CanCollide = false
	shard.Material = Enum.Material.Glass
	shard.Color = color
	shard.Transparency = 0.15
	shard.CFrame = CFrame.new(fromPos + Vector3.new(0, 0.5, 0), fromPos + dir * 3)
	shard.Parent = folder

	local endPos = fromPos + dir * 8 + Vector3.new(0, 0.5, 0)
	TweenService:Create(shard, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	for i = 1, 6 do
		local angle = i * 60
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 0.6, 1.0)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + offset * 3),
			Transparency = 1,
		}):Play()
