...
function SpecialVFX.flameRushTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(1.5, 0.6, 1.5)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
...
function SpecialVFX.iceShardsBurst(origin, range, color, folder)
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, angle, math.rad(70)) * CFrame.new(0, 0, range * 0.4)
		shard.Parent = folder
