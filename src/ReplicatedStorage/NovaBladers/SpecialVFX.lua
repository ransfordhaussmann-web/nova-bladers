...
function SpecialVFX.iceAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.5, 0.3, math.sin(angle) * 2.5))
...
function SpecialVFX.forgeCharge(controller, color, duration)
	...
	for i = 1, 6 do
		...
		ember.Color = color
		ember.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2) * 0.5, 0.5, math.random(-2, 2) * 0.5))
		ember.Parent = folder

		TweenService:Create(ember, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
	...
	heat.Shape = Enum.PartType.Cylinder
	heat.Size = Vector3.new(0.2, 3, 3)
	heat.Anchored = true
	heat.CanCollide = false
	heat.Material = Enum.Material.Neon
	heat.Color = color
	heat.Transparency = 0.3
	heat.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	heat.Parent = folder
...
function SpecialVFX.lavaTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
