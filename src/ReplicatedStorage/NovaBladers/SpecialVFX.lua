...
function SpecialVFX.slashArc(position, direction, color, folder)
	...
	for i = -1, 1 do
		...
		slash.CFrame = CFrame.new(position + offset + Vector3.new(0, 0.5, 0))
			* CFrame.Angles(0, math.atan2(direction.X, direction.Z), math.rad(45 + i * 30))
		slash.Parent = folder
...
function SpecialVFX.slashFinish(position, facing, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
	...
	burst.CFrame = CFrame.new(position + facing * 2 + Vector3.new(0, 0.5, 0))
	burst.Parent = folder
