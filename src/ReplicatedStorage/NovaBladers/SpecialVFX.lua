...
function SpecialVFX.bladeCycloneRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.solarOrbitTrail(origin, color, folder)
...
function SpecialVFX.phantomStrike(position, color, folder)
	...
	TweenService:Create(slash, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(12, 0.15, 0.4),
		Transparency = 1,
	}):Play()
	Debris:AddItem(slash, 0.35)
end

function SpecialVFX.unPhantomFade(controller)
	if controller.phantomFaded then
		controller.part.Transparency = controller._savedTransparency or 0
		controller.spinRing.Transparency = controller._savedRingTransparency or 0.4
		controller.phantomFaded = false
	end
end
