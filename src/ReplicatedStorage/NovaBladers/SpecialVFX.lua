...
function SpecialVFX.tideRipple(origin, range, color, folder)
	...
	TweenService:Create(wave, TweenInfo.new(0.4, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
...
function SpecialVFX.anchorSlam(position, range, color, folder)
	local slam = Instance.new("Part")
	slam.Shape = Enum.PartType.Cylinder
	slam.Size = Vector3.new(0.5, 2, 2)
	slam.Anchored = true
...
function SpecialVFX.stonePulse(origin, range, color, folder)
	...
	TweenService:Create(chunk, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(range * 1.5, 1.5, range * 1.5),
		Transparency = 1,
	}):Play()
