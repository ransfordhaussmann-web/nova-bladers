...
function SpecialVFX.ripperSlash(origin, direction, color, folder)
	...
	local mid = origin + direction * 1.8
	slash.CFrame = CFrame.new(mid, mid + direction) * CFrame.Angles(0, 0, math.rad(90))
	slash.Parent = folder
...
function SpecialVFX.auroraDomeBurst(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(6, 6, 6)
	burst.Anchored = true
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = color
