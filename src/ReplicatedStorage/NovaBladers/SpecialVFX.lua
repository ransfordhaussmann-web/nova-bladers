...
function SpecialVFX.bladeCycloneHit(position, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(4, 0.15, 0.8)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.2
	slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.random() * math.pi * 2, 0)
	slash.Parent = folder
...
function SpecialVFX.bladeFinisher(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Cylinder
	burst.Size = Vector3.new(0.3, 4, 4)
	burst.Anchored = true
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = color
...
function SpecialVFX.stoneAnchor(controller, color, duration)
	...
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(0, 0, 2.2)
		local stone = Instance.new("Part")
		stone.Size = Vector3.new(0.8, 1.2, 0.8)
		stone.Anchored = true
		stone.CanCollide = false
		stone.Material = Enum.Material.Slate
		stone.Color = color
		stone.CFrame = CFrame.new(controller.part.Position) * offset
		stone.Parent = folder
		Debris:AddItem(stone, duration + 0.2)
	end
end
...
function SpecialVFX.phantomSlash(position, facing, range, color, folder)
	for i = -1, 1 do
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(range * 0.8, 0.12, 0.6)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		...
		slash.CFrame = CFrame.new(position + right * i * 1.5, position + facing * 2)
		slash.Parent = folder
