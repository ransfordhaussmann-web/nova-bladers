...
function SpecialVFX.slashArc(position, facing, color, folder)
	...
	for i = -1, 1 do
		local mark = Instance.new("Part")
		mark.Size = Vector3.new(0.35, 1.8, 2.2)
		mark.Anchored = true
		mark.CanCollide = false
		mark.Material = Enum.Material.Neon
		mark.Color = color
		mark.Transparency = 0.2
		mark.CFrame = CFrame.new(position + right * (i * 1.2) + Vector3.new(0, 0.4, 0))
			* CFrame.lookAt(Vector3.zero, facing) * CFrame.Angles(0, math.rad(i * 35), 0)
		mark.Parent = folder
		TweenService:Create(mark, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.2, 2.4, 3),
		}):Play()
...
function SpecialVFX.crimsonFinisher(position, color, folder)
	...
	ring.CanCollide = false
	ring.Material = Enum.Material.Neon
	ring.Color = Color3.fromRGB(255, 180, 120)
	ring.Transparency = 0.25
...
function SpecialVFX.frostLock(position, range, color, folder)
	local pillar = Instance.new("Part")
	pillar.Shape = Enum.PartType.Cylinder
	pillar.Size = Vector3.new(0.5, range * 1.6, range * 1.6)
	...
	core.CanCollide = false
	core.Material = Enum.Material.Neon
	core.Color = Color3.fromRGB(200, 240, 255)
