...
function SpecialVFX.bladeSlash(controller, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.3, 3, 5)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.3
	slash.CFrame = CFrame.new(controller.part.Position, controller.part.Position + controller.facing)
	slash.Parent = folder
	TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.1, 5, 8),
	}):Play()
...
function SpecialVFX.phantomDash(position, dir, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.5, 0.5, 3)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
...
function SpecialVFX.phantomEdgeFlash(position, color, folder)
