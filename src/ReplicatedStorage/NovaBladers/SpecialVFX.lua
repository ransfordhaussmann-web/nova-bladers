...
function SpecialVFX.freezeAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	sparkles.SparkleColor = Color3.fromRGB(200, 245, 255)
	sparkles.Parent = frost
...
function SpecialVFX.crystalShard(origin, range, color, folder)
	...
	shard.CFrame = CFrame.new(origin + Vector3.new(0, 0.6, 0)) * CFrame.Angles(0, math.rad(math.random(0, 360)), math.rad(35))
	shard.Parent = folder
...
function SpecialVFX.phoenixBurst(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	...
	for i = 1, 4 do
		...
		wing.Size = Vector3.new(0.3, 0.8, 2.5)
		wing.Anchored = true
		wing.CanCollide = false
		wing.Material = Enum.Material.Neon
		wing.Color = Color3.fromRGB(255, 220, 100)
		wing.Transparency = 0.2
		local angle = (i / 4) * math.pi * 2
		wing.CFrame = CFrame.new(position + Vector3.new(math.cos(angle) * 2, 1.2, math.sin(angle) * 2))
			* CFrame.Angles(0, angle, math.rad(30))
		wing.Parent = folder

		TweenService:Create(wing, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = wing.CFrame * CFrame.new(0, 2, 0),
			Transparency = 1,
		}):Play()
