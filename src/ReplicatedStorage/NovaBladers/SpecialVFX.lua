...
function SpecialVFX.slashArc(origin, facing, color, folder)
	...
	arc.Size = Vector3.new(0.15, 2.8, 5.5)
	arc.Anchored = true
	...
	arc.CFrame = CFrame.new(origin + look * 2.5, origin + look * 2.5 + look)
	arc.Parent = folder

	local trail = Instance.new("Part")
	trail.Shape = Enum.PartType.Cylinder
	trail.Size = Vector3.new(0.12, 4.5, 4.5)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = Color3.fromRGB(255, 120, 80)
	trail.Transparency = 0.35
	trail.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(90))
	trail.Parent = folder
	...
	TweenService:Create(trail, TweenInfo.new(0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		flake.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.5, 0.8, math.sin(angle) * 2.5))
		flake.Parent = folder

		TweenService:Create(flake, TweenInfo.new(duration, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	for i = 1, 8 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * range, 1.2, math.sin(angle) * range)),
			Transparency = 1,
		}):Play()
