...
function SpecialVFX.flameAura(controller, color, duration)
	...
	local fireCore = Instance.new("Part")
	fireCore.Shape = Enum.PartType.Ball
	fireCore.Size = Vector3.new(3, 3, 3)
	fireCore.Anchored = true
	fireCore.CanCollide = false
	fireCore.Material = Enum.Material.Neon
	fireCore.Color = color
	fireCore.Transparency = 0.35
	fireCore.CFrame = CFrame.new(pos)
	fireCore.Parent = folder
	...
	TweenService:Create(fireCore, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(6, 6, 6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(fireCore, duration + 0.1)
end
...
function SpecialVFX.crownAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, math.rad(angle), math.rad(30))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
			CFrame = CFrame.new(pos + offset * 1.4 + Vector3.new(0, 0.6, 0)) * CFrame.Angles(0, math.rad(angle + 30), math.rad(30)),
			Transparency = 0.7,
		}):Play()
...
function SpecialVFX.iceShardPulse(origin, range, color, folder)
	for i = 0, 5 do
		local angle = i * 60
		local dir = Vector3.new(math.cos(math.rad(angle)), 0, math.sin(math.rad(angle)))
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.5, 0.3, 1.8)
		shard.Anchored = true
		shard.CanCollide = false
