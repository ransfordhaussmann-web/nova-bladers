...
function SpecialVFX.lungeSlash(position, facing, color, folder)
	...
	slash.Size = Vector3.new(0.3, 4, 7)
	slash.Anchored = true
	...
	slash.CFrame = CFrame.new(position + flatFacing * 2, position + flatFacing * 5)
...
function SpecialVFX.mirageClone(controller, color, offset, folder)
	...
	TweenService:Create(ghost, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		CFrame = ghost.CFrame + Vector3.new(offset.X * 0.3, 0.5, offset.Z * 0.3),
	}):Play()
