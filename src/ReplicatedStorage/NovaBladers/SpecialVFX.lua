...
function SpecialVFX.cycloneSwirl(position, radius, color, folder)
	local swirl = Instance.new("Part")
	swirl.Shape = Enum.PartType.Cylinder
	swirl.Size = Vector3.new(0.25, radius, radius)
	swirl.Anchored = true
	swirl.CanCollide = false
	swirl.Material = Enum.Material.Neon
	swirl.Color = color
	swirl.Transparency = 0.3
	swirl.CFrame = CFrame.new(position + Vector3.new(0, 0.4, 0)) * CFrame.Angles(0, 0, math.rad(90))
	swirl.Parent = folder

	TweenService:Create(swirl, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, radius * 1.6, radius * 1.6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(swirl, 0.35)
end

function SpecialVFX.cycloneFinale(position, range, color, folder)
	local burst = Instance.new("Part")
...
function SpecialVFX.stonePillars(controller, color, duration)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = Vector3.new(math.cos(math.rad(angle)) * 3.5, 0, math.sin(math.rad(angle)) * 3.5)
...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	shield.Size = Vector3.new(1.0, 5.5, 5.5)
...
function SpecialVFX.solarTrail(fromPos, toPos, color, folder)
	...
	ember.CFrame = CFrame.new(mid) * CFrame.Angles(0, 0, math.rad(90))
	ember.Parent = folder
...
function SpecialVFX.phantomDash(fromPos, toPos, color, folder)
	...
	ghost.CFrame = CFrame.new(fromPos) * CFrame.Angles(0, 0, math.rad(90))
	ghost.Parent = folder

	local dir = (toPos - fromPos)
	local len = dir.Magnitude
	if len > 0.1 then
		ghost.CFrame = CFrame.new(fromPos, toPos) * CFrame.Angles(0, 0, math.rad(90))
	end

	TweenService:Create(ghost, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		CFrame = CFrame.new(toPos) * CFrame.Angles(0, 0, math.rad(90)),
		Transparency = 1,
	}):Play()
	Debris:AddItem(ghost, 0.25)
end

function SpecialVFX.phantomSurge(position, range, color, folder)
	for i = 0, 2 do
		local angle = i * 120
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.3, 0.3, range)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.2
		slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(70))
		slash.Parent = folder

		TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.1, 0.1, range * 1.4),
		}):Play()
