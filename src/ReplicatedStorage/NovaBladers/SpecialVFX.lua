...
function SpecialVFX.frostShatter(origin, range, color, folder)
	...
	TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(range * 2, 1.5, range * 2),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.fangStrike(position, facing, color, folder)
	...
	slash.CFrame = CFrame.new(position + facing * 2 + Vector3.new(0, 0.5, 0), position + facing * 4)
	slash.Parent = folder
