...
function SpecialVFX.crystalShatter(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(origin + offset) * CFrame.Angles(math.random() * 0.5, angle, math.random() * 0.5)
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.15, 0.3, 0.08),
		}):Play()
...
function SpecialVFX.blazeSpiral(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
