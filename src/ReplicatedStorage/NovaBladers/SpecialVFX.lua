...
function SpecialVFX.slashArc(origin, facing, color, folder, radius)
	local arc = Instance.new("Part")
	arc.Size = Vector3.new(0.35, 0.8, radius or 5)
	arc.Anchored = true
	...
	arc.CFrame = CFrame.new(origin + flatFacing * ((radius or 5) * 0.45), origin)
	...
	TweenService:Create(arc, TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.1, 0.2, (radius or 5) * 1.4),
	}):Play()
