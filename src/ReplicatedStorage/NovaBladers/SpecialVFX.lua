...
function SpecialVFX.slashArc(fromPos, toPos, color, folder)
	...
	arc.Size = Vector3.new(0.15, 2.8, 0.4)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = 0.1
	arc.CFrame = CFrame.lookAt(mid, mid + dir) * CFrame.Angles(0, math.rad(90), math.rad(35))
	arc.Parent = folder

	TweenService:Create(arc, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.1, 3.6, 0.2),
	}):Play()
	Debris:AddItem(arc, 0.25)
end

function SpecialVFX.slashImpact(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.citadelRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "CitadelRing"
	ring.Shape = Enum.PartType.Cylinder
