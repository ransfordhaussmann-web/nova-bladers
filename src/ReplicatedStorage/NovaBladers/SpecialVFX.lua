...
function SpecialVFX.flameSpiral(origin, radius, color, folder)
	...
	arc.Size = Vector3.new(radius * 0.8, 0.6, radius * 0.8)
	arc.Anchored = true
	...
	TweenService:Create(arc, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(radius * 1.6, 0.3, radius * 1.6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(arc, 0.35)
end

function SpecialVFX.flameEruption(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.iceBarrier(controller, color, duration)
	...
	frost.Size = Vector3.new(1.2, 5, 5)
	frost.Anchored = true
