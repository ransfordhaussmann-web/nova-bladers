...
function SpecialVFX.slashArc(origin, facing, color, folder, arcIndex)
	local angle = (arcIndex or 0) * math.rad(60)
	local dir = CFrame.new(Vector3.zero, facing) * CFrame.Angles(0, angle, 0)
	local arc = Instance.new("Part")
	arc.Size = Vector3.new(0.3, 0.8, 5.5)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = 0.15
	arc.CFrame = CFrame.new(origin + Vector3.new(0, 0.6, 0)) * dir
	arc.Parent = folder

	TweenService:Create(arc, TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, 1.2, 7.5),
		Transparency = 1,
	}):Play()
