...
function SpecialVFX.fireOrbit(origin, radius, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.cometSlam(position, color, folder)
	...
	crater.Size = Vector3.new(0.3, 4, 4)
...
function SpecialVFX.iceShards(origin, color, folder)
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * 2, 0.8, math.sin(angle) * 2))
