...
function SpecialVFX.tidalWave(origin, range, color, folder)
	...
	TweenService:Create(wave, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, range * 2.5, range * 2.5),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.rendSlash(origin, facing, color, folder)
	...
	slash.CFrame = CFrame.new(origin, origin + facing) * CFrame.Angles(0, math.rad(90), 0)
	slash.Parent = folder
...
function SpecialVFX.phaseFade(controller, fade)
