...
function SpecialVFX.flameCharge(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	ring.CFrame = CFrame.new(controller.part.Position + Vector3.new(0, 1.2, 0)) * CFrame.Angles(0, 0, math.rad(90))
	ring.Parent = folder

	for i = 0, 5 do
		local angle = math.rad(i * 60)
		local spike = Instance.new("Part")
		spike.Size = Vector3.new(0.25, 1.2, 0.25)
		spike.Anchored = true
		spike.CanCollide = false
		spike.Material = Enum.Material.Neon
		spike.Color = Color3.fromRGB(220, 245, 255)
		spike.CFrame = CFrame.new(controller.part.Position + Vector3.new(math.cos(angle) * 2.5, 1.5, math.sin(angle) * 2.5))
		spike.Parent = folder
...
function SpecialVFX.iceShardBurst(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.4, 1.4, 0.4)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Neon
		shard.Color = color
		shard.Transparency = 0.2
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, angle, math.rad(30))
		shard.Parent = folder
