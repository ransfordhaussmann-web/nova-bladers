...
function SpecialVFX.bladeSlash(position, range, color, folder)
	local slash = Instance.new("Part")
	slash.Shape = Enum.PartType.Cylinder
	slash.Size = Vector3.new(0.12, range * 1.6, range * 1.6)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
	slash.Transparency = 0.25
	slash.CFrame = CFrame.new(position + Vector3.new(0, 0.4, 0)) * CFrame.Angles(0, 0, math.rad(90))
	slash.Parent = folder
...
function SpecialVFX.stoneAnchor(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local base = Instance.new("Part")
	base.Shape = Enum.PartType.Cylinder
	base.Size = Vector3.new(0.4, 5, 5)
	base.Anchored = true
	base.CanCollide = false
	base.Material = Enum.Material.Slate
	base.Color = color
	base.Transparency = 0.4
	base.CFrame = CFrame.new(controller.part.Position - Vector3.new(0, 0.3, 0)) * CFrame.Angles(0, 0, math.rad(90))
	base.Parent = folder
	Debris:AddItem(base, duration + 0.1)
end

function SpecialVFX.stonePillars(controller, color, folder)
	local pos = controller.part.Position
	for i = 0, 3 do
		local angle = i * 90 + 45
		local offset = Vector3.new(math.cos(math.rad(angle)), 0, math.sin(math.rad(angle))) * 5
		...
		pillar.Color = color
		pillar.CFrame = CFrame.new(pos + offset - Vector3.new(0, 0.5, 0))
		pillar.Parent = folder

		TweenService:Create(pillar, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + offset + Vector3.new(0, 1.2, 0)),
		}):Play()
		Debris:AddItem(pillar, 0.7)
	end
end

function SpecialVFX.groundCollapse(position, range, color, folder)
	local crack = Instance.new("Part")
	crack.Shape = Enum.PartType.Cylinder
	crack.Size = Vector3.new(0.25, 4, 4)
	crack.Anchored = true
...
function SpecialVFX.solarFlare(origin, range, color, folder)
	...
	flare.CFrame = CFrame.new(origin + Vector3.new(0, 1, 0))
	flare.Parent = folder
...
function SpecialVFX.heatTrail(position, color, folder)
	...
	ember.Parent = folder
	Debris:AddItem(ember, 0.25)
end

function SpecialVFX.phantomSplit(controller, color, folder)
	for i = 1, 3 do
...
function SpecialVFX.phantomDash(controller, targetPos, color, folder)
	...
	afterimage.Size = Vector3.new(0.6, 3.2, 3.2)
	...
	TweenService:Create(afterimage, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		CFrame = CFrame.new(targetPos) * CFrame.Angles(0, 0, math.rad(90)),
	}):Play()
	Debris:AddItem(afterimage, 0.35)
end

function SpecialVFX.phantomFade(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
