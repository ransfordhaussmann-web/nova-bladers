...
function SpecialVFX.frostChill(controller, color, duration)
	...
	for i = 1, 6 do
		...
		local angle = i * 60
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.8, math.sin(math.rad(angle)) * 2.5))
		shard.Parent = folder
...
function SpecialVFX.frostShards(origin, range, color, folder)
	for i = 1, 4 do
		...
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 2.5, 0.4)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Ice
		shard.Color = color
		shard.Transparency = 0.1
		local offset = Vector3.new(math.cos(math.rad(angle)) * range * 0.5, 1.5, math.sin(math.rad(angle)) * range * 0.5)
		shard.CFrame = CFrame.new(origin + offset) * CFrame.Angles(math.rad(25), math.rad(angle), 0)
		shard.Parent = folder
