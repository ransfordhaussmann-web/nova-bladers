...
function SpecialVFX.forgeHeat(controller, color, duration)
	...
	Debris:AddItem(heat, duration + 0.15)
end
...
function SpecialVFX.crystalShield(controller, color, duration)
	...
	for i = 0, 5 do
		...
		shard.CFrame = ring.CFrame * offset
		shard.Parent = folder
		Debris:AddItem(shard, duration)
	end
...
function SpecialVFX.shatterBurst(position, color, folder)
	...
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(position + dir * 2, position + dir * 4)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + dir * 6, position + dir * 10),
			Transparency = 1,
		}):Play()
