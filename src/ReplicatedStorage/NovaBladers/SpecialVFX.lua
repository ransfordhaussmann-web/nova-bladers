...
function SpecialVFX.igniteAura(controller, color, duration)
	...
	fireRing.Size = Vector3.new(0.25, 2.5, 2.5)
	fireRing.Anchored = true
	fireRing.CanCollide = false
	fireRing.Material = Enum.Material.Neon
	fireRing.Color = color
	fireRing.Transparency = 0.3
	fireRing.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	fireRing.Parent = folder
	...
	fire.Parent = fireRing

	TweenService:Create(fireRing, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, 6, 6),
		Transparency = 0.7,
	}):Play()
	Debris:AddItem(fireRing, duration + 0.15)

	SpecialVFX.chargeAura(controller, color, duration * 0.8)
end
...
function SpecialVFX.forgeSlam(position, color, folder)
	...
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = Color3.fromRGB(255, 180, 60)
...
function SpecialVFX.frostShards(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.4, 1.2, 0.25)
		shard.Anchored = true
		...
		local endPos = origin + Vector3.new(math.cos(angle) * range, 0.5, math.sin(angle) * range)
