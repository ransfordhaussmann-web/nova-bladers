...
function SpecialVFX.rendSlash(fromPos, toPos, color, folder)
	...
	slash.Size = Vector3.new(2.5, 0.15, 0.8)
	slash.Anchored = true
	...
	if dir.Magnitude > 0.01 then
		...
		slash.CFrame = CFrame.new(mid + Vector3.new(0, 0.4, 0), mid + dir)
	else
		slash.CFrame = CFrame.new(mid + Vector3.new(0, 0.4, 0))
	end
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(3.5, 0.1, 0.5),
	}):Play()
