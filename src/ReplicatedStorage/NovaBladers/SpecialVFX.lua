...
function SpecialVFX.bladeCyclone(origin, range, color, folder)
	...
	for i = 0, 2 do
		...
		blade.CFrame = CFrame.new(origin) * CFrame.Angles(0, math.rad(i * 120), math.rad(20)) * CFrame.new(0, 0.3, range * 0.5)
		blade.Parent = folder
...
function SpecialVFX.stoneShell(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local shell = Instance.new("Part")
	shell.Shape = Enum.PartType.Cylinder
	shell.Size = Vector3.new(1.8, 5, 5)
	shell.Anchored = true
...
function SpecialVFX.groundSlam(origin, range, color, folder)
	...
	for i = 0, 3 do
		...
		chunk.Color = Color3.fromRGB(100, 95, 90)
		chunk.CFrame = CFrame.new(origin + Vector3.new(math.cos(i * 1.5) * 2, 1, math.sin(i * 1.5) * 2))
		chunk.Parent = folder
		TweenService:Create(chunk, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = chunk.CFrame + Vector3.new(0, 3, 0),
			Transparency = 1,
		}):Play()
		Debris:AddItem(chunk, 0.45)
	end
end
...
function SpecialVFX.solarTrail(fromPos, toPos, color, folder)
	...
	trail.CFrame = CFrame.new(mid + Vector3.new(0, 0.2, 0)) * CFrame.Angles(0, 0, math.rad(90))
	trail.Parent = folder
...
function SpecialVFX.crossSlash(position, color, folder)
	for i = 0, 1 do
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.2, 0.2, 8)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.2
		slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 90), math.rad(45))
		slash.Parent = folder
