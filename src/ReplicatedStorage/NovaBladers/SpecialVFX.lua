...
function SpecialVFX.infernoIgnite(controller, color, duration)
	...
	flame.CFrame = CFrame.new(controller.part.Position)
	flame.Parent = folder
...
function SpecialVFX.frostCitadelRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FrostCitadel"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.frostShatter(origin, range, color, folder)
	...
	for i = 1, 4 do
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 0.4, 0.9)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Ice
		shard.Color = Color3.fromRGB(230, 250, 255)
