...
function SpecialVFX.iceCage(controller, color, duration)
	...
	cage.Size = Vector3.new(2.0, 5.5, 5.5)
	cage.Anchored = true
	cage.CanCollide = false
	cage.Material = Enum.Material.Ice
	cage.Color = color
...
function SpecialVFX.iceShatter(position, range, color, folder)
	for i = 1, 8 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos) * CFrame.Angles(0, angle, math.rad(90)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.meltTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.coreExplosion(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	core.Size = Vector3.new(3, 3, 3)
	core.Anchored = true
	...
	fire.Heat = 14
	fire.Color = color
	fire.SecondaryColor = Color3.fromRGB(255, 255, 120)
	fire.Parent = core
