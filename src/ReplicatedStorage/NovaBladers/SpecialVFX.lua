...
function SpecialVFX.spawnMirages(controller, color, folder)
	local offsets = {}
	local angles = { 72, -72 }
	for i, deg in angles do
		local offset = CFrame.Angles(0, math.rad(deg), 0) * Vector3.new(5, 0, 0)
		...
		ghost.Parent = folder
		Debris:AddItem(ghost, 1.2)
	end
	return offsets
end

function SpecialVFX.bladeSlash(origin, direction, color, folder)
	...
	slash.CFrame = CFrame.new(origin + Vector3.new(0, 0.5, 0), origin + flatDir)
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, 3.5, 7),
		Transparency = 1,
	}):Play()
