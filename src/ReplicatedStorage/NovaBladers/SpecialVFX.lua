...
function SpecialVFX.infernoErupt(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostCrystals(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.Size = Vector3.new(0.5, 1.2, 0.5)
		crystal.Anchored = true
		crystal.CanCollide = false
		crystal.Material = Enum.Material.Glass
		crystal.Color = color
		crystal.Transparency = 0.2
		crystal.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, math.rad(angle), math.rad(25))
		crystal.Parent = folder

		TweenService:Create(crystal, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + offset * 0.6) * CFrame.Angles(0, math.rad(angle), math.rad(25)),
			Transparency = 0.6,
		}):Play()
...
function SpecialVFX.frostShatter(origin, range, color, folder)
	...
	for i = 1, 4 do
		...
		local angle = i * 90
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(math.rad(angle)) * 1.5, 0.6, math.sin(math.rad(angle)) * 1.5))
