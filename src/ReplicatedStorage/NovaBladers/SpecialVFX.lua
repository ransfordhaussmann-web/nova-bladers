...
function SpecialVFX.forgeShatter(position, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	...
	for i = 0, 5 do
		...
		shard.Size = Vector3.new(0.6, 0.6, 1.4)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Neon
		shard.Color = Color3.fromRGB(255, 180, 80)
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 60), math.rad(25))
		shard.Parent = folder
...
function SpecialVFX.frostShatter(position, color, folder)
	...
	for i = 0, 7 do
		...
		crystal.CFrame = CFrame.new(position) * CFrame.Angles(math.rad(20), math.rad(i * 45), 0) * CFrame.new(0, 0, 3)
		crystal.Parent = folder

		TweenService:Create(crystal, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = crystal.CFrame * CFrame.new(0, 1.5, 2),
			Transparency = 1,
		}):Play()
