...
function SpecialVFX.bladeSlash(origin, facing, color, folder)
	...
	slash.CFrame = CFrame.new(origin + Vector3.new(0, 0.5, 0), origin + facing) * CFrame.Angles(0, math.rad(90), math.rad(45))
	slash.Parent = folder
...
function SpecialVFX.cycloneRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.stoneBastion(controller, color, duration)
	...
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(3.5, 0, 0)
...
function SpecialVFX.groundCrack(origin, range, color, folder)
...
function SpecialVFX.burnTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(1, 0.3, (fromPos - toPos).Magnitude)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.3
	trail.CFrame = CFrame.new(mid, toPos)
	trail.Parent = folder
...
function SpecialVFX.phantomAfterimage(controller, color, folder)
	...
	ghost.Size = controller.part.Size
	ghost.Anchored = true
...
function SpecialVFX.phantomFinisher(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
	...
	for i = 0, 2 do
		local angle = i * 120
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.12, 0.5, 5)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = Color3.fromRGB(200, 250, 255)
		slash.Transparency = 0.2
		slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(60))
		slash.Parent = folder
		TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = Vector3.new(0.08, 0.3, 8),
			Transparency = 1,
		}):Play()
