...
function SpecialVFX.lungeTrail(controller, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(1.5, 0.3, 4)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.25
	trail.CFrame = CFrame.new(controller.part.Position, controller.part.Position + controller.facing)
		* CFrame.new(0, 0, -2)
	trail.Parent = folder
	TweenService:Create(trail, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(2.5, 0.2, 6),
	}):Play()
...
function SpecialVFX.ripSlash(position, facing, color, folder)
	...
	slash.CFrame = CFrame.new(position + facing * 2.5, position + facing * 5)
	slash.Parent = folder
...
function SpecialVFX.graniteBastionWall(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	for i = 0, 5 do
		local angle = i * 60
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(3.5, 0.6, 0)
		local pillar = Instance.new("Part")
		pillar.Size = Vector3.new(1.2, 2.2, 0.8)
		pillar.Anchored = true
		pillar.CanCollide = false
		pillar.Material = Enum.Material.Slate
		pillar.Color = color
		pillar.CFrame = CFrame.new(controller.part.Position) * offset
...
function SpecialVFX.solarOrbitRing(origin, color, folder)
	local ring = Instance.new("Part")
	...
	ring.Color = color
	ring.Transparency = 0.2
	ring.CFrame = CFrame.new(origin + Vector3.new(0, 0.4, 0))
	ring.Parent = folder
...
function SpecialVFX.phantomSplit(controller, color, folder)
	...
	clone.Size = Vector3.new(0.8, 3.2, 3.2)
	clone.Anchored = true
	clone.CanCollide = false
	clone.Material = Enum.Material.ForceField
	clone.Color = color
	clone.Transparency = 0.6
	clone.CFrame = controller.part.CFrame * CFrame.new(2.5, 0, 0)
	clone.Parent = folder
	TweenService:Create(clone, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		CFrame = clone.CFrame * CFrame.new(4, 0, 0),
	}):Play()
	Debris:AddItem(clone, 0.55)
end

function SpecialVFX.mirageFeint(controller, color, folder)
	local after = Instance.new("Part")
	after.Shape = Enum.PartType.Cylinder
	after.Size = controller.part.Size
	after.Anchored = true
	after.CanCollide = false
	after.Material = Enum.Material.Glass
	after.Color = color
	after.Transparency = 0.7
	after.CFrame = controller.part.CFrame
	after.Parent = folder
	TweenService:Create(after, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
	}):Play()
	Debris:AddItem(after, 0.45)
end

function SpecialVFX.phantomStrike(controller, targetPos, color, folder)
