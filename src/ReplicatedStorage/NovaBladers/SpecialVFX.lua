...
function SpecialVFX.infernoErupt(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
...
function SpecialVFX.iceSurge(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.iceRecover(position, color, folder)
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
