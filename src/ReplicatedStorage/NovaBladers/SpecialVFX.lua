...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.random(-2, 2), 0.3, math.random(-2, 2)))
			* CFrame.Angles(math.rad(math.random(-20, 20)), math.rad(i * 60), math.rad(25))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(0, 2, 0)) * CFrame.Angles(0, math.rad(i * 60), math.rad(25)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.iceShardBurst(origin, range, color, folder)
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(origin, endPos) * CFrame.Angles(0, 0, math.rad(70))
		shard.Parent = folder
...
function SpecialVFX.fangBurst(position, color, folder)
	...
	for i = 0, 3 do
		...
		slash.Color = Color3.fromRGB(255, 200, 120)
