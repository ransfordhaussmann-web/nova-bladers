...
function SpecialVFX.stoneRoot(controller, color, duration)
	...
	crack.Size = Vector3.new(0.2, 4, 4)
	crack.Anchored = true
	crack.CanCollide = false
	crack.Material = Enum.Material.Slate
	crack.Color = color
	crack.Transparency = 0.3
	crack.CFrame = CFrame.new(controller.part.Position - Vector3.new(0, 0.5, 0)) * CFrame.Angles(0, 0, math.rad(90))
	crack.Parent = folder
	Debris:AddItem(crack, duration + 0.1)
end
...
function SpecialVFX.phantomBlink(controller, targetPos, color, folder)
	...
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.5, 0.5, (from - targetPos).Magnitude)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.4
	trail.CFrame = CFrame.new((from + targetPos) / 2, targetPos)
	trail.Parent = folder
