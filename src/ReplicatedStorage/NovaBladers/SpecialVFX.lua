...
function SpecialVFX.vortexCyclone(position, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	...
	for i = 1, 3 do
		...
		spark.CFrame = CFrame.new(position + Vector3.new(math.cos(i * 2) * 2, 0.3, math.sin(i * 2) * 2))
		spark.Parent = folder
...
function SpecialVFX.frostShatter(origin, range, color, folder)
	...
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.4, 0.6, 0.25)
		shard.Anchored = true
