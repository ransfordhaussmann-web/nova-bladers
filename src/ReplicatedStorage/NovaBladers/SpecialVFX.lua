...
function SpecialVFX.slashArc(position, facing, color, folder)
	...
	arc.CFrame = CFrame.new(position + flatFacing * 2.2, position + flatFacing * 4.5)
	arc.Parent = folder
...
function SpecialVFX.frostMist(position, color, folder)
	local mist = Instance.new("Part")
	mist.Shape = Enum.PartType.Ball
	mist.Size = Vector3.new(2, 2, 2)
	mist.Anchored = true
	...
	TweenService:Create(mist, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function SpecialVFX.frostShatter(position, range, color, folder)
	...
	shards.Size = Vector3.new(0.3, range * 1.5, range * 1.5)
