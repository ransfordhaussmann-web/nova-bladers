...
function SpecialVFX.ripTideTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
	...
	trail.CFrame = CFrame.new(mid + Vector3.new(0, 0.15, 0)) * CFrame.Angles(0, 0, math.rad(90))
	trail.Parent = folder
...
function SpecialVFX.rippleSweep(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.iceShatterPulse(origin, range, color, folder)
