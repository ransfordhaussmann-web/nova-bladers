...
function SpecialVFX.crimsonTrail(position, direction, color, folder)
	local mark = Instance.new("Part")
	...
	mark.CFrame = CFrame.new(position, position + direction) * CFrame.Angles(math.rad(90), 0, 0)
	mark.Parent = folder

	TweenService:Create(mark, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.3, 0.1, 3.5),
	}):Play()
...
function SpecialVFX.phantomFade(position, color, folder)
	for i = 1, 6 do
		local angle = i * 60
		local offset = Vector3.new(math.cos(math.rad(angle)) * 3, 0.2, math.sin(math.rad(angle)) * 3)
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			CFrame = CFrame.new(position + offset * 2),
		}):Play()
