...
function SpecialVFX.vortexSlash(origin, color, folder)
	local slashA = Instance.new("Part")
	slashA.Size = Vector3.new(0.3, 0.3, 8)
	slashA.Anchored = true
	slashA.CanCollide = false
	slashA.Material = Enum.Material.Neon
	slashA.Color = color
	slashA.Transparency = 0.15
	slashA.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(45))
	slashA.Parent = folder

	local slashB = slashA:Clone()
	slashB.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(-45))
	slashB.Parent = folder

	TweenService:Create(slashA, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, 0.15, 12),
		Transparency = 1,
	}):Play()
	TweenService:Create(slashB, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, 0.15, 12),
		Transparency = 1,
	}):Play()
	Debris:AddItem(slashA, 0.35)
	Debris:AddItem(slashB, 0.35)
end
...
function SpecialVFX.frostCitadel(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
