...
function SpecialVFX.flameShot(origin, direction, color, folder)
	local bolt = Instance.new("Part")
	bolt.Size = Vector3.new(0.9, 0.9, 2.2)
	bolt.Anchored = true
	...
	TweenService:Create(bolt, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		CFrame = CFrame.new(origin + direction * 5, origin + direction * 8),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	crown.Size = Vector3.new(0.25, 2.5, 2.5)
	crown.Anchored = true
	crown.CanCollide = false
	crown.Material = Enum.Material.Glass
	crown.Color = color
	crown.Transparency = 0.25
	crown.CFrame = CFrame.new(pos + Vector3.new(0, 1.2, 0)) * CFrame.Angles(0, 0, math.rad(90))
	crown.Parent = folder

	TweenService:Create(crown, TweenInfo.new(duration * 0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.35, 5.5, 5.5),
		Transparency = 0.5,
	}):Play()
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	for i = 1, shardCount do
		...
		shard.Size = Vector3.new(0.5, 0.15, 1.8)
		shard.Anchored = true
		...
		local outward = Vector3.new(math.cos(angle), 0.2, math.sin(angle)) * range * 0.45
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + outward) * CFrame.Angles(0, angle, math.rad(25)),
			Transparency = 1,
		}):Play()
	...
	TweenService:Create(pulse, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
