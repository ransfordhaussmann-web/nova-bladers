...
function SpecialVFX.ripTideSlash(origin, facing, color, folder)
	local right = facing:Cross(Vector3.yAxis).Unit
	for i = -1, 1, 2 do
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.15, 2.5, 5)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.25
		slash.CFrame = CFrame.new(origin + right * i * 1.5, origin + right * i * 1.5 + facing * 3)
			* CFrame.Angles(0, math.atan2(facing.X, facing.Z), math.rad(35 * i))
		slash.Parent = folder
...
function SpecialVFX.frostCrownShield(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local crown = Instance.new("Part")
	crown.Shape = Enum.PartType.Ball
	crown.Size = Vector3.new(5, 3, 5)
	crown.Anchored = true
	crown.CanCollide = false
	crown.Material = Enum.Material.Glass
	crown.Color = color
	crown.Transparency = 0.45
	crown.CFrame = CFrame.new(pos + Vector3.new(0, 0.5, 0))
	crown.Parent = folder

	for i = 0, 5 do
		local angle = i * 60
		local spike = Instance.new("Part")
		spike.Size = Vector3.new(0.3, 1.2, 0.3)
		spike.Anchored = true
		spike.CanCollide = false
		spike.Material = Enum.Material.Ice
		spike.Color = Color3.fromRGB(220, 245, 255)
		spike.Transparency = 0.2
		spike.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(angle), math.rad(50)) * CFrame.new(0, 1.5, 2.2)
		spike.Parent = folder
...
function SpecialVFX.frostShardBurst(position, color, folder)
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, angle, math.rad(30)) * CFrame.new(0, 0, 2)
		shard.Parent = folder
...
function SpecialVFX.neonResonanceRing(origin, range, color, accentColor, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.1, 1.8, 1.8)
	inner.Anchored = true
	inner.CanCollide = false
	...
	TweenService:Create(inner, TweenInfo.new(0.45, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.06, range * 2, range * 2),
		Transparency = 1,
	}):Play()
...
function SpecialVFX.neonResonanceBurst(position, color, accentColor, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	...
	halo.CFrame = CFrame.new(position) * CFrame.Angles(0, 0, math.rad(90))
	halo.Parent = folder
	...
	TweenService:Create(halo, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
