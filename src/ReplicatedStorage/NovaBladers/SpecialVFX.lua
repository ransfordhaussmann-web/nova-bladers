...
function SpecialVFX.iceCage(position, range, color, folder, duration)
	local cage = Instance.new("Part")
	cage.Name = "IceCage"
	cage.Shape = Enum.PartType.Cylinder
	cage.Size = Vector3.new(1.8, range * 1.6, range * 1.6)
	cage.Anchored = true
	cage.CanCollide = false
	cage.Material = Enum.Material.Ice
	cage.Color = color
	cage.Transparency = 0.35
	cage.CFrame = CFrame.new(position) * CFrame.Angles(0, 0, math.rad(90))
	cage.Parent = folder

	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(1.2, range * 1.2, range * 1.2)
	inner.Anchored = true
	inner.CanCollide = false
...
function SpecialVFX.meltdownBurst(position, range, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
