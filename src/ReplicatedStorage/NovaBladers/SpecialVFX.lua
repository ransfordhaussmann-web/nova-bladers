...
function SpecialVFX.infernoTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.infernoImpact(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.infernoEruption(position, color, folder)
	for i = 1, 6 do
		local angle = (i / 6) * math.pi * 2
		local offset = Vector3.new(math.cos(angle) * 3, 0, math.sin(angle) * 3)
		SpecialVFX.infernoImpact(position + offset, color, folder)
	end
end
...
function SpecialVFX.prismAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(0, 2.2, 0)) * CFrame.Angles(0, angle, math.rad(30)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.crystalLockRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "CrystalLock"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.crystalShatter(origin, range, color, folder)
	for i = 1, 4 do
		local angle = (i / 4) * math.pi * 2 + math.random() * 0.4
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.6, 1.4, 0.25)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(endPos) * CFrame.Angles(0, angle, math.rad(45)),
			Transparency = 1,
		}):Play()
