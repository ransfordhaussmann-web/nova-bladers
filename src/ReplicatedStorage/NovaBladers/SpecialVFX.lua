...
function SpecialVFX.flameWheelTrail(position, color, folder)
	local ember = Instance.new("Part")
	ember.Size = Vector3.new(1.4, 0.5, 1.4)
	...
	ember.CFrame = CFrame.new(position) * CFrame.Angles(0, 0, math.rad(90))
	ember.Parent = folder
...
function SpecialVFX.chillAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(0, 2.2, 0)) * CFrame.Angles(0, angle, math.rad(20)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.frostShards(position, color, folder)
	for i = 1, 6 do
		local angle = (i / 6) * math.pi * 2
		local shard = Instance.new("Part")
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + offset * 2.5 + Vector3.new(0, 1.5, 0)) * CFrame.Angles(0, angle, math.rad(35)),
			Transparency = 1,
		}):Play()
	...
	core.CanCollide = false
	core.Material = Enum.Material.Neon
	core.Color = Color3.fromRGB(220, 250, 255)
