...
function SpecialVFX.spiralSlash(position, color, folder)
	for i = 0, 2 do
		...
		slash.Size = Vector3.new(0.15, 0.15, 3.5)
		slash.Anchored = true
...
function SpecialVFX.spiralRend(position, dir, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
	...
	for i = 0, 3 do
		...
		arc.Size = Vector3.new(0.2, 0.2, 4)
		arc.Anchored = true
		arc.CanCollide = false
		arc.Material = Enum.Material.Neon
		arc.Color = Color3.fromRGB(255, 140, 80)
		arc.Transparency = 0.25
		arc.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(30))
		arc.Parent = folder
...
function SpecialVFX.iceBulwark(controller, color, duration)
	...
	for i = 0, 5 do
		...
		spike.CFrame = CFrame.new(controller.part.Position)
			...
			* CFrame.new(0, 0.5, 3.2)
		spike.Parent = folder
		Debris:AddItem(spike, duration)
	end
