...
function SpecialVFX.cometSlash(position, color, folder)
	local arc = Instance.new("Part")
	arc.Shape = Enum.PartType.Ball
	arc.Size = Vector3.new(2.5, 2.5, 2.5)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = 0.2
	arc.CFrame = CFrame.new(position + Vector3.new(0, 0.6, 0))
	arc.Parent = folder
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	for i = 1, 8 do
		...
		local angle = i * 45
		shard.CFrame = CFrame.new(position) * CFrame.Angles(math.rad(30), math.rad(angle), 0) * CFrame.new(0, 0, 1.5)
		shard.Parent = folder
