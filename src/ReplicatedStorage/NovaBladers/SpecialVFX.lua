...
function SpecialVFX.clawSlash(position, facing, color, folder)
	for i = -1, 1, 2 do
		...
		local side = Vector3.new(-facing.Z, 0, facing.X) * i * 0.8
		slash.CFrame = CFrame.new(position + side, position + side + facing) * CFrame.Angles(math.rad(70), 0, 0)
		slash.Parent = folder
...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	glow.Size = Vector3.new(1.5, 5.5, 5.5)
	glow.Anchored = true
...
function SpecialVFX.phantomMirage(controller, color, duration)
	...
	for i = 1, 3 do
		local angle = (i - 2) * 0.9
		local offset = Vector3.new(math.sin(angle) * 3, 0.3, math.cos(angle) * 3)
...
function SpecialVFX.bladeSlash(position, facing, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.15, 0.8, 8)
	slash.Anchored = true
	slash.CanCollide = false
	slash.Material = Enum.Material.Neon
	slash.Color = color
