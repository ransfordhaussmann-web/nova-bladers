...
function SpecialVFX.crystalPrism(controller, color, duration)
	...
	prism.Name = "CrystalPrism"
	prism.Shape = Enum.PartType.Cylinder
	prism.Size = Vector3.new(1.8, 5.5, 5.5)
	prism.Anchored = true
	prism.CanCollide = false
	prism.Material = Enum.Material.Glass
	prism.Color = color
	prism.Transparency = 0.35
	prism.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	prism.Parent = folder
...
function SpecialVFX.iceShatter(position, range, color, folder)
	...
	for i = 1, 8 do
		...
		local endPos = position + Vector3.new(math.cos(angle) * range * 0.6, 1.2, math.sin(angle) * range * 0.6)
...
function SpecialVFX.fireCharge(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
...
function SpecialVFX.fireCycloneTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.infernoEruption(position, range, color, folder)
	local pillar = Instance.new("Part")
	pillar.Shape = Enum.PartType.Cylinder
	pillar.Size = Vector3.new(range * 1.2, range * 0.8, range * 0.8)
	pillar.Anchored = true
	pillar.CanCollide = false
	pillar.Material = Enum.Material.Neon
	pillar.Color = color
	pillar.Transparency = 0.2
	pillar.CFrame = CFrame.new(position + Vector3.new(0, range * 0.3, 0)) * CFrame.Angles(0, 0, math.rad(90))
	...
	TweenService:Create(pillar, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(range * 0.3, range * 2.2, range * 2.2),
		Transparency = 1,
	}):Play()
