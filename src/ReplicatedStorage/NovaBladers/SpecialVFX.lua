...
function SpecialVFX.emberCycloneRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.emberFlare(position, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
	...
	TweenService:Create(core, TweenInfo.new(0.38, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(11, 11, 11),
		Transparency = 1,
	}):Play()
	Debris:AddItem(core, 0.42)
end
...
function SpecialVFX.glacierShroud(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	...
	task.delay(duration, function()
		if dome.Parent then
			TweenService:Create(dome, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
				Transparency = 1,
				Size = Vector3.new(8, 6, 8),
			}):Play()
			Debris:AddItem(dome, 0.3)
		end
	end)
...
function SpecialVFX.frostPulse(origin, range, color, folder)
	...
	local sparkles = Instance.new("Sparkles")
	sparkles.SparkleColor = Color3.fromRGB(200, 235, 255)
...
function SpecialVFX.avalancheWave(origin, range, color, folder)
	local slab = Instance.new("Part")
	slab.Size = Vector3.new(range * 1.6, 0.6, range * 1.6)
	slab.Anchored = true
	...
	TweenService:Create(slab, TweenInfo.new(0.55, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(range * 3.2, 0.3, range * 3.2),
		Transparency = 1,
	}):Play()
	Debris:AddItem(slab, 0.6)
end
