...
function SpecialVFX.ripperSlash(position, facing, color, folder)
	for i = -1, 1, 2 do
		...
		local side = CFrame.Angles(0, math.rad(i * 55), math.rad(70))
		slash.CFrame = CFrame.new(position + Vector3.new(0, 0.5, 0), position + facing) * side
		slash.Parent = folder

		TweenService:Create(slash, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.15, 4, 1.2),
		}):Play()
...
function SpecialVFX.auroraBurst(position, range, color, folder)
	...
	halo.Size = Vector3.new(0.25, 4, 4)
	halo.Anchored = true
	halo.CanCollide = false
	halo.Material = Enum.Material.Neon
	halo.Color = Color3.fromRGB(200, 150, 255)
