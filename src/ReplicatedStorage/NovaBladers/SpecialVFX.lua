...
function SpecialVFX.glacierShatter(position, range, color, folder)
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(25))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(math.cos(math.rad(angle)) * range * 0.5, 1.5, math.sin(math.rad(angle)) * range * 0.5),
			Transparency = 1,
		}):Play()
