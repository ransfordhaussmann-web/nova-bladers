...
function SpecialVFX.slashArc(origin, facing, color, folder)
	...
	arc.CFrame = CFrame.new(origin + right * 1.8 + Vector3.new(0, 0.6, 0), origin + facing * 2)
	arc.Parent = folder
...
function SpecialVFX.barrierDome(controller, color, duration)
	...
	glow.Size = Vector3.new(6.5, 6.5, 6.5)
	glow.Anchored = true
...
function SpecialVFX.shatterBurst(origin, range, color, folder)
	...
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, angle, math.rad(30)) * CFrame.new(0, 1, range * 0.4)
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			CFrame = shard.CFrame * CFrame.new(0, 0, range * 0.3),
		}):Play()
