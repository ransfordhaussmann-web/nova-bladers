...
function SpecialVFX.phantomSplit(controller, color, folder)
	...
	for i = 1, 3 do
		...
		ghost.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.3, math.sin(math.rad(angle)) * 2.5))
...
function SpecialVFX.phantomAfterimage(position, color, folder)
	local ghost = Instance.new("Part")
	ghost.Shape = Enum.PartType.Cylinder
	ghost.Size = Vector3.new(0.4, 3.0, 3.0)
	ghost.Anchored = true
	ghost.CanCollide = false
	...
	ghost.Parent = folder
	Debris:AddItem(ghost, 0.18)
end

function SpecialVFX.phantomVanish(controller, targetPos, color, folder)
	local start = controller.part.Position
	controller.part.CFrame = CFrame.new(Vector3.new(targetPos.X, start.Y, targetPos.Z))
	...
	burst.CFrame = CFrame.new(controller.part.Position)
	burst.Parent = folder
