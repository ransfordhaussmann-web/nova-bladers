...
function SpecialVFX.cycloneSpiral(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 6 do
		...
		flame.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2, 0.8, math.sin(angle) * 2))
		flame.Parent = folder
...
function SpecialVFX.emberEruption(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.iceVault(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "IceVault"
	ring.Shape = Enum.PartType.Cylinder
