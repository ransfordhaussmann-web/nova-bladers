...
function SpecialVFX.vortexSlash(origin, range, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.3, range * 0.8, 0.8)
	...
	slash.CFrame = CFrame.new(origin) * CFrame.Angles(0, math.random() * math.pi * 2, math.rad(75))
	slash.Parent = folder

	TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.15, range * 1.6, 0.15),
		Transparency = 1,
	}):Play()
	...
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(0.12, range * 1.4, range * 1.4)
	ring.Anchored = true
	ring.CanCollide = false
	ring.Material = Enum.Material.Neon
	ring.Color = color
...
function SpecialVFX.iceAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.5, 0.6, math.sin(angle) * 2.5))
...
function SpecialVFX.frostShards(origin, range, color, folder)
	...
	burst.Size = Vector3.new(0.15, 2, 2)
	burst.Anchored = true
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = color
