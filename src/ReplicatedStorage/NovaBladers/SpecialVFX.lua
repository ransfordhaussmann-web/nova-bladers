...
function SpecialVFX.cometIgnite(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
...
function SpecialVFX.cometTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.cometImpact(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.glacierFreeze(controller, color)
	local folder = SpecialVFX.ensureFolder(controller)
	local frost = Instance.new("Part")
	frost.Shape = Enum.PartType.Ball
	frost.Size = Vector3.new(7, 2.5, 7)
	frost.Anchored = true
	frost.CanCollide = false
	frost.Material = Enum.Material.Ice
	frost.Color = color
	frost.Transparency = 0.3
	frost.CFrame = CFrame.new(controller.part.Position - Vector3.new(0, 1, 0))
	frost.Parent = folder
	...
	for i = 1, 4 do
		...
		local angle = i * 90
		shard.CFrame = CFrame.new(controller.part.Position + Vector3.new(math.cos(math.rad(angle)) * 2, 0.5, math.sin(math.rad(angle)) * 2))
...
function SpecialVFX.glacierBarrier(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "GlacierBarrier"
	ring.Shape = Enum.PartType.Cylinder
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(1.3, 5, 5)
	inner.Anchored = true
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
...
function SpecialVFX.glacierShatter(origin, range, color, folder)
	local wave = Instance.new("Part")
	wave.Shape = Enum.PartType.Cylinder
	...
	for i = 1, 6 do
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 0.8, 0.4)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = Color3.fromRGB(200, 240, 255)
		...
		local angle = i * 60
		local offset = Vector3.new(math.cos(math.rad(angle)) * range * 0.5, 0.8, math.sin(math.rad(angle)) * range * 0.5)
