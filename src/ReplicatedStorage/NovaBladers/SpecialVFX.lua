...
function SpecialVFX.bladeSlash(position, facing, color, folder)
	for i = -1, 1, 2 do
		...
		local offset = CFrame.Angles(0, math.rad(i * 35), 0) * CFrame.new(0, 0, 3)
		slash.CFrame = CFrame.new(position, position + facing) * offset
		slash.Parent = folder

		TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.15, 0.4, 9),
		}):Play()
...
function SpecialVFX.stoneShatter(position, color, folder)
	for i = 1, 6 do
		...
		shard.Color = color
		shard.CFrame = CFrame.new(position + Vector3.new(math.random(-2, 2), 0.5, math.random(-2, 2)))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + Vector3.new(math.random(-4, 4), 3, math.random(-4, 4)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.solarNova(position, color, folder)
	local nova = Instance.new("Part")
	nova.Shape = Enum.PartType.Ball
	nova.Size = Vector3.new(3, 3, 3)
	nova.Anchored = true
	nova.CanCollide = false
	nova.Material = Enum.Material.Neon
	nova.Color = color
	nova.Transparency = 0.1
	nova.CFrame = CFrame.new(position)
	nova.Parent = folder

	TweenService:Create(nova, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(14, 14, 14),
		Transparency = 1,
	}):Play()
	Debris:AddItem(nova, 0.5)
end
...
function SpecialVFX.phantomVanish(controller, color, duration)
	...
	task.delay(duration, function()
		...
		if mist.Parent then mist:Destroy() end
	end)
end

function SpecialVFX.phantomDash(position, dir, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.5, 0.5, 4)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.3
	trail.CFrame = CFrame.new(position + dir * 2, position + dir * 4)
	trail.Parent = folder
...
function SpecialVFX.edgeCrossSlash(position, facing, color, folder)
	for angle = 0, 90, 90 do
		local slash = Instance.new("Part")
		slash.Size = Vector3.new(0.2, 0.6, 8)
		slash.Anchored = true
		slash.CanCollide = false
		slash.Material = Enum.Material.Neon
		slash.Color = color
		slash.Transparency = 0.1
		slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), 0) * CFrame.Angles(0, 0, math.rad(90))
		slash.Parent = folder

		TweenService:Create(slash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = Vector3.new(0.1, 0.3, 12),
			Transparency = 1,
		}):Play()
		Debris:AddItem(slash, 0.4)
	end
