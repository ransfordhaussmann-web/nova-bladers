...
function SpecialVFX.blazeSpiral(position, color, folder)
	local arc = Instance.new("Part")
	arc.Shape = Enum.PartType.Ball
	arc.Size = Vector3.new(1.4, 1.4, 1.4)
	arc.Anchored = true
...
function SpecialVFX.blazeFinale(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.Size = Vector3.new(0.35, 1.2, 0.35)
		...
		TweenService:Create(crystal, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 4, 1.5, math.sin(angle) * 4))
				* CFrame.Angles(0, angle, math.rad(25)),
			Transparency = 0.7,
		}):Play()
...
function SpecialVFX.iceLockRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Name = "IceLock"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.frostShatter(position, color, folder)
	...
	for i = 1, 8 do
		...
		shard.Size = Vector3.new(0.5, 1.4, 0.2)
		shard.Anchored = true
