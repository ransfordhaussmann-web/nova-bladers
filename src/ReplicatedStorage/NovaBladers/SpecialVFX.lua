...
function SpecialVFX.flameCorkscrewTrail(position, facing, color, folder)
	local mark = Instance.new("Part")
	mark.Size = Vector3.new(1.4, 1.4, 1.4)
	mark.Shape = Enum.PartType.Ball
	mark.Anchored = true
	mark.CanCollide = false
	mark.Material = Enum.Material.Neon
...
function SpecialVFX.blazeErupt(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.glacierBarrier(controller, color, duration)
	...
	crystals.Size = Vector3.new(4.5, 4.5, 4.5)
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	shard.Size = Vector3.new(0.2, 3, 3)
	shard.Anchored = true
	...
	local sparkles = Instance.new("Sparkles")
	sparkles.SparkleColor = Color3.fromRGB(230, 250, 255)
