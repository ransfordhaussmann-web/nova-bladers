...
function SpecialVFX.iceAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		local angle = i * 60
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.5, math.sin(math.rad(angle)) * 2.5))
...
function SpecialVFX.glacierRing(controller, color, duration)
	...
	inner.CanCollide = false
	inner.Material = Enum.Material.Glass
	inner.Color = Color3.fromRGB(200, 235, 255)
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	for i = 1, 4 do
		...
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = Color3.fromRGB(230, 245, 255)
		shard.Transparency = 0.1
		local angle = i * 90
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(math.rad(angle)) * 2, 0.3, math.sin(math.rad(angle)) * 2))
		shard.Parent = folder
