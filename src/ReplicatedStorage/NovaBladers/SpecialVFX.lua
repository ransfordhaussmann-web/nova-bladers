...
function SpecialVFX.iceFreezeShell(controller, target, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local anchor = (target and target.part) or controller.part
	...
	shell.CFrame = CFrame.new(anchor.Position)
	shell.Parent = folder
...
function SpecialVFX.iceShards(origin, color, folder, count)
	for i = 1, count or 4 do
		local angle = (i / (count or 4)) * math.pi * 2
...
function SpecialVFX.solarIgnite(origin, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
...
function SpecialVFX.solarBurst(position, color, range, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
