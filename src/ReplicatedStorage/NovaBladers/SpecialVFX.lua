...
function SpecialVFX.crimsonSlash(position, facing, color, folder)
	...
	slash.Size = Vector3.new(0.3, 0.8, 5)
	slash.Anchored = true
...
function SpecialVFX.stoneBarrier(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "StoneBarrier"
	ring.Shape = Enum.PartType.Cylinder
	...
	glow.Size = Vector3.new(1.2, 4.5, 4.5)
	glow.Anchored = true
	glow.CanCollide = false
	glow.Material = Enum.Material.Neon
	glow.Color = Color3.fromRGB(200, 195, 180)
	glow.Transparency = 0.5
	glow.CFrame = ring.CFrame
	glow.Parent = folder
...
function SpecialVFX.phantomClones(controller, targetPos, color, count, folder)
	local start = controller.part.Position
	local dir = (targetPos - start)
	dir = Vector3.new(dir.X, 0, dir.Z).Unit
	for i = 1, count do
		local offset = dir * (4 * i) + Vector3.new(math.random(-2, 2), 0.5, math.random(-2, 2))
		local clone = Instance.new("Part")
		clone.Shape = Enum.PartType.Cylinder
		clone.Size = Vector3.new(0.8, 2.5, 2.5)
		clone.Anchored = true
		clone.CanCollide = false
		clone.Material = Enum.Material.Glass
		clone.Color = color
		clone.Transparency = 0.5
		clone.CFrame = CFrame.new(start + offset) * CFrame.Angles(0, 0, math.rad(90))
		clone.Parent = folder

		TweenService:Create(clone, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(targetPos + offset * 0.3) * CFrame.Angles(0, 0, math.rad(90)),
			Transparency = 0.9,
		}):Play()
