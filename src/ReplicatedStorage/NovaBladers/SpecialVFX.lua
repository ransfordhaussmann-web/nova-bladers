...
function SpecialVFX.shatterBurst(origin, range, color, folder)
	...
	for i = 1, 6 do
		...
		frag.Material = Enum.Material.Concrete
		frag.Color = color
		frag.CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * 2, 0.3, math.sin(angle) * 2))
		frag.Parent = folder
		TweenService:Create(frag, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + Vector3.new(math.cos(angle) * range * 0.6, 0.5, math.sin(angle) * range * 0.6)),
			Transparency = 1,
		}):Play()
