...
function SpecialVFX.coilAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local aura = Instance.new("Part")
	aura.Shape = Enum.PartType.Cylinder
	aura.Size = Vector3.new(0.3, 4.5, 4.5)
	aura.Anchored = true
	aura.CanCollide = false
	aura.Material = Enum.Material.Neon
	aura.Color = color
...
function SpecialVFX.coilRushTrail(controller, targetPos, color, folder)
	local trail = Instance.new("Part")
	trail.Shape = Enum.PartType.Cylinder
	trail.Size = Vector3.new(0.2, 2.5, 2.5)
	trail.Anchored = true
	trail.CanCollide = false
	trail.Material = Enum.Material.Neon
	trail.Color = color
	trail.Transparency = 0.2
	trail.CFrame = CFrame.new(controller.part.Position, targetPos) * CFrame.Angles(0, 0, math.rad(90))
	trail.Parent = folder
...
function SpecialVFX.coilStrike(position, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
