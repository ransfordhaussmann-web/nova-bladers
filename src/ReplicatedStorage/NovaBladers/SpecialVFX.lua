...
function SpecialVFX.iceShard(fromPos, toPos, color, folder)
	...
	local tween = TweenService:Create(shard, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
	...
	tween.Completed:Connect(function()
		...
		Debris:AddItem(impact, 0.35)
	end)

	Debris:AddItem(shard, 0.3)
end

function SpecialVFX.freezeRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(0.2, 2, 2)
	ring.Anchored = true
