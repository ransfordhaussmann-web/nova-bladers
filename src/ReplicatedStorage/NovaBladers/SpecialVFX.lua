...
function SpecialVFX.flameSpiral(position, color, folder)
	...
	spiral.Color = color
	spiral.Transparency = 0.15
	spiral.CFrame = CFrame.new(position)
...
function SpecialVFX.iceShardRing(origin, range, color, folder)
	for i = 0, 5 do
		local angle = i * 60
		local offset = Vector3.new(math.cos(math.rad(angle)) * range * 0.5, 0.6, math.sin(math.rad(angle)) * range * 0.5)
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + offset * 1.8) * CFrame.Angles(0, math.rad(angle), math.rad(25)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.iceShatter(position, color, folder)
	...
	for i = 0, 7 do
		...
		shard.CFrame = CFrame.new(position + dir * 2) * CFrame.Angles(0, math.rad(angle), math.rad(30))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + dir * 6) * CFrame.Angles(0, math.rad(angle), math.rad(30)),
			Transparency = 1,
		}):Play()
