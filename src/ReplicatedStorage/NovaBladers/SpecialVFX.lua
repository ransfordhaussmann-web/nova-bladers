...
function SpecialVFX.slamWindup(controller, color, duration)
...
function SpecialVFX.slamImpact(position, color, folder)
	...
	local dust = Instance.new("Part")
	dust.Shape = Enum.PartType.Ball
	dust.Size = Vector3.new(5, 2, 5)
	dust.Anchored = true
	dust.CanCollide = false
	dust.Material = Enum.Material.SmoothPlastic
	dust.Color = Color3.fromRGB(80, 60, 50)
...
function SpecialVFX.crystalShell(controller, color, duration)
	...
	shards.Size = Vector3.new(0.3, 4.5, 4.5)
	...
	shards.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
