...
function SpecialVFX.solarCharge(controller, color, duration)
	...
	for i = 1, 6 do
		...
		spark.Material = Enum.Material.Neon
		spark.Color = color
		spark.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 3, 0.5, math.sin(angle) * 3))
		spark.Parent = folder
...
function SpecialVFX.glacierFreeze(controller, color)
	...
	crystals.Size = Vector3.new(0.4, 3, 3)
	...
	crystals.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
	crystals.Parent = folder
	Debris:AddItem(crystals, 0.55)
end
