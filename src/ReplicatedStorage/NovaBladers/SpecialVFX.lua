...
function SpecialVFX.slashArc(controller, color, folder, isFinisher)
	local pos = controller.part.Position
	local facing = controller.facing
	local arc = Instance.new("Part")
	arc.Size = Vector3.new(isFinisher and 5 or 3.5, 0.35, isFinisher and 7 or 5)
	arc.Anchored = true
	arc.CanCollide = false
	arc.Material = Enum.Material.Neon
	arc.Color = color
	arc.Transparency = isFinisher and 0.1 or 0.25
	local look = CFrame.lookAt(pos, pos + facing)
	arc.CFrame = look * CFrame.new(0, 0.4, -(isFinisher and 3.5 or 2.5))
	arc.Parent = folder

	TweenService:Create(arc, TweenInfo.new(isFinisher and 0.35 or 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = arc.Size + Vector3.new(isFinisher and 3 or 1.5, 0, isFinisher and 4 or 2),
	}):Play()
	Debris:AddItem(arc, isFinisher and 0.4 or 0.25)
end
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + offset * 1.8) * CFrame.Angles(0, angle, math.rad(25)),
			Transparency = 1,
		}):Play()
