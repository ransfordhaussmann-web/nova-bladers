...
function SpecialVFX.forgeShard(position, color, folder)
	for i = 1, 4 do
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		crystal.Size = Vector3.new(0.3, 1.2, 0.3)
...
function SpecialVFX.frostCrown(controller, color, duration)
	...
	crown.Size = Vector3.new(0.6, 5.5, 5.5)
	crown.Anchored = true
...
function SpecialVFX.frostBurst(origin, range, color, folder)
	...
	for i = 1, 8 do
		...
		local endPos = origin + Vector3.new(math.cos(angle) * range * 0.6, 1.5, math.sin(angle) * range * 0.6)
