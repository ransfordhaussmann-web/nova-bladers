...
function SpecialVFX.slashArc(origin, facing, angleOffset, color, folder)
	local dir = CFrame.Angles(0, math.rad(angleOffset), 0) * CFrame.new(facing)
	local slashDir = dir.LookVector
	...
	slash.CFrame = CFrame.new(origin + Vector3.new(0, 0.6, 0), origin + slashDir * 5 + Vector3.new(0, 0.6, 0))
	slash.Parent = folder
...
function SpecialVFX.domeShatter(position, color, folder)
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(position) * CFrame.Angles(math.rad(math.random(-30, 30)), math.rad(angle), 0) * CFrame.new(0, 0, 3)
		shard.Parent = folder
	...
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = Color3.fromRGB(200, 255, 250)
