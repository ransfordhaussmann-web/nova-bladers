...
function SpecialVFX.corkscrewTrail(position, color, folder)
	local mark = Instance.new("Part")
...
function SpecialVFX.corkscrewFlare(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	for i = 0, 5 do
		...
		shard.CFrame = CFrame.new(origin + dir * 2, origin + dir * (range * 0.5))
		shard.Parent = folder
