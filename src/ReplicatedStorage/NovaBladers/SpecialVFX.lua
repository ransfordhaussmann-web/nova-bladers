...
function SpecialVFX.emberCyclone(origin, range, color, folder)
	local cyclone = Instance.new("Part")
	cyclone.Shape = Enum.PartType.Cylinder
	cyclone.Size = Vector3.new(0.4, 3, 3)
	cyclone.Anchored = true
	cyclone.CanCollide = false
	cyclone.Material = Enum.Material.Neon
	cyclone.Color = color
	cyclone.Transparency = 0.25
	cyclone.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(90))
	...
	TweenService:Create(cyclone, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.2, range * 2, range * 2),
		Transparency = 1,
	}):Play()
	Debris:AddItem(cyclone, 0.4)
end

function SpecialVFX.emberTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
	local trail = Instance.new("Part")
...
function SpecialVFX.frostShield(controller, color, duration)
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.5, 5, 5)
	inner.Anchored = true
	inner.CanCollide = false
...
function SpecialVFX.iceShard(origin, range, color, folder)
	...
	shard.Size = Vector3.new(0.6, 1.8, 0.3)
	shard.Anchored = true
	...
	TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.3, 0.9, 0.15),
	}):Play()
	Debris:AddItem(shard, 0.45)
end

function SpecialVFX.freezeBurst(position, color, folder)
	local core = Instance.new("Part")
	core.Shape = Enum.PartType.Ball
