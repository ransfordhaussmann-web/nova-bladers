...
function SpecialVFX.bladeWhirl(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.bladeSlash(position, color, folder)
	...
	slash.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(math.random(0, 360)), math.rad(75))
	slash.Parent = folder
...
function SpecialVFX.stonePillars(controller, color, duration)
	...
	for i = 0, 3 do
		...
		TweenService:Create(pillar, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			...
			CFrame = CFrame.new(pos + offset + Vector3.new(0, 2, 0)),
		}):Play()
...
function SpecialVFX.sunTrail(fromPos, toPos, color, folder)
	local mid = (fromPos + toPos) / 2
...
function SpecialVFX.flareBurst(origin, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.phantomFade(controller, color, duration)
	...
	TweenService:Create(shimmer, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
function SpecialVFX.edgeSalvo(position, range, color, folder)
	for i = 0, 2 do
		local angle = i * 120 + math.random(-20, 20)
		local blade = Instance.new("Part")
		blade.Size = Vector3.new(0.2, 0.2, range)
		blade.Anchored = true
		blade.CanCollide = false
		blade.Material = Enum.Material.Neon
		blade.Color = color
		blade.Transparency = 0.25
		blade.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(60))
		blade.Parent = folder
