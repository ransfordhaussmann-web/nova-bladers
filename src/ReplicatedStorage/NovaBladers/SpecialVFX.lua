...
function SpecialVFX.cycloneRing(origin, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.cycloneSlam(position, color, folder)
	local slam = Instance.new("Part")
	slam.Shape = Enum.PartType.Ball
	slam.Size = Vector3.new(2, 2, 2)
	slam.Anchored = true
	slam.CanCollide = false
	slam.Material = Enum.Material.Neon
	slam.Color = color
	slam.Transparency = 0.2
	slam.CFrame = CFrame.new(position)
	slam.Parent = folder
	TweenService:Create(slam, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(9, 9, 9),
		Transparency = 1,
	}):Play()
	Debris:AddItem(slam, 0.4)
end

function SpecialVFX.bladeSlash(position, color, folder)
	local slash = Instance.new("Part")
	slash.Size = Vector3.new(0.3, 0.8, 3.5)
	slash.Anchored = true
...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	task.delay(duration + 1.35, function()
...
function SpecialVFX.stonePulse(origin, range, color, folder)
	...
	pulse.Material = Enum.Material.Rock
	pulse.Color = color
...
function SpecialVFX.stoneCrack(position, color, folder)
	for i = 1, 4 do
		local crack = Instance.new("Part")
		crack.Size = Vector3.new(0.3, 0.2, 2.5)
		crack.Anchored = true
		crack.CanCollide = false
		crack.Material = Enum.Material.Slate
		crack.Color = color
		crack.Transparency = 0.15
		crack.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(i * 90), 0)
		crack.Parent = folder
		TweenService:Create(crack, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
		}):Play()
		Debris:AddItem(crack, 0.4)
	end
end
...
function SpecialVFX.phantomTrail(controller, color, folder)
...
function SpecialVFX.phantomEcho(position, color, folder)
	...
	echo.Size = Vector3.new(2, 2, 2)
	echo.Anchored = true
	echo.CanCollide = false
	echo.Material = Enum.Material.ForceField
	echo.Color = color
	echo.Transparency = 0.35
	echo.CFrame = CFrame.new(position)
	echo.Parent = folder
	TweenService:Create(echo, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(6, 6, 6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(echo, 0.3)
end
