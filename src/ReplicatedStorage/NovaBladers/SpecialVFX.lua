...
function SpecialVFX.stoneBrace(controller, color, duration)
	...
	slab.Shape = Enum.PartType.Block
	slab.Size = Vector3.new(4, 0.6, 4)
	slab.Anchored = true
	slab.CanCollide = false
	slab.Material = Enum.Material.Slate
	slab.Color = color
	slab.Transparency = 0.2
	slab.CFrame = CFrame.new(controller.part.Position - Vector3.new(0, 0.5, 0))
	slab.Parent = folder

	TweenService:Create(slab, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(5.5, 0.8, 5.5),
		Transparency = 0.6,
	}):Play()
	Debris:AddItem(slab, duration + 0.1)
end

function SpecialVFX.stoneCounter(origin, range, color, folder)
	...
	shard.CFrame = CFrame.new(origin) * CFrame.Angles(math.random(), math.random(), math.random())
	shard.Parent = folder
...
function SpecialVFX.solarFlare(controller, color, duration)
	...
	flare.CFrame = CFrame.new(controller.part.Position + Vector3.new(0, 1.5, 0))
	flare.Parent = folder
...
function SpecialVFX.solarOrbitSpark(position, color, folder)
	local spark = Instance.new("Part")
	spark.Shape = Enum.PartType.Ball
...
function SpecialVFX.mirageSplit(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	for i = -1, 1, 2 do
		...
		ghost.CFrame = CFrame.new(pos + Vector3.new(i * 2.5, 0, 0)) * CFrame.Angles(0, 0, math.rad(90))
		ghost.Parent = folder

		TweenService:Create(ghost, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			CFrame = ghost.CFrame + Vector3.new(i * 1.5, 0, 0),
		}):Play()
		Debris:AddItem(ghost, duration + 0.1)
	end
end

function SpecialVFX.phantomBlink(controller, targetPos, color, folder)
	local start = controller.part.Position
	local afterimage = Instance.new("Part")
	afterimage.Shape = Enum.PartType.Cylinder
	afterimage.Size = Vector3.new(0.9, 3.2, 3.2)
	...
	afterimage.CFrame = CFrame.new(start) * CFrame.Angles(0, 0, math.rad(90))
	afterimage.Parent = folder

	TweenService:Create(afterimage, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
	}):Play()
	...
	if dir.Magnitude > 0.5 then
		local snapPos = start + dir.Unit * math.min(dir.Magnitude, 10)
		controller.part.CFrame = CFrame.new(Vector3.new(snapPos.X, start.Y, snapPos.Z), snapPos + dir.Unit)
	end
end

function SpecialVFX.phantomSlash(position, color, folder)
	for i = 0, 2 do
		local angle = i * 120
		...
		arc.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(60))
		arc.Parent = folder

		TweenService:Create(arc, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = Vector3.new(0.1, 5.5, 0.3),
			Transparency = 1,
		}):Play()
