...
function SpecialVFX.vortexSpiral(controller, color, duration)
	...
	for i = 1, 6 do
		...
		spark.Material = Enum.Material.Neon
		spark.Color = color
		spark.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * radius, 0.3, math.sin(math.rad(angle)) * radius))
		spark.Parent = folder

		TweenService:Create(spark, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
...
function SpecialVFX.frostAura(controller, color, duration)
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(math.rad(angle)) * 2.5, 0.6, math.sin(math.rad(angle)) * 2.5))
			* CFrame.Angles(0, math.rad(angle), math.rad(30))
		shard.Parent = folder
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	for i = 1, 4 do
		...
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.4, 1.2, 0.2)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + dir * range * 0.6),
			Transparency = 1,
		}):Play()
