...
function SpecialVFX.clawRip(position, facing, color, folder)
	...
	for i = -1, 1, 2 do
		...
		slash.CFrame = CFrame.new(position + right * i * 1.2, position + right * i * 1.2 + facing * 2)
...
function SpecialVFX.auroraDome(controller, color, duration)
	...
	shimmer.Size = Vector3.new(0.2, 5.5, 5.5)
	shimmer.Anchored = true
	shimmer.CanCollide = false
	shimmer.Material = Enum.Material.Neon
	shimmer.Color = Color3.fromRGB(200, 255, 250)
	shimmer.Transparency = 0.4
	shimmer.CFrame = CFrame.new(controller.part.Position) * CFrame.Angles(0, 0, math.rad(90))
...
function SpecialVFX.frostPulse(origin, range, color, folder)
	...
	flakes.Material = Enum.Material.Glass
	flakes.Color = Color3.fromRGB(220, 255, 255)
	flakes.Transparency = 0.4
	flakes.CFrame = CFrame.new(origin + Vector3.new(0, 1, 0))
	...
	TweenService:Create(flakes, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(4, 4, 4),
		Transparency = 1,
	}):Play()
	Debris:AddItem(ring, 0.55)
	Debris:AddItem(flakes, 0.45)
end
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(position + dir * 1.5, position + dir * 4)
		shard.Parent = folder
