...
function SpecialVFX.fireOrbitSpark(position, color, folder)
	...
	spark.CFrame = CFrame.new(position + Vector3.new(math.random(-2, 2) * 0.3, 0.4, math.random(-2, 2) * 0.3))
	spark.Parent = folder
...
function SpecialVFX.frostBurrowCloud(controller, color)
	...
	shard.CFrame = CFrame.new(controller.part.Position - Vector3.new(0, 0.6, 0))
	shard.Parent = folder
