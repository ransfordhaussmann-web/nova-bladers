...
function SpecialVFX.prismShards(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 6 do
		...
		shard.Size = Vector3.new(0.3, 0.8, 0.5)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.2
		local offset = Vector3.new(math.cos(angle) * 2.5, 1.2, math.sin(angle) * 2.5)
		shard.CFrame = CFrame.new(pos + offset) * CFrame.Angles(0, angle, math.rad(30))
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(pos + offset * 0.6 + Vector3.new(0, 0.5, 0)) * CFrame.Angles(0, angle, math.rad(60)),
			Transparency = 0.7,
		}):Play()
...
function SpecialVFX.prismBarrier(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "PrismBarrier"
	ring.Shape = Enum.PartType.Cylinder
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.8, 4.5, 4.5)
	inner.Anchored = true
	inner.CanCollide = false
...
function SpecialVFX.shardBurst(origin, range, color, folder)
	for i = 1, 8 do
		local angle = (i / 8) * math.pi * 2
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.25, 0.6, 0.4)
		shard.Anchored = true
		shard.CanCollide = false
		...
		shard.CFrame = CFrame.new(origin + dir * 1.5)
		shard.Parent = folder
