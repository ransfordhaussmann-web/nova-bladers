...
function SpecialVFX.fireWheelRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FireWheel"
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.flameArc(origin, facing, range, color, folder)
	local tip = origin + facing * range
	local arc = Instance.new("Part")
	arc.Size = Vector3.new(range, 0.6, 1.2)
	arc.Anchored = true
	...
	TweenService:Create(arc, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(range, 0.2, 0.4),
	}):Play()
...
function SpecialVFX.frostMist(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
...
function SpecialVFX.iceVeil(controller, color, duration)
	...
	veil.Material = Enum.Material.Ice
	veil.Color = color
...
function SpecialVFX.crystalShard(position, color, folder)
	for i = 1, 4 do
		...
		shard.Size = Vector3.new(0.5, 1.8, 0.5)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.15
		local offset = Vector3.new(math.cos(angle) * 3, 1.2, math.sin(angle) * 3)
		...
		TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + offset * 0.8,
			Transparency = 1,
		}):Play()
