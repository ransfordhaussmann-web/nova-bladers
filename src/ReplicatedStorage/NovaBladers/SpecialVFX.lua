...
function SpecialVFX.crystalShell(controller, color, duration)
	...
	facets.Shape = Enum.PartType.Ball
	facets.Size = Vector3.new(4, 4, 4)
	facets.Anchored = true
	facets.CanCollide = false
	facets.Material = Enum.Material.Neon
	facets.Color = Color3.fromRGB(220, 245, 255)
	facets.Transparency = 0.5
	facets.CFrame = shell.CFrame
...
function SpecialVFX.crystalShatter(position, color, folder)
	for i = 1, 8 do
		...
		TweenService:Create(shard, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + Vector3.new(math.cos(angle) * 5, 2, math.sin(angle) * 5),
			Transparency = 1,
		}):Play()
