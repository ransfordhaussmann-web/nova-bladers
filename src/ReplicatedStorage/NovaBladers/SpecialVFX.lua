...
function SpecialVFX.iceShatter(origin, range, color, folder)
	...
	for i = 1, 8 do
		...
		local angle = i * 45
		shard.CFrame = CFrame.new(origin + Vector3.new(math.cos(math.rad(angle)) * 2, 0.8, math.sin(math.rad(angle)) * 2))
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + Vector3.new(math.cos(math.rad(angle)) * range * 0.5, 2, math.sin(math.rad(angle)) * range * 0.5),
			Transparency = 1,
		}):Play()
