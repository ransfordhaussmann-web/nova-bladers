...
function SpecialVFX.slashArc(origin, facing, color, folder)
	...
	arc.CFrame = CFrame.new(origin + Vector3.new(0, 0.6, 0), origin + facing * 4 + Vector3.new(0, 0.6, 0))
	arc.Parent = folder

	TweenService:Create(arc, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 1,
		Size = Vector3.new(0.1, 0.2, 6),
	}):Play()
...
function SpecialVFX.shatterBurst(origin, range, color, folder)
	for i = 1, 6 do
		...
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.6, 0.9, 0.35)
		shard.Anchored = true
		shard.CanCollide = false
		...
		local target = origin + Vector3.new(math.cos(angle) * range, 0.3, math.sin(angle) * range)
...
function SpecialVFX.coronaRing(origin, range, color, folder)
	...
	glow.CFrame = CFrame.new(origin + Vector3.new(0, 1.2, 0))
	glow.Parent = folder
