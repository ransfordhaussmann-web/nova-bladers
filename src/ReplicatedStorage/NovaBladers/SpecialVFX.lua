...
function SpecialVFX.flameWheel(origin, color, folder)
	local wheel = Instance.new("Part")
	wheel.Shape = Enum.PartType.Cylinder
	wheel.Size = Vector3.new(0.2, 4, 4)
	wheel.Anchored = true
	wheel.CanCollide = false
	wheel.Material = Enum.Material.Neon
	wheel.Color = color
	wheel.Transparency = 0.25
	wheel.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(90))
	wheel.Parent = folder
	...
	TweenService:Create(wheel, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, 8, 8),
		Transparency = 1,
	}):Play()
	Debris:AddItem(wheel, 0.55)
end
...
function SpecialVFX.frostVeilRing(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "FrostVeilRing"
	ring.Shape = Enum.PartType.Cylinder
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.9, 4, 4)
	inner.Anchored = true
	inner.CanCollide = false
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	for i = 0, 5 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + dir * range) * CFrame.Angles(0, math.rad(angle), math.rad(30)),
			Transparency = 1,
		}):Play()
