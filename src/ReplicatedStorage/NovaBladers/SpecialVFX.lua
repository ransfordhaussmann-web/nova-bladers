...
function SpecialVFX.edgeSlash(position, facing, color, folder)
	...
	for _, offset in { -1, 1 } do
		...
		local pos = position + right * offset * 1.5
		slash.CFrame = CFrame.new(pos, pos + facing)
		slash.Parent = folder
		TweenService:Create(slash, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 1,
			Size = Vector3.new(0.05, 0.3, 4.5),
		}):Play()
		Debris:AddItem(slash, 0.25)
	end
end

function SpecialVFX.edgeFinish(position, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
