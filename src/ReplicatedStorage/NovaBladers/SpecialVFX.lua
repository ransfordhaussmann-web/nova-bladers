...
function SpecialVFX.fireCyclone(origin, range, color, folder)
	...
	cyclone.Size = Vector3.new(0.25, range * 1.6, range * 1.6)
	cyclone.Anchored = true
	cyclone.CanCollide = false
	cyclone.Material = Enum.Material.Neon
	cyclone.Color = color
	cyclone.Transparency = 0.35
	cyclone.CFrame = CFrame.new(origin + Vector3.new(0, 1.2, 0)) * CFrame.Angles(0, 0, math.rad(90))
	...
	Debris:AddItem(cyclone, 0.35)
end

function SpecialVFX.fireBurst(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
	burst.Size = Vector3.new(2, 2, 2)
	burst.Anchored = true
...
function SpecialVFX.iceVeil(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "IceVeil"
	ring.Shape = Enum.PartType.Cylinder
	...
	frost.Size = Vector3.new(4.5, 4.5, 4.5)
	frost.Anchored = true
	frost.CanCollide = false
	frost.Material = Enum.Material.Ice
	frost.Color = Color3.fromRGB(200, 240, 255)
	frost.Transparency = 0.5
	frost.CFrame = CFrame.new(controller.part.Position)
	frost.Parent = folder
...
function SpecialVFX.frostField(origin, range, color, folder)
	...
	TweenService:Create(field, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Transparency = 0.85,
	}):Play()
	Debris:AddItem(field, 0.55)
end
...
function SpecialVFX.iceShatter(position, range, color, folder)
	for i = 1, 6 do
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame + Vector3.new(math.cos(angle) * range * 0.6, 2, math.sin(angle) * range * 0.6),
			Transparency = 1,
		}):Play()
