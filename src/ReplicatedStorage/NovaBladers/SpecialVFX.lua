...
function SpecialVFX.flameSpiral(origin, radius, color, folder)
	...
	TweenService:Create(ring, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.1, radius * 0.6, radius * 0.6),
		Transparency = 1,
	}):Play()
	Debris:AddItem(ring, 0.4)
end

function SpecialVFX.blazeEruption(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.iceWallRing(controller, color, duration, range)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "IceWall"
	ring.Shape = Enum.PartType.Cylinder
	ring.Size = Vector3.new(1.2, range * 1.4, range * 1.4)
	ring.Anchored = true
	ring.CanCollide = false
	...
	local inner = Instance.new("Part")
	inner.Shape = Enum.PartType.Cylinder
	inner.Size = Vector3.new(0.9, range, range)
	inner.Anchored = true
	inner.CanCollide = false
