...
function SpecialVFX.cycloneVortex(controller, color, duration)
	...
	for i = 1, 6 do
		...
		flame.Size = Vector3.new(0.5, 1.2, 0.5)
		flame.Anchored = true
		flame.CanCollide = false
		flame.Material = Enum.Material.Neon
		flame.Color = color
		flame.Transparency = 0.15
		flame.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 3, 0.6, math.sin(angle) * 3))
		flame.Parent = folder
	...
	vortex.Size = Vector3.new(0.3, 5, 5)
...
function SpecialVFX.emberErupt(position, range, color, folder)
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostAura(controller, color, duration, range)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position

	local frost = Instance.new("Part")
	frost.Shape = Enum.PartType.Cylinder
	frost.Size = Vector3.new(0.2, range * 1.2, range * 1.2)
	frost.Anchored = true
...
function SpecialVFX.glacierWall(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local ring = Instance.new("Part")
	ring.Name = "GlacierWall"
	ring.Shape = Enum.PartType.Cylinder
	...
	crystals.Size = Vector3.new(1.0, 5, 5)
...
function SpecialVFX.iceShards(origin, range, color, folder)
	for i = 1, 4 do
		...
		shard.CFrame = CFrame.new(origin) * CFrame.Angles(0, angle, math.rad(45))
		shard.Parent = folder

		local target = origin + Vector3.new(math.cos(angle) * range, 0.5, math.sin(angle) * range)
