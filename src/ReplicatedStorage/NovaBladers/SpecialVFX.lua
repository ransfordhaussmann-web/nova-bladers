...
function SpecialVFX.iceShatter(position, range, color, folder)
	for i = 1, 8 do
		...
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, angle, math.rad(25)) * CFrame.new(0, 0, range * 0.4)
		shard.Parent = folder

		TweenService:Create(shard, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = shard.CFrame * CFrame.new(math.cos(angle) * range * 0.6, 1.5, math.sin(angle) * range * 0.6),
			Transparency = 1,
		}):Play()
