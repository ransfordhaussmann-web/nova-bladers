...
function SpecialVFX.iceShard(position, color, folder)
	...
	shard.CFrame = CFrame.new(position + Vector3.new(math.random(-2, 2), 2.5, math.random(-2, 2)))
		* CFrame.Angles(math.rad(math.random(-30, 30)), math.rad(math.random(0, 360)), 0)
	shard.Parent = folder

	TweenService:Create(shard, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
	...
	burst.CanCollide = false
	burst.Material = Enum.Material.Neon
	burst.Color = Color3.fromRGB(200, 245, 255)
