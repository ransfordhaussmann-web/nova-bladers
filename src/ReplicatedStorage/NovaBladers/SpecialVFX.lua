...
function SpecialVFX.bastionShield(controller, color, duration)
	...
	shield.Size = Vector3.new(1.8, 5, 5)
...
function SpecialVFX.stonePulse(origin, range, color, folder)
	...
	pulse.Size = Vector3.new(0.4, 2.5, 2.5)
	pulse.Anchored = true
	pulse.CanCollide = false
	pulse.Material = Enum.Material.Slate
	pulse.Color = color
...
function SpecialVFX.stoneSlam(origin, range, color, folder)
	local slab = Instance.new("Part")
	slab.Shape = Enum.PartType.Cylinder
	slab.Size = Vector3.new(0.5, 3, 3)
	slab.Anchored = true
	slab.CanCollide = false
	slab.Material = Enum.Material.Concrete
	slab.Color = color
	slab.Transparency = 0.2
	slab.CFrame = CFrame.new(origin) * CFrame.Angles(0, 0, math.rad(90))
	slab.Parent = folder

	TweenService:Create(slab, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {
		Size = Vector3.new(0.3, range * 2.2, range * 2.2),
		Transparency = 1,
	}):Play()
	Debris:AddItem(slab, 0.55)
end

function SpecialVFX.solarCharge(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	...
	glow.CFrame = CFrame.new(controller.part.Position)
	glow.Parent = folder
...
function SpecialVFX.solarFlare(position, color, folder)
	...
	flare.Size = Vector3.new(1.2, 1.2, 1.2)
	flare.Anchored = true
...
function SpecialVFX.phantomSplit(controller, color, folder)
	...
	for i = 1, 3 do
		...
		local offset = Vector3.new(math.cos(angle) * 5, 0.3, math.sin(angle) * 5)
...
function SpecialVFX.getPhantomPositions(controller, count)
	local positions = {}
	local origin = controller.part.Position
	local target = controller.specialTarget
	local center = target and target.part and target.part.Position or (origin + controller.facing * 10)

	for i = 1, count do
		local t = i / (count + 1)
		local angle = (i - 1) * (math.pi * 2 / count) + math.random() * 0.5
		local offset = Vector3.new(math.cos(angle) * 4, 0, math.sin(angle) * 4)
		positions[i] = origin:Lerp(center, t) + offset
	end
	return positions
end
...
function SpecialVFX.phantomDash(fromPos, toPos, color, folder)
	local trail = Instance.new("Part")
	trail.Size = Vector3.new(0.6, 0.6, 0.6)
	trail.Shape = Enum.PartType.Ball
	trail.Anchored = true
	trail.CanCollide = false
...
function SpecialVFX.phantomFade(position, color, folder)
	local fade = Instance.new("Part")
	fade.Shape = Enum.PartType.Ball
	fade.Size = Vector3.new(3, 3, 3)
	fade.Anchored = true
	fade.CanCollide = false
	fade.Material = Enum.Material.ForceField
	fade.Color = color
	fade.Transparency = 0.4
	fade.CFrame = CFrame.new(position)
	fade.Parent = folder

	TweenService:Create(fade, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = Vector3.new(10, 10, 10),
		Transparency = 1,
	}):Play()
	Debris:AddItem(fade, 0.45)
end
