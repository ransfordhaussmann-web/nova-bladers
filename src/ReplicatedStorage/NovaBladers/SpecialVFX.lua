...
function SpecialVFX.graniteRoot(controller, color, duration)
	...
	for i = 0, 3 do
		local angle = i * 90
		local offset = CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(2.2, -0.5, 0)
		...
		TweenService:Create(pillar, TweenInfo.new(duration * 0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			...
			CFrame = CFrame.new(pos) * offset * CFrame.new(0, 1.2, 0),
		}):Play()
...
function SpecialVFX.phantomSplit(controller, color, duration)
	...
	for i = -1, 1, 2 do
		...
		ghost.CFrame = CFrame.new(pos + Vector3.new(i * 3, 0, 0)) * CFrame.Angles(0, 0, math.rad(90))
		ghost.Parent = folder

		TweenService:Create(ghost, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Transparency = 0.9,
			CFrame = CFrame.new(pos + Vector3.new(i * 5, 0, i * 2)) * CFrame.Angles(0, 0, math.rad(90)),
		}):Play()
