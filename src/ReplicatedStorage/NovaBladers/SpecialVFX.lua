...
function SpecialVFX.flareBurst(position, color, folder, range)
	range = range or 7
	local burst = Instance.new("Part")
	burst.Shape = Enum.PartType.Ball
...
function SpecialVFX.frostArmor(controller, color, duration)
	...
	shell.Name = "FrostArmor"
	shell.Shape = Enum.PartType.Cylinder
	shell.Size = Vector3.new(1.8, 5.5, 5.5)
	shell.Anchored = true
...
function SpecialVFX.iceShard(origin, direction, color, folder)
	...
	shard.CFrame = CFrame.new(origin, origin + direction)
	shard.Parent = folder

	TweenService:Create(shard, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		CFrame = CFrame.new(origin + direction * 6, origin + direction * 7),
		Transparency = 1,
	}):Play()
