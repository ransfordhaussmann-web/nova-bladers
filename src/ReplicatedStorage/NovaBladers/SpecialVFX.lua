...
function SpecialVFX.slashArc(position, facing, color, folder)
	...
	arc.CFrame = CFrame.new(position + Vector3.new(0, 0.6, 0), position + facing) * CFrame.Angles(0, math.rad(90), 0)
	arc.Parent = folder
...
function SpecialVFX.stoneWall(controller, color, duration)
	...
	pillar.CFrame = CFrame.new(controller.part.Position + Vector3.new(0, 1.2, 0))
...
function SpecialVFX.phaseFade(controller, hidden)
	SpecialVFX.setUnderground(controller, hidden)
	if hidden then
...
function SpecialVFX.phaseWarp(position, color, folder)
