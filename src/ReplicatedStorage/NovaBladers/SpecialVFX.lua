...
function SpecialVFX.crossSlashTrail(origin, facing, color, folder)
	local right = facing:Cross(Vector3.yAxis).Unit
	for i = -1, 1, 2 do
		local mark = Instance.new("Part")
		mark.Size = Vector3.new(0.5, 0.15, 3.2)
		mark.Anchored = true
		mark.CanCollide = false
		mark.Material = Enum.Material.Neon
		mark.Color = color
		mark.Transparency = 0.2
		mark.CFrame = CFrame.lookAt(origin, origin + facing) * CFrame.Angles(0, 0, math.rad(45 * i))
		mark.Parent = folder
...
function SpecialVFX.iceShatter(origin, range, color, folder)
	for i = 0, 5 do
		...
		shard.Size = Vector3.new(0.5, 0.8, 0.35)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Glass
		shard.Color = color
		shard.Transparency = 0.1
		local offset = Vector3.new(math.cos(math.rad(angle)), 0.3, math.sin(math.rad(angle))) * (range * 0.45)
		...
		TweenService:Create(shard, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(origin + offset * 1.8) * CFrame.Angles(0, math.rad(angle), math.rad(60)),
			Transparency = 1,
		}):Play()
