...
function SpecialVFX.crystalAura(controller, color, duration)
	local folder = SpecialVFX.ensureFolder(controller)
	local pos = controller.part.Position
	...
	for i = 1, 6 do
		...
		shard.CFrame = CFrame.new(pos + Vector3.new(math.cos(angle) * 2.5, 0.8, math.sin(angle) * 2.5))
...
function SpecialVFX.crystalBurst(position, color, folder)
	...
	for i = 0, 5 do
		local angle = i * 60
		local shard = Instance.new("Part")
		shard.Size = Vector3.new(0.25, 0.8, 0.25)
		shard.Anchored = true
		shard.CanCollide = false
		shard.Material = Enum.Material.Neon
		shard.Color = color
		shard.CFrame = CFrame.new(position) * CFrame.Angles(0, math.rad(angle), math.rad(40))
		shard.Parent = folder
		TweenService:Create(shard, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			CFrame = CFrame.new(position + Vector3.new(math.cos(math.rad(angle)) * 4, 1, math.sin(math.rad(angle)) * 4)),
			Transparency = 1,
		}):Play()
...
function SpecialVFX.fireRing(origin, range, color, folder)
	local ring = Instance.new("Part")
	ring.Shape = Enum.PartType.Cylinder
...
function SpecialVFX.crownBurst(position, color, folder)
	...
	for i = 0, 4 do
		...
		spike.Size = Vector3.new(0.4, 1.5, 0.4)
		spike.Anchored = true
		spike.CanCollide = false
		spike.Material = Enum.Material.Neon
		spike.Color = Color3.fromRGB(255, 220, 80)
		spike.CFrame = CFrame.new(position) * CFrame.Angles(math.rad(-40), math.rad(angle), 0)
		spike.Parent = folder
		TweenService:Create(spike, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = Vector3.new(0.2, 3, 0.2),
			Transparency = 1,
		}):Play()
		Debris:AddItem(spike, 0.45)
	end
...
function SpecialVFX.mirageBurst(position, color, folder)
	...
	for i = 1, 3 do
		...
		ghost.CFrame = CFrame.new(position + Vector3.new(math.random(-2, 2), 0, math.random(-2, 2)))
