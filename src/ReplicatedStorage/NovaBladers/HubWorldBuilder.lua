...
local function buildZone(parent, origin, zoneDef, onPromptTriggered)
	...
	local pad = makePart({
		...
		CFrame = CFrame.new(origin + zoneDef.offset + Vector3.new(0, zoneDef.size.Y * 0.5, 0)),
	})
	...
	makePart({
		...
		CFrame = CFrame.new(origin + zoneDef.offset + Vector3.new(0, 0.1, 0)),
	})
	...
	if zoneDef.id ~= "HallOfFame" then
		...
		prompt:SetAttribute("Action", zoneDef.action)
		prompt.Parent = pad

		if onPromptTriggered then
			prompt.Triggered:Connect(function(player)
				onPromptTriggered(player, zoneDef)
			end)
		end
	else
		buildLeaderboardBoard(zoneFolder, origin + zoneDef.offset, zoneDef)
...
function HubWorldBuilder.build(onZoneTriggered)
	...
	if existing and existing:GetAttribute("Built") then
		return existing
	end
	if existing then
		existing:Destroy()
	...
	for _, zoneKey in HubConfig.ZONE_ORDER do
		...
		buildZone(zonesFolder, origin, zoneDef, onZoneTriggered)
	end
...
function HubWorldBuilder.findLeaderboardBoard()
	...
	local zone = zones:FindFirstChild("HallOfFameZone")
	if not zone then
		return nil
	end
