...
async function formatTable(format, head, rows) {
	switch (format) {
		...
		case "csv": return new Promise((resolve, reject) => {
...
async function reportSection(type, format, logger, section) {
	...
	const footnotes = format !== "csv" ? getFootnotes(type, header) : [];
...
function formatPropertyReport(property, index, format) {
	...
	for (const key in property) {
		...
		else if (key.match(/size/i) && format !== "csv") row[key] = value > 0 ? formatBytes(value) : "";
		else if (typeof value === "number") row[key] = format !== "csv" ? formatLong(value) : value;
...
const PAD_EMOJI = /* @__PURE__ */ new Set([
	"🫖",
	"🖼",
	"⏯"
]);
...
async function checkKTXSoftware(logger) {
	if (!await commandExists("ktx") && !process.env.CI) throw new Error(`Command "ktx" not found. Please install KTX-Software ${KTX_SOFTWARE_VERSION_MIN}+, from:
...
async function validate(input, options, logger) {
	...
	return validator.validateBytes(new Uint8Array(buffer), {
		...
		if (options.format === "csv") await printCSV(report);
		else {
...
async function printCSV(report) {
	...
	console.log(await formatTable("csv", HEADER, messages.map(Object.values)));
}
...
program.version("4.4.1").description("Command-line interface (CLI) for the glTF Transform SDK.");
