...
| **Crimson Forge** | Hammer plates, forge ring, orange metal tip |
| **Frost Prism** | Ice core, glass facets, crystal prism ring |

All 6 beys support **Creator Store / Studio imports** via `modelRef.studioModelName` (clone from `ReplicatedStorage/NovaBladers/Models/`). Procedural layers are used as fallback when no model is present.
