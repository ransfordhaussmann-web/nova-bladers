...
| **Crimson Edge** | 4 crimson blades with neon edges, fast spin ring |
| **Frost Anchor** | Ice pillars, frost ring, heavy anchor plate |
...
Most toolbox hits are **UGC accessories**, not game-ready spin tops. Fan games often use **free toolbox models** with mixed quality.

### How to add a Creator Store model

1. Open **Roblox Studio**
2. **View → Toolbox → Creator Store**
3. Search using the `creatorStoreSearch` hint in `BeyCatalog.lua` (e.g. `attack spinning top metal blade`)
...
Studio model folders: `NovaStriker`, `IronShell`, `VoltDash`, `ShadowBite`, `CrimsonEdge`, `FrostAnchor`
