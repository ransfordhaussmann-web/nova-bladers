...
| **Blaze Wheel** | Fire spokes, inferno ring, fast outer flame layer |
| **Frost Veil** | Ice crystals, frost shield ring, glass veil layer |
...
Or use the legacy `modelAssets` field — both work. Procedural layers are skipped when `meshId` is set; spin ring still added.
