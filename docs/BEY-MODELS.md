...
| **Crimson Vortex** | 4 flame blades, vortex ring, crimson spin layers |
| **Glacier Mantle** | Ice crystal segments, frost spikes, glass frost shield |
...
5. `BeyCatalog.modelRef.studioModelName` must match the folder name

### Studio model names (all 6 beys)
...
When a matching model exists in `Models/`, it is cloned instead of the procedural build.
