...
| **Crimson Forge** | Forge plate, hammer blades, ember spin ring |
| **Frost Crown** | Ice crown spikes, glass aura, crown jewel |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys expose `modelRef.studioModelName` and `creatorStoreQuery` in `BeyCatalog.lua` for optional Creator Store imports under `ReplicatedStorage/NovaBladers/Models/`.
