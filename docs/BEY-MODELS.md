...
| **Blaze Drift** | Flame ring, orange heat spikes, fast outer glow |
| **Frost Crown** | Ice shell segments, crystal spikes, frost aura |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All six beys support **Creator Store / Studio imports** via `modelRef.studioModelName` under `ReplicatedStorage/NovaBladers/Models/<Name>`. Procedural layers are the fallback when no imported model exists.
