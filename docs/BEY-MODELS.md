...
| **Crimson Vortex** | Spiral attack blades, expanding red vortex ring |
| **Glacier Mantle** | Ice shell segments, frost crystals, glass mantle |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Optional **Creator Store / Studio import**: each bey has a `modelRef.studioModelName`. Place imported models under `ReplicatedStorage/NovaBladers/Models/<Name>` — procedural layers are skipped when a model is found.
