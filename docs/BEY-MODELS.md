...
| **Crimson Forge** | Ember core, 4 flame blades, heat haze ring |
| **Frost Crown** | Ice segments, crystal spikes, frost crown ring |
...
### Studio model folder (all 6 beys)

Place imported Creator Store models under `ReplicatedStorage/NovaBladers/Models/`:

| Bey | Folder name | Creator Store search hint |
|-----|-------------|---------------------------|
| Nova Striker | `NovaStriker` | attack pegasus / storm top |
| Iron Shell | `IronShell` | defense metal spinning top |
| Volt Dash | `VoltDash` | stamina flat spinning top |
| Shadow Bite | `ShadowBite` | dark balance spinning top |
| Crimson Forge | `CrimsonForge` | flame attack spinning top |
| Frost Crown | `FrostCrown` | ice crystal spinning top |

`BeyModelBuilder` clones from `Models/<studioModelName>` when present; otherwise uses `modelAssets.meshId` or procedural layers.
