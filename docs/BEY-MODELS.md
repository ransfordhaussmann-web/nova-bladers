...
| **Blaze Quill** | 4 fire-feather blades, ember ring, fast spin |
| **Tide Anchor** | 3 anchor flukes, wave shield ring, chain link |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

**6 Beys total** — Nova Striker, Iron Shell, Volt Dash, Shadow Bite, Blaze Quill, Tide Anchor.
...
3. Place under `ReplicatedStorage/NovaBladers/Models/`:
   - `NovaStriker` — Attack bey (Sketchfab import)
   - `BlazeQuill` — Attack bey (Creator Store / custom import)
   - `TideAnchor` — Defense bey (Creator Store / custom import)
4. Set `PrimaryPart`, weld parts, name `Hull` on collision part
5. In `BeyCatalog.lua`, set `modelRef.studioModelName` to match the folder name
6. Procedural fallback builds automatically when no Studio model is present
