...
| **Crimson Vortex** | Jagged attack wings, crimson energy disc, fast outer halo |
| **Glacier Mantle** | Ice mantle segments, frost crystal spikes, heavy bumper ring |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Crimson Vortex and Glacier Mantle also support optional Creator Store imports via `modelRef.studioModelName` in `BeyCatalog.lua` (place models under `ReplicatedStorage/NovaBladers/Models/`).
