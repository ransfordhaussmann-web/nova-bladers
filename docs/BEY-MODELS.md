...
| **Crimson Comet** | 4 flare spikes, heat ring, orange comet tip |
| **Glacier Helm** | Ice shell segments, glass crown ring, frost layers |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support **Creator Store / Studio imports** via `modelRef.studioModelName` — drop a model into `ReplicatedStorage/NovaBladers/Models/<Name>` and it replaces the procedural mesh at runtime.
...
```lua
modelRef = {
    studioModelName = "CrimsonComet",  -- must match Models folder name
    targetSize = 3.5,
},
-- or for a single MeshPart asset:
modelAssets = {
...
3. Place under `ReplicatedStorage/NovaBladers/Models/<BeyId>` (e.g. `NovaStriker`, `CrimsonComet`, `GlacierHelm`)
