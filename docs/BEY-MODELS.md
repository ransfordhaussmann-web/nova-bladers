...
| **Blaze Wheel** | Flame petals, ember glow ring, fast attack spin |
| **Frost Veil** | Crystal shell segments, ice spikes, glass veil ring |
...
Or use `creatorStore` (same fields) for Blaze Wheel / Frost Veil — search terms and `studioModelName` are already in the catalog.

8. Procedural layers are skipped when `meshId` is set; spin ring still added.

### Studio model folder import

For **Blaze Wheel** and **Frost Veil**, place imported models under:

- `ReplicatedStorage/NovaBladers/Models/BlazeWheel`
- `ReplicatedStorage/NovaBladers/Models/FrostVeil`

The builder clones from `Models/<studioModelName>` when present, otherwise uses procedural layers.
