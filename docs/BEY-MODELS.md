...
| **Crimson Fang** | 4 crimson attack blades, fast spin ring, metal core |
| **Glacier Spin** | Ice shell segments, glass frost ring, heavy stamina core |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support **Creator Store imports** via `modelRef.studioModelName` (clone from `Models/` folder) or `modelAssets.meshId`.
