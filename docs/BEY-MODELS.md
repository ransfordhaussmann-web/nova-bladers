...
| **Frost Crown** | Ice spikes, glass crown ring, slow defensive spin layers |
| **Ember Fang** | Three fire claws, ember flame ring, fast attack spin |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

### Creator Store import (Frost Crown, Ember Fang)

Place imported models under `ReplicatedStorage/NovaBladers/Models/` as `FrostCrown` and `EmberFang`.  
Alternatively set `modelAssets.meshId` in `BeyCatalog.lua` after copying the asset ID from Studio.
