...
| **Crimson Fang** | 3 flame fangs, ember ring, fast crimson spin layers |
| **Frost Halo** | Ice shell segments, glass halo ring, crystal tip |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support optional Creator Store meshes via `modelAssets.meshId` in `BeyCatalog.lua`.
Studio imports go under `ReplicatedStorage/NovaBladers/Models/<BeyId>`.
