...
| **Crimson Edge** | Flame blades, ember core, heat haze ring |
| **Frost Core** | Ice crystals, glacier shell, frost shield ring |
...
3. Place under `ReplicatedStorage/NovaBladers/Models/<BeyId>` (e.g. `CrimsonEdge`)
4. Set `PrimaryPart`, weld parts, name `Hull` on collision part
5. `BeyModelBuilder` clones from `Models/<studioModelName>` when present

Each bey has a `modelRef.studioModelName` in `BeyCatalog.lua` matching the folder name under `Models/`.
