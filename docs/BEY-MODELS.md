...
| **Crimson Flare** | Flame petals, heat ring, fast neon spin layer |
| **Aqua Spiral** | Glass core, spiral fins, water ring |
...
| Bey | Type | Special | Creator Store slot |
|-----|------|---------|-------------------|
| **Crimson Flare** | Attack | Crimson Blaze Drive | `modelAssets.meshId` or Studio model `CrimsonFlare` |
| **Aqua Spiral** | Balance | Aqua Cyclone Vortex | `modelAssets.meshId` or Studio model `AquaSpiral` |

Both ship with procedural 3D fallbacks. To use a Creator Store mesh, uncomment `modelAssets` in `BeyCatalog.lua` and paste the rbxassetid.
