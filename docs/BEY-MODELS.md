...
| **Crimson Edge** | Flame spiral blades, heat ring, burning performance tip |
| **Frost Core** | Crystal shards, frost ring, counter-rotating ice shell |
...
5. At runtime `BeyModelBuilder` clones from `Models/<studioModelName>` when present

Each bey has a `modelRef.studioModelName` in `BeyCatalog.lua`:
