...
| **Crimson Fang** | 4 spiral fangs, red rend ring, fast attack spin |
| **Frost Coil** | Ice-glass coil segments, frost aura, heavy defense ring |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support **Creator Store / Studio import** via `modelRef.studioModelName` in `BeyCatalog.lua`.
Procedural layers are used as fallback when no model is in `Models/`.
