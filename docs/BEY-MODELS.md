...
| **Crimson Fang** | Four crimson blades, fast attack ring |
| **Granite Fort** | Stone blocks, granite rampart, heavy dual rings |
| **Solar Drift** | Sun rays, golden halo, wide stamina ring |
| **Phantom Edge** | Ghost blades, translucent wisp, cyan force-field |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Each new bey has an optional `modelAssets.meshId` slot — paste a Creator Store mesh ID in `BeyCatalog.lua` to replace the procedural model.
