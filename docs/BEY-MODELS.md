...
| **Crimson Fang** | 4 fang blades, blood-red neon ring, aggressive attack profile |
| **Frost Crown** | Ice shell segments, crystal crown spikes, frost shield ring |
...
|-----|---------------|---------------|
| **Crimson Fang** | `Models/CrimsonFang` | optional `meshId` in `BeyCatalog.lua` |
| **Frost Crown** | `Models/FrostCrown` | optional `meshId` in `BeyCatalog.lua` |

Procedural fallback builds automatically when no Studio model or `meshId` is set.
