...
| **Crimson Vortex** | 4 vortex blades, crimson energy ring, fast spin layers |
| **Glacier Shield** | Ice crystal segments, frost shield ring, slow outer rim |
...
### Studio import slots (Creator Store / custom meshes)

| Bey | Models folder name | Notes |
|-----|-------------------|-------|
| Nova Striker | `NovaStriker` | See `docs/SKETCHFAB-NOVA-STRIKER.md` |
| Crimson Vortex | `CrimsonVortex` | Toolbox → spinning top / attack bey |
| Glacier Shield | `GlacierShield` | Toolbox → defense / ice-themed top |

After import: `ReplicatedStorage → NovaBladers → Models → <name>`. The game clones these instead of procedural layers when present.
