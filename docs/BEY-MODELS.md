...
| **Crimson Fang** | Blood-red fangs, neon ring, aggressive attack blades |
| **Frost Core** | Ice crystal segments, glass frost shield, slow counter-spin |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

### Creator Store import (Crimson Fang, Frost Core)

1. Studio → **Toolbox → Creator Store** → search `spinning top`
2. Insert model into Workspace, scale to ~3–4 studs wide
3. Move to `ReplicatedStorage/NovaBladers/Models/` and rename:
   - `CrimsonFang`
   - `FrostCore`
4. Or paste **MeshId** into `BeyCatalog.lua` → `modelAssets.meshId` on the bey entry

Procedural layers are used until a Studio model or `meshId` is set.
