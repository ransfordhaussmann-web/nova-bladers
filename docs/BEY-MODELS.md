...
| **Crystal Tide** | Glass prism shards, tide ring, cyan stamina glow |
| **Ember Core** | Magma core, flame blades, heat ring |
...
**Crystal Tide** and **Ember Core** ship with `modelAssets` placeholders — paste a Creator Store `meshId` in `BeyCatalog.lua` to replace the procedural model. Alternatively import FBX models as `CrystalTide` / `EmberCore` under `Models/`.
