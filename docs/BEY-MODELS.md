...
| **Crimson Fang** | 4 ripper blades, blood-red neon ring, glass tip |
| **Aurora Crest** | Glass crest petals, aurora glow, shield cap |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Creator Store models can replace procedural meshes — place under `ReplicatedStorage/NovaBladers/Models/` as `CrimsonFang` or `AuroraCrest` (see `modelRef.studioModelName` in catalog).
