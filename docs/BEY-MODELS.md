...
| **Starfall Rush** | Crimson comet fins, star core, fast attack ring |
| **Thunder Loop** | Dual counter-rotating loop rings, lightning arcs |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support optional Creator Store / imported models via `modelRef.studioModelName` in `BeyCatalog.lua`. Place models under `ReplicatedStorage/NovaBladers/Models/<studioModelName>`.
