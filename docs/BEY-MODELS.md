...
| **Crimson Ember** | Flame ring, 4 fire spikes, ember glow (Attack) |
| **Glacier Core** | Ice shell segments, frost shield, crystal tip (Defense) |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

**Crimson Ember** and **Glacier Core** ship with procedural models plus optional Creator Store / Studio import slots (`modelRef.studioModelName` or `modelAssets.meshId` in `BeyCatalog.lua`).
