...
| **Crimson Vortex** | Spiral blades, counter-rotating vortex ring, red attack core |
| **Glacier Mantle** | Ice shell segments, frost crown, slow heavy spin |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Each bey has a `modelRef.studioModelName` — drop a Creator Store mesh into `ReplicatedStorage/NovaBladers/Models/<Name>` to override the procedural build.
