...
| **Crimson Blaze** | Flame blades, ember ring, fast attack spin |
| **Frost Crown** | Ice crown segments, glass spikes, slow heavy spin |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Each bey has a `modelRef.studioModelName` — drop a Creator Store / imported model into
`ReplicatedStorage/NovaBladers/Models/<Name>` in Studio to override the procedural mesh.
