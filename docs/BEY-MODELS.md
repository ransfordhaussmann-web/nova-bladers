...
| **Crimson Ripper** | 4 saw blades, red vortex ring, aggressive attack profile |
| **Frost Monarch** | Ice crystals, glass frost shield, heavy ice bumper |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All **6 beys** include `modelRef.studioModelName` and optional `modelAssets` in `BeyCatalog.lua` — paste a Creator Store `meshId` to replace procedural layers.
