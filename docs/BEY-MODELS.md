...
| **Crimson Edge** | Flame ring, 4 attack blades, fast spin layer |
| **Frost Halo** | Ice shell segments, glass halo ring, frost shield |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Each bey has a `modelRef` in `BeyCatalog.lua` with `studioModelName` and `searchTerms` for Creator Store import.
