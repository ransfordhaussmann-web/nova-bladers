...
| **Crimson Edge** | 4 flame blades, ember ring, aggressive red spin layers |
| **Frost Halo** | Ice shell segments, crystal shards, glass frost halo |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

**6 Beys** total — selection UI scrolls when the roster grows.
...
5. `BeyModelBuilder` clones from `Models/` when present; otherwise procedural build

`modelRef.studioModelName` in `BeyCatalog.lua` must match the folder/model name in Studio.
