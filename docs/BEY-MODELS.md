...
| **Crimson Fang** | Four fang blades, crimson blood ring, aggressive spin |
| **Frost Ring** | Ice shell segments, crystal shards, frost aura |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All six beys support optional **Creator Store / imported Studio models** via `modelRef.studioModelName` in `BeyCatalog.lua` (procedural fallback if missing).
