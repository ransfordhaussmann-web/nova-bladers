...
| **Crimson Fang** | 4 red fang blades, crimson spin ring, aggressive attack profile |
| **Frost Core** | Ice crystals, glass core, frost shield ring |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

### Creator Store model hooks

These beys support optional Studio-imported meshes via `modelRef.studioModelName`:
...
Place imported models under `ReplicatedStorage/NovaBladers/Models/<Name>`. Procedural layers are used as fallback when no model is present.
