...
| **Crimson Fang** | Four fang blades, crimson blood ring, aggressive attack profile |
| **Frost Ring** | Ice shell segments, crystal spikes, frost aura ring |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

### Creator Store import (all 6 Beys)

Each bey has `modelRef.studioModelName` in `BeyCatalog.lua`. Import a Creator Store model in Studio under:
