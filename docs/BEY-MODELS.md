...
| **Crimson Blaze** | Ember layer, 4 flame blades, heat glow ring |
| **Frost Crown** | Ice crown segments, glass spikes, frost aura |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support optional Creator Store / Studio imports via `modelRef.studioModelName` in `BeyCatalog.lua` (procedural fallback when no model is present).
