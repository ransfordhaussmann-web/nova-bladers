...
| **Ember Fang** | 4 flame fangs, ember glow ring, attack tip (Creator Store `modelAssets` optional) |
| **Frost Crown** | Ice spikes, glass crown ring, frost base (Creator Store `modelAssets` optional) |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All 6 beys support optional Creator Store meshes via `modelAssets.meshId` or Studio imports under `Models/`.
