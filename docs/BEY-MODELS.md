...
| **Crimson Fang** | Twin crimson blades, heat ring, aggressive spin |
| **Frost Ring** | Ice shell segments, frost aura, crystal accents |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

All six beys support optional Creator Store imports via `modelRef.studioModelName` in `BeyCatalog.lua`.
...
```lua
modelRef = {
    studioModelName = "IronShell",  -- folder name under Models/
},
```
