...
| **Frost Core** | Ice shell segments, glass crystals, frost ring |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

### Creator Store import (all 6 Beys)

Each bey has `modelRef.studioModelName` in `BeyCatalog.lua`. Drop a Creator Store model into:

`ReplicatedStorage → NovaBladers → Models → <studioModelName>`

The game clones and scales it automatically; procedural layers are the fallback.
