...
| **Crimson Fang** | 4 sharp fang blades, heat ring, glass tip (Attack) |
| **Frost Coil** | Ice crystal segments, coil ring, frost aura (Defense) |
...
3. Place under `ReplicatedStorage/NovaBladers/Models/<BeyId>` (e.g. `CrimsonFang`, `FrostCoil`)
4. Set `PrimaryPart`, weld parts, name `Hull` on collision part
5. `BeyModelBuilder` clones from folder when present; otherwise procedural layers
