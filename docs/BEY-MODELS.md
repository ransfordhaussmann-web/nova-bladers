...
| **Crimson Reaver** | 4 neon blades, ember ring, fast crimson spin layers |
| **Frost Ward** | Ice shell segments, glass crystal cap, slow frost ring |

Layers **spin visually** while the bey moves (RPM affects spin speed + ring opacity).

Each bey also has a `modelRef.studioModelName` — drop a Creator Store model into `Models/` to replace the procedural look.
