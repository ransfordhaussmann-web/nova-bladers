...
| **Blaze Wheel** | Flame petals, ember core, outer fire glow |
| **Frost Veil** | Ice plates, glass veil ring, frost mist |
...
Each bey in `BeyCatalog.lua` has a `creatorStore.searchTerms` array for Studio Toolbox searches. Paste a `meshId` into `creatorStore.meshId` to override the procedural model.
